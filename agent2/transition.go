...
// Consumed against the delivered Signal window, captures the candidate
...
func Continue(consumed uint32, effects ...Effect) (Transition, error) {
	owned, err := cloneEffects(effects)
	if err != nil {
		return Transition{}, err
	}
	return Transition{kind: TransitionKindContinue, consumed: consumed, effects: owned}, nil
}

// Wait moves the Process to Waiting for an Engine-minted WaitID already stored
// in the candidate ExecutionState.
func Wait(consumed uint32, waitID WaitID) (Transition, error) {
	if !waitID.Valid() {
		return Transition{}, fmt.Errorf("%w: wait ID: %w", ErrInvalidTransition, ErrInvalidIdentity)
	}
	return Transition{kind: TransitionKindWait, consumed: consumed, waitID: waitID}, nil
}

// Pause requests an explicit scheduling pause with a bounded diagnostic reason.
func Pause(consumed uint32, reason string) (Transition, error) {
	...
	return Transition{kind: TransitionKindPause, consumed: consumed, reason: reason}, nil
}
...
func Complete(consumed uint32, output Output) (Transition, error) {
	if !output.Valid() {
		return Transition{}, fmt.Errorf("%w: output: %w", ErrInvalidTransition, ErrInvalidOutput)
	}
	return Transition{kind: TransitionKindComplete, consumed: consumed, output: output}, nil
}
...
func Fail(consumed uint32, failure Failure) (Transition, error) {
	if !failure.Valid() {
		return Transition{}, fmt.Errorf("%w: failure: %w", ErrInvalidTransition, ErrInvalidFailure)
	}
	return Transition{kind: TransitionKindFail, consumed: consumed, failure: failure}, nil
}

// Kind returns the requested lifecycle intent.
func (t Transition) Kind() TransitionKind { return t.kind }

// Consumed returns the length of the delivered Signal prefix to commit.
func (t Transition) Consumed() uint32 { return t.consumed }
...
func (t Transition) MarshalJSON() ([]byte, error) {
	...
	wire := transitionWire{Kind: t.kind.String(), Consumed: t.consumed}
	switch t.kind {
	case TransitionKindContinue:
...
func transitionFromWire(kind TransitionKind, wire transitionWire) (Transition, error) {
	...
	case TransitionKindContinue:
		...
		return Continue(wire.Consumed, wire.Effects...)
	case TransitionKindWait:
		...
		return Wait(wire.Consumed, *wire.WaitID)
	case TransitionKindPause:
		...
		return Pause(wire.Consumed, wire.Reason)
	case TransitionKindComplete:
		if len(wire.Effects) > 0 || wire.WaitID != nil || wire.Reason != "" || len(wire.Output) == 0 || wire.Failure != nil {
