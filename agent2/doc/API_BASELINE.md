...
Baseline 2 对根 Kernel 的唯一公开修订是 `ChildWaitOpened.Spec`：Planning 的真实 child Action 证明 Strategy 必须校验 Engine 确认的完整 child-wait request，而不能只相信 WaitID。Baseline 3 没有改变根公开 API 或 snapshot/tree wire；P6 恢复合同只修复 terminal commit 关闭 mailbox wait 的私有不变量。

## 4. 明确不在基线中的能力

Baseline 3 不冻结 Platform catalog/routing、OTel decorator、应用迁移 API 或 P7 尚未实现的组合 helper。`flow` 保持独立 in-process 库，不形成 Agent adapter API 或依赖；未来编辑器图只能在更高层编译成已验证的 Workflow Definition，不能反向扩张 Kernel 或恢复 wire。
