...
const moduleImportPath = "github.com/Tangerg/lynx/agent2"
	...
	for packagePath, dependencies := range actualDependencies {
		...
		for dependency := range dependencies {
			if _, ok := allowed[dependency]; !ok {
				t.Errorf("production package %q imports forbidden agent2 package %q", packagePath, dependency)
			}
		}
	}
...
func assertExternalPackageBoundary(t *testing.T, packagePath string, sourcePath string, importPath string) {
	t.Helper()
	switch {
	case isPackageOrChild(importPath, "github.com/Tangerg/lynx/agent"):
		t.Errorf("%s imports the replaced agent module %q", sourcePath, importPath)
...
func assertPackageSet(t *testing.T, actual map[string]struct{}) {
	...
	for packagePath := range actual {
		if _, ok := allowedPackageDependencies[packagePath]; !ok {
			t.Errorf("production package %q is not part of the accepted agent2 package graph", packagePath)
		}
	}
