...
func TestSHA256GeneratorDeterministic(t *testing.T) {
	generator := documentpipeline.NewSHA256IDGenerator(nil)
...
func TestSHA256GeneratorSaltSeparatesStreams(t *testing.T) {
	...
	plain, err := documentpipeline.NewSHA256IDGenerator(nil).Generate(t.Context(), doc)
	if err != nil {
		t.Fatal(err)
	}
	salted, err := documentpipeline.NewSHA256IDGenerator([]byte("tenant-A")).Generate(t.Context(), doc)
	if err != nil {
		t.Fatal(err)
	}
	otherSalt, err := documentpipeline.NewSHA256IDGenerator([]byte("tenant-B")).Generate(t.Context(), doc)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSHA256GeneratorRejectsInvalidDocument(t *testing.T) {
	...
	if _, err := documentpipeline.NewSHA256IDGenerator(nil).Generate(t.Context(), doc); err == nil {
		t.Fatal("invalid metadata produced an ID")
	}
	if _, err := documentpipeline.NewSHA256IDGenerator(nil).Generate(t.Context(), nil); !errors.Is(err, documentpipeline.ErrNilDocument) {
...
func TestSHA256GeneratorCopiesSalt(t *testing.T) {
	salt := []byte("tenant-A")
	generator := documentpipeline.NewSHA256IDGenerator(salt)
	salt[0] = 'X'
	doc := newDocument(t, "doc")
	...
	want, err := documentpipeline.NewSHA256IDGenerator([]byte("tenant-A")).Generate(t.Context(), doc)
	if err != nil {
		t.Fatal(err)
	}
	if got != want {
