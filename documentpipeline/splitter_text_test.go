...
func TestTextSplitterDefaultsToNewline(t *testing.T) {
	splitter, err := documentpipeline.NewTextSplitter(documentpipeline.TextSplitterConfig{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTextSplitterPreservesMetadata(t *testing.T) {
	splitter, err := documentpipeline.NewTextSplitter(documentpipeline.TextSplitterConfig{Separator: "|"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSplitterRejectsNilDocument(t *testing.T) {
	...
	if _, err := splitter.Split(t.Context(), []*document.Document{nil}); !errors.Is(err, documentpipeline.ErrNilDocument) {
