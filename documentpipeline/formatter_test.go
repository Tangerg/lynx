...
func TestFormatterFunc(t *testing.T) {
	...
	formatter := documentpipeline.FormatterFunc(func(doc *document.Document) (string, error) {
...
func TestTextFormatterReturnsDocumentText(t *testing.T) {
	doc, _ := document.NewDocument("hi", nil)
	got, err := (documentpipeline.TextFormatter{}).Format(doc)
	if err != nil {
		t.Fatal(err)
	}
	if got != "hi" {
...
func TestSimpleFormatterPreservesTypedMetadataBoundary(t *testing.T) {
	...
	formatted, err := documentpipeline.NewSimpleFormatter(documentpipeline.SimpleFormatterConfig{}).Format(doc)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFormattersRejectInvalidDocuments(t *testing.T) {
	if _, err := (documentpipeline.TextFormatter{}).Format(nil); !errors.Is(err, documentpipeline.ErrNilDocument) {
		t.Fatalf("TextFormatter error = %v, want ErrNilDocument", err)
	}

	doc, _ := document.NewDocument("body", nil)
	doc.Metadata = metadata.Map{"broken": []byte("{")}
	_, err := documentpipeline.NewSimpleFormatter(documentpipeline.SimpleFormatterConfig{}).Format(doc)
