...
Package bedrockkb wraps AWS Bedrock Knowledge Bases as a semantic and hybrid
searcher.
...
from vectorstore.SearchOptions.TopK, and SearchOptions.Mode is mapped directly
to Bedrock's SEMANTIC or HYBRID search type. Provider-specific reranking and
implicit-filter configuration remain store configuration. Filter
...
`Document.ID` from the result's `DocumentId` or `Location` (e.g. the S3 URI of
the source object); a result without either stable identity is rejected. See
