...
- 已完成三模块回归验证：
  - `go test ./...`（`core`）通过（第三十六轮后复跑）。
  - `go test ./...`（`agent`）通过（第三十六轮后复跑）。
  - `go test ./...`（`app/runtime`）通过（第三十六轮后复跑）。
  - `go vet ./...`（`core`）通过（第三十六轮后复跑）。
  - `go vet ./...`（`agent`）通过（第三十六轮后复跑）。
  - `go vet ./...`（`app/runtime`）通过（第三十六轮后复跑）。
  - `go build ./...`（`core`）通过（第三十六轮后复跑）。
  - `go build ./...`（`agent`）通过（第三十六轮后复跑）。
  - `go build ./...`（`app/runtime`）通过（第三十六轮后复跑）。
- 已完成目标模块低误伤异味扫描：
  - 常量 `fmt.Errorf("...")` 未命中。
  - `TODO` / `FIXME` / `HACK` 未命中。
