...
// any existing row with that id (restore semantics). See [session.Store.Restore].
...
// Delete is idempotent — deleting an unknown id is not an error (matches
...
// (see [session.Store.RenameIfUntitled]). The WHERE guard collapses the titler's
// check-and-set into one statement so a concurrent user rename can't be
...
// [session.Store.SetMetadata]) + refreshes UpdatedAt. ErrNotFound for unknown
// id.
