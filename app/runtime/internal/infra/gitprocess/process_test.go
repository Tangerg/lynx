...
func TestEnvironmentOwnsGitControlsAndExplicitOverrides(t *testing.T) {
	...
	t.Setenv("LYNX_GITPROCESS_SENTINEL", "preserved")
	t.Setenv("LYNX_GITPROCESS_OVERRIDE", "ambient")

	environment := Environment(
		"GIT_DIR=/owned/repository",
		"LYNX_GITPROCESS_OVERRIDE=explicit",
	)
	if !slices.Contains(environment, "GIT_DIR=/owned/repository") {
		t.Fatal("command-owned GIT_DIR override was not installed")
	}
	if !slices.Contains(environment, "LYNX_GITPROCESS_SENTINEL=preserved") {
		t.Fatal("unrelated host environment was not preserved")
	}
	if !slices.Contains(environment, "LYNX_GITPROCESS_OVERRIDE=explicit") {
	...
	for _, entry := range environment {
		...
		if entry == "LYNX_GITPROCESS_OVERRIDE=ambient" {
