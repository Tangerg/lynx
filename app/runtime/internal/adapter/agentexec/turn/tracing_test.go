...
func TestTerminalDiscardFailureIsRecordedUntilCleanupSucceeds(t *testing.T) {
	...
	handle, err := controller.StartTurn(t.Context(), runs.RootExecutionStart{SessionID: "s", Message: "hi"})
	if err != nil {
...
func TestStartTurn_PropagatesEntryTrace(t *testing.T) {
	...
	controller := mustTurn(turn.New(turnDeps(stub)))
	handle, err := controller.StartTurn(entryCtx, runs.RootExecutionStart{SessionID: "s", Message: "hi"})
	if err != nil {
		t.Fatalf("StartTurn: %v", err)
	}
