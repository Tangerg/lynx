...
// findTurn looks up per-turn state by ID under the controller's mutex. Returns
...
func (s *controller) findTurn(id string) (*turnState, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	state, ok := s.turns[id]
	if !ok {
...
func (s *controller) ProcessID(_ context.Context, handle Handle) (string, error) {
