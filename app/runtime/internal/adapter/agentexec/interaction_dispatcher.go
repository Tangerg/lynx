...
func (observed *observedInteractionTool) prepare(
	...
	if observed.authorizer == nil || !observed.interpreter.UsesStandardPolicy(name) {
		return observed.applyDoomLoopBrake(name, arguments, false, "")
	}
	...
	return observed.applyDoomLoopBrake(name, arguments, decision.Denied, decision.Reason)
}
...
func (observed *observedInteractionTool) applyDoomLoopBrake(
	...
	return arguments, true, fmt.Sprintf(
		"tool %q was called with the same arguments and unchanged result %d times; change approach before retrying",
		name,
		interactionDoomLoopThreshold,
	), nil
}
...
func toolInvocationID(invocation interaction.ToolInvocation) string {
	return "tool:" + invocation.EffectID().String() + ":" + strconv.FormatUint(uint64(invocation.ToolCallIndex()), 10)
}
