...
func (ix *Indexer) Available(ctx context.Context) (bool, error) {
	...
	case err == nil:
		return true, nil
	case errors.Is(err, codebaseindex.ErrNoEmbeddingModel):
		return false, nil
	default:
		return false, err
	}
...
func (ix *Indexer) Search(ctx context.Context, cwd, query string, topK int) ([]codebaseindex.Hit, error) {
	...
	return codebaseindex.Rank(vecs[0], chunks, topK), nil
}

// Status reports cwd's current index state — the live in-memory status, falling
// back to the persisted meta on a cold process.
func (ix *Indexer) Status(ctx context.Context, cwd string) (codebaseindex.Status, error) {
	ix.mu.Lock()
	s, ok := ix.status[cwd]
	ix.mu.Unlock()
	if ok {
