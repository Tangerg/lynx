...
type Chunk = codebaseindex.Chunk
type Meta = codebaseindex.Meta
type Hit = codebaseindex.Hit

const (
	StateReady = codebaseindex.StateReady
	StateError = codebaseindex.StateError
)

var ErrNoEmbeddingModel = codebaseindex.ErrNoEmbeddingModel
...
func TestNoEmbeddingModel(t *testing.T) {
	ix := NewIndex(newMemStore(), func(context.Context) (Embedder, error) { return nil, ErrNoEmbeddingModel }, newMemSource())
...
func TestAvailabilityPreservesResolverFailure(t *testing.T) {
	...
	ix := NewIndex(newMemStore(), func(context.Context) (Embedder, error) { return nil, wantErr }, newMemSource())
