...
func registerInterrupts(registry *Registry) {
	...
	Query(registry, MethodMeta{
		...
		Stability: stable,
	}, func(router *Router, ctx context.Context, request protocol.ListInterruptsRequest) (*protocol.Page[protocol.PendingInterruptSet], error) {
		return router.api.ListInterrupts(ctx, request)
	})
}
