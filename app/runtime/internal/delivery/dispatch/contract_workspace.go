...
func registerWorkspace(r *Registry) {
	...
	// subscribing for the global skills / MCP events is always available
	// (AUX_API §1). The condition treats `watches: []` as "no watches", so an
	// explicitly empty list and an absent one behave alike.
	Stream(r, MethodMeta{
		...
		return d.api.SubscribeWorkspace(ctx, in)
	}, workspaceEventFramer)
}
