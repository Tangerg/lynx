...
// session, dispatching it, and steering the live turn — plus the run-boundary
// maintenance (effects factory + title) the delivery layer drives off it. The
// session/run lifecycle write-sets and single-writer admission gates that must be
// coordinated with a turn live on the sessions coordinator (see [Server.sessions]),
// and the session read projections on the queries coordinator (see [Server.queries]).
