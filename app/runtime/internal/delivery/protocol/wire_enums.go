...
var wireEnums = map[reflect.Type][]string{
	...
	reflect.TypeFor[RunOutcomeType]():            {string(OutcomeCompleted), string(OutcomeError), string(OutcomeMaxSteps), string(OutcomeMaxBudget), string(OutcomeCanceled), string(OutcomeInterrupt)},
	reflect.TypeFor[RunStatus]():                 {string(RunStatusRunning), string(RunStatusFinished)},
