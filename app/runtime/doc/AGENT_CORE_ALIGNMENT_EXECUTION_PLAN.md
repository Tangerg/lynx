...
- 测试与门禁：
  ...
  - `GOWORK=off go test -race -count=1 ./...`：通过；
  - `git diff --check`：通过。
- Commit / Push：`B8_IMPLEMENTATION_COMMIT`，待本批提交后回填并推送。
...
| B8 Runtime HITL | 已完成 | 2026-07-17 | 2026-07-17 | `B8_IMPLEMENTATION_COMMIT` | typed envelope；root/child approval/hooks；restart/cancel/run_lost；tree cleanup；workspace / standalone / race 全绿 |
