...
> 状态：**进行中，当前批次 B8 Runtime HITL envelope、恢复与取消集成**
>
> 建立日期：2026-07-16
>
> 审查基线：`92b4147a5afd7294a144196043d54858bb80b89d`
>
> 最近完成检查点：`124014513`（B7 完成记录）
>
> 当前进度：**7 / 10**
>
...
| 项目 | 当前事实 |
|---|---|
| 当前批次 | B8 — Runtime HITL envelope、恢复与取消集成 |
| 已完成 | B1-B7 |
| 待完成 | B8-B10 |
| 当前动作 | 审计 approval / question 从 Agent suspension 到 Run interrupt、resume、cancel 的完整链路 |
| 本批首要验收 | typed envelope；root/child approval 与 hooks 一致；nested HITL 可恢复；cancel/approve 竞态单胜者 |
| 下一批 | B9 — `agentexec` 职责与资源所有权收敛 |
...
| G-06 | P0 | **部分解决**：B6 已让 per-run provider/model、accounting、budget 与 cancel 覆盖委派树；approval、tool hooks / middleware 与 interrupt policy 留在 B8 | 委派树装配 | B8 |
...
| B8 | Runtime HITL envelope、恢复与取消集成 | B7 | 进行中 |
...
- 开始时间：2026-07-17
- 当前实际 scope：
  - 审计 Agent suspension 到 Runtime application `RunEvent` / pending interrupt 的翻译链路；
  - 审计 approval 与 question 是否仍依赖未判别 JSON shape；
  - 审计 tool observer middleware、approval policy 与 pre/post hooks 是否覆盖所有 child；
  - 审计 parked Run 的 root process tree restore、缺失 child 的 `run_lost` 映射；
  - 审计 approve、deny、cancel 的线性化与 terminal cleanup。
- 当前约束：
  - B7 已提供 generic nested suspension、递归 restore/resume 与 child snapshot 完整性验证，B8 不重复实现 Agent continuation；
  - Runtime 只在 adapter 防腐边界解释 approval / question，业务类型不得反向进入 Agent/Core；
  - `task` 在 per-tool gating 完成并由测试证明前继续保持保守风险等级；
  - 当前尚未形成 B8 完成结论，任何实现结果都必须在测试、race、standalone 门禁后补录。
- 新发现：待完成当前代码审计后记录。
- 与计划偏差：暂无。
- 公开 API 影响：当前无；若发现必须改变现行 wire shape 或破坏 exported API，按 §0.3 暂停咨询。
- 测试命令与结果：待实施。
- race 结果：待实施。
- standalone 结果：待实施。
- Commit / Push：待实施。
- 剩余风险：G-06 的 approval/hooks/interrupt policy、G-07 的 Runtime 集成部分、G-12 尚未关闭。
...
| B8 Runtime HITL | 进行中 | 2026-07-17 | — | — | typed interrupt envelope；approval/hooks；restart/cancel 集成 |
