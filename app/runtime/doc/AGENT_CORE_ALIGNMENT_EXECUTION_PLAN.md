...
- Runtime SQLite ProcessStore 已采用新版 Core 的 `Apply(ProcessSnapshotChange)` 契约（无 revision / CAS），旧 Agent Core 仍要求旧 `Save` 签名和旧 snapshot shape；
...
- 实际实现：
  ...
  - `Engine.Save` 对活动 nested tree 按 root → leaf claim 顺序捕获一致快照，release checkpoint 后再由 store 在单次 `Apply` 中持久化整棵树，防止 parent relation 与 child snapshot 来自不同时间点；
  ...
  - child session 按原 process id 与 parent conversation lineage 恢复：已有 session 保留内部 agent annotations，缺失 session 确定性重建；
...
- 行为测试：
  ...
  - child session lineage / agent annotations 恢复；
...
- 完成时间：2026-07-17
- 审计结论：
  - `agentexec.Engine` 当前混合七类变化理由：Agent process 执行、system prompt、steering 写入、turn-boundary maintenance、工具目录、MCP live 门面、技能目录与 capability closers；只有前三项中的 process/prompt/action 部分属于 execution 防腐层；
  - `turn.engineDep` 的五个方法中，只有 `StartTurn` / `RestoreTurn` 是 execution runtime；`InjectUserMessage`、`MaybeCompact`、`MaybeExtract` 的真实消费者都是 `turn.Dispatcher`，应由 Dispatcher 直接持有各自窄依赖；
  - 工具目录当前经 `toolset.Built.Tools → agentexec.Config.Tools → Engine.tools → Engine.Tools()` 重复转存；`task` 是唯一由 Agent runtime 创建的工具，应注入 toolset 自有 catalog，而不让 Engine 成为 catalog；
  - MCP 的 status/catalog/connection/registry 四个 live port 已由 `toolset.Build` 产出，Engine 的七个 MCP 方法只做 nil guard + 原样转发；`application/integrations` 应直接定义并消费这四片接口；
  - workspace skill discovery 只需要 `SkillsGlobalDir` + `promptsource.ListSkills`，经 Engine 中转没有 execution 语义；
  - `processStarter` / `processRestorer` / `processControl` 均只有 `agent/runtime.Engine` 一个真实实现；现有替换测试只制造具体 runtime 不可能返回的状态，不构成替换价值；
  - capability closers 当前由 toolset 创建、经 Engine 转存并由 `Engine.Close` 正序执行；这既让 Host 不是最终所有者，也违反本批要求的逆序关闭；
  - `Host.Close` 虽标注幂等，却使用值接收者且没有自身 once/error cache；幂等性依赖下游偶然实现，不能证明“唯一、逆序、幂等”。
- 实施边界：
  - Engine 只保留 Agent deploy/process create/restore/control、prompt/action 所需状态；删除 maintenance、MCP、catalog、skills、closers 中转面；
  - Dispatcher 分别接收 steering、compactor、extractor consumer-side 窄接口；
  - toolset Resolver 持有唯一 catalog snapshot，`UseTaskTool` 同时更新 role resolver 与 catalog；工具 registry 移入 toolset adapter 并直接消费该 source；
  - integrations 拆为 status/catalog/connection/registry 四片 consumer-side live port，bootstrap 直接注入 `toolset.Built`；
  - workspace skill catalog 由 bootstrap 直接绑定 promptsource；
  - Engine 直接持有唯一具体 `*agent/runtime.Engine`，删除三组单实现 process interface；
  - Host 直接持有 `built.Closers`，按依赖逆序且 once-only 关闭；bootstrap 后续失败仍回收已经创建的 tool resources，但不提前接管调用方 `Config.Resources`。
- 公开面影响：仅调整 `app/runtime/internal` 下 exported construction shape（`agentexec.Config`、`turn.Dependencies`、`integrations.Config`、`Host.Close` method set）；不改 Agent/Core exported API，不改 Runtime wire。该变化已由本执行文档 B9 目标和用户“严格按文档推进”明确授权，不保留旧字段或代理方法。
- 实际实施：
  - `agentexec.Engine` 直接持有唯一 `*agent/runtime.Engine`，删除 `processStarter` / `processRestorer` / `processControl` 及其不可能状态测试；公开方法收敛为 `StartTurn`、`RestoreTurn`、`ResumableProcess`，system prompt 仅保留包内 action 使用；
  - 从 Engine 与 `agentexec.Config` 删除 steering、compactor、extractor、tool catalog、MCP、skills 和 closers 中转；`turn.Dispatcher` 直接持有 execution、steering、compactor、extractor 四片 consumer-side 依赖；
  - maintenance adapter 直接实现 turn-owned result/port 契约；workspace skill discovery 由 bootstrap 直接绑定 `promptsource.ListSkills`；
  - tool catalog 归 `toolset.Resolver`，`UseTaskTool` 原子更新 role resolver 与 catalog；diagnostic registry 移入 toolset 并直接消费具体 Resolver，不再经过 Engine；
  - MCP status/catalog/connection/registry 接口归真正消费者 `application/integrations`，toolset adapter 实现，bootstrap 直接装配；`agentexec/toolport` 只保留 Agent execution 所需的 ToolResolver；
  - `bootstrap.Host` 以共享不可变 `hostLifetime` 保存原始 shutdown graph，保证 Host 副本、并发 Close 和 Stack 改写下仍关闭同一组资源；关闭顺序固定为 integrations → codebase → active runs → active turn tree → boundary effects → tool closers 逆序 → process resources 逆序；
  - bootstrap 采用分阶段 ownership guard：tool environment 与 Dispatcher 只有在 Host 构造完成后才转移；任何后续失败均按依赖逆序回收，调用方 `Config.Resources` 只在成功后转移。
- 新发现：
  - 原 Host 的 value receiver + 下游偶然幂等不足以证明跨副本唯一关闭；仅共享 `sync.Once` 仍会让不同副本携带可分叉的资源字段，因此关闭图必须与 once 一起共享；
  - `*Resolver(nil)` 注入接口会形成非 nil typed-interface，可能以 nil Agent extension 失败；装配边界现显式消除 typed-nil，registry 改为直接接收具体 Resolver；
  - MCP HTTP 集成测试原先先关 server、后关 SSE client，会让 `httptest.Server.Close` 等待活跃连接；清理改为后创建客户端先关闭，符合逆序释放原则。
- 与计划偏差：无；所有变更均在 B9 已记录 ownership 边界内，B8 行为契约和 Runtime wire 未改变。
- 测试与门禁：
  - `MODULE=app/runtime scripts/check.sh build vet test lint`：通过；
  - `go test -race -count=1 ./...`：通过；
  - `make check-standalone`：通过；
  - `GOWORK=off go test -race -count=1 ./...`：通过；
  - `git diff --check`：通过；
  - 新增 Engine method-set / Config proxy fitness test、application/delivery concrete Engine import guard、Host 全关闭顺序与并发跨副本幂等测试、engine/post-dispatcher bootstrap failure 回收测试、toolset registry/MCP ownership 测试。
- Commit / Push：`e945323c1`，实现与完成记录已推送。
- 剩余风险：G-14、G-15 已关闭；B10 只同步全局架构/扩展文档与最终 fitness/残留清理。
