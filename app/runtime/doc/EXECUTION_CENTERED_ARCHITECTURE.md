...
func (c *Coordinator) Start(ctx context.Context, spec StartSpec, projector func(SegmentView) Projector) (<-chan Event, error)
func (c *Coordinator) Subscribe(ctx context.Context, runID, after string) (<-chan Event, bool)
func (c *Coordinator) BeginCancel(ctx context.Context, runID, reason string) (...)
```

`Coordinator` 是内部焊死的具体类型，不为它抽没人实现的 SPI。

`Executor`、`Projector`、`Effects` 等 port 由 `application/runs` 定义，因为该包是消费者。Resume 不另造一个平行协调器：delivery/lifecycle 先解析 parked handle，再用带 `Activate` 的 `StartSpec` 打开同一条 segment 管线；`Activate == nil` 表示 fresh，避免 `Resume bool` 与 callback 组合出非法状态。
...
Application 在 `application/runs` 定义唯一的、封闭的 transport-neutral `EngineEvent` 事件族：
...
`turn` adapter 直接发出这套 application event；它只用 type alias 保留本地短名，不维护第二份 lockstep struct。这样 delivery 的 `Projector` 只依赖 `runs.EngineEvent`，不会再把 `execution.Event` 静默 downcast 回 `turn.Event`。Interrupt 也是 typed union（approval/question），失败的 payload 不能再退化成空卡片。

Journal 上的 `runs.Event` 仍是 transport-neutral 信封（RunID / SegmentID / Seq / Timestamp / durability / terminal），但 Payload 不再是无界 `any`，而是 `runs.Projection` marker；`protocol.StreamEvent` 是 delivery 的实现。monotonic cursor 由 `runs.Coordinator` 自持，delivery 只做 `evt_` framing。
