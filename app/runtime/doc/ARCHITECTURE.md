...
- Agent2 是唯一托管执行内核，Runtime 不复制它的 Process loop、tree scheduler 或 snapshot 解释器。
...
Run 生命周期是 Application 的应用事务边界和 Delivery 的产品观察边界。Agent2 Process 是执行实现，不是产品生命周期本身。
...
Agent2 Engine 是 Process 和 Process tree 的唯一推进者。Runtime 只发出开始、恢复、语义输入、取消等请求，并消费已发生事实；不得在外部再建一套 Turn controller、Tool loop、child scheduler 或 continuation state machine。

### 2.5 事务属于 Application

跨 Run、Session、Transcript、Interrupt、Goal 和 checkpoint 的原子写入由具体应用用例拥有。Domain 保护单个模型的不变量；Adapter 执行消费方端口；Agent2 不认识 Store、transaction、CAS、lease、schema epoch 或业务幂等。

### 2.6 防腐而非转发

Adapter 必须翻译两侧语义。只把 Agent2、SQLite 或 Provider 的类型原样转发给内环，不叫适配，而是抽象泄露。
...
| 术语 | 唯一含义 | 明确不表示 |
|---|---|---|
| `Session` | 一段持久产品会话，拥有 workspace 绑定、默认模型和根 Run admission 约束 | Agent2 Process、网络连接 |
| `Run` | 用户看到的一次完整逻辑执行，跨中断与恢复保持同一身份 | 一次模型调用、Agent2 Execution |
| `Segment` | 一个 Run 中连续输出的一段；首启和每次恢复分别打开新 Segment | Run、Agent2 Step |
...
| `Interrupt` | 一个 Run tree 等待外部答案或裁决的产品请求 | Agent2 Signal payload 本身 |
| `Plan` | 当前工作请求的步骤状态 | Goal、Todo 的别名 |
| `Goal` | 跨多个 Run 持续推进的自主目标 | 当前 Run 的 Plan |
| `Executor` | Application 消费的、实现 Run 执行能力的端口 | Agent2 Engine 的别名 |
| `Executor member` | Application 为路由或产品 child Run 映射保存的不透明执行树成员身份 | 可由 Application 解释的 Process 对象 |
| `Checkpoint` | Application 拥有的恢复聚合，包含 Host metadata 和不透明 executor payload | Agent2 snapshot 的第二份可解释模型 |
| `Run event` | Application 已提交并可向 Delivery 发布的产品事实 | Agent2 Event 或 Delta 的原样转发 |
...
- `interaction.WorkingContext` 是一个 Agent2 Process 精确恢复所需的私有执行状态。
...
Agent2 Event 只承载 Framework 生命周期事实；Delta 是有界、可能丢失、恢复后不重放的临时输出。Runtime 不得用 Delta 拼接最终结果或构建权威 Transcript。

每个外部调用使用一个 dispatch-attempt scope 串起 outer Agent2 Dispatcher decorator、model client decorator 和 Tool decorator。调用前的 authoritative `started` write 必须先成功；失败时不得调用外部能力。外部调用一旦开始，最终响应/结果/usage 的 durable write 失败就必须由 outer Dispatcher 作为 indeterminate outcome 返回，使 Agent2 记录 unknown settlement；不能把“外部已成功、Runtime 写盘失败”伪装成普通 Tool error 或模型失败并允许自动重试。chunk/progress projection 失败只产生可观察 drop，不改变 Effect settlement。
...
- Agent2 `TreeSnapshot` 的不透明编码。

只有 `adapter/agentexec` 可以编解码 TreeSnapshot 外层并调用 Agent2 的校验/恢复 API。任何 Runtime 层都不能解析其中的 Process snapshot、ExecutionState payload、Interaction phase、mailbox 或 Strategy protocol。
...
它不拥有 executor reference/checkpoint、Conversation 内容、Transcript Item 排序、Agent2 Process 状态或 Store 操作。executor binding 是 Application 为驱动外部执行端口保存的应用状态，不是 Run entity 的领域属性。现有 `domain/execution` 将在重构中治本改名为 `domain/run`，不保留 alias。
...
`accounting` 拥有模型 token、调用次数、价格和产品预算值。Agent2 Usage 只提供 Steps、Effects、Signals、Delta 等框架资源事实，不能替代产品 accounting。
...
- Agent2、SQLite、JSON-RPC、HTTP、OTel SDK、文件系统和外部 Provider SDK；
...
- 不导入 Adapter、Infra、Delivery、Bootstrap、Agent2 或外部驱动 SDK；
...
Delivery 只依赖 Protocol、Application 和必要的 Domain projection values；不得导入 Agent2、Infra、具体 persistence、agentexec 或持有 Run lifecycle state。
...
生产代码中，Agent2 import 应收敛到 `adapter/agentexec` 及其明确的 Framework-integration 叶子。Domain、Application、Delivery、Infra 和通用 Toolset 不得导入 Agent2。
...
每棵 root Run tree 使用独立 Engine、精确 root/child Deployment 集和 caller-owned resolver。当前迁移不需要 Agent2 Platform；只有产品出现真实的多 Deployment 发布、版本选择和治理用例时才引入。
...
```text
Agent2 pending input + quiescent tree
...
Managed Delegate 的 child Process 是 first-class child Run 的来源。模型响应中的 Delegate ToolCall 必须先 durable commit；Agent2 `ProcessAdmitter` 随后在 Process 发布前调用 Application-owned admission use case，用 prospective identity 和同一 Delegate child key 原子创建不可见的 child opening reservation、产品 child Run identity 和父因果 binding。`EventProcessStarted` 只作为对账唤醒；agentexec 验证 live Process/关系后产生 executor fact，Application 提交后才把 child Run 公开为 Running。

Admission 成功不等于 Process 已启动。root 的 `Engine.Start` 失败由直接调用者把已存在 Opening Run 终结为 start failure；进程崩溃后，任何没有 checkpoint/started fact 的 Opening root 都按 recovery loss 收口。child 则必须从 Framework 得到 prospective identity 对应的 conclusive started/aborted outcome，或使用“admission 后 publication 不再失败”的中性 reservation 合同。Agent2 Baseline 9 尚未提供这条事实，因此 P7 在补齐并冻结该 Framework 合同前不得接入 durable child Run admission；Runtime 不用超时、私有 ID 算法或父 Effect 顺序猜测结果。

不是所有 Framework child 都自动成为产品 Run。Planning/Workflow 的内部 child 是否投影为产品 Run，由 agentexec 根据 exact Deployment 和组合语义决定；不得把产品 `Run` 标记塞入 Agent2 ProcessRelation 或 ChildSpec。
...
1. agentexec 在完整 tree safe boundary 取得 prepared change；该 capability 在 `Apply` 或 `Discard` 前冻结 source tree，但不暴露 Agent2 plan、lock 或 snapshot type；
...
4. durable commit 成功后调用 `Apply`，把仍被冻结的 live tree 线性化到已持久化的 resulting state；
5. 进程在 commit 后、Apply 前崩溃时，重启直接恢复已提交 resulting checkpoint；Apply 跨过 Framework commit gate 后即使调用 context 取消也必须完成；若仍出现无法证明的 apply 失败，销毁旧 live tree并从 resulting checkpoint 恢复，恢复失败则 `RunLost`。

prepared change 必须 one-shot、`Discard` 幂等且有 Host-owned deadline；agentexec 取得后立即注册 `defer Discard`，只有 Apply 成功才解除。Framework 不允许因调用方遗漏或 transaction 卡住而无限冻结 tree。

当前 Agent2 `WaitingSubtreeCancellationPlan` 是纯 prospective value，规划返回后会释放 tree quiescence，不能单独满足步骤 1 的应用原子性。因此 P7 前必须先在 Agent2 增加中性 prepared-change capability；Runtime 不持久化 plan、不复刻 source digest，也不重新引入含糊的通用 Mutation lease。

### 7.6 终态翻译

Run 终态不能由 `error` 文本或 `context.Canceled` 猜测。agentexec 同时读取 Application 已记录的控制意图和 Agent2 `Termination`，再产生应用事实：

| Agent2/Host 事实 | Runtime 产品结果 |
|---|---|
...
同一 Agent2 cause 在 root/child 和不同 Application intent 下可以映射为不同产品细因，因此映射属于 agentexec + Application 合作边界，不下沉 Agent2，也不散落在 Delivery。
...
Application port 只表达产品真正需要的执行能力，不能镜像 Agent2 Engine 的全部方法。
...
- Agent2 owns Process/Effect/child loop；
...
Agent2 Event/Delta listener 都是 observation/wake-up 边界，不是 Application durable commit callback。listener failure/panic 不会回滚 Framework 状态，因此 Application 不能仅因“收到一个 Event”就假定产品事实已经可靠提交。per-Run pump 必须通过 Agent2 public Process status/result、Strategy public helper 和 quiescent complete-tree capture 对账 waiting/terminal；Event 丢失后仍能在下一次 wake、control 或 recovery reconciliation 收敛。terminal truth 最终来自 `Process.Await`/`Result`，不是 Event payload。
...
- Domain、Application、Delivery、Infra 和通用 Toolset 对 Agent2 零 import；
...
- 不把 Runtime 的 transaction、approval、pricing、history、workspace 或 Run 类型下沉到 Agent2；
- 不把 Agent2 Process、Signal、Effect、Strategy payload 暴露到产品协议；
