...
- Goal lifecycle 与当前 Run command 在同一中栏具有明确作用域：Goal banner 的可见 stop/resume 文案包含 Goal 身份，composer 的 Run stop 保持简洁；两者继续调用各自既有 command owner，不靠重复按钮名或 aria-only 补丁隐藏命令差异。
...
P119 当前受影响 Frontend 全量为 321 files / 1994 tests，typecheck/lint/format/knip/circular/context/API/style/design/token/locales/bundle 门禁全绿；其中 production Composer→agent bridge 的中文输入法混合文本提交序列已在 Chromium 与 WebKit 闭环，Markdown table inline/preview 明暗 golden、RTL/CJK、18px smoke 同步通过。KaTeX lazy CSS 门禁确认启动 CSS 从 136.1KB 降至 107.0KB，未放宽预算。该证据不包含 P119 最终 `--detectAsyncLeaks`、三栏完整矩阵、production package/codesign 与 fresh DB/renderer/Runtime SIGKILL 封板。
...
- P118 完成定义内没有已知未闭环项。P119 尚待最终 async-leak、三栏完整 visual/WCAG/keyboard/IME/CJK/light-dark/Retina/WebKit、Wails production package/codesign 和 fresh DB/renderer/Runtime SIGKILL 封板；commentary/final answer phase 与原生图片 Download 仍分别等待 Runtime public surface、Desktop binding 的显式边界授权。
