...
- P114-02c 已关闭 Runtime event opening 的非终止恢复状态：HTTP/SSE fetch、首个 JSON-RPC response frame或 workspace resolution若永不结算，旧 loop没有 opening deadline，因而不会 abort request、上报 disconnect、进入 reconnect或触发 successor snapshot。现在 `WorkspaceEventLoop` 为每次 opening拥有独立10秒 handshake deadline；timeout先以稳定机器错误终结 deadline，再 abort exact attempt，使 RpcClient pending correlation和stream资源沿同一 signal释放，随后只走既有 connection verification与有界backoff owner。忽略取消的迟到 opening会被观察并关闭其 iterable；一旦 opening成功，deadline loser立即显式release，accepted stream继续由generation signal长寿命拥有，绝不在10秒后被误杀。暂时失败后的下一次 opening成功仍先建立tail再执行唯一`invalidateAll`，因此不会永久停在retired read/pending loading，也没有 snapshot-before-tail旁路。Codex Rust exec-server的`RpcClient::call_with_timeout/call_inner`在deadline后删除exact pending request，且pending注册与disconnect检查同锁、reader断线按接收顺序drain pending；本批吸收“opening必须有有限终态、deadline只清exact pending capability、accepted resource另有lifetime”，不复制其多进程transport task、call-slot semaphore或cleanup overflow策略；
- P114-02d 已关闭 HITL resume 在 Runtime 换代与成功回执丢失交错下的永久 submitting：旧 `runs.resume` opening 被 generation retirement 正确撤权后，若 durable transaction 未提交且 successor snapshot仍包含原 atomic interrupt barrier，旧 callback不会再调用 coordinator，原 response batch过去会永远持有按钮 latch。现在充血 `InterruptResponseBatch` 同时拥有 staged responses、submission generation、successor recovery boundary与迟到 settlement identity；Session material显式区分任意 `viewRevision` 和只由 durable authoritative snapshot commit推进的 `authoritativeRevision`。generation retirement本身只建立等待基线，不允许重复提交；successor snapshot提交后，barrier消失表示 continuation已由权威 material接管，barrier仍在则整批回滚为可重试。live event、command error与optimistic write不能伪装成 mutation settlement，旧 accept/reject仍受 exact batch identity隔离。只读对照 Codex Rust app-server：running thread resume先发送权威 response/Goal snapshot，再按 RequestId顺序重放 pending server requests，client response只 `take` exact callback；本批吸收“权威恢复后再开放 pending interaction、response只结算原 identity”，不复制其多 connection、server-to-client request replay或 thread listener package形状，因为 Lyra单 client/server以 SQLite pending projection和 durable `runs.resume` mutation为唯一产品路径；
- P114-03a 已关闭 Agent Memory mutation、串行队列与 TanStack cache projection 的跨代拼接：原模块级 `memoryChanges` 在 Plugin Host/Runtime replacement 后仍保留旧 admission，旧响应可写共享 successor cache，同键排队命令还会在真正执行时动态取得 successor gateway。现在充血 `AgentMemoryMutationOwner` 以 exact Plugin Host installation 和 Runtime generation 拥有固定 gateway、按 memory identity 分区的 command tail、retirement signal、response commit 与 query repair；successor-first replacement 同步拒绝旧代在途及排队命令，非协作迟到响应只被观察而无提交权，stale installation lifecycle 不能轮换或释放 current generation。accepted mutation response 是该命令已证明的权威事实，cache repair 失败不再反向伪造 command failure；UI 仅把 generation retirement 当作中性的生命周期结算。Workspace events 通过公开 setup-time lifecycle port 驱动 owner replacement，没有反向依赖 workspace 私有 Application，也没有全局串行瓶颈、延时、刷新绕过或第二 cache。Codex Rust `remote_file_system`、`client_recovery` 与 search owner 的 `Arc::ptr_eq` exact-current cleanup/recovery 规则被吸收为 admission/settlement capability；其共享请求缓存、多连接 recovery 与 filesystem/search 产品形状没有进入 Lyra；
- P114-03b 已关闭 Provider 配置、utility/embedding role、live probe 与四组 cache projection 的跨代拼接：三个模块级 serial queue 和动态 current gateway 已删除；充血 `ProviderMutationOwner` 以 exact Host/Runtime generation固定一个 adapter/client capability，按 provider identity、utility role和embedding role分别串行，probe同样受代际 settlement约束。不同 provider不再被全局阻塞；successor先建新generation再retire旧代，旧在途/排队command及probe立即失去提交权，stale installation callback不能轮换successor。authoritative provider/role response由owner提交准确resource，后续query repair失败不伪造command失败；Runtime connection generation通过真实Plugin service订阅直达owner，UI的save/role/probe把retirement结算为空闲而不展示机器错误。Codex Rust `client_recovery` 只允许exact current `RpcClient`发起recovery，connector metadata store将cache绑定backend/auth scope且拒绝较弱迟到material覆盖较强current记录；本批吸收exact transport scope、resource-aware commit与stale settlement no-op，不复制其多连接recovery、account partition或process-global connector cache；
- P114-03c 已关闭 MCP Server 配置、授权轮询与 cache projection 的跨代拼接：模块级全局队列、动态 current gateway 与游离 poll timer 已删除；充血 `MCPServerMutationOwner` 以 exact Host/Runtime generation 固定 gateway/client capability，按 server identity 串行 CRUD，并统一拥有 authorization attempt id、caller+generation cancellation、poll timer、authoritative response commit 与 query repair。Host replacement 和真实 Runtime generation 变化都先建立 successor 再同步退役 predecessor；旧在途/排队 command、授权网络请求及 timer 立即失去提交权，旧 attempt id 不会被拿到 successor transport 轮询，stale installation lifecycle 不能轮换或释放 current owner。不同 server 可独立推进，accepted response 不因后续 cache repair 失败伪装成 command failure，UI 将换代结算视为 neutral idle，JSON 批量导入只报告已 durable 接受的事实。Codex Rust `client_recovery` 的 exact `Arc::ptr_eq` recovery gate，以及 connection cancellation token 对 pending server decision 的共同 lifetime 被吸收为 exact transport/attempt settlement 不变量；其多连接、network-policy server request 与 callback registry 没有进入 Lyra 单 client/server 产品；
