...
- P113–P120 已完成；Work Index、Agent Narrative、Composer standing stack 与 Context Dock 的 Codex 深度对齐、权威前端接线和 production renderer/Runtime 恢复验收均已闭环。
...
- Goal lifecycle 与当前 Run command 在同一中栏具有明确作用域：Goal standing row 只呈现 lifecycle、objective 与 pause/resume，预算、额度、花费、步数、model 与 last move 不成为永久 chrome；composer 的 Run stop 保持简洁。两者继续调用各自既有 command owner，不靠重复按钮名或 aria-only 补丁隐藏命令差异。
...
- Plan 只在 active Run 期间以环形进度和“第 N / M 步”pill 呈现；完整 Session plan 由同一 projection 在 hover/focus tooltip 展开，不复制成 disclosure card、progress bar 或第二 expanded state。Plan 与 Goal 均位于 composer overlay，空 contribution 不留下固定边线或高度。
...
P120 Frontend 基线为 322 files / 1993 tests；Plan/Goal/Context/normal completion 的独立红例与 production owner 单元测试全绿。agent browser 矩阵新增真实 Context tooltip 与 Plan/Goal composer-stack 几何守卫，并刷新标题栏、普通终态及 light/dark golden；完整 visual/WCAG/keyboard/IME/CJK/light-dark/Retina/WebKit 矩阵 312 tests 全绿，typecheck/lint/format/knip/design/token/chrome/locales 门禁保持全绿。P119 的完整 Runtime/Desktop/Wails recovery 基线未被修改，P120 也没有改变 Protocol、Artifact、SQLite schema、公共 Go API 或 Agent Framework baseline。
...
- P120 完成定义内没有已知未闭环项。commentary/final answer phase 与原生图片 Download 仍分别等待 Runtime public surface、Desktop binding 的显式边界授权，因此未由 Frontend 猜测或伪造接线。

## 11. 当前结论

P0–P120 已把主要缺陷从“调用处补判断”上移到领域不变量、Application transaction、进程/renderer generation、credential lifecycle、read-model 与 presentation owner；P120 又把中央 Plan/Goal/Context/terminal narrative 从“移动旧 chrome”收敛为 Codex 的紧凑 standing grammar，并让 Context 占用只消费真实 Runtime footprint。后续工作必须从真实产品反例开始；若不能说明唯一 owner、提交能力和失败后的 durable winner，就不能以新增 helper、刷新或兼容路径进入生产代码。
