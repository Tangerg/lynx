...
| 能力 | 消费方端口 | 默认实现/来源 |
|---|---|---|
| 长期知识 | `domain/knowledge.Store` | `infra/storage.FileKnowledgeStore` |
...
- `adapter/agentexec/turn` 自己定义 unexported 两方法 `engineDep`，因为 Dispatcher 才是
  Start/Restore 的消费者；
- `turn.Dispatcher` 是 process-control consumer boundary；不要再在 application、
  delivery 或 bootstrap 外面套一层 Manager/Facade；
- `turn.Executor` 是把 Dispatcher 投影为 application/runs ports 的具体 adapter；
...
- `adapter/agentexec.Engine`、`adapter/agentexec/turn.Executor`；
...
- 不给 coordinator、Engine、managed interaction、transport 或 Dispatcher 再套换名 facade；
