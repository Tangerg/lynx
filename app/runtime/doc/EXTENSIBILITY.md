...
> 当前目录命名：`internal/kernel` / `internal/domain` / `internal/delivery` / `kernel/turn`（2026-06-14 重命名落地，见 [`GREENFIELD_ARCHITECTURE.md`](GREENFIELD_ARCHITECTURE.md) §9）。
...
| **上下文压缩** | `kernel.Compactor` | `maintenance.Compactor` | 外部压缩服务 |
| **事实提取** | `kernel.Extractor` | `maintenance.Extractor` | 外部记忆抽取 |
| **规划** | `engine.Planner` | `maintenance.Planner` | 外部 planner |
| **转向注入** | `kernel.SteeringSink` | `conversation.Messages` | 少见，但可换 |
...
| **会话 / 时间线 / 中断 / provider 存储** | `session.Store` / `transcript.Store` / `interrupts.Store` / `provider.Registry` | sqlite 实现 | 可换后端（语义仍 lyra 专有，主要为测试+后端替换） |

**怎么注入外部实现**（关键——SPI 必须能被组合根塞进去）：
kernel 消费的端口在 `kernel.Config` 暴露为接口字段；`runtime.New` **仅当字段为 nil 时**才建内部默认实现，否则**尊重注入值**。所以：

```go
// 组合根（cmd/lyra 或测试）：塞一个 mem0/HTTP 桥接的实现
eng := engine.Config{
    Knowledge: myMem0KnowledgeStore,     // 实现 knowledge.Store
    Compactor: myHTTPCompactor,          // 实现 kernel.Compactor
...
- ❌ **把可替换端口焊死**（直接 `import` 具体 `maintenance.Compactor` 去消费，而非经 `kernel.Compactor` 端口）。
