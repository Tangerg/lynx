...
P63 已令 maintenance compaction 在最后一轮、短历史、超大 recent suffix 与空摘要下保持收敛且不丢历史。下一轮继续反证 run/goal boot recovery、subscription close/retarget、跨 Session 晚到事件、事务失败和真实崩溃恢复，重点检查“提交成功但返回错误”与“启动 reconcile 返回成功但状态未落地”两类不可观察后果；每轮提交前执行 Runtime→Agent/Agent→Runtime 双向依赖与 staged path 审计。
