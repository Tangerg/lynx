...
P66 已关闭 child-start callback ledger 在正常终态、Session 生命周期与进程强杀后的永久泄露，并保持所有回收与 owning write-set 原子。下一轮继续反证跨 Session 晚到事件、stream opening/close 的资源所有权、事务 commit 结果与真实进程强杀恢复；每轮提交前执行 Runtime→Agent/Agent→Runtime 双向依赖与 staged path 审计。
