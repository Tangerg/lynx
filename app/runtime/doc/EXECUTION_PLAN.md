...
> 状态：P1–P82、P83-01–P83-18 已完成；发布后反证审计持续进行
>
...
P83-18 已让始终挂载的 Session sidebar read、默认 Runtime Data Provider、SDK pagination 与 transport 共享唯一 Query generation signal：`sessions.changed`、global replacement 或 renderer teardown 都会同时释放旧首页/continuation correlation 与 fetch，迟到分页不能覆盖 successor。P83 后续继续按 1–2 client 产品模型审计默认 Runtime Data Provider；下一步优先核对 Workspace project catalog，因为它由已提交的 Session 列表事实驱动 refresh，但 `projectIndexRefresh` 仍直接 invalidation，provider/SDK `workspaces.list` 也尚未传递 cancellation，首次无缓存读取可能同时吞掉 committed refresh 并保留旧 transport owner。随后按真实挂载频率扩展到 workspace/settings reads，继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
