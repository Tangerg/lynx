...
P65 已令 Run boot recovery 在墙钟回拨下仍按 durable high watermark 单调终结，并关闭 HITL park/resume 期间旧 Segment 的寻址与 registry 删除竞态。下一轮继续反证事务提交结果不确定、跨 Session 晚到事件、stream opening/close 的资源所有权和真实进程强杀恢复；每轮提交前执行 Runtime→Agent/Agent→Runtime 双向依赖与 staged path 审计。
