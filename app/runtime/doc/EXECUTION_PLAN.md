...
- 三栏 Codex 对照审计已进入中央 transcript：Codex `thread-scroll-layout` 以底部距离为权威读数，footer 高度由 ResizeObserver 驱动，并在 wheel/pointer/keyboard 用户滚动时立即退出自动跟随；采用这一 owner/escape 机制，拒绝其 reverse-scroll DOM 形状，因为 Lynx 既有 `use-stick-to-bottom` 已拥有等价的距离与用户逃逸语义。Lynx 当前额外用固定 250ms RAF 窗口反复写 `scrollTop = scrollHeight`，真实反例是在长会话刚打开后用户向上滚动，下一帧仍被抢回底部；失败测试已锁定 `1000` 覆盖用户选择的 `240`。唯一修复 owner 是 `MessageStream` 的 mount geometry reconciliation，不能用更长 timer 或第二 scroll state 掩盖。
