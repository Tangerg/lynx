...
- File preview 的相邻滚动红例确认：plain DOM 已含目标行，导航 effect 会先居中一次；Shiki 异步 materialization 改变 `highlighted` 后又重放同一滚动，能覆盖用户在这段时间内取得的阅读位置。语法 materialization 不是新的 file navigation intent；红例要求它只替换 token，不再调用 `scrollIntoView`。
