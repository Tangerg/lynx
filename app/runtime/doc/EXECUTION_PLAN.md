...
> 状态：P1–P82、P83-01–P83-06 已完成；发布后反证审计持续进行
>
...
P83-09 已让客户端保存的 mutation identity 在每次请求 admission 时由当前 Runtime store 自证，关闭 discovery 校验后的同 endpoint replacement TOCTOU；mismatch 在 claim 和业务执行前失败，不会在新 store 上重放旧 intent。P83 后续按 1–2 client 产品模型继续验证命令响应丢失、重复恢复、Runtime restart 与 renderer replacement 的交错，尤其检查重连 snapshot、已挂载 HITL/Plan/Goal/Run/Tool read model、迟到/乱序事件以及 mutation settlement 是否仍只有一个 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
