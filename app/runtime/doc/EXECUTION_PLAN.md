...
> 状态：P1–P82、P83-01–P83-11 已完成；发布后反证审计持续进行
>
...
P83-11 已把 reattach opening 的迟到资源纳入 Runtime generation ownership：replacement 可立即释放旧 snapshot，迟到返回的 event iterable 会被主动关闭且不能接管 successor live projection。P83 后续按 1–2 client 产品模型继续验证命令响应丢失、重复恢复、Runtime restart 与 renderer replacement 的交错，下一步优先反证 active Run stream 断线后的 `runStreamReattach` opening 是否也可能因不合作取消保留旧代 ownership，并继续检查迟到/乱序事件及 mutation settlement 是否仍只有一个 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
