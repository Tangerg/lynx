...
P71 已让优雅关闭在 SQLite/Host/lease 释放前排空 Delivery 已知 outcome，失败保持可重试 ownership，并以 durable first result 统一当前进程与重启后的 replay。下一轮继续处理跨进程 crash 留下的 empty claim：完成结果仍按 24 小时保留，但没有可证明 outcome 的 reservation 不能因时间流逝自动变成“可重新执行”，否则同 key 在 crash 后可能第二次运行业务 handler/invalidation；每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
