...
> 状态：P1–P82、P83-01–P83-19 已完成；发布后反证审计持续进行
>
...
P83-19 已让 Session-derived Workspace project catalog 在 committed projection refresh 时先退休旧 cache writer，并让默认 Data Provider、SDK `workspaces.list` 与 transport 共享唯一 Query generation signal；首次无缓存 read 不再吞掉一次性 Session commit，旧 correlation/fetch 与迟到结果也不能越代。P83 后续继续按 1–2 client 产品模型审计默认 Runtime Data Provider；下一步优先核对始终挂载的 Header diff stat：`files.changed` 与 global replacement 已通过统一 policy 替换 `WORKSPACE_FILES_CHANGED_KEY` writer，但 provider/Workspace SDK `workspace.changes.list` 仍未传递 cancellation，旧文件状态请求会在 cwd retarget、Runtime replacement 或 committed file event 后继续占有 transport generation。随后按真实挂载频率扩展到 file tree/read/diff/search 与 settings reads，继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
