...
P67 已关闭 Run journal authoritative overflow 在 consumer 尚未开始 range 时的 subscriber registry 永久保留，并反证 Desktop 跨 Session/Segment 晚到帧已有唯一 lifecycle/identity owner。下一轮继续反证 stream opening 失败、transport end、reattach 与 Runtime Close 交错时的 exact teardown，以及事务 commit 结果在旁路事件和消费者投影中的一致性；每轮提交前执行真实 HITL/Plan/Goal、Runtime→Agent/Agent→Runtime 双向依赖与 staged path 审计。
