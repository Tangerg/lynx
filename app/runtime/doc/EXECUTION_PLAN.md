...
P80 已闭合 Desktop composition、RPC receive loop、deadline race loser 与 UI/query 测试夹具的异步资源所有权，并以完整 async detector 证明 278 files / 1729 tests 零泄露。下一轮 P81 继续从真实 renderer/window 退出、Runtime 中途重启与旁路事件重连切点反证：验证异步 dispose 无法被 unload 等待时 replacement generation 仍可安全接管，所有已挂载 Session/Run/HITL/Plan/Goal/Workspace read model 经唯一 event supervisor 与 cold read 收敛；同时继续枚举未覆盖的 API 组合、失败注入与并发顺序。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
