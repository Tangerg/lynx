...
> 状态：P1–P82、P83-01–P83-16 已完成；发布后反证审计持续进行
>
...
P83-16 已让常驻 pending-work 的 install-wide `interrupts.list` 首/续页共享 Query generation signal：event replacement、Runtime replacement 或 renderer teardown 都能释放旧 RPC/HTTP owner，而不是只 fence cache 写回。P83 后续继续按 1–2 client 产品模型审计绕过通用 Data Provider 的 mounted query；下一步优先核对 Session usage chip 与跨 Session Usage pane 的 queryFn 是否把 cancellation 贯穿 `usage.session` / `usage.summary` transport，并验证 `runs.changed`、global replacement 与 unmount 交错下旧统计 read 不保留 correlation 或覆盖 successor。随后扩展到其余默认 Runtime provider，继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
