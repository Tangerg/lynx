...
> 状态：P1–P82、P83-01–P83-21 已完成；发布后反证审计持续进行
>
...
P83-21 已让 FileTree 根目录/展开目录的参数化 Query、默认 Data Provider、bound paged Workspace SDK 与 transport 共享唯一 generation signal；cwd/path retarget、`files.changed`、global replacement 或 renderer teardown 都会释放旧首页和全部 continuation，迟到旧目录分页不能越代。P83 后续继续按 1–2 client 产品模型审计默认 Runtime Data Provider；下一步优先核对文件预览 `workspace.files.read`：用户快速选中另一文件或切换 active cwd 时 query key 会改变，但 provider/bound SDK 尚未传递 cancellation，旧大文件读取会在新预览已显示后继续占有 transport owner。随后扩展到 diff/search/file-head 与 settings reads，继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
