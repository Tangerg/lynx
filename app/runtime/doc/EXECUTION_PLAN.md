...
P83-10 已把 mounted Session durable snapshot 纳入 Runtime generation ownership：global replacement 可立即退休旧 items/runs/interrupts/plan 读取，successor 统一重建 HITL/Plan/Run/Tool，Goal query 与它共享同一 resync boundary；迟到旧代结果不能写回。P83 后续按 1–2 client 产品模型继续验证命令响应丢失、重复恢复、Runtime restart 与 renderer replacement 的交错，尤其检查连续重连、snapshot 后 reattach、迟到/乱序事件以及 mutation settlement 是否仍只有一个 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
