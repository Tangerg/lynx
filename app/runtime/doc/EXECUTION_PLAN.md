...
P81 已把 container 的 quiescent dispose 接到真实 renderer before-unload，并用同库 Runtime 进程完整停启证明 durable Session、idempotency namespace 与重订阅事件收敛。下一轮 P82 聚焦 event subscription opening/iteration 对取消不合作的切点、Runtime 重启时已挂载多 context read model 的统一失效，以及 window close 与 replacement renderer 同时接管 mutation identity 的交错；继续用真实 HITL/Plan/Goal/旁路事件、失败注入与 async detector 反证。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
