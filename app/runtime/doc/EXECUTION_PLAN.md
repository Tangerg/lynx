...
P72 已阻止 crash-abandoned empty claim 因 24 小时到期而重新执行业务 handler/invalidation，retention 只回收有 authoritative outcome 的 completed result。下一轮继续审计 Desktop 对 logical mutation identity 的所有权：运输断线已复用同 key，但应用/桌面重启、组件卸载和显式 retry 后是否会为 settlement-unknown 命令错误地产生新 key，尤其覆盖 Session/Run、HITL、Plan、Goal 和全部旁路 command；每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
