...
- 右栏 tab overflow 红例确认：strip 已用 Codex 同类 trailing fade 表达“右侧还有内容”，但 renderer 恢复到末尾 tab 或 picker 打开末尾 tab 时只更新 active destination，没有把 active tab 带回可见区；内容已切换而选中身份仍在裁切区外。active tab identity 是现成的唯一滚动意图；红例要求变化时用浏览器 nearest scrolling 显示它，不增加 ResizeObserver、overflow boolean 或持续测量状态。
