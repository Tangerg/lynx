...
P83-06 已让 Tool start、terminal Item、operational journal 与 Run lifecycle 在各自事实边界内拥有唯一原子 write-set；终态失败保留上一个 durable running 边界，清理或恢复可从同一 identity 收敛且不会发布半个 finish。P83 后续按 1–2 client 产品模型继续验证命令响应丢失、重复恢复、Runtime restart 与 renderer replacement 的交错，尤其检查已结算 mutation identity、重连 snapshot 和迟到事件是否仍只有一个 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
