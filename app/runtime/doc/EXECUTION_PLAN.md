...
P82 已把 Workspace event stream 的唯一活跃代际所有权收回 Application owner，重复启动不再依赖调用方先撤销旧 signal，并确认完整重订阅失效覆盖 mounted read model 而不引入跨 context 反向依赖。下一轮 P83 聚焦 subscription opening/iteration 对取消不合作时的有界退出、Runtime 重启后已挂载 HITL/Plan/Goal/Run/Tool read model 的统一收敛，以及 window close 与 replacement renderer 接管 mutation identity 的交错；并按真实 1–2 client 使用模型验证断线重连、冷启动、事务失败与崩溃恢复，不堆砌无产品意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
