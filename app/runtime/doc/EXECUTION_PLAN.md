...
P76 已闭合 Desktop 同进程 client 热切换、跨 renderer 迟到 settlement fencing、storage handoff 写失败、write-then-throw 候选 identity 与 SDK 双资源关闭，确认所有修复停留在 RPC/Runtime plugin storage Adapter 边界。下一轮 P77 继续验证“same-key replay 之后产品权威 cold read 真正收敛 UI”：分别在 Session create/delete、Goal start/stop/resume、Run start/resume/HITL answer、Schedule、Provider/Role、Knowledge/Memory/Skill/File invalidation 的 response-before/after-commit cutpoint 注入崩溃与断线，补 owner heartbeat 丢写/lease 到期的两个真实 renderer 竞争，并确认每个 mutation 的 read model 与事件 sidecar 不依赖旧进程内存。每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
