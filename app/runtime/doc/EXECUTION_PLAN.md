...
> 状态：P1–P82、P83-01–P83-13 已完成；发布后反证审计持续进行
>
...
P83-13 已把 stream 退出后的 exact `runs.get` 纳入 pump generation ownership：replacement 可立即释放旧 live owner，迟到 RunRef 不能写回 successor projection。P83 后续按 1–2 client 产品模型继续验证命令响应丢失、重复恢复、Runtime restart 与 renderer replacement 的交错，下一步优先核对 mounted Goal query 与 HITL/Plan/Run/Tool material snapshot 在连续 Runtime replacement 中是否共享同一 successor 事实，尤其反证旧 Goal read 的迟到结果能否覆盖新代；继续检查迟到/乱序事件及 mutation settlement 是否仍只有一个 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
