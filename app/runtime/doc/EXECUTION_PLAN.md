...
> 状态：P1–P82、P83-01–P83-02 已完成；发布后反证审计持续进行
>
...
P83-02 已让 Runtime event 全量 resync 明确替换 mounted Agent 的旧 live generation；不合作的 Run opening/iterator 也会有界释放，HITL/Plan/Goal/Run/Tool read model 能在同一新代分别经 durable Agent snapshot 与 Query invalidation 收敛。P83 后续继续聚焦 window close 与 replacement renderer 接管 mutation identity 的真实交错，并按 1–2 client 产品模型验证断线重连、冷启动、事务失败与强杀恢复，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
