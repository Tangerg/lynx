...
> 状态：P1–P82、P83-01–P83-05 已完成；发布后反证审计持续进行
>
...
P83-05 已让外部 Tool 调用与 running Tool read model 共享同一个 authoritative transaction；SIGKILL 后 open invocation、Transcript Item 和 Run lifecycle 会统一收敛为 incomplete/lost。P83 后续按 1–2 client 产品模型继续验证事务失败、命令结算不明、事件发布与重连 snapshot 的交错，尤其检查 running Item 持久化后终态覆盖失败、重复恢复和客户端替换时是否仍只有一个事实 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
