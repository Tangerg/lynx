...
P77 已闭合 Desktop HTTP transport 的 quiescent close，并以真实 Runtime 验证 Session、Run streaming opening 与 Schedule 全 mutation 在提交前/后断线时通过 same-key replay、cold read 和事件收敛。下一轮 P78 继续扩展同一 fault oracle 到 Goal start/stop/resume、Run resume/HITL answer、Provider/Role、Knowledge/Memory/Skill/File invalidation，并补 owner heartbeat 丢写/lease 到期的两个真实 renderer 竞争；同时验证 runtime event sequence gap、重订阅与进程冷启动之后，各产品 read model 不依赖旧 client 内存。每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
