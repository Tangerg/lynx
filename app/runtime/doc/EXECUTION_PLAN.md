...
P75 已把 journal 从会 lost-update 的整表 snapshot 收敛为 verified entry/owner records，并闭合 quota/吞错、迁移中断、retention 收缩、多窗口活跃/崩溃接管与 lifecycle dispose；同时修复真实高负载下 Goal durable terminal 领先 stream boundary 时的 cleanup 错误投影。下一轮 P76 继续验证“same-key replay 之后产品权威 cold read 真正收敛 UI”：分别在 Session create/delete、Goal start/stop/resume、Run start/resume/HITL answer、Schedule、Provider/Role、Knowledge/Memory/Skill/File invalidation 的 response-before/after-commit cutpoint 注入崩溃与断线，覆盖同 endpoint token 热换、两个真实 renderer 同时恢复、owner heartbeat 丢写再恢复，并确认每个 mutation 的 read model 与事件 sidecar 不依赖旧进程内存。每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
