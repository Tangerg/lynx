...
P73 已使 Session create、Run start/resume（含 HITL）与 Goal lifecycle 在同一 Desktop 进程内跨 bounded settlement failure、组件 owner 释放和显式产品 retry 保留原 logical mutation identity，同时严格不合并 fresh 同形产品意图。下一轮 P74 继续审计 Desktop 冷重启与其余 replayable response commands：建立不持久化敏感参数、不跨 Runtime endpoint/instance 错投、且能由所属产品权威 read model 结算的恢复模型；同时统一所有旁路 mutation 的有界 attempt signal，验证 Session metadata/fork/rollback/delete、approval/provider/MCP/memory/skill/hook/schedule/codebase 等全部命令不会在 settlement-unknown 后换 key。每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
