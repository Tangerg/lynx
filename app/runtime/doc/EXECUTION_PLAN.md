...
P70 已关闭已知业务结果因 receipt claim 丢失而永久卡在 `idempotency_in_progress` 的缺口，恢复路径只重新绑定并持久化同一 outcome，不重复命令或旁路 invalidation；MCP session/request/handshake 三类 lifetime 合同也已由确定性 oracle 分责。下一轮继续反证优雅关闭时仍留在进程内的 pending completion 是否在 SQLite/lease 关闭前可靠排空，以及跨进程 crash/abandoned claim 与 exactly-after-commit invalidation 的可证明边界；每轮提交前执行真实 HITL/Plan/Goal、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
