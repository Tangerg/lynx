...
P69 已关闭 Runtime Close 在已接受 unary/stream source 尚未真实退出时提前关闭 Host/SQLite 依赖并释放 lease 的缺口，未开始 range 的流也由 Endpoint owner 主动收敛。下一轮继续反证事务 commit、idempotency replay 与旁路 invalidation 在并发消费者投影中的 exactly-after-commit 一致性；每轮提交前执行真实 HITL/Plan/Goal、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
