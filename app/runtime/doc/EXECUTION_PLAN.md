...
- [x] P114-01 建立 renderer replacement/final close 单 owner：旧 bootstrap、mutation/RPC、query/material writer、stream callback 与 teardown settlement 全部在同步 retirement 后失去提交权；replacement、重复 dispose、迟到响应与冷启动 mutation handoff 共享唯一 generation identity；
- [x] P114-02 建立 Runtime process/event generation：覆盖冷启动、断线、同 endpoint 重启、SIGKILL、事务失败和成功回执丢失；重复、乱序和迟到事件只在所属 incarnation 内推进，survivor read model 由一次权威 recovery 接管；
- [x] P114-03 逐项审计 mutation/query/optimistic cache/event/material snapshot 的共同 read model，删除多 writer 与独立刷新旁路，使 Session/HITL/Plan/Goal/Run/Tool/usage/navigation 都有准确的 Application owner和提交边界；
- [x] P114-04 完成 Desktop 真实接线反证与修复：Run Summary、Terminal、Diff、Tool selection、Goal、Plan、审批在多轮长对话、compaction、HITL continuation、Session切换、Dock折叠/恢复和renderer替换中保持 exact identity；
- [x] P114-05 按 Codex 主参考、zcode/minimax 补充参考完成 UI 对照台账和像素级打磨；先证明 interaction/data wiring，再调整 theme token、布局、状态反馈和细节，不以静态 mock 截图掩盖生命周期问题；
- [x] P114-06 持续执行 Runtime 单元/SQLite 不变量、Frontend 全门禁与 `--detectAsyncLeaks`、真实 HTTP/进程恢复和必要的 Wails/browser 产品验证；少做脱离产品的定向 race/fuzz；
- [x] P114-07 每个独立纵切更新本文和 Capability Ledger，精确暂存本批文件、提交并推送；agent-browser 使用后关闭全部会话并确认无 daemon、Chrome、测试或 Runtime 残留，不关闭用户已有 Wails/Vite 进程，始终不修改或暂存 `app/cli`。
...
| 日期 | 阶段 | 完成事实 | 验证 |
|---|---|---|---|
| 2026-08-18 | P114-04ac（Approval mode / visible mutation intent owner；P114 milestone close） | 在审批模式durable写入已经由`AgentCommandOwner`按Runtime generation串行化后，从真实单客户端操作继续反证receipt到authoritative query projection之间的UI交接：点击Auto时旧界面仍把Balanced标成选中、没有busy语义且其余模式仍可再次提交；补上pending后，第二条红例又证明RPC成功回执先于React Query prop更新时会发生`Auto → Balanced → Auto`一帧回跳。根因不是后端缺少重试，而是`ModeRow`只渲染standing query，没有对象拥有“用户意图已准入→RPC已接受→authoritative projection已追平”这段presentation lifecycle。现在同一组件内的判别状态`ApprovalModeIntent`只拥有`pending → accepted-awaiting-projection`：准入即显示目标模式、以`aria-busy`和既有spinner反馈并禁用整组选项；accepted receipt只推进settlement，不提前撤销目标，直到父级mode等于accepted值才释放；失败或generation retirement仅撤销exact intent并恢复standing value。重复点击、点击当前值与pending期间的第二命令在presentation boundary被拒绝；durable value仍只有原query material writer，没有optimistic cache、第二global store、新owner模块、timer/delay、reload或兼容路径。前端主参考study Codex `agent-settings-B3R-25Yb.js:2140-2375`中field-key pending set在mutation期间禁用approval control并保持可见反馈；本批只吸收对应的pending心智模型，不复制multi-scope/managed configuration。后端参考Codex Rust `app-server/src/config_manager_service.rs:190-300`的expected-version write boundary及`config_manager_service_tests.rs:496-550`的write/read-after验收；Lyra现有serialized command与authoritative query边界已满足，因此未改Runtime/Protocol/SQLite。按最终确认，本里程碑固定为单client、单Runtime、同时启动/关闭的产品心智，不为假想多客户端、高并发或异常组合增加防御层；此前P114提交在该模型下保留。 | 两条红例修复前分别稳定得到“Auto未选中、Balanced仍选中、无busy且控件可用”和“accepted receipt后Auto失选且控件提前恢复”；修复后focused `--detectAsyncLeaks` 1 file / 2 tests全绿，覆盖pending、receipt/projection窗口、重复准入拒绝、失败回滚及后续正确命令。Frontend `npm run check`为329 files / 1974 tests全绿，完整`--detectAsyncLeaks`同为329 / 1974且253.09s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，105 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2741.5/2929.7 KB（93.6%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为67.3/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，测试/Runtime进程退出；`app/cli`未修改或暂存。用户确认本批完成即形成新里程碑，不再开启下一轮代码改动 |
| 2026-08-18 | P114-04ab（Goal / single Session material transaction owner） | 在P114-04aa已经把authoritative snapshot response绑定exact projection generation后，继续主动寻找由query、event与material snapshot共同写入的read model，先构造真实单客户端红例：predecessor `goals.get`已在途，successor `sessions.snapshot`先把Goal `{objective: successor, used.runs: 1}`提交到同一Session，旧query随后返回`{objective: predecessor, used.runs: 0}`；期望successor保持可见，旧实现稳定被覆盖为predecessor。根因不是少一次invalidate，而是P112只让Goal伴随Session snapshot提交，却仍保留Goal DATA_PROVIDER、QueryClient key、mounted query hook与独立event invalidation，standing state存在两个合法writer；mutation response、query cache、event repair与Agent material也没有共享一个generation。首次尝试为Goal增加独立generation Store后主动否决：它会让Agent与Goal无法原子换代，还需要额外preserve/retire协议。最终breaking物理删除`goals.get` Desktop query/data provider/public key及其特殊invalidation，不保留alias或兼容双路径；`AgentSessionMaterialRead`改为纯`projectAssociatedSharedMaterial(shared)`，Goal adapter以generic contributor从同一次Runtime `sessions.snapshot`事务预计算neutral value，Agent inner ring只看`shared` key、不依赖Goal/Runtime/RPC/SQLite词汇。Plan/HITL/Run/Tool与Goal companion在同一个immutable view里只由exact `AgentViewRefreshToken`提交：败者snapshot即使迟到并完成纯投影，也没有任何store写入口；plugin在提交前dispose则staged value自动撤权。`GoalCommandOwner`只保留exact Runtime generation的命令顺序与settlement，mutation receipt收窄为Session correlation；成功、业务失败、成功回执丢失均等待mounted Session lifecycle owner做同一authoritative synchronization，`goals.changed`与scoped resync也只触发这一个Session writer。Goal Banner/Launcher直接读取Agent shared material，Banner key加入admitting generation，使server replacement即使复用Session ID与Goal creation stamp也退休predecessor disclosure state；visual fixture同样把Goal放进唯一Agent view，不再建立测试专用第二store。没有optimistic Goal、query refresh、延时、Abort假取消、双store、兼容query或Agent领域泄漏。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:482125-482205`：authorization generation+lease在每个await后重新`canApply`，同一exact conversation/turn material共同失效；本批吸收“关联物料必须与admitting generation一起提交”，不复制multi-Host/conversation hydration registry。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_state.rs:99-125`中replacement先取消predecessor再推进listener generation、`request_processors/thread_lifecycle.rs:386`只允许exact generation teardown，并结合Lyra既有SQLite Session snapshot transaction；不复制multi-connection listener package。zcode/minimax未发现更强的Goal transaction owner，未制造补充路径。Runtime operation、Protocol、SQLite schema、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前红例稳定得到expected `successor / runs=1`、actual `predecessor / runs=0`。修复后focused `--detectAsyncLeaks`为14 files / 94 tests，覆盖generic shared contributor唯一key、plugin dispose、winning/losing snapshot、live write race、Runtime/port replacement、command settlement/repair、Goal generation presentation、event/scoped resync与query generation余项。Frontend `npm run check`为328 files / 1972 tests全绿，完整`--detectAsyncLeaks`同为328 / 1972且185.66s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，105 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2741.5/2929.7 KB（93.6%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，测试/Runtime进程退出；`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04aa（Session refresh / exact projection generation capability） | P114-04z让mounted Session generation不再ABA后，继续反证authoritative snapshot writer自身是否真正消费该identity：单客户端中predecessor Session发起snapshot read，窗口/导航随后drop该slice，同一Session remount并发起successor read；两代局部`refreshSequence/viewRevision`都重置为`1/0`，旧响应迟到时旧实现稳定返回commit `true`并把predecessor snapshot写进successor generation，继而挤掉successor响应。根因是`AgentViewRefreshToken`只有短命entry局部计数，store虽已拥有renderer级单调`viewEpoch`，query capability却在async边界丢弃它。现在breaking将refresh token收敛为只读不可拆分`{generation, requestSequence, viewRevision}`；`beginViewRefresh`把本次read实际获权的exact generation写入token，history rewrite则先从同一`projectionGenerationSequence`分配successor generation再发token；`commitViewRefresh`必须同时匹配mounted generation、同代request sequence与view revision，close/remount、connection retirement、server replacement、late response及同代newest-wins因而共用一个admission规则。HITL response与Run cancel审计确认已先比较同一monotonic generation再比较各自revision，无需第二修复路径；没有tombstone、AbortController假取消、随机ID、timeout、refresh重试或兼容token。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:482125-482205`：pending load只复用exact authorization generation+lease，且每个await后重新执行`canApply`；本批吸收query response必须携带并重新证明admitting generation，不复制multi-Host/conversation hydration registry。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_state.rs:99-125`中replacement先取消predecessor并单调推进listener generation，以及`request_processors/thread_lifecycle.rs:386`只允许exact generation teardown；不复制multi-connection listener package。zcode/minimax未发现更强的Session snapshot writer capability，未制造补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前单客户端close/remount红例稳定得到expected predecessor commit `false`、actual `true`；修复后focused `--detectAsyncLeaks`为5 files / 56 tests，覆盖snapshot、history rewrite正向提交、HITL recovery、Run cancel与selector generation。用户报告的流式尾部点赞/点踩闪烁路径另复跑3 files / 25 tests全绿，确认P114-04a/04l/04p/04v/04w治理在当前分支仍有效而`main`尚未包含这些提交。Frontend `npm run check`为328 files / 1972 tests全绿，完整`--detectAsyncLeaks`同为328 / 1972且157.64s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2742.1/2929.7 KB（93.6%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04z（mounted Session / monotonic projection generation） | P114-04y之后继续从Session navigation反证generation自身：关闭/裁剪同一Session会物理删除Agent slice，随后重新打开时`emptyEntry()`把`viewEpoch`重置为0；期望remount generation严格大于predecessor，旧实现稳定得到`0 > 0`。因此即使Plan已消费generation，“dismiss A→关闭A→切B→重开A”仍可在相同Session/Plan revision上发生ABA，HITL recovery与Run cancellation的exact token也依赖一个会复用的身份。根因是每个短命Session entry各自用`epoch + 1`分配身份，slice既拥有大体积material又错误拥有renderer级generation clock。现在Agent Zustand store作为既有单一material owner内聚`projectionGenerationSequence`高水位：首次mount、history rewrite、connection retirement与server replacement都从同一单调序列分配，drop只释放Session material而不回退clock；同一个global retirement给全部mounted Session同一exact generation，下一次remount再取得successor。没有tombstone map、Plan特判、local mirror、随机token、timer、refresh或兼容路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:482125-482205`：pending load只复用相同authorization generation+lease，且每个async边界重新证明`canApply`；本批吸收generation不可ABA及successor必须重新获权，不复制multi-Host/conversation hydration registry。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_state.rs:99-125`中`set_listener`先取消predecessor再单调推进`listener_generation`，以及`request_processors/thread_lifecycle.rs:386`只允许exact generation cleanup；不复制multi-connection listener package。zcode/minimax未发现更强的Session remount generation clock，未制造补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前store红例稳定得到expected remount generation greater than predecessor、actual `0 > 0`；修复后focused `--detectAsyncLeaks`为6 files / 58 tests，并证明一次retirement的mounted Sessions共享一个exact generation。Frontend `npm run check`为328 files / 1971 tests全绿，完整`--detectAsyncLeaks`同为328 / 1971且168.78s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2742.1/2929.7 KB（93.6%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。用agent-browser验证standalone Vite因缺少Wails native bridge在kernel startup fail-closed，未把空壳当产品证据；随后关闭全部agent-browser session/daemon与本批Vite，未触碰用户已有Chrome，测试/Runtime进程无残留；`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04y（Plan presentation / projection generation identity） | 继续反证Plan在Runtime endpoint replacement、进程恢复与Session复用下的本地presentation所有权：generation 1中用户dismiss revision 7的Plan后，以同一Session ID和同一revision 7安装successor server projection并写入“Successor server plan”；期望新代Plan重新出现，旧实现实际返回`null`。根因是`SessionPlan.identity`只有`[sessionId, revision]`，而generic `useSharedState`只发布裸shared value，丢弃Agent store已经拥有且会在`replace-server`/authoritative rewrite推进的`viewEpoch`；组件local dismissal因此把predecessor用户选择跨代借给successor material。现在breaking物理替换port clause为`useSharedMaterial<T>(): { generation, value }`，无alias/双路径；adapter用同一次Zustand selector原子读取exact Session的`viewEpoch + shared subtree`并以shallow equality保持无关projection写入零重绘。充血immutable `SessionPlan`共同拥有`generation/revision/steps`，identity收敛为`[sessionId, generation, revision]`：同代同Plan保留dismiss选择，Session切换、server replacement或authoritative projection rewrite同步退休旧选择，successor即使复用业务revision也不会继承；没有新增Plan专用port/store/维护文件。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:482125-482205`在每个异步边界用authorization generation + lease证明pending snapshot仍可应用；本批吸收本地presentation必须携带admitting projection generation，不复制其multi-Host/conversation registry。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_state.rs:99`、`:125`与`request_processors/thread_lifecycle.rs:386`的listener generation及exact-generation cleanup；本批映射为Desktop successor projection不能继承predecessor本地选择，不复制多connection listener package。zcode/minimax未发现更强的Plan dismissal代际owner，未制造补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前红例稳定得到expected successor Plan非空、actual `null`；修复后focused `--detectAsyncLeaks`为5 files / 31 tests，覆盖Session/revision/generation复合identity、same revision跨server重新出现、shared material原子epoch/value与既有progress/view model。Frontend `npm run check`为328 files / 1969 tests全绿，完整`--detectAsyncLeaks`同为328 / 1969且172.62s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2741.8/2929.7 KB（93.6%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04x（current root / exact terminal turn material owner） | P114-04w继续相邻反证后发现“何时显示”虽已归accepted-visible generation，“属于哪一轮”仍由两个不相交事实决定：`RootRunOutcome`独立订阅全局current root outcome/metrics，`MessageStream`只把它挂到数组最后一行。真实单客户端窗口为旧Run已finished，新用户消息先以`runId=null`乐观进入transcript，新Run尚未收到Runtime接纳；期望旧Completed footer仍属于旧assistant turn，旧实现实际把它从`assistant-terminal-footer`搬到`local-successor-user`下方，形成跨轮拼接和一次布局跳动。根因同时暴露Agent Session port把同一个current root拆成attention/outcome/metrics三个selector，Run Summary、Context Usage、Composer admission与footer各自抽样碎片，没有一个对象拥有identity与所有派生事实。现在breaking将三个port clause物理收敛为唯一`useCurrentRootRun(): AgentRunView | null`，无alias/forwarder；application既有`runReadModel.ts`升级为充血且immutable的`CurrentRootMaterial`，一次捕获exact Run并共同拥有`runId/status/running/attention/outcome/metrics`，`terminalTurnIndex(rows)`只允许finished current root认领自己最后一个owned narrative row。`MessageStream`不再用`isLast`猜footer归属；unassigned optimistic successor、其他Run与无narrative的迟到终态都不能承接，accepted successor成为current root后旧close material立即退役；failed/timedOut/lost由recovery banner拥有，模型直接拒绝terminal footer权，不再挂空`mt-4` wrapper。`RootRunOutcome`变为纯exact-material view，不再自行采样全局状态；Run Summary、Context Usage和Composer input admission也消费同一对象，旧`useCurrentRootAttention/useCurrentRootOutcome/useCurrentRootMetrics`public出口全部删除。P114-04w的visible generation仍决定该exact row内何时进入布局，因此identity与可见结算是串联的两个必要边界。没有把runId塞进local state、按最后行回退、CSS隐藏、delay/timer、refresh、第二footer query或兼容双路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/subagent-activity-chip-group-DsFB2D8Y.js:9192-9200`从同一exact `conversationId + turnId + completed`生成streaming identity、`:9438-9467`让action/goal/fork material继续携带同一`turnId`且服从`completed`；本批吸收终态material不能离开exact turn，不复制Codex多conversation/Host、analytics或annotation。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs:1312-1338`让ItemCompleted与AgentMessageDelta携带exact `thread_id + turn_id`，delta另带`item_id`；本批把provenance保持到Desktop current-root read model，不改协议。zcode/minimax未发现更强的optimistic successor/terminal turn owner，未制造补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前两条红例稳定得到：outcome owner expected `assistant-terminal-footer`、actual `local-successor-user`；failure footer index expected -1、actual 0。修复后focused `--detectAsyncLeaks`为4 files / 12 tests，覆盖finished Run最后exact row、unassigned successor隔离、running/idle/failure无terminal权、exact root订阅稳定及无关projection零重绘。Frontend `npm run check`为328 files / 1968 tests全绿，完整`--detectAsyncLeaks`同为328 / 1968且156.49s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2740.8/2929.7 KB（93.6%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04w（Run outcome / terminal visible generation owner） | 继续沿用户报告的流尾布局闪烁做相邻反证：让最后一个assistant turn从running进入complete，但保留smooth Markdown reveal backlog；期望Run Summary的Completed/Canceled/Limit outcome仍不进入布局，旧实现实际立即挂载`RootRunOutcome`。根因是P114-04v虽然让点赞/点踩action服从accepted-visible generation，Session级footer却仍在`MessageStream`尾部仅凭全局`!running`独立写布局，绕过了exact terminal `TranscriptRow`及其visible projection owner；同一次Run结束会出现durable终态已到、可见文本尚未追平的真实窗口。现在不新增store/context/维护模块，由`MessageStream`只把最后一轮的footer内容组合给`MessageBlock`，原`MessageVisibleMaterialOwner`唯一决定action与outcome何时共同materialize：只有exact terminal row的block/Tool/delegated Run/Run owner和全部accepted-visible projection均settled，footer才进入DOM；backlog存在、临时非running或predecessor settlement均不能借权，追平后正常出现且不会永久loading。最后一轮本就拥有`READING_GUTTER`，只把既有`mt-4` footer移入同一所有权边界，视觉几何和交互样式不变。没有CSS隐藏、opacity占位、delay/timer、滚动补偿、强制remount、第二outcome state或兼容路径。前端继续以study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/subagent-activity-chip-group-DsFB2D8Y.js:9192-9200`的exact turn `completed/isStreaming`同源、`:9438-9467`的action material与同一`completed + turnId`共同失效为主参考；后端继续以Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs:1312-1338`中delta/completion携带exact `thread_id + turn_id + item_id`的provenance为边界。本批只将该原则扩展到Desktop turn footer，不复制Codex多conversation/Host、analytics或协议形状；zcode/minimax未发现更强的terminal footer generation owner。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前新增DOM布局反例稳定红：terminal Run转complete且visible backlog仍在时，expected outcome absent，actual已出现`root-run-outcome`；修复后focused `--detectAsyncLeaks`为3 files / 22 tests，另证明visible generation追平后footer正常materialize。Frontend `npm run check`为327 files / 1964 tests全绿，完整`--detectAsyncLeaks`同为327 / 1964且141.60s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2740.4/2929.7 KB（93.5%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04v（terminal action / accepted visible generation owner） | 用户再次报告真实中间内容区流式输出时底部点赞/点踩操作栏出现并跟随流尾、造成页面闪烁。本批主动反证P114-04l的visible backlog与P114-04p的exact Run完成态仍不充分：先让一个finished assistant row在reduced-motion下稳定挂载`message.actions`，再给同一finished Message追加更长的accepted text并恢复smooth reveal；期望新文本进入布局前旧操作栏已撤权，旧实现最终DOM虽然已无操作栏，Slot render计数却从1增至2，证明父层先按上一代“无presenting token”让按钮参与一次布局，子Markdown到layout effect才发现backlog并撤下。根因是`MessageVisibleMaterialOwner`只保存“当前仍presenting”的token Set：projection一旦settled就从集合删除，owner完全忘记它结算的是哪一版accepted transcript，因而把predecessor settlement借给successor text generation。现在原owner升级为每个mounted projection持有`{accepted generation, settled}`的充血状态：terminal action准入同时要求durable block/Tool/delegated/exact Run已settled，且每个可见projection结算的generation与当前terminal `TranscriptRow`完全相同；late terminal row一到，generation mismatch在同一次父render就返回active，`message.actions`根本不创建，projection追平后才发布一次settled。活跃流使用owner自身作为稳定generation，避免每token改变Context；projection generation更新与真实unmount retirement拆开，active→active不发布，旧token teardown也不能退休successor。没有opacity/CSS隐藏、debounce、timer、滚动补偿、强制remount或第二action state。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/subagent-activity-chip-group-DsFB2D8Y.js:9192-9200`把exact conversation/turn的`completed`直接投影为`isStreaming`，`:9438-9467`让action material与同一`completed + turnId`共同失效；本批吸收action与accepted turn generation必须同源，不复制Codex多conversation/Host、annotation或analytics形状，也不声称其实现Lyra本地smooth reveal owner。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs:1312-1338`让ItemCompleted与AgentMessageDelta都携带exact `thread_id + turn_id`，delta另带`item_id`；本批把exact provenance延伸到Desktop accepted-visible generation，不改协议。zcode/minimax未发现更强的smooth-reveal/action generation owner，未制造补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前新增DOM布局反例稳定红：late terminal text后expected action Slot累计render不变，actual额外render 1次；修复后focused `--detectAsyncLeaks`为4 files / 41 tests，覆盖late accepted generation同步撤权、predecessor settlement隔离、多projection teardown与active→active零通知。Frontend `npm run check`为326 files / 1963 tests全绿，完整`--detectAsyncLeaks`同为326 / 1963且140.60s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2740.4/2929.7 KB（93.5%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04u（Runtime endpoint / single server-scope replacement） | 单客户端红例先证明Connection设置虽然能持久化新Runtime endpoint，却由UI直接调用`window.location.reload()`销毁renderer；期望应用层接管server replacement且reload为0次，旧实现实际reload为1次。刷新掩盖了更深的owner空洞：endpoint configuration只会写地址，connection generation、mounted Session material与TanStack query仍各自保留旧server事实；若去掉刷新就可能把相同Session/query key的两个server跨代拼接。现在breaking将`RuntimeEndpointConfiguration.write`改为语义明确的`replace`且无兼容alias，由充血`RuntimeConnectionOwner.replaceEndpoint`唯一拥有事务顺序：同步发布空白connection generation使现有mutation/query/event subscriber先撤权，再提交标准化endpoint与持久化，随后发布server-scope replacement，最后由同一controller启动successor inspection；即使Runtime process generation字符串相同也生成新connection generation。独立的`RuntimeServerScopePorts`只表达server replacement，不污染每个stream test double；Workspace owner收到它后以`replace-server`清空mounted Agent Session projection并reset query cache，区别于瞬时断线仍可保留last-committed offline facts。`AgentViewRefreshOwner`同步把exact mounted view置为`EMPTY_AGENT_SESSION_VIEW`、推进epoch/revision/refresh sequence并把authoritative revision归零，旧refresh token与迟到response均不能回写；successor read正常接管。配置storage mirror也随Runtime owner dispose，不再泄露subscriber。Connection UI保持本地draft/错误状态，不再reload、remount、delay、双query或localStorage恢复旁路，八语言文案统一改为“应用并重新连接”。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:142601-142634`在await前冻结authorization generation且完成后仍要求exact generation，`:482125-482205`在每个async边界前后检查`canApply`并用authorization generation+lease标识pending snapshot；本批吸收replacement先撤旧写权、successor只接受exact scope material，不复制Codex multi-host/conversation snapshot registry。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs:262-272`只给exact current且未断开的`Arc<RpcClient>`准入、`:309-326`只让exact failed current进入Recovering；本批将exact-current transition映射到单server endpoint replacement，不复制多client registry。zcode/minimax未发现更强的单client server-scope transition，未发明补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前ConnectionPane focused红例稳定得到reload expected 0、actual 1；修复后focused `--detectAsyncLeaks`为11 files / 125 tests，覆盖transaction顺序、同process新connection generation、endpoint adapter/storage cleanup、query reset、mounted Session立即清空、successor权威提交及predecessor迟到拒绝。Frontend `npm run check`为326 files / 1960 tests全绿，完整`--detectAsyncLeaks`同为326 / 1960且138.69s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2740.1/2929.7 KB（93.5%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为66.9/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04t（Diagnostic Tool draft / Runtime invocation material owner） | 单客户端红例先证明Diagnostic Tool虽然已由`DiagnosticToolOwner`发布exact Runtime material generation，UI却把generation直接用作整个row的React key：用户展开`grep`、输入`{"path":"README.md"}`并得到结果后，同一Host发生Runtime replacement；期望只退休旧结果且row继续展开、draft保持，旧实现实际把按钮恢复为“Configure grep diagnostic”，Collapsible body卸载，输入退回`{}`。这不是少一个缓存，而是generation边界过宽：open/arguments/JSON validation属于mounted user presentation，running/RPC error/result属于Runtime invocation；旧组件还用同一个`error`同时承载本地parse failure与远端failure。现在不新增单消费者维护文件，在既有Tools view内明确成`DiagnosticToolRowPresentation`与`DiagnosticToolInvocationMaterial`两个状态owner：Session/workspace的`workspaceKey + tool.id`仍可重建整行，Runtime generation只key exact invocation material；外层保留open、controlled arguments draft及draft validation，内层独占running guard、RPC error和formatted result。replacement同步安装空白successor invocation，predecessor subtree与其迟到continuation一起失去DOM写权，同时用户草稿无需备份/恢复；编辑draft只清与该draft相关的validation和predecessor invocation error。没有整行remount、localStorage、timer/delay、refresh、第二result store、强制重试或兼容路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:142601-142634`：authorization lease在await前冻结generation，完成后仍要求exact generation才返回；`:204421-204435`用exact operation object决定`isPending`，旧operation不能替successor声明pending。本批吸收generation lease与pending material必须归exact operation，不声称Codex存在Lyra direct diagnostic surface，也不复制multi-host operation registry。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs:262-272`仅让exact connected且未断开的`Arc<RpcClient>`准入process start、`:309-326`仅让exact failed current进入Recovering；本批把exact-current admission映射到invocation material retirement，不复制多client transport recovery。zcode/minimax未发现更强的direct diagnostic draft/material owner，未据此发明补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused 1/1稳定红：generation replacement后expected“Close grep diagnostic”，actual只剩“Configure grep diagnostic”，body与draft均被销毁；修复后focused `--detectAsyncLeaks`为2 files / 5 tests，覆盖material generation推进、旧settlement retirement、结果清空及row/draft保留。Frontend `npm run check`为326 files / 1953 tests全绿，完整`--detectAsyncLeaks`同为326 / 1953且126.50s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2739.1/2929.7 KB（93.5%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为67.0/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04s（Provider role mutation feedback generation owner） | 两条单客户端红例先证明Settings Provider owner虽然已经发布Runtime material generation，Utility/Embedding角色保存仍绕过它各自持有`useState(error)`：generation 1保存失败显示`retired role failure`后切到generation 2，旧错误仍留在DOM；保存Promise在途时触发器实际`disabled=false`且没有“Saving…”反馈，可重复提交并让query、mutation与组件local state竞争同一read model。进一步审计发现共享hook已经被Provider credential保存、删除和MCP form使用，却仍以实现历史命名为`useProbe`，领域名反而掩盖了它实际承担的通用异步反馈所有权。现在breaking物理删除`useProbe`并统一为有四类真实消费者的`useAsyncFeedback`，无alias/forwarder：immutable `materialGeneration + feedback`与monotonic intent revision共同拥有busy/ok/error、reset/fail与settlement，generation mismatch在render同步呈现idle successor，layout transition再使旧continuation永久失去提交权；同一material只有latest intent可发布结果。Utility与Embedding删除local error writer并消费Provider mutation material generation；saving期间禁用trigger、用既有`loop` spinner和`providers.saving`反馈，迟到retirement保持中性，不伪装失败。角色draft与query material继续存活，accepted value仍只由authoritative query发布；没有optimistic假值、强制remount、delay/timeout、refresh、第二mutation store或兼容双路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:503853`接收`isSaving`、`:503934`在saving时禁用editable control、`:504044`同时禁用Back：本批吸收保存反馈与控件可用性必须同属一个settings mutation state，不声称其实现了Lyra的Runtime generation owner。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs:267-271`只在`Arc::ptr_eq(current, expected)`时承认失败client、`:318-326`只让exact failed current进入Recovering；本批把exact generation admission映射到前端feedback settlement，不复制multi-client registry或transport recovery。zcode/minimax未发现更强的generation-bound Provider角色反馈owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused两条反例稳定得到generation 2仍显示`retired role failure`，以及在途trigger `disabled=false`且“Saving…”缺失；修复后focused `--detectAsyncLeaks`为7 files / 26 tests，覆盖Utility/Embedding跨代错误退休、在途禁用/反馈、latest intent与Provider/MCP既有路径。Frontend `npm run check`为326 files / 1953 tests全绿，完整`--detectAsyncLeaks`同为326 / 1953且128.19s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2738.8/2929.7 KB（93.5%），CSS为131.2/131.8 KB（99.5%），settings lazy chunk为67.0/195.3 KB。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04r（Session identity / promoted material transition owner） | 两条单客户端导航红例先证明同一个Session identity boundary有两套location提交。其一从`ses_a + view=diff + dock=terminal`关闭active Session，selection model正确选出`ses_b`，但旧adapter只提交`{session: ses_b}`，实际仍为`view=diff`；其二从已删除deep-link `ses_gone + view=run-summary + dock=diff`做authoritative reconciliation，正确survivor为`ses_live`，却同样继承`run-summary`。显式`selectSession`早已提交`view=null`，所以根因不是Navigator缺少清理能力，而是close/reconcile绕过唯一Session transition，各自只维护一半location；这让predecessor promoted Diff/Run Summary material在successor Session shell中复活。现在adapter内唯一`goToSession`把hold-open、cold-start memory和location提交组成一个identity transition，任何真正进入另一active Session的路径都从这里退休predecessor `view`；close active Session走push，authoritative纠正已删除deep-link走replace。反向反例同时冻结不过度清理：关闭后台Session或reconciliation只裁剪peers且active identity不变时，当前`view`保持；`dock`始终不由此transition改写，继续交给既有per-Session workspace scope owner恢复。空Session不会制造`holdOpen("")`，Application `sessionSelectionModel`仍只拥有open/active identity，未泄露Diff/Run Summary等Delivery终态词汇。生产`navigator().go({session...})`已收敛到该adapter唯一transition，没有逐调用点`view:null`补丁、强制remount、刷新、timer、第二location writer或兼容路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:512830`：exact active thread/conversation identity决定sidebar selection；同文件`:515100`把同一`activeThreadKey`交给thread shell。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_state.rs:99`、`:125`与`request_processors/thread_lifecycle.rs:386`：listener generation属于ThreadState，replacement推进generation，旧task只有仍为exact generation才能清current listener；本批吸收同一identity transition和stale-owner cleanup fencing，不复制multi-thread UI、多connection listener registry或unload策略。zcode/minimax未发现更强的Session/material transition owner，未增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused两条新增反例分别得到successor `view=diff`与`view=run-summary`而非null；修复后focused `--detectAsyncLeaks`为3 files / 17 tests，另证明后台close/reconcile保持当前promoted view且Dock不被误清。Frontend `npm run check`为325 files / 1950 tests全绿，完整`--detectAsyncLeaks`同为325 / 1950且126.78s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2738.8/2929.7 KB（93.5%），CSS为131.2/131.8 KB（99.5%）。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04q（explicit Runtime reconnect lifecycle material） | P114-04o已经让`runtime.subscribe`失联同步退休旧connection generation，却仍调用冷启动投影，把已证明的连接丢失重新标成`checking`；同时真实Chat/Composer完全不消费service status，只有Settings页可见。两条单客户端红例先固定这个owner裂缝：失联后期望phase为`reconnecting`、实际为`checking`；同一snapshot下期望Composer旁出现“Runtime connection was lost. Reconnecting…”，实际DOM只有输入控件。根因不是缺一条toast，而是connection owner没有表达“从ready失联”与“从未检查”的领域差异，UI若自行记忆连接历史又会成为第二lifecycle writer。现在breaking扩展`RuntimeServicePhase`为显式`reconnecting`，`RuntimeConnectionOwner.reportConnectionLoss(expectedGeneration)`在exact generation匹配时原子发布`connection/process/capabilities = null + reconnecting`后才启动successor inspection；重复/旧loss仍无权改变current。恢复失败保持空白权利并进入`unavailable`，自动retry期间不伪装checking，只有成功inspection发布新的ready connection generation；command availability同样拒绝reconnecting。Composer附近的`RuntimeConnectionNotice`留在既有`FloatingComposer`维护单元，不新增单消费者薄模块：active transcript和empty session都消费同一service snapshot，冷启动checking不闪提示，reconnecting以warning/status呈现，unavailable以error/alert及Connection settings动作呈现；notice在overlay measurement内部，因此尾部预留随真实高度结算，不覆盖消息，也没有固定高度猜测。交互使用现有SystemMessage、语义色/token、`disclosureTransition`与`AnimatePresence initial=false`，只做opacity/4px位移；八语言同时补齐status、重连、不可用和设置文案。测试把Motion浏览器帧调度同步替换为语义桩，生产动效不变；完整泄露门禁最初由此抓到1个happy-dom RAF，修复后归零。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js:463224`的“Connection to {connectionName} was lost. Reconnecting…”Composer反馈及`:497168`的Sidebar Reconnecting心智；本批吸收失联/冷启动必须由owner区分、反馈紧邻被撤权控制，不复制SSH/Host、多thread、telemetry或多连接UI。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs`与`exec-server/src/rpc.rs`：exact current client先从Connected切Recovering，pending注册/断连结算服从同一身份边界；本批不改Lyra后端，只让P114-04o已经建立的exact owner完整发布其状态。zcode/minimax未发现更强的单client reconnect lifecycle，未制造补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused稳定得到expected `reconnecting`、actual `checking`，DOM expected reconnect copy、actual仅Composer；修复后focused `--detectAsyncLeaks`为3 files / 16 tests，覆盖同步撤权、旧inspection迟到零写、恢复失败持续无权、自动retry发布successor、cold checking静默、reconnecting反馈与unavailable设置动作。Frontend `npm run check`为325 files / 1947 tests全绿，type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，106 public context edges无环，1004 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2738.7/2929.7 KB（93.5%）；完整`--detectAsyncLeaks`为325 / 1947且126.24s零泄露。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。真实HTTP HITL丢回执用例在一次全套泄露轮次中曾返回`session_busy`，沿terminal journal/admission release与same-key replay链审计后，单例真实HTTP+SQLite+泄露检测通过；未把该红例归因于本批，若再次同形出现将独立升级。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04p（exact Run-owned message action materialization） | 用户再次报告真实流式尾部反例后，没有把P114-04a的item-local block边界或P114-04l的visible Markdown generation当成充分条件，而是构造第三个交错：当前assistant的agentMessage Item已经`complete`且可见文本恰好追平，但它所属exact Run仍为`running`；同时current-root attention因HITL/recovery/交接短暂非running。修复前领域selector实际把该行判为`settled`，组件实际挂载`message.actions`，点赞/点踩整排因此进入文档流并继续跟随同一Run后续Item下移；`waiting`存在同形缺口。根因是`TranscriptRow`只携带Message、Tool与delegated facts，没有携带`Message.runId`所指向Run的生命周期；全局`isRunning`只能隐藏当前看到的操作条，既不是该消息的owner，也不能证明整个Run不再追加Item。现在在既有transcript projection维护单元内建立不可拆分`TranscriptRunOwner`值对象，不新拆单消费者薄模块：每一行绑定exact Run ID与`running/waiting/finished`，optimistic未分配行显式为`unassigned`；cache只把owner生命周期作为identity，Run progress/metrics在status不变时保持row引用，跨状态边界才精准重绘。`messageActionMaterialization`对assistant先要求自己的exact Run finished，再与既有block/Tool/delegated Run settlement及`MessageVisibleMaterialOwner`的mounted text generation取交集；successor/current Run无权替predecessor结算。nested delegated renderer的输入breaking收窄为真正需要的`message + facts`，不伪造顶层Run owner。操作条在准入前完全不进DOM/布局，终态后仍沿既有hover/pinned心智；没有opacity/CSS隐藏补丁、delay、timer、强制remount、refresh、第二snapshot或兼容双路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/plan-summary-item-content-C24tzGn_.js`与`subagent-activity-chip-group-DsFB2D8Y.js`：rating/copy等body-after material服从exact thread/turn与item completion；本批吸收terminal action必须绑定其原turn owner，不声称其本地存在Lyra smooth reveal owner。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs`、`turn.rs`与`event_mapping.rs`：message delta/ItemCompleted保留exact thread+turn+item，而Turn lifecycle另有自己的身份与终态；本批吸收Item complete不能代替Turn complete，不复制multi-thread/Host协议。zcode/minimax未发现更强的exact action lifecycle owner，未据此发明补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused两条反例稳定红：领域层expected `active`、actual `settled`，DOM层expected null、actual mounted `message.actions`；修复后focused `--detectAsyncLeaks`为3 files / 26 tests，覆盖running/waiting/unassigned/finished、root attention交接、visible backlog、owner teardown与同status progress引用稳定。Frontend `npm run check`为324 files / 1943 tests全绿，完整`--detectAsyncLeaks`同为324 / 1943且125.05s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，105 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2737.7/2929.7 KB（93.4%）。Runtime全量test（含architecture与SQLite）、vet、build通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04o（exact Runtime connection/event generation owner） | 两条真实单客户端红色反例先证明断线恢复仍会跨代借权：`runtime.subscribe`结束后，测试期望当前connection generation同步变为`null`，实际仍为`runtime_1`，只在后台调用可能长达10秒的`verifyServiceConnection()`；若旧健康inspection已在途，该调用还会被coalesce，旧能力、mutation/query/material owner与loading因此继续伪装为live。第二条反例期望workspace event loop以`start(signal, "runtime_1")`携带exact stream generation，实际只收到signal，迟到结束只能撤销“当前连接”。进一步反证同一Runtime进程重连仍复用服务端`processGeneration`，以及模块级generation reader/subscriber可被退役Plugin installation继续调用并借到successor。根因是服务端进程identity同时承担客户端连接/事件identity，连接投影只描述状态而没有拥有transition；断流把撤权推迟到异步health probe，controller又把recover与旧inspection合并，状态退役、RPC恢复和事件loop没有同一个owner。现在breaking将setup port从`runtimeGeneration/verifyServiceConnection`收敛为`connectionGeneration/reportConnectionLoss(expectedGeneration)`，并建立实例级充血`RuntimeConnectionOwner`：它唯一拥有本installation的opaque connection generation、订阅、exact loss report与dispose；退役实例读取恒为null、不会收到successor通知，也不能报告successor失联。健康inspection若仍连接同一进程则保持connection generation稳定；断流只在owner和expected generation均精确匹配时同步发布空白checking state，立即清除process/capabilities并触发既有mutation/read-model retirement，然后强制启动新的recovery inspection；即使进程未重启，重新建连也生成新的connection generation。`RuntimeServiceController.recover()`现在显式替换active inspection：旧attempt协作abort且deadline非协作释放，cleanup只按exact attempt identity清理，迟到旧结果不能覆盖successor或安装timer。workspace event loop绑定exact generation；旧loop loss、重复dispose、迟到response与已abort delay均不可撤销successor，也不留下reconnect timer。公共service status物理删除无产品调用者的`verify`，没有兼容alias、双路径、刷新绕过、延时竞态掩盖或第二event stream。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs`与`exec-server/src/rpc.rs`：恢复只在`Arc::ptr_eq(current, failed_rpc_client)`时于锁内从Connected切Recovering并先通知connection change，pending request注册与disconnected检查/清空在同一锁边界；本批吸收exact-current、transition-first retirement与stale cleanup fencing，不复制其多connection registry、channel/client重建或session transport。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.js`：明确呈现“Reconnecting…”/connection lost而不是让ready UI等待探测；本批吸收失联即撤销ready material的页面心智，不复制SSH/Host、多thread或遥测形状。zcode/minimax未发现更强的exact connection owner，未增加补充路径。Runtime Go、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused反例稳定得到expected generation null、actual `runtime_1`，且event loop缺失exact generation；修复后覆盖失联同步撤权、旧inspection abort/迟到零写、同进程重连新generation、健康探测代际稳定、旧loss no-op、subscriber仅见null+successor、退役owner隔离、event loop exact generation与零遗留timer。Frontend `npm run check`为324 files / 1938 tests全绿，完整`--detectAsyncLeaks`同为324 / 1938且125.83s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，105 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2737.3/2929.7 KB（93.4%）。Runtime全量`go test -count=1 ./app/runtime/...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04n（Run opening Runtime generation owner） | 单客户端红色反例先让`runs.start`在predecessor Runtime丢失回执，Session的opening controller随后随connection handoff退休，再以完全相同的用户输入通过successor driver重试。修复前第二次结果实际仍为`run_predecessor/seg_predecessor`，successor client调用0次、旧`MutationPromise.retry()`调用1次；`runs.resume`存在同形缺口。根因是P114-02b只撤销mounted Session的presentation/stream pump写权，RPC Agent plugin拥有的`RunOpeningSettler`却只服从Host dispose：它把未知settlement按method+params保留，Runtime process generation变化后仍把旧promise（连同predecessor client、RpcClient pending correlation和stream closure）借给successor调用。这样process-local retained identity与durable mutation journal同时声称跨代接管，破坏唯一owner。现在RPC Agent plugin breaking声明`RUNTIME_STREAM_PORTS`依赖并观察exact `runtimeGeneration`；充血`DefaultRuntimeRunsGateway`将opening settler变为可替换generation owner，换代时先发布全新settler再dispose predecessor，同步abort全部start/resume attempts、retained delivery identity和accepted stream，同代capability/health通知不误退役。successor只能重新通过当前client开门；同一Runtime generation内两次有界delivery与显式产品retry仍复用原promise，跨process/renderer/cold-start的same-key恢复则唯一交给既有durable mutation journal及服务端idempotency store。没有按错误类型兼容分支、参数改写、delay、刷新、第二stream、动态client fallback或重复journal。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_state.rs`、`request_processors/thread_lifecycle.rs`与`exec-server/src/rpc.rs`：listener replacement先取消predecessor并递增`listener_generation`，旧task teardown只有generation仍相等才可清current listener；RPC pending注册与disconnect检查在同一锁内，closed connection不能再准入request。本批吸收exact generation admission/retirement与stale cleanup fencing，不复制其多connection、per-thread listener registry、call-slot或cleanup transport。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.js`：UI把reconnecting作为独立lifecycle feedback，steer继续携带`expectedTurnId`而不把“当前连接”当成旧命令identity；本批只吸收重连不能复活旧in-memory command owner与exact turn address的心智，不复制Host follower、多thread或遥测。zcode/minimax未发现更强的opening generation owner，未增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前插件级红测稳定得到expected successor Run、actual predecessor Run；修复后focused `--detectAsyncLeaks`为5 files / 18 tests，覆盖start retained identity、resume retained identity、accepted stream retirement、同代通知不误退役、同generation product retry继续same-key。Frontend `npm run check`为324 files / 1934 tests全绿，完整`--detectAsyncLeaks`同为324 / 1934且137.35s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，105 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2736.5/2929.7 KB（93.4%）。Runtime全量`go test -count=1 ./app/runtime/...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04m（exact Run cancellation generation owner） | 两层真实单客户端红色反例先证明取消链跨代借权。第一层在renderer/Session replacement窗口保留旧Reasoning：该卡明确呈现`session-predecessor + run-predecessor`，但“立即回答”调用无参数`stopCurrentRootRun()`，exact cancel实际0次，命令会在事件执行时重新读取active Session并停止successor root；同源`cancelActiveSessionRun(runId)`也让Delegated Narrative和mounted Timeline只携带Run、临提交时再借active Session。第二层在`runs.cancel`已经发给旧Runtime后执行`retire-live`并递增Agent event generation：旧成功回执仍实际把`run-retired-cancel`从running折成finished/canceled，随后`onSettled`又通过动态client触发successor recovery。根因是UI mutation target丢失Session维度，且`RunCancellationController`只有React effect级`cancelled`布尔，没有Runtime generation、可退休settlement或client ownership；store提交令牌又只有`viewRevision`，而Runtime handoff只推进`viewEpoch`，所以旧RPC response、failure revalidation、error与repair都能跨越代际边界。现在breaking物理删除`cancelActiveSessionRun`，公开唯一复合命令`cancelSessionRun({sessionId, runId})`：Reasoning从exact MessageContext绑定其消息Run，Delegated Narrative和Timeline直接绑定durable `AgentRunView.sessionId + id`，并校验projection内Run的Session一致；只有Composer Stop/Escape保留语义正确的“当前root”。每个mounted Session内部建立可替换cancel generation，以`RetirableTaskCohort`拥有在途response、failure revalidation与settlement，首次准入时惰性捕获本代client；`retire-live`、`replace-live`和unmount同步退休旧代，successor发布全新controller，旧成功/失败/迟到revalidation均不能commit、repair、toast或释放successor。store再以不可拆分`viewEpoch + viewRevision`提交令牌守住material事务边界，event generation变化即使遗漏上层retire也拒绝旧RunRef。没有active-Session fallback alias、兼容双API、timeout、delay、retry掩盖、refresh绕过或第二Run snapshot；全门禁发现无消费者的公开`RunCommandTarget`类型后将其收回实现边界，没有为内部形状扩张public surface。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.js`：interrupt先验证expected/current turn仍为in-progress，再以exact `{threadId, turnId}`发送`turn/interrupt`，turn mismatch不会重定向到新的active turn；本批吸收presentation-held composite target与server-side exact precondition，不复制其Host follower、descendant cleanup、两步Escape确认或multi-thread形状。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/turn.rs`与`common.rs`：`TurnInterruptParams`同时要求`thread_id + turn_id`，请求按thread串行，steer另以`expected_turn_id`拒绝当前turn不匹配；本批吸收取消不能退化为“当前对象”，不复制其协议。zcode/minimax未发现更强的cancel generation owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 第一条修复前组件反例稳定得到expected exact cancel、actual 0；第二条修复前恢复反例稳定得到expected running、actual finished/canceled，并输出旧settlement触发的successor recovery。修复后focused `--detectAsyncLeaks`为6 files / 60 tests，覆盖旧renderer exact Session/Run、跨Session拒绝、Delegated exact target、in-flight retirement、retire后拒绝、event epoch CAS、旧client迟到零写与successor client独立准入。Frontend `npm run check`为323 files / 1931 tests全绿，完整`--detectAsyncLeaks`同为323 / 1931且122.82s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2746.6/2929.7 KB（93.8%）。Runtime全量`go test ./...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04l（message visible material owner） | 用户真实报告先冻结为单客户端红色反例：assistant文本仍在mounted流式generation中平滑收尾，exact Item与root Run紧接着切到complete/非running时，底部点赞、点踩、复制等action Slot已经挂载，实际按钮随仍增长的尾字下移并造成整行闪烁；测试期望`message.actions`不存在，实际得到已挂载Slot。根因不是动画参数，而是durable block material与可见presentation material各自判断完成：`messageActionMaterialization`只看block/Tool/delegated Run和root attention，`MarkdownMessage`却在其后仍由`useStreamReveal -> useCommitThrottle -> useDeferredValue`拥有一段可见backlog，终态动作因此能跨过尚未结算的DOM generation。现在为每个exact `Session ID + Message ID`建立充血`MessageVisibleMaterialOwner`：每个mounted Markdown/Reasoning可见projection用独立token报告自己实际渲染的`source === text`，动作准入取durable material与全部可见projection settlement的交集；只有两者都settled才发布action row。owner仅在presenting/settled边界通知message chrome，逐token增长仍局部渲染；旧projection按entry token精确退休，不能删除successor projection。冷启动已settled文本首屏直接准入，reduced-motion收尾会正常结算，不会永久隐藏。没有截断平滑输出、CSS隐藏占位、timer/delay、强制remount、root-status特判、refresh绕过或第二message snapshot。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/subagent-activity-chip-group-DsFB2D8Y.js`与`plan-summary-item-content-C24tzGn_.js`：assistant Markdown显式消费item-local `completed`作为`isStreaming`，copy/rating等终态material独立置于body之后并服从exact thread/turn completion；本批吸收动作是presentation之后的terminal material，Lyra因保留本地smooth drain而进一步显式拥有visible generation，不声称参考实现存在相同owner。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs`与`event_mapping.rs`：`AgentMessageDeltaNotification`和`ItemCompletedNotification`分别携带exact thread/turn/item identity并独立映射，接受delta与durable completion不能被root状态合并猜测。未复制其multi-thread/turn协议；zcode/minimax未发现更强的visible material owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前新增组件反例稳定得到expected absent、actual mounted `message.actions`；修复后focused `--detectAsyncLeaks`为3 files / 27 tests，覆盖durable终态但visible backlog仍active、可见代收尾后动作正常发布，以及predecessor teardown不能退休successor token。Frontend `npm run check`为323 files / 1924 tests全绿，完整`--detectAsyncLeaks`同为323 / 1924且160.98s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2746.1/2929.7 KB（93.7%）。Runtime全量`go test ./...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04k（approval action exact owner identity） | 真实单客户端红色反例先制造renderer replacement/Session切换窗口：退役Session中已挂载的审批卡和当前active Session都拥有同名`approval_same`，当前material明确给出active `Session + root Run + item`，但键盘approve仍命中退役卡注册的closure，实际旧action调用1次、当前Session durable resume调用0次；同Session内root replacement也会借到旧root action。根因是独立薄模块`approvalActions.ts`用module-level `Map<itemId, actions>`把展示层closure提升成全局事实，丢失Session与root Run两个owner维度；material owner、mounted renderer bridge和键盘mutation因此能跨代拼接。现在物理删除该薄模块与公开的item-only registrar，建立充血`ApprovalActionRegistry`，以不可拆分`Session ID + root Run ID + item ID`复合身份拥有action entry，并按entry实例精确注销，旧renderer teardown不能删除successor注册。`useApprovalSubmit`在审批卡挂载代捕获exact Session并绑定root/item registrar；键盘路径只用当前material证明的三元组查找，exact命中时继续复用卡内编辑参数与remember选择，任一维不匹配则只提交当前Session自己的durable direct stage，绝不借用旧closure。没有global clear、兼容alias、双registry、timer、delay、强制remount或query refresh。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs`：command approval request显式携带`thread_id + turn_id + item_id`及可选callback approval id，回复不能退化为item-only；前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js`：approval request存入exact conversation，desktop notification由`conversationId + requestId`识别，回复前仍在该conversation内解析request。本批吸收展示action必须服从领域owner复合身份，不复制其multi-host、多pending-request协议或turn-scoped产品形状。zcode/minimax未发现更强且适合Lyra单client/server的action owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前新增反例稳定得到expected retired action 0 calls、actual 1 call；修复后focused `--detectAsyncLeaks`为4 files / 23 tests，覆盖跨Session同item拒绝、同Session跨root拒绝、exact三元组命中及direct durable fallback。Frontend `npm run check`为322 files / 1921 tests全绿，完整`--detectAsyncLeaks`同为322 / 1921且125.89s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2745.2/2929.7 KB（93.7%）。Runtime全量`go test ./...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04j（Terminal Tool selection projection owner） | 真实单客户端红色反例先把selection分成完整Tool material与Terminal command子集：authoritative projection同时含exact `file-edit-1`和`cmd-1`，用户当前选择仍存在的`file-edit-1`时，只读Terminal保持挂载就会用`TerminalViewModel`的local latest-command fallback触发`selectTool("cmd-1")`，实际全局writer调用1次；因此用户在Terminal Dock打开时展开file-edit/read/inline Tool，或Tool route与Dock切换交错，选择会被一个不展示该Tool的后台子集视图篡改。根因是Terminal把“本surface需要高亮哪个command”的派生答案当成“Session当前选择哪个exact Tool item”的领域事实，与ChatStream基于完整authoritative Tool集合的compaction/recovery对账形成两个自动writer。现在物理删除Terminal对`useSelectWorkspaceTool`的依赖和回写effect：不可变充血`TerminalViewModel`继续在本地拥有exact-command命中、latest fallback、command snapshot与material generation，供高亮、定位和pinned-tail使用，但其子集答案不再逸出surface；显式用户点击/Tool route仍写Session-scoped Context Dock selection，完整Tool material的成员淘汰只由ChatStream的唯一reconciler处理。没有按Tool类别加兼容分支、delay、DOM key、selection copy或第二store。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/terminal-panel-CxtrniPa.js`：Terminal panel以exact `sessionId ?? conversationId`建立自己的error/lifecycle边界，所有register/write/output replacement都绑定local terminal identity，不从terminal buffer反向改写conversation item selection；本批吸收surface-local派生状态不得拥有外部selection，不复制其交互式PTY、tab registry、ResizeObserver或RAF调度。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs`与`event_mapping.rs`：CommandExecution、FileChange、MCP、Dynamic/Collab Tool各自保持exact item id，command output delta只更新点名`thread_id + turn_id + item_id`；本批吸收command只是Tool item集合中的一个准确variant，command子集不能把另一个仍存在的item identity重写成自己。zcode/minimax未发现更强的跨Tool-category selection owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused红测稳定得到expected 0 calls、actual `selectTool("cmd-1")` 1 call，且`file-edit-1`明确存在于同一authoritative projection；修复后focused `--detectAsyncLeaks`为2 files / 8 tests，证明Terminal仍本地高亮latest command且不改写exact non-command selection，并保留同长度authoritative output replacement追尾。Frontend `npm run check`为322 files / 1918 tests全绿，完整`--detectAsyncLeaks`同为322 / 1918且124.88s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2744.8/2929.7 KB（93.7%）。Runtime全量`go test ./...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04i（Diff file-focus intent owner） | 真实单客户端反例先证明 mounted Diff 的文件导航不是一次可结算的 intent：Dock 通过 React `Activity` 保持挂载后，先从 Tool 路由到 `src/a.ts`、再路由到 `src/b.ts`，第二次目标虽然被标为 active，却因组件级 `anchoredRef` 已置位而不再滚动，实际 scroll 记录只有 `src/a.ts`。根因是 Session store 用一个 `activeFile:string` 同时冒充 standing selection 与 navigation generation，Diff 再用“首挂载只消费一次”的 ref 补足缺失身份；query replacement、Runtime resync、重复同路径用户 intent 与多文件 Tool 清除旧目标因此没有统一 owner。现在 breaking 删除 `activeFile`/`setActiveFile`/`useActiveWorkspaceFile` 全链，建立不可变充血 `WorkspaceFileFocus(path, revision)`：Session-scoped Context Dock store 是唯一 revision owner，每次 `focusFile`（包括重复同路径）产生 successor intent；Tool route 只提交自己证明的 exact file，多文件 route 显式提交空路径以退休旧选择。Diff surface 只在空focus明确结算或目标DOM确实存在并完成定位后消费exact revision；intent早于query/material到达时保持pending，目标出现后只定位一次，已消费revision上的mode/resync/material replacement则不会把用户重新拉回旧文件。File tree点击也只发 focus intent，不再并行直接滚动；Files/Diff/public port/application/adapter共用同一 snapshot，旧字符串 API 已物理删除。没有DOM key强制重建、timer、delay、query refresh、第二selection cache或兼容路径。前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/pull-request-code-review-CivfTo5J.js`：每次 exact path selection 都更新选择并对对应 DOM target 执行 `scrollIntoView`，本批吸收“每个导航 intent 都应定位目标”的心智，不复制其PR/cloud/multi-host形状。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server-protocol/src/protocol/v2/item.rs`与`event_mapping.rs`：file-change update由exact `thread_id + turn_id + item_id`及changes归属，PatchApply的`call_id`也先映射到exact item identity；本批吸收文件material provenance不能退化为path-only identity，不复制其协议或turn-scoped产品模型。zcode/minimax未发现更强的durable Diff focus owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 第一条修复前组件红测期望scroll记录为`[src/a.ts, src/b.ts]`，实际只有`[src/a.ts]`；提交前反证又证明focus先到、query为空时revision会被过早消费，文件到达后实际scroll仍为`[]`。最终focused及`--detectAsyncLeaks`为6 files / 47 tests，覆盖mounted successor intent、focus-before-material、query replacement不重滚、同路径新revision重滚、Session store revision与多文件Tool清旧focus。Frontend `npm run check`为322 files / 1917 tests全绿，完整`--detectAsyncLeaks`同为322 / 1917且123.27s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2744.8/2929.7 KB（93.7%）。Runtime全量`go test ./...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04h（Terminal output material generation owner） | 真实单客户端反例先证明 Terminal 的追尾 revision 不是 material identity：同一 command 的 live preview `1234567` 被 authoritative completion 整体替换为同为7字符、但含4行的 `1\n2\n3\n4` 时，DOM 高度增长而旧 `tailSignature = command count + output length sum` 前后都为8，Pinned Terminal 的 effect 不运行并永久停在旧底部。根因是 presentation 从内容长度猜 projection generation，且过程式 view model 同时混入 selected Tool，既会发生碰撞，也没有一个对象拥有 command snapshot、latest/fallback selection 与 material lifetime。现在 breaking 删除 `tailSignature` 与函数式 DTO，建立不可变充血 `TerminalViewModel`：每次 exact Agent Tool projection replacement 产生一个 successor 实例，实例冻结该代 command snapshot并拥有count/empty/latest/selected-command fallback行为；Tool selection变化只派生选择，不伪造新的command material。Terminal 的 pinned-tail effect直接服从该实例identity，因此live delta、authoritative replacement、status completion、cold snapshot或Runtime resync只要替换Tool projection就会追尾，同长度/同计数不再碰撞；用户滚离底部仍由既有pinned owner阻止自动抢滚动，P101的Session-scoped Tool selection继续是唯一navigation owner。没有文本hash、长度checksum、timer、delay、ResizeObserver兜底或第二output cache。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/core/src/unified_exec/async_watcher.rs`：每个output delta携带exact `call_id + chunk`，终态再从同一transcript生成aggregated output，字节数不承担事件身份；前端主参考study Codex `/Users/tangerg/Desktop/study/codex/deob/webview/terminal-panel-CxtrniPa.js`：`onInitLog`以whole replacement写buffer，`onData`逐事件写入，并在写前捕获是否位于底部、写后按accepted event请求`scrollToBottom`，不从字符串长度推断变更。本批吸收exact output event/replacement与pinned-before-write心智，不复制Codex交互式PTY、multi-session registry、ResizeObserver sizing或RAF调度，因为Lyra Terminal是只读的Session command log。zcode/minimax未发现更强的durable Terminal material owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused为1 file / 6 tests且新增反例稳定失败：expected different revision、actual `8 === 8`；修复后focused及`--detectAsyncLeaks`为2 files / 7 tests，组件级证明同长度多行replacement会把pinned scroll container从旧位置追到新`scrollHeight`。Frontend `npm run check`为321 files / 1912 tests全绿，完整`--detectAsyncLeaks`同为321 / 1912且122.28s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2744.6/2929.7 KB（93.7%）。Runtime全量`go test ./...`（含architecture与SQLite）及vet通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04g（Run Summary root attention owner） | 真实单客户端 HITL 反例先证明 Run Summary 丢失了 durable Run 的正式状态：Agent fold 与 cold snapshot 都精确保存`running / waiting / finished`，但 summary presentation把current root拆成`runId`与`running:boolean`；审批/提问使Run进入waiting且没有terminal entry时，digest只能把`false`猜成`unknown`，UI在HITL continuation期间稳定显示“Unknown”，resume后再跳回“Running”。根因不是视觉文案，而是 read model 边界把充血判别状态降维为boolean，并同时暴露了一个只有Run Summary消费的`useCurrentRootRunId` public/port/adapter链。现在Run Summary直接消费既有`AgentRootAttention`这一exact root identity + lifecycle状态，`RunDigestSource`不再接受可拼错的ID/boolean；无terminal时running保持running、waiting明确投影为waiting，finished但无terminal才保留unknown以暴露不完整material。workspace badge以warning tone和8语言“Needs input/等待输入”呈现，approval section继续显示exact pending/decision事实；cold recovery与live continuation服从同一个durable root owner，不依赖最近事件或loading。无生产消费者的`useCurrentRootRunId`从public API、Application port、adapter selector和composition物理删除；没有从approval card反推Run状态、没有兼容boolean、refresh、timer或第二cache。后端主参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/thread_status.rs`：pending permission/user-input即使execution不running也保持`ThreadStatus::Active`并携带`WaitingOnApproval / WaitingOnUserInput` flags；前端主参考study Codex `app-initial-BuOiDQdP.js`把这些事实明确显示为“Awaiting approval / Needs input”。本批吸收“waiting是active read-model事实而非unknown”和准确反馈词汇；不复制Codex多request类型优先级、multi-host或resume-state状态图，因为Lyra单client/server已有Run waiting aggregate与Pending interrupt tree。zcode/minimax未发现更强的durable waiting summary owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused为1 file / 11 tests且新增反例稳定失败：expected `waiting`、actual `unknown`；修复后focused `--detectAsyncLeaks`为2 files / 18 tests，覆盖HITL waiting、running/terminal/canceled/limit/unknown状态与warning badge。Frontend `npm run check`为320 files / 1910 tests全绿，完整`--detectAsyncLeaks`同为320 / 1910且121.18s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，1000 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2744.6/2929.7 KB（93.7%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04f（Plan replacement material owner） | 真实单客户端红色反例先证明 Plan 的领域身份与 banner material owner 相互矛盾：Runtime/Agent 注释和后端 aggregate 都明确 Plan 是 Session-scoped whole replacement，但 banner 的 disclosure key、dismissed state 和可见性却只绑定 current root `runId`；同一 Run 内关闭 Plan A 后提交内容不同的 Plan B，旧 UI 仍把 B 判定为已关闭，反例精确得到 replacement 文本 `null`。这也意味着同一 Plan 因 root Run 变化会被错误重建、不同 Session 若碰巧共享 Run-shaped identity 可借用展开状态，而 workspace Plan panel 与 banner 又各自重新计算进度。现在 breaking 收窄公共 Agent Plan surface：不可变充血`SessionPlan`由 exact active Session ID 与 Runtime whole-replacement revision 构成 identity，冻结 accepted steps并内聚active-step/progress行为；banner dismissal、React disclosure lifetime和visible policy全部绑定`SessionPlan.identity`，同 replacement的无关render保留选择，successor revision或另一Session同步退休旧展开/关闭material。Plan workspace view同样消费这个模型，不再第二次过程式统计完成数；只供Agent内部Tool-call projection使用的`activePlanStep`/`planProgress`从公共API移除，避免无外部生产消费者的抽象泄露。没有文本hash、runId兼容分支、强制refresh、timeout或第二状态路径。前端主参考study Codex `app-initial-BuOiDQdP.js`的implement-plan request：所有读取、提交与移除都冻结 exact `conversationId + turnId`，不会按当前conversation的任意Plan猜身份；后端主参考Codex Rust `app-server/src/bespoke_event_handling.rs`与`exec/tests/event_processor_with_json_output.rs`：`TurnPlanUpdatedNotification`携带exact thread/turn，前一todo list完成后的新update创建新list identity。本批吸收replacement必须有显式复合身份与新replacement不借旧presentation state；不复制Codex turn-scoped临时list，因为Lyra Plan由SQLite durable Session aggregate拥有，canonical revision才是whole replacement identity。zcode/minimax未发现更强的durable Plan replacement owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused为1 file / 2 tests且新增反例稳定失败：expected replacement text non-null、actual `null`；修复后focused `--detectAsyncLeaks`为4 files / 26 tests，覆盖同replacement稳定、successor revision、同revision跨Session、dismissal隔离、rich progress/active behavior和workspace projection。Frontend `npm run check`为320 files / 1909 tests全绿，完整`--detectAsyncLeaks`同为320 / 1909且128.20s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2744.5/2929.7 KB（93.7%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04e（Goal command generation / incarnation owner） | 两条真实单客户端红色反例先证明 Goal 的命令与可见 material 都缺少准确 identity：旧 Stop 已在途且 Resume 已按同 Session 排队时替换 Host，两个 Promise 在 replacement 边界仍均为 `pending`，排队命令随后会从模块级 singleton 动态借 successor gateway；另一个 Goal 使用相同 objective、不同 `createdAt` 替换旧 incarnation 时，banner 仍继承前一 Goal 的 allowance 展开状态。根因是模块级 keyed queue、动态 gateway locator、adapter 方法内动态 `getContainer().client()` 和 query repair 共同组成了跨 Host/Runtime 的隐式 owner，而 UI 又把可编辑 objective 当作 Goal identity；Runtime generation replacement 没有进入 Goal plugin，accepted command receipt 与 standing query repair 也被错误合并成一个成败。现在物理删除 singleton gateway 配置与模块队列，建立充血 `GoalCommandOwner -> GoalCommandGeneration`：每个 exact Plugin Host / Runtime generation 固定一个 exact `LyraClient` gateway，以 `RetirableTaskCohort` 和 per-Session tail 共同拥有 start/stop/resume 的准入、顺序、回执校验、query repair 与 settlement；successor-first replacement 同步退休旧代，在途和排队命令立即中性结算，旧 RPC、迟到 repair 与 stale disposer 不再拥有 successor 写权。命令回执只证明该 mutation 被接受，standing Goal 继续由 query/event material owner 恢复；repair 失败不能把已接受命令改写为失败，也不能覆盖原命令错误。Goal plugin 通过真实 Runtime stream generation 轮换 owner，UI 只把 generation retirement 视为生命周期结算；banner 以 immutable `Session + createdAt` 作为当前协议下最窄 incarnation identity，同 objective 新 Goal 不再借用旧 disclosure/busy material。前端主参考 study Codex `app-initial-BuOiDQdP.reconstructed.js`：conversation action 在 admission 时冻结 exact `conversationId + hostId`，rename transition queue 使用同一 resource key；本批吸收 exact resource admission 与 facade-owned ordering，不复制 multi-host、undo/redo 或 telemetry。后端主参考 Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`：`ConnectionRequestId`合并 connection/request identity，thread cancellation 先从 pending registry 摘除 exact entries 再结算 callbacks；本批吸收 transition-first retirement 与 exact settlement，不复制 multi-connection registry。zcode/minimax 未发现更强的 Goal command generation owner，未据此增加补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring 与 Go Agent Framework 合同均未变化 | 修复前 command focused 反例稳定得到 expected `retired/retired`、actual `pending/pending`，同 objective/new `createdAt` 的 banner 反例显示旧 `Hide the allowance`；修复后 focused `--detectAsyncLeaks` 为 6 files / 35 tests，覆盖 Host replacement、真实 Runtime generation、在途/排队 settlement、successor exact client、迟到回执、accepted receipt + repair failure、Session mismatch 和 Goal incarnation material reset。Frontend `npm run check` 为 320 files / 1905 tests 全绿，完整 `--detectAsyncLeaks` 同为 320 / 1905 且 120.00s 零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle 全绿，104 public context edges 无环，999 locale keys 跨 8 语言一致，87/87 operations + 3/3 sidecars + 16/16 events 保持覆盖，entry bundle 为 2744.2/2929.7 KB（93.7%）。Runtime architecture 与 SQLite invariant focused tests 通过；Desktop Wails v3 Go test/vet/build 通过（仅既有 macOS linker warning）。未使用 agent-browser，`app/cli` 未修改或暂存 |
| 2026-08-18 | P114-04d（Inline copy material/intent owner） | 真实单客户端红色反例先证明流式Tool output已经从`old output`替换为`replacement output`后，旧clipboard Promise迟到成功仍把replacement按钮标成`Copied`；现有Run Summary、Shiki code block与Tool output三处各自用renderer-local boolean/timer表达复制反馈，既不绑定被复制的exact material，也没有intent revision，其中Tool output甚至不在卸载时清timer。于是Run A反馈可借给Run B、流式代码/输出的旧文本回执可标记新文本，同一material的旧成功也能越过较新失败并延长/缩短另一intent的timer。现在三条路径统一使用`useCopyFeedback`这一有三个真实消费者的material owner：每个mounted consumer以exact plaintext为resource identity、以monotonic revision拥有copy admission、settlement、可见accepted revision与唯一reset timer；layout ownership transition在replacement paint/event前退休旧revision，只有exact current material+latest intent+mounted owner可发布反馈，失败、换料、回退到同文本与unmount都会使旧continuation失去写权，timer由同一owner清理。旧`setCopied`/copy timer写路径已物理清零，没有timeout竞态掩盖、DOM key强制重建或第二反馈cache。前端主参考study Codex `app-initial-BuOiDQdP.reconstructed.js`的`CopyButton`：统一可访问`Copied`反馈并用mounted guard阻止卸载后的timer写入；本批吸收集中反馈与teardown，但拒绝其点击即置boolean、无await/material identity且裸`setTimeout`的形状。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`的`ConnectionRequestId` exact identity与take/retire结算原则，映射为material+intent复合身份；不复制multi-connection registry。zcode/minimax未发现更强的流式copy feedback generation owner，未据此发明补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前Tool output focused反例1/1稳定红，DOM明确同时显示replacement文本与`aria-label=Copied`；修复后focused `--detectAsyncLeaks`为2 files / 3 tests，覆盖跨material迟到success、同material latest intent、unmount timer retirement。Frontend `npm run check`为319 files / 1900 tests全绿，完整`--detectAsyncLeaks`同为319 / 1900且119.63s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2742.9/2929.7 KB（93.6%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04c（Workspace view Session material owner） | 真实单客户端红色反例先证明同一个workspace view在Dock与中间提升区拥有不一致的Session lifetime：`ses_first`中已挂载的stateful promoted view实例为1，切到`ses_second`且view id不变时旧实现仍显示实例1；因此Diff折叠/滚动、Run Summary复制反馈、Tool/Codebase选择/loading/error等组件瞬态material都可能从上一Session借给下一Session。源码反证确认Dock单独经过`SessionOwnedDock`，promoted/full路径却直接挂载`WorkspaceViewBody`，同一领域视图被presentation placement切成两个生命周期owner。现在`ChatPanel`内聚唯一`SessionOwnedWorkspaceState`结构边界，并由Dock与promoted/full共同使用：exact Session ID变化时整棵workspace view material同步退休；同一Session内Dock tab仍由`Activity`保持选择/滚动并在隐藏时清effect，持久化的per-Session dock navigation仍由既有store拥有。原先只有`ChatPanel`使用的`SessionOwnedDock`及其独立维护文件/测试被物理删除，反例并入真正拥有两种placement的shell测试，避免再为一个调用点制造薄模块。没有逐视图补丁、refresh、timeout、opacity或第二状态路径。前端主参考study Codex `app-initial-BuOiDQdP.reconstructed.js`的tab/window generation：失活transition推进generation，reader只接受`tab.generation === current generation`的引用；后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`把request identity与connection identity合并并在connection close按exact identity退休context。本批吸收exact owner boundary与generation mismatch不可借用，不复制Codex多窗口/多connection状态形状；zcode/minimax未发现更强的统一Dock+promoted Session owner，未据此发明补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前promoted focused反例1/1稳定红并显示`workspace-instance:1`；修复后focused `--detectAsyncLeaks`为1 file / 2 tests，覆盖promoted与Dock两种placement的Session退休。Frontend `npm run check`为317 files / 1897 tests全绿，完整`--detectAsyncLeaks`同为317 / 1897且121.81s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2742.5/2929.7 KB（93.6%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-04b（Codebase search material generation owner） | 真实单客户端红色反例先证明 P114-03g 只退休了 Codebase command settlement、没有退休已经显示的搜索 material：`CodebaseWorkspaceSurface`先成功显示`retired.ts:7-7`，随后同一 Host 收到 Runtime generation replacement，旧命令 owner 已切代但旧命中仍留在 DOM；focused 2 tests 中新增反例稳定失败。根因是`CodebaseCommandOwner`拥有RPC、reindex tail与status repair的generation，React组件却独立拥有hits/error/searching/reindexing，且surface只以cwd作为Session navigation identity；因此相同workspace上的Runtime重启不会重建material owner，旧索引结果可伪装成successor事实。现在充血`CodebaseCommandOwner`同时发布唯一material generation，Host install、Runtime replacement与final disposal在命令cohort退休的同一transition推进epoch；Codebase surface用`useSyncExternalStore`消费该epoch，明确把查询文本留作用户草稿，把hits/error/searching/reindexing放入generation-keyed material子树。这样Runtime换代保留可重试的查询文本，但同步清空旧结果、错误和loading；旧不合作RPC仍由既有cohort拒绝且无DOM发布权，cwd外层identity继续负责Session/workspace切换，没有refresh、timeout或第二结果路径。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs` 的connection-scoped request context清理与take-once settlement；前端主参考study Codex `app-initial-BuOiDQdP.reconstructed.js` 的authorization lease generation（acquire前冻结generation、invalidation递增并拒绝旧lease）及git live-query generation gate（旧generation update不得写query）。本批吸收transition-first retirement、generation lease与用户draft/Runtime material分离，不复制多connection、多Host、专用live-query subscription/retry协议；zcode/minimax未发现更强的独立Codebase结果generation owner，未据此发明补充路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复后Codebase focused `--detectAsyncLeaks`为3 files / 9 tests，覆盖Session/cwd切换、Host/Runtime迟到search/reindex与visible material retirement。Frontend `npm run check`为317 files / 1896 tests全绿，完整`--detectAsyncLeaks`同为317 / 1896且118.90s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2742.5/2929.7 KB（93.6%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03o（Message feedback command/material generation owner） | 三条真实单客户端红色反例先证明消息反馈同时缺少resource与generation owner：同一消息先赞后踩时，较新的踩已成功，较早赞的迟到失败仍按旧`previous`把全局Map删空；adapter/Host dispose后已admit命令在微任务边界仍`settlement=undefined`，必须等待不合作旧RPC；来自`ses_original`的消息在Session切换后提交时，adapter动态读取active Session并实际把反馈写到`ses_successor`。根因是Application只有一套renderer-global、仅按messageId索引的Map和`createSingletonPort`，React组件又持有第二份local optimistic state；每次RPC独立并发、失败无intent revision，adapter在方法体动态读取Session与container，反馈插件也未订阅Runtime generation。现在物理删除singleton locator与组件local writer，建立充血`MessageFeedbackOwner → MessageFeedbackGeneration → MessageFeedbackAggregate`：每个exact `Session + Item` aggregate同时拥有accepted/selected material、intent revision、同资源串行tail与`RetirableTaskCohort` settlement；改选按用户顺序进入durable append，旧失败只能影响自己的intent，最新失败精确回到last accepted。Host安装、Runtime replacement与final disposal先切owner、立即reject在途/排队调用并整体清空旧按钮material，迟到response再无发布权；按identity分桶的subscription只重绘对应消息，换代才统一退休。Message Context breaking前移为`{sessionId,message}`，由transcript owner普通prop/context下传，反馈在点击admission时即冻结exact Session/Run/Item；adapter在composition捕获exact `LyraClient`，不再借当前Session或current container。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`：`ConnectionRequestId`把request绑定connection，callback以take/drain一次结算并在connection transition清空；本批吸收exact identity、transition-first retirement与一次结算，不复制multi-connection registry。前端主参考study Codex `plan-summary-item-content-C24tzGn_.js`：turn rating始终携带exact threadId+turnId，selection属于当前turn material；补充对照zcode `index-DxQm3wxO.js`同样携带taskId+assistantMessageId并在持久化失败时回滚。采用exact conversation/message identity与可访问pressed feedback；不复制Codex fire-and-forget analytics/report flow，也不复制zcode可并发local rollback、reaction burst或toggle-to-null，因为其旧失败仍可覆盖新intent且Lyra要求durable ack。minimax未发现可比的逐消息rating owner，仅有全局feedback/comment UI，未据此发明第二路径。用户报告的流式尾部按钮闪烁继续由P114-04a `a65c2f4f4`的active-turn action material边界负责，本批没有加动画或opacity掩盖。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused 2 files / 3 tests全部稳定红：新踩被回滚为`undefined`、dispose时settlement仍`undefined`、请求Session从original串为successor。修复后focused普通与`--detectAsyncLeaks`为4 files / 10 tests，覆盖改选串行、older failure、latest rollback、Runtime/Host replacement、final dispose、同Item跨Session隔离、exact client与真实插件generation接线。Frontend `npm run check`为317 files / 1895 tests全绿，完整`--detectAsyncLeaks`同为317 / 1895且119.52s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，104 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2742.0/2929.7 KB（93.6%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03n（Conversation archive command/material generation owner） | 四条真实单客户端红色反例先证明会话归档没有状态 owner：旧 Host 的文件选择器悬挂时安装 successor，选择结果会在旧命令 continuation 中动态读取全局 gateway 并调用 successor `sessions.import`；旧 import RPC 已在途时 replacement 不结算命令，迟到 response 继续 rehydrate、query repair、Session navigation 与 success toast；旧 export RPC 失败会在 successor 时代读取当前 active snapshot并触发local download；原生文件对话框直接取消只发`cancel`，旧adapter只监听`change`，Promise与后续import永久pending。前三例在replacement微任务边界均稳定得到`settled=false`并观察跨代副作用，第四例在cancel后同样未结算。根因是文件机制和Runtime gateway由两个独立`createSingletonPort`持有，Application use case在每个await后重新借全局当前值；runtime adapter方法体又动态`getContainer()`，Workspace mutation lifecycle完全不知道picker/RPC/query writer/navigation/download，browser adapter的取消合同也没有真实事件owner。现在物理删除两套singleton locator，browser adapter只构造`FileTransferPort`并以一次性settler统一处理cancel、空选择、读取成功/失败；Runtime adapter在composition时捕获exact `LyraClient`并安装充血`ConversationArchiveOwner`。owner的每个Host/Runtime generation以同一个`RetirableTaskCohort`拥有picker、export/import RPC、rehydrate、Session-list repair、navigation、toast和download，replacement同步中性结算旧命令，底层不合作Promise仍被观察但再无continuation写权。重复import意图合并为一个picker/一个mutation；authoritative import与rehydrate成功后，即使collection repair真实失败仍导航，不把query repair误作业务失败，但repair在途遇replacement则整体退休。导出在命令开始的同步边界一次捕获exact Session ID、snapshot、时间与fallback payload，Session切换或RPC失败不再拼接另一Session material；retired failure不得进入fallback。Workspace composition root把owner接入唯一`replaceRuntimeGeneration`与dispose。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`：`ConnectionRequestId`绑定exact connection，response/context以take-once结算，connection/turn transition先retain/drain pending；本批吸收exact identity、transition-first retirement与一次结算，不复制多connection callback registry。前端主参考study Codex `app-initial-BuOiDQdP.reconstructed.js`：copy-conversation action先以exact conversationId完成history materialization再读取markdown，`ABo`只向consumer暴露conversation行为并在内部携带host/conversation identity；补充对照zcode在Host unregister时集中fail其`pendingOwnerCommands`。本批采用exact action owner、material-before-effect与Host teardown，不复制Codex undo/telemetry、zcode多Host bus或timeout政策；minimax未发现更强的conversation archive owner，未据此发明第二路径。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前generation focused 1 file / 3 tests全部稳定红，replacement settlement均为false且三类迟到副作用均可观察；browser cancellation 1/1另行稳定红在`settledAtCancel=false`。修复后focused普通与`--detectAsyncLeaks`均为3 files / 10 tests，继续覆盖rehydrate在途、query repair在途、repair真实失败、重复import single-flight、exact client绑定与同Host Runtime generation。Frontend `npm run check`为314 files / 1887 tests全绿，完整`--detectAsyncLeaks`同为314 / 1887且119.64s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle为2739.4/2929.7 KB（93.5%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03m（test-only singleton command bypass removal） | 架构红色反例先扩展既有port-surface门禁：与其只检查`*Port`方法是否有非测试调用，新规则同时检查每个`export const x = port.get`是否存在真实产品caller；旧代码稳定报出正好3条旁路：Provider、Agent Memory与Codebase gateway accessor的生产调用数均为0，唯一caller全是adapter测试。源码反证确认三者的composition root仍为这些测试专用locator安装/撤销第二套全局状态，而产品命令已分别由`ProviderMutationOwner`、`AgentMemoryMutationOwner`、`CodebaseCommandOwner`独占generation、resource queue与material repair；任何未来调用该accessor的代码都能绕开同资源串行、replacement retirement和迟到隔离。现在物理删除三套`createSingletonPort`、configure/get导出及adapter冗余dispose，保留真正有意义的Application gateway interface作为依赖倒置合同；adapter测试breaking改为与产品相同的公开Application facade，因此Provider role测试实际经过authoritative commit/query repair，Agent Memory add/pin经过resource owner，Codebase reindex经过generation owner。真实facade也立刻暴露并修正了旧direct-gateway测试掩盖的utility/embedding query GC timer清理责任。门禁永久拒绝“只有测试调用的singleton accessor”，错误信息要求删除测试旁路而非增加伪产品caller。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`：pending callback/context map完全封装在`OutgoingMessageSender`内，只通过connection-scoped register/take/drain行为访问，没有向测试发布raw map getter；本批吸收封装与唯一owner，不复制其多connection registry。前端主参考study Codex `ABo` action facade：consumer只调用`renameThread/archiveThread`，Host request和`hostId+conversationId` transition queue留在facade内部；补充对照zcode的`pendingOwnerCommands`也只由Host manager按exact owner存取。本批采用测试与产品共用行为入口，不复制Codex undo/redo、telemetry或zcode多Host router；Lyra仍严格单client/server。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前`check:port-surface`稳定红并精确列出3个test-only command accessor；修复后focused `--detectAsyncLeaks`为6 files / 20 tests零泄露，覆盖三类真实facade mapping/commit、Host replacement、同Host Runtime generation与迟到隔离。首次改走真实Provider facade时泄露检测抓到测试未清理role query的GC timer，补齐真实read model teardown后复跑清零。Frontend `npm run check`为311 files / 1877 tests全绿，完整`--detectAsyncLeaks`同为311 / 1877且119.80s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/增强port-surface/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle降至2737.7/2929.7 KB（93.4%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03l（Agent Session command settlement generation owner） | 五条红色反例先证明同一真实恢复缺口：Session summary conditional-write、create、delete、rollback pre-read与fork已由旧Plugin Host发出后安装successor，旧continuation虽然靠`isCurrent()`避免迟到写入，却全部继续pending直到测试手工释放非协作旧RPC；因此renderer replacement/Runtime restart能留下永久按钮loading、未结束的对话框提交和被旧请求占住的调用Promise。源码反证进一步发现rename/favorite在真实command admission前先等待不受owner约束的`cancelQueries`，steer、abandoned draft delete与rollback rehydrate也只在迟到完成后检查代际。根因是`AgentCommandOwner`只拥有optimistic effect、single-flight key和Session revision queue，却没有拥有这些队列所等待的真实settlement，状态写权与调用生命周期被拆开。现在充血owner的同一`RetirableTaskCohort`成为所有Agent Session产品命令的唯一admission/settlement边界：create/fork single-flight与summary queue在execute时登记真实Promise，rename/favorite的query cancellation、delete、draft cleanup、rollback snapshot/command/rehydrate、running-turn steer均通过同一cohort；replacement同步reject旧在途与排队调用，optimistic effect只回滚自己的字段，successor按独立Session revision queue立即准入，旧底层Promise仍被观察但迟到response、navigation、query/material repair和error toast均无写权。create/fork把正常业务错误与generation retirement在唯一外层结算，删除分散的内层catch，不使用timeout、刷新绕过或第二loading路径。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`：`ConnectionRequestId`把context绑定exact connection，connection close只移除该代context，callback以take/drain一次结算；本批吸收exact owner、transition-time cancellation与take-once，不复制multi-connection/thread request registry。前端主参考study Codex `app-initial-BuOiDQdP.reconstructed.js`：rename command以`hostId+conversationId`组成transition queue key，Host summary读取以generation拒绝旧subscription/read；补充对照zcode按exact owner Host删除pending command并清timeout。本批采用资源级串行、Host generation teardown与terminal feedback归属，不复制undo/redo telemetry、多Host router或timeout政策；Lyra仍严格单client/server。用户报告的流式尾部点赞/点踩闪烁另由已推送P114-04a `a65c2f4f4`复核：active turn不挂载action Slot，本批未增加动画/opacity掩盖。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused 4 files / 22 tests中新增5条replacement反例稳定失败，分别观测到summary RPC outcome=`pending`及create/delete/rollback/fork的`settledBeforeOldRPC=false`；修复后focused `--detectAsyncLeaks`为8 files / 34 tests，覆盖真实settlement即时retire、successor独立准入、旧response零navigation/material、optimistic rollback与既有revision carry。Frontend `npm run check`为311 files / 1877 tests全绿，完整`--detectAsyncLeaks`同为311 / 1877且118.91s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle 2737.9/2929.7 KB（93.5%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03k（Task cohort / Settings feedback material generation owner） | 三条红色反例先冻结两个同源缺口：`useProbe`已经显示完成态`ok`后切换material generation仍继续显示旧成功；Provider与MCP mutation owner没有可观察material generation，install、同Host Runtime replacement与final disposal都无法整体退休已完成反馈。源码反证同时发现Provider、Hook、Schedule、Agent Memory仍让每个已完成operation挂在同一个长期不结算的retirement Promise上；正常settlement虽然返回，`Promise.race` loser reaction却保留到换代，长会话会累积内存并在Runtime重启时制造与历史命令数线性相关的微任务尖峰。此前Codebase、Knowledge、Skill、Diagnostic Tool、MCP与Agent command又各复制一份手写settler set，形成同一lifecycle机制的十个维护实现。现在breaking将窄名`serialTaskQueue`改为`taskQueue`，在这一个已有多消费者维护单元中建立充血`RetirableTaskCohort`：每个generation显式admit真实Promise，resolve/reject立即take并解除retirement挂接，retire只reject仍pending的settlement，底层不响应cancel时迟到结果仍被观察但不能重新结算；十个Application owner统一委托该cohort并在retire清空自己的resource tail/accepted material，不再拥有手写settler或共享retirement race。Provider与MCP owner成为各自transient feedback的唯一可观察时钟；`useProbe`把busy/ok/error和在途token绑定到该generation，generation变化在render阶段立即隐藏旧feedback、commit前以layout lifecycle撤销迟到写权，同时保留credential/MCP表单草稿而不靠remount清空用户输入。Provider UI回归精确证明旧`Connection OK`消失但未保存base URL仍保留；MCP表单复用同一generation-bound probe。剩余Application `Promise.race`仅是拥有显式release/abort的Run opening deadline与Workspace stream opening deadline，不再存在owner retirement loser。后端参考Codex Rust `/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`：`ConnectionRequestId`绑定exact connection，请求context/callback分别由map持有，response/error只take一次，connection close只retain掉该connection；`thread_state.rs`以`listener_generation`先撤旧listener再发布新代。本批吸收exact owner、take-once与replacement-first retirement，不复制多connection fan-out、thread replay registry或server request协议。前端主参考study Codex `app-initial-BuOiDQdP.reconstructed.js`的MCP reauth：loading只属于当前prompt实例，Host侧以`hostId+server`管理prompt/error并在startup status恢复时整体移除；补充对照zcode `deob/main/index.js`按Host精确删除pending owner command/session delivery并清timer。本批采用generation-owned pending/terminal反馈与exact teardown，不复制OAuth prompt、multi-Host router、timeout policy或telemetry；Lyra继续严格单client/server。Runtime operation、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同均未变化 | 修复前focused 3 files / 18 tests新增3条稳定失败：旧feedback实际仍为`ok`，两个owner均报`materialGeneration is not a function`；修复后focused 13 files / 58 tests覆盖cohort完成/非协作迟到/retire、十类owner既有generation合同、Provider/MCP material clock、完成态与在途feedback retirement及草稿保留。Frontend最终`npm run check`为311 files / 1875 tests全绿，完整`--detectAsyncLeaks`同为311 / 1875且117.63s零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle 2737.9/2929.7 KB（93.5%）。首次完整check仅因全矩阵并行冷`go build`令Runtime HTTP `beforeAll`超过60s、44场景未入场；无残留检查后隔离44/44在13.50s通过，随后完整check一次性复跑全绿，没有延长timeout或修改生产逻辑。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03j（Diagnostic Tool / MCP server command generation owner） | 先在旧生产路径增加三条真实adapter反例，其中两条稳定红：旧Host的`tools.invoke`已在途时安装successor，旧response仍错误resolve为可展示结果，得到`operation unexpectedly resolved`；一个已安装的Tool catalog gateway在调用MCP reconnect时动态读取current container，实际调用successor client而exact retired client调用数为0。继续沿material层反证发现，即使只退休在途Promise，已完成的诊断结果、参数与running局部状态仍按`workspace+tool` React key跨同Host Runtime重启保留。根因是`ToolCatalogGateway`把两个不同聚合的command拼在一个singleton port：MCP reconnect绕过已有`MCPServerMutationOwner`，诊断Tool又由动态client、局部Promise与React material各自持有代际；MCP自身还保留只供adapter测试读取的第二singleton gateway。现在物理删除`toolCatalogGateway` port、`runtimeToolCatalogGateway` adapter及MCP singleton getter，MCP reconnect breaking并入同一充血`MCPServerMutationOwner`：每次installation捕获exact `LyraClient`，按server identity与create/update/enable/delete共享tail，重叠reconnect admission合并为同一Promise，不同server保持独立；accepted admission只触发MCP server/tools projection repair，Host/Runtime replacement同步退休在途/排队命令，UI用同步ref+本地admission latch封住React state生效前重复点击，connecting/terminal仍只服从Runtime事件，retirement为neutral settlement。直接诊断Tool建立独立`DiagnosticToolOwner`与consumer-owned `DiagnosticToolGateway`：按`cwd+tool`串行同资源调用、精确绑定Host/Runtime generation、旧在途/排队response零material；owner同时发布自己的material generation，`DiagnosticToolRow`以该identity整体重建，因此Runtime重启会一次退休已完成result、arguments、error和running，不靠effect逐字段清空。两类owner都只登记真实在途settler，已完成命令不再把共享`Promise.race` loser保留到换代。跨context唯一公开面breaking收敛为`serverCatalog`，Workspace不再接触Settings私有owner。后端首先复核Lyra Runtime `internal/application/mcp/connections.go` / `coordinator.go`与`internal/infra/mcp/reconnect.go`：application按server替换dial generation，Infra只在exact current attempt下swap session/tools，Desktop据此让同一server的配置与reconnect共享owner而没有复制live dial。Codex Rust对照`/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`的connection-scoped request identity/take-once callback、`thread_state.rs`的listener generation，以及`core/src/session/mcp.rs`的exact-client hard refresh；吸收exact settlement与generation-first replacement，不复制多connection fan-out、thread registry或Codex Apps专用refresh。前端主参考`/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js`的`isStreaming` item反馈与MCP reconnect/reauth显式pending/error，补充查看zcode/minimax的running状态反馈；本批采用即时disabled/spinner、明确error与代际整体退役，不复制其OAuth、插件市场、daemon queue或私有telemetry形状 | 红测修复前1 file / 3 tests中新增2条稳定失败；修复后focused `--detectAsyncLeaks`为7 files / 29 tests，覆盖Host replacement、同Host Runtime generation、在途/排队diagnostic retirement、completed material retirement、exact client、MCP reconnect同资源合并/配置排序、successor Host接管、UI同步latch与neutral retirement。Frontend `npm run check`与完整`--detectAsyncLeaks`均为311 files / 1870 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle 2737.6/2929.7 KB（93.4%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03i（Knowledge command/query/draft generation owner） | 两条不依赖新API的真实adapter红例先证明：旧Host `knowledge.get`或`knowledge.update`已经越过workspace open并等待Runtime response时安装successor，两个旧promise仍错误resolve；旧save随后继续全量invalidate共享successor Knowledge query。继续审计发现列表`knowledge.list`本已返回三个scope的完整content/revision，行展开却再发一次`knowledge.get`并以局部`loadSequence`提交编辑器，形成无事件代际关系的第二snapshot writer；若direct read比list新，save settlement还会把旧list的不同opaque revision误判为save后的更新而回退内容。根因是singleton port、动态current container gateway、direct read/save settlement、query projection、CAS baseline和React draft没有共同Host/Runtime/document owner；`knowledgeConfig`与唯一UI消费者使用的`knowledgeEditor`又把同一document lifecycle拆成两组过程式函数。现在删掉config/editor生产文件与port singleton getter，建立充血`KnowledgeOwner`和不可变`KnowledgeDraft`：每次Host installation捕获exact `LyraClient`，generation只登记真实在途settler并按semantic document identity串行save，`home`忽略cwd保持全局一个资源，cwd/project按workspace+scope隔离；不同资源独立推进。Host replacement与同Host Runtime generation replacement同步reject旧read、在途save和queued save，非协作迟到response零commit/repair，stale installation不能影响successor。save成功先按scope提交response证明的事实：cwd/project只upsert原`[knowledge,{cwd}]`，home同步所有已挂载Knowledge projection；project/home可能共享physical alias但wire不暴露canonical path，因此不猜测其他project cache，由现有`knowledge.changed`与repair收敛。accepted response不因repair失败伪造成command失败，revision conflict和ambiguous error保留原failure并先修复read path。`KnowledgeDraft`自己拥有open/dirty/edit/revert/reconcile/settleSave/rebase，用户在途编辑不会被save覆盖，conflict rebase保留draft；UI把generation retirement结算为neutral idle。列表已是唯一完整material，展开只切换Collapsible而不再read/loading，CAS conflict才通过current generation exact read；删除8个语言包中的`knowledge.loading/loadError`两个死key。首次全门禁先以published-boundary守卫反证局部变量`document`与浏览器全局词汇冲突，按领域改名`snapshot`而未加allowlist；第二次由locale守卫精确找到16个死翻译并物理删除。后端以Lyra Runtime `internal/application/workspace/knowledge.go`为主事实：充血`Knowledge`先解析scope/physical root，再以expected revision CAS，成功返回committed Entry后接管AuthoredWatch与`knowledge.changed`；Desktop保持此边界，没有把Knowledge并入Skills/Memory。Codex Rust只读对照`/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`的exact callback take-once/transition cancel，吸收settlement capability而不复制多connection registry。前端对照`/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js`的project instructions：saving/load-error有明确form反馈，成功rename response通过`c8c`同步project detail/infinite page/list projection；本批吸收authoritative multi-projection commit与保存反馈，但其rename+instructions+theme合并mutation、undo、cloud project cache及无revision CAS形状均不适合Lyra，冗余展开loading路径明确不采纳 | 修复前focused为1 file / 4 tests且新增2条Host replacement反例稳定失败，均得到`operation unexpectedly resolved`；修复后focused `--detectAsyncLeaks`为3 files / 15 tests，覆盖Host replacement、真实in-place Runtime generation、direct read/在途/排队save retirement、同document串行/跨scope独立、home跨cwd唯一identity、authoritative exact/home全投影commit、repair失败、adapter exact scope/wire conflict与rich draft全状态迁移。Frontend `npm run check`与完整`--detectAsyncLeaks`均为309 files / 1861 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，999 locale keys跨8语言一致，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖，entry bundle 2734.7/2929.7 KB（93.3%）。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03h（Skill curation resource / generation owner） | 两条红色反例先证明同一真实产品缺口：user Skill `restore` 尚在旧独立library mutation中时，同名user proposal `approve`会通过另一套proposal mutation并发进入Runtime，修复前观测到restore结算前approve调用数为1；旧Host `archive`在途时安装successor，旧promise仍错误resolve并可继续失效共享query。根因是Skill library与proposal review被拆成两套只被各自config使用的单消费者port/adapter，mutation settlement、同一Skill资源顺序、proposal→library投影与Host/Runtime generation没有共同owner；两边只在成功后全量invalidate，既没有提交response已证明的事实，也无法表达Runtime canonical workspace与Desktop Session alias query的关系。现在删除`skillLibraryConfig`、`skillProposalsConfig`及其两套port/adapter，按Runtime既有充血`application/workspace.Skills`的真实业务边界建立一个Desktop `SkillCurationOwner`、一个跨边界`SkillCurationGateway`和一个捕获exact `LyraClient`的adapter。owner内部generation只登记真实在途settler，按`user/name`或`project/workspace/name`分区tail；user library与user proposal因此共享同一资源顺序，不同Skill独立推进。Host replacement与同Host Runtime generation replacement同步reject旧在途/排队命令，非协作迟到response零commit/repair，stale installation不能轮换successor。archive/restore精确提交managed lifecycle并同步所有已挂载user catalog；approve/reject按workspace/name/revision/scope移除exact proposal，user approve精确提升到managed与全部user-visible catalog，project approve从proposal cache捕获原始`WorkspaceCatalogQuery`，即使Runtime返回canonical workspace也只写回最初列出它的Session alias scope。accepted mutation与cache repair分相，repair失败不伪造command失败；两个UI action latch保持原owner，只将generation retirement恢复为neutral idle。生产薄模块从六个减为三个，旧路径物理删除，没有兼容转发、第二cache或delay。后端首先对照Lyra Runtime `application/workspace/skills.go`，其`Skills`本就共同拥有discovery/library/proposals，Desktop据此合并边界而没有把独立Knowledge错误并入；再对照`/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`的exact callback take-once/cancel语义，吸收settlement capability而不复制多connection registry。前端对照`/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js`的Skill/plugin list/install/uninstall/read动作：其可见反馈服从一次exact host request，但不存在Lyra immutable proposal→same library resource的产品语义；本批不复制其多host、plugin cache、package或telemetry形状 | 修复前focused为1 file / 2 tests且两条反例稳定失败，分别得到restore结算前approve调用数1与`operation unexpectedly resolved`；修复后focused `--detectAsyncLeaks`为2 files / 10 tests，覆盖同资源跨命令串行、Host replacement、真实in-place Runtime generation、迟到response零repair、accepted library commit/repair失败、user proposal全投影提升、canonical workspace→原alias query scope及adapter exact-client映射。Frontend `npm run check`与完整`--detectAsyncLeaks`均为308 files / 1853 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03g（Codebase command / workspace material generation owner） | 两条真实adapter红色反例先证明：旧Runtime `codebase.reindex`与`codebase.search`卡在RPC response时安装successor gateway，旧promise仍错误resolve；reindex把`op_retired`提交进current status projection，search继续触发successor query invalidation。继续审计发现adapter每次命令动态取得`getContainer().client()`，Application在accepted response后全量invalidate所有cwd的status；Codebase view的query/hits/busy/error则由不随Session/cwd切换的常驻组件拥有，旧workspace迟到结果可以出现在新workspace标题下。根因是RPC settlement、status query writer与React result material没有共同Host/Runtime/workspace identity。现在增加充血`CodebaseCommandOwner`：每次Host installation固定exact client/gateway，内部generation统一拥有只登记真实在途operation的settler集合、按cwd分区的reindex tail、search/reindex settlement、authoritative operation commit与exact `[codebase-status,{cwd}]` repair；正常resolve/reject立即移除settler，Host replacement与同Host Runtime generation replacement同步拒绝旧在途/queued command，非协作迟到response只能被观察而零commit/repair，stale installation不能轮换或释放successor。accepted search/reindex response不因status repair失败伪造command失败，不同cwd独立推进。Workspace composition root把codebase与Agent Memory共同接入既有公开mutation lifecycle；Codebase结果surface按cwd建立React实例身份，Session navigation直接卸载旧query/hits/busy/error owner，迟到setState没有新material目标；同步ref latch封住React disabled state生效前的重复提交，UI把generation retirement视为neutral idle。只读对照后端`/Users/tangerg/Desktop/codex/codex-rs/app-server/src/request_processors/search.rs`、`fuzzy_file_search.rs`与`outgoing_message.rs`：相同cancellation token先cancel前一flag，cleanup仅在`Arc::ptr_eq(current, exact_flag)`时remove，session Drop与callback共同检查canceled/latest query，request response只take exact callback。前端对照`/Users/tangerg/Desktop/study/codex/deob/webview/app-initial-BuOiDQdP.reconstructed.js`的fuzzy search hook：host+roots决定session lifetime，callback持exact lease object且cleanup先清current lease再stop session，query effect另以local canceled guard拒绝迟到material。本批吸收workspace-scoped lifetime、exact current cleanup与material lease，不复制Codex专用fuzzy session/fallback协议、多host registry或多client产品形状；Lyra继续严格单client/server并沿既有`codebase.*`协议 | 修复前focused为1 file / 3 tests且2条replacement反例稳定失败，均得到`operation unexpectedly resolved`；修复后focused `--detectAsyncLeaks`为3 files / 8 tests，覆盖Host replacement、真实in-place Runtime generation、迟到search/reindex零commit、同cwd串行/跨cwd独立、accepted response/cache repair失败与Session navigation后旧result零material。Frontend `npm run check`与完整`--detectAsyncLeaks`均为307 files / 1845 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03f（Approval batch / Agent Runtime generation owner） | clear-all replacement红色反例先证明：settings按可见rules逐项调用`forgetRule`，第一条旧Runtime RPC在途时安装successor Agent gateway；旧`forgetRule`在await后发现owner已过期却静默return success，外层loop随后重新捕获successor owner/runtime并把第二条rule发到新进程，整批最终错误resolve。审计还发现approval mode/rule accepted response会因query invalidation失败被反向报告为command失败，`AgentCommandOwner`只随Plugin Host安装轮换，真实同Host Runtime generation变化不会撤销approval、Session command或usage writer。根因是batch identity仍在settings过程式循环，Agent owner只有若干Map/tail而没有在途settlement capability，Agent bootstrap也未消费Runtime generation service。现在clear-all通过Agent public facade一次传入rule id set；既有充血`AgentCommandOwner`增加独立approval-rule serial tail与只登记真实在途operation的settler集合：正常resolve/reject立即移除settler，Host/Runtime retirement同步reject全部在途而不依赖共享`Promise.race` loser或timeout。`forgetRules`入口一次捕获exact owner/runtime，逐项accepted后立即以`setQueriesData`从所有Session-scopedrules projection移除exact id；后续item失败保留已durable事实并只repair一次，换代阻止剩余RPC，cache repair失败不伪造已接受delete。mode同样精确commit response并把repair failure视为后续读路径。Agent gateway installation breaking返回lifecycle对象；真实Agent bootstrap要求公开`RUNTIME_STREAM_PORTS`，generation变化同步轮换command owner、unary Session mutation settler和Session usage writer，final cleanup只dispose其current installation；Approvals UI把retirement结算为neutral idle。首次全门禁捕获settings红测越过Agent public facade引用私有adapter，测试重构为“settings一次公开batch委托 + Agent内部真实replacement”并保持边界，无allowlist。只读对照`/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`与`request_processors/thread_lifecycle.rs`：Codex client response/cancel都先按exact RequestId `take_request_callback`，transition按thread一次drain/cancel pending callbacks，迟到response找不到旧callback；本批吸收exact take-once settlement、transition-first batch cancellation和partial accepted事实，不复制server-to-client request、多connection fan-out或thread registry，因为Lyra严格单client/server且approval rule由Runtime durable owner负责 | 修复前红测精确得到整批错误resolve且successor收到`rule-2`；修复后受影响focused泄露组为9 files / 45 tests，边界收敛组4 files / 11 tests，覆盖Host replacement、真实in-place Runtime generation、立即settlement、stale disposer、mode/rule串行、partial accepted delete、cache repair失败、Session mutation settler与usage writer回归。Frontend `npm run check`与完整`--detectAsyncLeaks`均为306 files / 1839 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-18 | P114-03e（Schedule mutation/revision/navigation generation owner） | 两条产品红例先证明：旧Runtime的`schedules.runNow`已在途时安装successor gateway，旧response仍正常resolve并在query invalidation后执行`selectAgentSession("ses_retired")`；同一schedule基于revision 7连续toggle两次时，原五条过程式command没有共同tail，两次RPC并发发送旧revision，第二条错误返回revision 8而非基于首个权威response重放到9。根因是singleton gateway、动态`getContainer().client()`、revision CAS、schedule cache writer和Session navigation没有共同Host/Runtime generation；create/update/enable/delete/run-now还各自定义不同的提交与刷新顺序。现在删掉仅承担singleton转发的`application/ports/scheduleGateway.ts`，充血`ScheduleMutationOwner`直接内聚在既有`scheduleCommands.ts`；每次Host installation固定exact gateway/client，内部generation统一拥有retirement、按schedule identity分区的command tail、latest accepted/cache revision基线、authoritative resource commit、failure ambiguity repair和run navigation。Host replacement与真实Runtime connection generation subscription都先建立successor再同步retire predecessor；旧在途/queued command立即以稳定机器错误结算，非协作迟到response不能写successor cache或导航，stale lifecycle不能轮换/释放current owner。相同schedule严格串行，后续enable intent用前一response完整config/revision重放；edit update在提交时继承更新的enabled+revision，避免表单陈值反向覆盖并行toggle；不同schedule独立推进。create/update/enable response只upsert其证明的resource，delete只移除exact identity；accepted run identity即使cache repair失败仍导航，UI把retirement恢复为neutral idle。没有全局队列、delay、第二cache、refresh绕过或双navigation路径。只读对照`/Users/tangerg/Desktop/codex/codex-rs/thread-store/src/thread_metadata_sync.rs`与`exec-server/src/client_recovery.rs`：Codex pending metadata patch携带generation且只有仍匹配时才mark applied/清pending，failed client只有`Arc::ptr_eq`证明仍current才改变recovery owner；本批吸收ordered revision settlement、exact post-await apply与stale cleanup no-op，不复制thread metadata merge、多连接recovery或其package形状，因为Lyra schedule由Runtime durable CAS和单Desktop owner负责 | 修复前红测精确得到旧run正常resolve并导航，以及两次相同revision并发、第二response仍为8；修复后focused `--detectAsyncLeaks`为3 files / 13 tests，包含真实Plugin Host+Runtime generation service接线，并覆盖Host/in-place Runtime replacement、旧run navigation隔离、同schedule revision重放、update enablement继承、跨schedule独立推进、authoritative cache commit及accepted run/cache repair失败。Frontend `npm run check`与完整`--detectAsyncLeaks`均为304 files / 1833 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-17 | P114-03d（Hook trust mutation/query generation owner） | Host replacement 红色反例先证明：旧 `hooks.setTrust` 在途且同 project 第二条 intent仍排在Application模块级`hookTrustMutations`时安装successor gateway，旧promise仍错误resolve；queued predecessor会在真正执行时通过singleton getter和adapter内动态`getContainer().client()`进入successor transport。根因是process-global keyed queue、current gateway/client、failure ambiguity repair、hooks query invalidation与UI submitting settlement没有共同Host/Runtime generation。现在删掉只负责singleton配置转发的`application/ports/hookTrustGateway.ts`，没有再造owner微模块；充血`HookTrustMutationOwner`直接内聚在既有`hookTrust.ts`，为每次Host installation固定exact gateway/client，内部generation统一拥有retirement、按project root分区的tail、command settlement与cache repair。Host replacement和真实Runtime connection generation subscription都先建立successor generation再同步retire predecessor；旧在途/queued command立即以稳定机器错误结算，非协作迟到response被观察但不能invalidate successor cache，stale installation lifecycle不能轮换或释放current owner。不同project独立推进，同project保持intent顺序；失败response先尝试权威重读再保留原failure，accepted security command不因后续cache repair失败伪装成mutation failure。HooksPane把retirement恢复为neutral idle而不显示重连伪错误，没有delay、第二cache、refresh绕过或双gateway路径。只读对照`/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs`：Codex只有`Arc::ptr_eq`证明failed client仍current才允许异步failure改变recovery owner；本批吸收exact transport admission/settlement与stale cleanup no-op，不复制多连接recovery或connection registry，因为Lyra严格单client/server且project trust由一个Runtime owner负责 | 修复前真实adapter反例稳定得到旧promise错误resolve；修复后focused `--detectAsyncLeaks`为3 files / 6 tests，包含真实Plugin Host+Runtime generation service接线，并覆盖Host replacement、in-place Runtime replacement、在途/排队迟到、同project顺序、跨project独立推进、failure ambiguity repair及accepted command/cache repair失败。Frontend `npm run check`与完整`--detectAsyncLeaks`均为303 files / 1827 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，103 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-17 | P114-03c（MCP Server mutation/auth generation owner） | 两组 replacement 红色反例先证明：旧 `mcpServers.setEnabled` 在途且同 server 第二条命令仍排在模块级全局队列时安装 successor，旧 promise 仍错误 resolve，queued predecessor 随后可通过动态 container gateway 调用 successor；旧 Runtime 创建 authorization attempt 后替换 client，原 poll closure 会把旧 attempt id 发到 successor transport并错误完成授权。根因是 CRUD queue、current gateway、authorization attempt/timer、MCP server/tool cache writer 与 UI settlement 没有共同 Host/Runtime generation；即使 closure 捕获了 adapter，其方法仍在每次调用时重新读取可变 container client。现在充血 `MCPServerMutationOwner` 为每次 Host installation 固定 exact gateway/client，内部 generation 统一拥有 lifetime `AbortController`、retirement settlement、按 server identity 分区的 command tail、probe、authorization attempt id、caller+generation combined signal、poll timer、authoritative response commit 与 query repair。Host replacement和真实 Runtime connection generation subscription均先建立 successor 再同步 retire predecessor；旧在途/queued command立即以稳定机器错误结算，授权 timer与网络请求同时取消，忽略取消的迟到 response 被观察但零 commit，旧 attempt id 永不进入 successor transport，stale installation lifecycle不能轮换或释放 current owner。不同 server 独立推进，同 server保持顺序；accepted response已证明 command 成功，后续 query invalidation失败不倒置为mutation failure。UI save/delete/toggle/auth/probe把retirement恢复为neutral idle，JSON批量导入保留已durable接受的部分事实而不伪报完整成功；没有delay、第二cache、refresh绕过或双授权路径。只读对照`/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs`：Codex只有`Arc::ptr_eq(current, failed_rpc_client)`成立才允许旧client触发recovery，connection cancellation token同时终止pending network-policy decision，异步response在reply前再次核对exact session/client；本批吸收exact transport identity、attempt与connection共同lifetime及stale settlement no-op，不复制多连接recovery、network-policy server request或callback registry，因为Lyra严格单client/server且MCP配置/授权由一个Runtime owner负责 | 修复前两条真实adapter反例稳定得到旧promise错误resolve，且旧authorization attempt被successor gateway轮询；修复后focused `--detectAsyncLeaks`为6 files / 21 tests，包含真实Plugin Host+Runtime generation service接线，并覆盖Host replacement、in-place Runtime replacement、在途/排队迟到、stale lifecycle、authorization timer/network abort、旧attempt隔离、同server顺序、跨server独立推进和cache repair失败。Frontend `npm run check`与完整`--detectAsyncLeaks`均为301 files / 1823 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，102 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-17 | P114-03b（Provider mutation/query generation owner） | replacement红色反例先证明：旧`providers.update`在途且第二条同resource命令仍排在模块级`providerChanges`时安装successor gateway，旧promise仍错误resolve并可把retired provider response提交进共享successor cache，queued predecessor随后通过动态`providerGateway()`调用successor transport；同源审计发现utility role、embedding role各有独立process-global queue，live probe虽会写行内material却完全没有generation。根因是三个队列、current gateway getter、provider/models/role/codebase cache writer与probe UI没有共同Host/Runtime owner；原provider全局队列还无业务理由地阻塞不同provider。现在充血`ProviderMutationOwner`为每次Host installation固定exact gateway和client capability，内部`ProviderMutationGeneration`统一拥有retirement、in-flight settlement、按provider identity/utility role/embedding role分区的tail、probe、authoritative response commit和query repair。Host replacement与真实Runtime connection generation subscription都先建立successor generation再同步retire predecessor；旧在途/queued command及probe立即以稳定机器错误结算，非协作迟到response被观察但零commit，stale installation lifecycle不能轮换successor。不同provider与两个role可独立推进，同resource仍保持顺序。provider/role response只提交自己证明的resource，后续invalidate失败不倒置为command failure；UI既有save/role/probe owner把retirement恢复为neutral idle，不显示重连伪错误，未新增effect或第二material路径。只读对照`/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/client_recovery.rs`与`connectors/src/metadata_store.rs`：Codex只有`Arc::ptr_eq`证明failed client仍current才进入recovery，connector metadata cache绑定backend/auth scope且较弱迟到metadata不能覆盖fresh tool material；本批吸收exact transport scope、resource-aware commit与stale settlement no-op，不复制多连接recovery、account partition或process-global connector cache，因为Lyra严格单client/server且provider配置由Runtime durable owner负责 | 修复前真实adapter反例稳定得到旧promise错误resolve；修复后focused `--detectAsyncLeaks`为6 files / 19 tests，包含真实Plugin Host+Runtime generation service接线，并覆盖Host replacement、in-place Runtime replacement、在途/排队迟到、stale installation、probe retirement、同resource顺序、跨provider/role独立推进和cache repair失败。Frontend `npm run check`与完整`--detectAsyncLeaks`均为299 files / 1816 tests且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，101 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-17 | P114-03a（Agent Memory mutation/cache generation owner） | replacement 红色反例先证明：旧 Agent Memory `setPinned` 已在途、同一 memory 的第二条命令仍排在模块级 `memoryChanges` 时安装 successor gateway，旧 promise 会正常 resolve并把 retired response写进共享 successor cache，排队的 predecessor command随后又通过动态 gateway getter发往 successor transport。根因是配置模块同时散持 process-global serial tail、当前 gateway getter、optimistic cache writer与全量 invalidation，却没有 Plugin Host或Runtime generation identity；全局队列还让无关 memory彼此阻塞。现在充血 `AgentMemoryMutationOwner` 为每次 Host installation独占 exact gateway，并以内部 `AgentMemoryMutationGeneration` 统一拥有按 memory identity分区的 admission/tail、in-flight settlement、retirement、response commit和query repair。successor先发布owner再retire predecessor；Host replacement与同Host Runtime process replacement都同步撤销旧代，排队及在途promise立即以稳定机器错误结算，忽略取消的迟到response仍被观察但无法commit。stale installation的`replaceRuntimeGeneration`/`dispose`只作用于自身，不能轮换successor；accepted response已经证明command成功，后续cache invalidation失败只保留已提交事实，不伪造 mutation failure。Workspace events只依赖composition root提供的公开 lifecycle port，首次全门禁发现的events→workspace private Application越层依赖已删除而未加白名单；UI只把owner retirement当正常换代，不展示伪错误。只读对照`/Users/tangerg/Desktop/codex/codex-rs/exec-server/src/remote_file_system.rs`、`client_recovery.rs`和`app-server/src/request_processors/search.rs`：Codex仅在`Arc::ptr_eq`证明仍为current request/client/cancel flag时清理或发起恢复；本批吸收exact admission/settlement、successor-first replacement与stale cleanup no-op，不复制共享filesystem request cache、多连接recovery或search产品结构，因为Lyra严格单client/server且Agent Memory由一个Runtime owner负责 | 修复前真实adapter反例稳定得到旧promise错误resolve、successor cache被旧response覆盖且旧queued closure可调用新gateway；修复后focused `--detectAsyncLeaks`为5 files / 24 tests，覆盖Host replacement、in-place Runtime replacement、在途/排队迟到、stale lifecycle、accepted response/cache repair以及无关memory独立推进。Frontend `npm run check`为297 files / 1809 tests，干净完整`--detectAsyncLeaks`同为297 / 1809且零泄露；type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，100 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖。第一次完整泄露运行中同文件既有“resume stream回执丢失”HTTP E2E出现一次`session_busy`，exact 1/1重跑与干净全量重跑均通过，没有跳过、延时或生产绕过。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
| 2026-08-17 | P114-02d（HITL ambiguous resume settlement owner） | 红色反例先证明：完整 approval/question atomic barrier已调用旧 Runtime 的`runs.resume`，随后 Runtime generation replacement撤权 Run opening；如果旧 transaction没有durable commit且successor权威 snapshot仍返回同一pending set，`runOpening.retire()`会按设计抑制旧代`onSettled/onStartError`，但原`InterruptResponseCoordinator`只在interrupt消失时清batch，因而两个card持续`submitting`、用户永远不能重试。根因是response batch把transport callback当成唯一终态，又把所有session material write混为同一种revision，无法表达“旧代已撤权、successor durable truth尚未提交、successor确认仍pending”三个阶段。现在原过程式`ResponseBatch`重塑为充血`InterruptResponseBatch`，内聚responses、atomic completeness、submission generation、recovery authority baseline、superseded settlement与迟到callback identity；Session entry新增只在`commitViewRefresh`成功时推进的`authoritativeRevision`，与live event、command error、optimistic mutation都会推进的`viewRevision`严格分离。generation retirement只记录successor基线并保持按钮锁定，避免断线空窗重复提交；该generation的下一次durable projection commit若删除barrier，则material已经证明continuation并退役旧选择，若保留barrier，则整批调用error hooks回滚为可重试。旧accept即使迟到也因map object identity不能resolve已退役batch；普通material write明确不能冒充回执。没有timeout、刷新旁路、双coordinator或基于错误文本的兼容分支。只读对照`/Users/tangerg/Desktop/codex/codex-rs/app-server/src/outgoing_message.rs`与`request_processors/thread_lifecycle.rs`：Codex在running thread resume response与Goal snapshot之后才按RequestId顺序replay pending server requests，response通过`take_request_callback`只完成exact callback；本批吸收authoritative-recovery-before-pending-admission与exact response settlement，不复制其多connection fan-out、server-to-client request replay、thread unload或listener registry，因为Lyra严格单client/server且由SQLite pending projection与durable`runs.resume` mutation拥有产品事实 | 修复前focused红测稳定得到successor相同pending snapshot后`approvalError/questionError`均为0；修复后受影响6 files / 55 tests与focused `--detectAsyncLeaks`全绿，新增交错证明retirement空窗仍锁定、期间command-error/material revision不能释放、successor authoritative commit保留barrier后准确回滚、迟到accept零写入。Frontend完整`npm run check`与`--detectAsyncLeaks`均为296 files / 1805 tests，type/lint/format/knip/circular/context/published-boundary/layer/port/API consumer/style/design/token/chrome/locales/bootstrap/bundle全绿，100 public context edges无环，87/87 operations + 3/3 sidecars + 16/16 events保持覆盖且异步泄露为零。Runtime architecture与SQLite invariant focused tests通过；Desktop Wails v3 Go test/vet/build通过（仅既有macOS linker warning）。Runtime、Protocol、SQLite、Artifact、Desktop Agent inner ring与Go Agent Framework合同未变化；未使用agent-browser，`app/cli`未修改或暂存 |
...
P113先以语义fitness清理Runtime过细package、胖接口、构造与lifecycle坏味道；P114再把单client、单Runtime的renderer replacement/final close、connection/process event generation、Session authoritative transaction与Desktop Run/Tool/HITL/Plan/Goal/审批presentation统一到exact owner。P114-04ac最终补齐审批模式从用户intent、accepted receipt到authoritative projection的可见交接，流式尾部actions、Run outcome、Session navigation和Goal material等前序治理在同一产品心智下保持成立。后端参考Codex Rust，前端主参考study Codex并以zcode/minimax作补充，但只吸收owner、lifecycle、transaction与交互反馈原则，不复制多client、多Host或第二状态路径。全量Runtime、SQLite、Desktop、Frontend与异步泄露门禁绿色；本里程碑在P114-04ac封板，不再继续下一轮代码改动，`app/cli`始终未修改或暂存。
