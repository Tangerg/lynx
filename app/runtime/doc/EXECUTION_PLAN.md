...
> 状态：P1–P82、P83-01–P83-03 已完成；发布后反证审计持续进行
>
...
P83-03 已把 window close、迟到 settlement、retention cleanup 与 replacement renderer 接管 mutation identity 的读写窗口收敛为持久化 generation fence，旧代不能再覆盖或删除 successor。P83 后续按 1–2 client 产品模型继续验证断线重连、冷启动、事务失败、强杀恢复以及重复/乱序事件，优先寻找 HITL、Plan、Goal、Run/Tool read model 在恢复边界的不一致，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
