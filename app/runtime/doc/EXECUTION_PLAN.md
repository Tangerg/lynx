...
> 状态：P1–P82、P83-01–P83-15 已完成；发布后反证审计持续进行
>
...
P83-15 已让 global replacement 与每个定向 committed event 都建立新的 Query generation：事件前 writer 先退休，successor read 独占 cache commit，迟到旧事实不能吞掉一次性变化信号。P83 后续继续按 1–2 client 产品模型区分“cache fencing 已成立”与“底层 foreign read/resource 已实际退休”，下一步优先审计常驻 Inbox badge 的全局 pending-work 分页读取：确认 Query cancellation 是否贯穿所有 cursor attempt 到 RPC transport，并验证连续 `interrupts.changed`、Runtime replacement 与 renderer teardown 不会保留旧分页 owner；随后再检查 Session usage 等 mounted provider。继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
