...
> 状态：P1–P82、P83-01–P83-17 已完成；发布后反证审计持续进行
>
...
P83-17 已让 Session usage chip 与 cross-session Usage pane 的 hook、Application port、Adapter、SDK 和 transport 共享 Query generation signal：`runs.changed`、global replacement 或 renderer teardown 都会释放旧统计 correlation/fetch，迟到响应不能覆盖 successor。P83 后续继续按 1–2 client 产品模型审计默认 Runtime Data Provider；下一步优先核对始终挂载的 Session 列表及其全部 cursor attempt，因为 `sessions.changed` 与 Runtime replacement 虽已替换 cache generation，当前 provider/SDK `sessions.list` 仍未传递 cancellation。随后按真实挂载频率扩展到 workspace/projects/settings reads，继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
