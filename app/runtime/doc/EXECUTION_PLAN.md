...
P74 已把冷重启 key 恢复绑定到 Runtime durable replay store 身份，并让生成 registry 中全部 replayable mutation 进入同一 privacy-preserving journal，同时保持 Runtime Application/Domain、Desktop Agent 内层与 Go Agent Framework 零感知。下一轮 P75 继续以 crash cutpoint 和多窗口并发反证 journal：覆盖 localStorage quota/禁用、同 endpoint 热换 token、两窗口同时恢复同一 persisted entry、Runtime retention 动态收缩、已完成 response 与浏览器崩溃之间的每个切点，并核对各产品权威 cold read 能否把 replay result 收敛到 UI，而不是仅证明 transport same-key。每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
