...
- 右栏 tab overflow 红例确认：strip 已用 Codex 同类 trailing fade 表达“右侧还有内容”，但 renderer 恢复到末尾 tab 或 picker 打开末尾 tab 时只更新 active destination，没有把 active tab 带回可见区；首次 nearest-scroll 修复在 production visual 中又证明单向 fade 不成立——向右后的左边缘从 `preview` 半词硬切入，用户看不出左侧还有 tab。active tab identity 是唯一导航滚动意图；strip 自己的 scroll geometry 是两侧 overflow 提示的唯一 presentation fact，不得上升到 Dock store 或 URL。
