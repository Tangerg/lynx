...
> 状态：P1–P82、P83-01–P83-12 已完成；发布后反证审计持续进行
>
...
P83-12 已把 active Run 异常 EOS 后的 replay/tail opening 纳入同一 Runtime generation ownership：replacement 可立即释放旧 pump，迟到 event iterable 会被主动关闭，cold projection read 也绑定当前 signal。P83 后续按 1–2 client 产品模型继续验证命令响应丢失、重复恢复、Runtime restart 与 renderer replacement 的交错，下一步优先反证 stream 退出后的 exact `runs.get` settlement read 是否会因不合作取消保留旧 live owner，并继续检查迟到/乱序事件及 mutation settlement 是否仍只有一个 owner，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
