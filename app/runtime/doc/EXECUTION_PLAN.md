...
> 状态：P1–P82、P83-01–P83-04 已完成；发布后反证审计持续进行
>
...
P83-04 已让每条 Runtime event subscription 独占单调 recovery watermark；前向 gap 只触发一次 authoritative replacement，重复/迟到旧帧不能再倒退序列或反复重建 HITL、Plan、Goal、Run/Tool read model。P83 后续按 1–2 client 产品模型继续验证冷启动、事务失败、强杀恢复和命令结算不明，优先寻找持久化 commit、事件发布、重连 snapshot 三者之间的失败窗口，不堆砌无业务意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
