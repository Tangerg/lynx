...
> 状态：P1–P82、P83-01 已完成；发布后反证审计持续进行
>
...
P83-01 已让 Workspace event subscription opening/iteration 在底层不合作取消时仍能有界让位，并为可等待 owner 暴露 lifecycle join，而不把 transport 抽象带进 Application。P83 后续继续聚焦 Runtime 重启后已挂载 HITL/Plan/Goal/Run/Tool read model 的统一收敛，以及 window close 与 replacement renderer 接管 mutation identity 的交错；并按真实 1–2 client 使用模型验证断线重连、冷启动、事务失败与崩溃恢复，不堆砌无产品意义的多写 race。每轮提交前继续执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
