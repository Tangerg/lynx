...
- 右侧 Codex 对照采用 app-shell 将 panel visibility 与 tab membership 分离、关闭/重开不销毁 tab 的机制；拒绝照搬其独立 bottom panel 和“空 panel”状态，因为 Lynx 的 Terminal 是只读 Agent command material，且现有 URL destination 必须继续唯一拥有 Dock 是否可见。Lynx 当前 active destination 能从 URL 恢复，但其他已打开 tab、focused diff file 与 file preview target 只在 renderer 内存；replacement 后右栏静默退化成单一 active tab，file preview 还会变空。失败测试已锁定同一 Session 的 `explorer/diff/file`、last `diff`、`src/runtime.ts:42` 在 renderer module replacement 后全部丢失。唯一修复 owner 是 `contextDockStore` 自己的 per-Session presentation scope；只允许持久化 tab/file-target memory，active destination 仍由 URL 拥有，不向 Runtime、query cache 或第二导航状态写事实。
