...
P61 已把 Run-boundary Memory/Skill 后台写收敛到独立 Application mutation owners，并移除 Desktop 的跨聚合失效猜测。下一轮继续反证 run recovery、goal recovery、maintenance compaction 与 authored-resource watcher 的冷启动/失败后可观察后果，以及 subscription close/retarget、跨 Session 晚到事件、事务失败和真实崩溃恢复；每轮提交前执行 Runtime→Agent/Agent→Runtime 双向依赖与 staged path 审计。
