...
P78 已闭合低层 RpcClient 在 recv EOS 与并发 close 下跳过/提前完成 transport teardown 的同源生命周期缺陷，并以真实 Runtime 验证 Goal、HITL resume、Provider/Role、Knowledge/Memory、MCP 与 managed Skill 在提交后丢回执时仍只执行一次并由 cold read/event 收敛。下一轮 P79 聚焦两个真实 renderer 的 owner heartbeat 丢写、lease 到期/抢占交错与 Runtime 进程重启：以独立 client/storage adapter 模拟 owner crash、replacement 接管和旧 owner 迟到 settlement，再验证 runtime event sequence gap、重订阅与冷启动后所有已挂载 read model 不依赖旧进程内存。每轮提交前继续执行真实 HITL/Plan/Goal、全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
