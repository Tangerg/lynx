...
P68 已关闭 Desktop Run/Runtime stream 在自然终止但 consumer 尚未开始或尚未再次读取时的 RpcClient listener/request retention，并保持 buffered terminal 对 late consumer 可见。下一轮继续反证 Runtime Close 与并发 opening/reattach 的服务端 exact teardown，以及事务 commit 结果在旁路事件和消费者投影中的一致性；每轮提交前执行真实 HITL/Plan/Goal、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计。
