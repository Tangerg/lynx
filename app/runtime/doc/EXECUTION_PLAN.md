...
> 状态：P1–P82、P83-01–P83-20 已完成；发布后反证审计持续进行
>
...
P83-20 已让 Header diff stat / Files view 的 active-cwd Query、默认 Data Provider、bound Workspace SDK 与 transport 共享唯一 generation signal；cwd retarget、`files.changed`、global replacement 或 renderer teardown 都会释放旧 change-summary correlation/fetch，迟到旧 cwd 事实不能越代。P83 后续继续按 1–2 client 产品模型审计默认 Runtime Data Provider；下一步优先核对 FileTree 的 `workspace.files.list` 全部 cursor attempt：其参数化 key 同样随 active cwd/path 切换，但 provider 与 bound paged SDK 尚未传递 cancellation，旧目录的 continuation 可能在新树已显示后继续占有 transport owner。随后扩展到 file read/diff/search 与 settings reads，继续覆盖命令响应丢失、迟到/乱序事件和 mutation settlement 单一 identity owner，不堆砌无业务意义的多写 race。每轮提交前执行全部后端 API consumer、Runtime→Agent/Agent→Runtime 双向依赖、agent-browser/测试进程回收与 staged path 审计；`app/cli` 继续只读且不暂存。
