...
- **进度**:**已完成**。落地形态(经与用户对齐后**改为 SQLite 持久化**,而非落文件树):
  ...
  - **写(eviction)**:`agentexec` 的 `toolResultEvictionMiddleware` 注册在 observer **外层** —— UI 事件仍见全文,仅 history/LLM 收 head+tail 占位符;session 从 `turnctx.TurnSession(ctx)` 每调用读取(root/subtask 自动各归其 session);offload 失败降级为全文(best-effort);结构化转发 ConcurrencyKey/ReturnsDirect/MutationPaths。
  ...
  - **清理**:session 删除级联(`sessionStores.deleteSession`)在同一事务里 DropSession blob。
...
```text
Current item: A3(分级压缩阶梯) — A1、A2 已完成
Recommended next: A3(压缩健壮性一组的最后一项;依赖已就绪:A1 safe-cutoff 已守、A2 offload 原语已在 SQLite)
Note for A3: A2 落盘走 SQLite ToolResultStore(非文件路径),A3 步骤⑤/summary 内嵌"落盘路径"应改为内嵌 read_tool_result 的 blob id 引用,与 A2 对齐。
Last completed code checkpoint: A2(commit feat(runtime): offload oversized tool results to SQLite)
```
