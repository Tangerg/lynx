...
>
> **状态**:**里程碑已完成**。A1/A2/A3、B4/B5、C7/C8 已交付;B6 经源码核查确认前提不存在,按 YAGNI 明确不做。
...
- **进度**:**Batch 1(核心自著述)已完成**(用户定"完整版")。落地:
  ...
  - **Batch 2(管理面)已完成**:curator 生命周期 **ACTIVE↔ARCHIVED**(目录式 `_archive/`,只移不删,可 restore;infra archive/restore/list + workspace coordinator + `skills.library.list/archive/restore` RPC + wire golden Go/TS + 前端 "Skill Library" 视图,skills.changed 失效刷新)。全栈全绿。**有据不做**:STALE 中间态(目录式与 ARCHIVED 等价 / "加载但打标"需给只读加载路径引入 state 耦合);专用审计日志(企业治理面,单用户桌面过度范围,HITL 已 durable 记录人审)。
...
- **目标**:提供"每日账本 → 策展 memory"的两级持久记忆(区别于原始会话日志),同时让人写的指令永远不被 agent 覆盖。
- **落点**:`app/runtime/domain/knowledge` + `adapter/maintenance/agentexec` + `infra/sqlite`。
- **实现**:
  - 抽取结果先规范化为独立 markdown facts,以 project + digest 去重后逐条追加到 SQLite ledger;每条保留 `YYYY-MM-DD` daily partition、session provenance、全局递增 sequence。混合旧/新 facts 时只跳过重复项,不整批误删。
  - 每项目一个 curated row 同时持有**完整正文 + watermark + updated_at**。发布是单事务整行替换,并以 expected-watermark CAS 防两个并发 curator 互相覆盖;ledger 永不删除。
  - gate:首批立即策展;之后达到 pending 数量或最大年龄再策展,单次 pending 有界。模型必须产出完整替换正文并遵守 token 预算;空响应/超预算/LLM/DB 失败均**不推进 watermark**,事实仍留在 ledger 等下次自然处理。
  - `LYRA.md` 边界已定:它继续是唯一用户可编辑的文件状态,agent 永不写。AgentScope 的逻辑 `memory/YYYY-MM-DD.md` / `MEMORY.md` 在 lyra 映射成 SQLite ledger/curated projection,遵循 runtime"单 SQLite,仅 LYRA.md 文件例外"的强约束。
  - prompt 顺序:`~/.lyra/LYRA.md`(用户全局) → agent curated(project) → `<cwd>/LYRA.md`(用户项目) → AGENTS.md;最具体的人写项目规则最后出现,优先级高于模型自策展内容。
- **验收**:daily partition + 逐 fact 去重 + project 隔离 + 并发 append + curator CAS 单赢家 + 原子正文/watermark + 失败/超预算不推进 + 后续恢复 + prompt 优先级测试全绿。
- **风险/边界**:这是单用户/单 SQLite 的项目级记忆,不是多租户知识库;ledger 保留原始事实用于审计与重新策展,不做自动物理删除。
- **优先级**:C。
- **进度**:**已完成**。
...
| C8 | 每日账本 → watermark 策展记忆 | C | app/runtime knowledge/maintenance/sqlite | ✅ 已完成(ledger + atomic curated projection) | 无 |

### 4.2 当前执行指针

```text
Current item: none — AgentScope 吸纳里程碑完成;B6 为有据不做,其余条目全部交付。
Last completed code checkpoint: C8 每日 ledger → watermark-gated atomic curated memory
```
