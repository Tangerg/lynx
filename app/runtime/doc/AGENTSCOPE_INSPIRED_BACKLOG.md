...
- **进度**:**已完成**。落地形态(经与用户对齐后**改为 SQLite 持久化**,而非落文件树):
  - **存储**:`sqlite` 新增 `tool_result_blobs` 表(schema v5→v6,`case 5:` 迁移守卫防清库)+ `ToolResultStore`(Offload/Fetch/DropSession,session 隔离)。
  - **写(eviction)**:offload 收进 `agentexec` 的 observation chokepoint(`observedTool.Call`)—— 占位符同时流向 transcript(`OnToolCallEnd`)与 model(返回值),session 从 `turnctx.TurnSession(ctx)` 每调用读取;offload 失败降级为全文(best-effort)。占位符格式+解析在共享 `component/offload`(`Placeholder`/`ID`)。
  ...
  - **单一真源(C1 精修,已 ship)**:原 A2 全文存两份(blob + transcript 全文)= 为省管线的捷径;现 **blob 为唯一全文源**,其余只存占位符,展示在读取边界(`sqlite/TranscriptStore.List`)从 blob 回补 → items.list/export 见全文、LLM 见占位符、live 瞬态预览(reload 即全文)。独立 eviction middleware 已删除。
  - **清理**:session 删除级联(`sessionStores.deleteSession`)DropSession blob —— **对正确性完整**(无 blob 活过其会话)。rollback 提前清是存储优化、需 run_id 穿 4+ 层(turn 层只载 turn_id 非 run_id),**刻意不做**。
  - **配置**:`toolResultOffload.threshold`(viper,默认 50000 字节=开启;显式 0/负=关闭),贯穿 config→bootstrap→agentexec。
  - **测试**:sqlite 往返/session 隔离/DropSession/**v5→v6 迁移不丢数据**;中间件全路径(超阈 offload、小结果不动、错误透传、无 session 保全、offload 失败降级、读回排除、能力转发);读回工具(分页/未找到/空 id/无 session/window rune 安全)。全绿。
...
- **进度**:**Batch 1(核心自著述)已完成**(用户定"完整版")。落地:
  ...
  - **Batch 2(剩余,管理面)**:curator 生命周期(ACTIVE→STALE→ARCHIVED 只归档不删)+ 审计日志 + `workspace.skills.*` 管理 RPC + 前端 review/archive 视图。需前端,较大。
...
| B4 | 受治理技能自著述(HITL-gated) | P2 | skills + approval + tools | 🟡 Batch 1 完成(propose_skill + 人审晋升);Batch 2=管理面/curator/审计/前端 | 无 |
...
```text
Current item: B4 Batch 2(管理面:curator/审计/RPC/前端) — A 组 + B5 + B4 Batch 1 已完成;B6 判定不做
B4 Batch 1(核心自著述:propose_skill + 静态扫描 + 草稿 + 人审晋升门)已 ship。
Recommended next: B4 Batch 2 需前端(workspace.skills.* RPC + review/archive 视图 + curator 状态 + 审计)——较大,可作为独立一批。
Last completed code checkpoint: B4 Batch 1(commit feat(runtime): propose_skill — governed agent skill self-authoring)
```
