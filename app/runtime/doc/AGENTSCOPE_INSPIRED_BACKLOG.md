...
- **进度**:**已完成**。落地形态(经与用户对齐后**改为 SQLite 持久化**,而非落文件树):
  - **存储**:`sqlite` 新增 `tool_result_blobs` 表；schema v7 把 blob 与 canonical item 显式绑定，并将 v5/v6 数据无损迁移到强类型关系。`ToolResultStore` 负责 Offload/Bind/Discard/Fetch/List/Restore/PurgeUnbound/DropSession，所有读取保持 session 隔离。
