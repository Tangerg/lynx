...
**当前边界正确,不变。** Tool loop 在共享基础设施 `core/model/chat/middleware/tool/`(12 文件)。

- agent 的 `core/ProcessContext.Chat()` 内部装配 tool middleware,通过 `ChatClient` interface 使用(不直接 import `core/model/chat/middleware/tool`)。
- lyra 的 `kernel/engine.go` 直接装配 `tool.NewMiddleware(cfg)`,因为 lyra 是应用,可以直接依赖基础设施的具体类型。

**greenfield 澄清**:agent 的 `core/` 定义 `ChatClient` interface → `core/model/chat.Client` 隐式满足它 → `core/model/chat/middleware/tool.NewMiddleware` 返回一个 `chat.CallMiddleware`。agent 的 `ProcessContext.Chat()` 内部通过 `ChatClient` interface + middleware 装配使用 tool loop。这条依赖链不要求 agent `core/` import `core/model/chat` 的具体类型。
