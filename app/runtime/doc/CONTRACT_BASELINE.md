...
- `runs.event_segment_id` / `runs.event_commit_id` 保存当前 active Segment 最近一次完整顶层 `EventCommit` 的 opaque 技术回执；单 Run pump 在收到结算前不会发出下一笔 canonical commit，因此 latest marker 足以核验 SQLite 已 COMMIT 但 success receipt 丢失的 Model/Tool/Transcript/Conversation/Progress 或 terminal write-set。terminal 保留最终 marker；Suspend、Resume、Restore 与 recovery 不沿用旧代 marker；
