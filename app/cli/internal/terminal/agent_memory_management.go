...
func (a *app) ShowAgentMemory(argument string) error {
	...
	target, err := parseAgentMemoryTarget(argument, a.session.Workspace)
	if err != nil {
		return err
	}
...
func (a *app) showAgentMemory(target agentmemory.Target) {
	a.runRuntimeReaderQuery("loading "+string(target.Scope)+" agent memory", agentMemoryOperation, runtimeReaderAgentMemory,
...
func (a *app) loadAgentMemoryItem(argument, label string, apply func(agentmemory.Target, agentmemory.Item)) error {
	...
	target, identity, err := parseAgentMemoryIdentity(argument, a.session.Workspace)
	if err != nil {
		return err
	}
	a.status.note(label)
