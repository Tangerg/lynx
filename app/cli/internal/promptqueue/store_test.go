...
func TestStoreKeepsSessionQueuesIsolatedAndSnapshotsDetached(t *testing.T) {
	store := New()
	first, err := store.Enqueue("one", agent.Message{Text: "first"})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := store.Enqueue("two", agent.Message{Text: "other session"}); err != nil {
		t.Fatal(err)
	}
	second, err := store.Enqueue("one", agent.Message{Text: "second"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStoreUpdatesMovesRemovesAndClearsByStableIdentity(t *testing.T) {
	store := New()
	first, _ := store.Enqueue("session", agent.Message{Text: "first"})
	second, _ := store.Enqueue("session", agent.Message{Text: "second"})
	third, _ := store.Enqueue("session", agent.Message{Text: "third"})

	if err := store.Update("session", second.ID, agent.Message{Text: "edited"}); err != nil {
		t.Fatal(err)
	}
...
func TestStoreRejectsInvalidMessagesWithoutMutation(t *testing.T) {
	store := New()
	if _, err := store.Enqueue("", agent.Message{Text: "valid"}); !errors.Is(err, ErrSessionIDRequired) {
		t.Fatalf("empty session returned %v", err)
	}
	if _, err := store.Enqueue("session", agent.Message{}); err == nil {
