...
The interactive client uses a stable agent shell: session and workspace identity at the top, a responsive Lyra brand banner in an empty session, selectable transcript content in the center, a compact live plan and run status near the bottom, and a framed multiline composer whose footer shows the provider-qualified model and run limits. The banner is presentation-only and disappears with the first conversation block. Optional regions yield their space on constrained terminals; at extreme sizes the shell reduces to transcript, status, and a one-row composer, then restores the full chrome without losing the draft or keyboard focus. It supports session switching, runtime-backed workspace inspection, search, protocol-representable text/image attachments, structured approval and question workflows, provider/model and global approval-mode pickers, tool details, plugin inspection, and plugin reload/unload. `lyra completion <bash|zsh|fish|powershell>` generates shell completion.
...
- `/codebase`, `/codebase-search <query>`, and `/codebase-reindex` expose semantic-index status, scored code spans, and a confirmed background rebuild independently from literal workspace grep. Reindex admission preserves the runtime operation identity; subsequent status reads remain authoritative.
...
- `/knowledge`, `/knowledge-read <cwd|projectRoot|home>`, and `/knowledge-edit <scope>` expose the human-authored LYRA.md cascade. The resize-safe editor accepts multiline text, preserves it verbatim, allows intentional clearing, and reads the saved scope back from the runtime.
...
- `/hooks` audits global and project lifecycle hooks, including their exact shell command or declarative injection, source, matcher, timeout, active state, and project trust root. `/hooks-trust` and `/hooks-revoke` re-read that root, require explicit confirmation, and refetch the authoritative catalog after mutation.
...
| --- | --- | --- |
| Domain and ports | `internal/agent`, `internal/agentmemory`, `internal/authoringcontext`, `internal/changefeed`, `internal/codebase`, `internal/diagnostictool`, `internal/feedback`, `internal/goal`, `internal/hookpolicy`, `internal/knowledge`, `internal/mcp`, `internal/modelconfig`, `internal/schedule`, `internal/sessiontransfer`, `internal/settings`, `internal/skills`, `internal/usage`, `internal/workspace` | Validated agent, context, diagnostics, semantic-index, hook-governance, feedback, runtime-management, invalidation, portable-session, and configuration contracts with consumer-owned narrow ports. |
...
- keep governed agent memory and human-authored LYRA.md knowledge as separate bounded contexts, preserve memory provenance/review state, and omit workspace context from runtime-global scopes;
