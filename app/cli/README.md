...
- `/memory [project|user]` and the `memory-*` commands expose governed agent-maintained memory without conflating it with user-authored instructions. Pending automatic facts retain their session/day provenance, approve/reject and delete require confirmation, and successful mutations re-read the selected scope.
