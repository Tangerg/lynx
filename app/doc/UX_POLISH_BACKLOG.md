...
| Lever | 现状 | 影响 | 工 |
|---|---|---|---|
| emit `run.progress`(activity/usage/step)+ plumb maxSteps | ✅ activity/step(toolStart)+ usage/cost(每 round 边界,OnUsage→UsageReported)已 emit;maxSteps plumb 仍 TODO | 解锁实时活动/成本/步数(T1.2) | M |
