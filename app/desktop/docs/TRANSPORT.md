...
```
pkg/
├── coreapi/          # SSOT —— Go interface + types + codegen struct tags
├── coreimpl/         # 实现（调 LLM、跑 tool、存状态）
├── transport/
│   ├── transport.go  # Transport interface + Message envelope
│   ├── inprocess/
│   ├── wails/
│   └── http/
├── rpcadapter/       # JSON-RPC Message ↔ CoreAPI 方法 dispatch
└── coreclient/       # client 侧 wrapper（inproc.go / wails.go / http.go）

cmd/
├── lyra-tui/         # Bubble Tea —— 直接 import coreapi + coreimpl
└── lyra-desktop/     # Wails 外壳 —— import coreimpl + transport/wails

frontend/src/transport/   # TS 镜像：transport.ts / wails.ts / http.ts
schemas/                  # 生成物：methods.yaml / events.yaml
```
