...
> 已有的写法看 `PLUGINS.md`（设计提案）+ `PLUGINS_IMPL.md`（实现指南），本文不重复 API 细节。
...
2. **真正的能力上限不在"还差几个 `register*`"，而在三处结构性缺口**：
   - (A) **扩展点是封闭的** —— 只有 kernel 能定义贡献点；插件不能为别的插件开新插槽（JetBrains 的核心能力）。
...
| 维度 | Lyra 现状 | 文件 |
|---|---|---|
| 贡献模型 | `definePlugin({ setup({host}) })` 命令式注册，24 namespace / 31 个 `register*` | `sdk/host.ts` |
| 注册表 | Zustand owned-map，`addOwned`(单槽覆盖) / `addOwnedMulti`(复合键多槽)，冲突告警 | `sdk/registry.ts` |
...
| 后端通信 | `host.rpc.{get,post}`(JSON-RPC) / `host.agui.on`(CUSTOM 事件) / `host.data.registerProvider` | `host.ts`, `docs/API.md` |
...
- **Lyra 对位**：扩展点**全部由 kernel 在 `host.ts` 写死**。插件唯一的"开点"手段是
  `host.state.slice(name)`（约定名共享状态），离 JetBrains 的类型化扩展点差一截。**这是最大的上限差距。**
...
- ✅ **per-slot 强类型 `register*`（不抽 factory）** —— CLAUDE.md 已论证过，类型推断收益 > LOC，**保持**。

---

## 4. 能力上限的四个真实瓶颈

### 瓶颈 A：扩展点封闭（kernel-only）
...
definePlugin({
  ...
  version: "1.0.0",
  capabilities: ["workspace", "rpc", "state", "commands", "shortcuts", "notify"],
  setup({ host }) {
    host.workspace.registerView({
      id: "editor", title: "Editor", icon: "filetext", component: EditorView,
    });
    host.commands.register({ id: "editor.save", title: "Save File", run: () => save() });
    host.shortcuts.register({ key: "Mod+S", run: () => save() });
  },
});
```
...
- `PLUGINS.md`（设计提案 v2）/ `PLUGINS_IMPL.md`（实现指南 + SDK 参考 + 内置插件清单）
