...
- **id 时间可排序**：id 内嵌创建时间戳，**字典序 ≈ 创建时间序**（升序锚定历史、降序锚定 recent-first 列表）。由此：
  历史稳定排序、游标分页、日志排序**都不需要额外的 seq 字段**——分页 `cursor`（§4.11）即可直接用一个资源 id 表达。
...
- 事件名小写 `domain.action`：`run.started` / `run.finished` / `item.started` / `item.delta` / `item.completed` /
...
| 必需面 | 用途 |
| --- | --- |
| `runtime.initialize` | 握手 + 协商（必经） |
| `sessions.create` | 开一个会话 |
| `runs.start` + 消费 `notifications.run.event` 里的 `item.*` + `run.finished` | 发消息、流式显示、知道何时结束 |
| `items.list` | 重开会话补历史 |
...
§4.5 的可复用结构（`DiffRow` / `FileChangeEntry` / `SearchHit` / `WebSearchResult`）。
...
| `edit` / `write` | `path`, … | `{ changes: FileChangeEntry[] }` | diff 卡片 |
...
interface FileChangeEntry {
  path: string;
  type: "add" | "modify" | "delete" | "rename";   // 判别/分类字段统一用 type（§2.1）
  diff?: DiffRow[];
}
...
interface GrepResult { matches: GrepMatch[]; total: number }       // total ≥ matches.length（可能被 limit 截断）
interface GrepMatch  { path: string; lineNumber: number; text: string }
```

> 本地搜索命中（`SearchHit`：文件 + 行）与网络检索结果（`WebSearchResult`：url + 标题）是**两个互斥类型**，不合并成
> 一个松形状（避免"一个结果同时带 path 和 url"这种非法可表达态）。
...
- **cursor 不透明**：client 不解析其内容。server 可直接用"上一页最后一个资源的 id"作 cursor（id 时间可排序，§2.4），
  也可用任意编码——client 只回传上次拿到的 `nextCursor`。
...
> （`toolArguments`→`tool.arguments`，`toolOutput`→`tool.result`）。见 §5.2。
...
| `item.completed` | ✅ true | —— |
| `state.snapshot` | ✅ true | —— |
| `run.progress` | ⬜ false | `run.finished.result`（usage / steps / costUsd） |
...
| `item.delta{toolOutput}` | ⬜ false | `tool.result`（completed，§4.4） |
...
- 入参 `InitializeRequest`：
  ...
  | `capabilities` | `ClientCapabilities`（§9） | 是 | client 能渲染的事件 / 能处理的 interruptTypes / opt-out |
...
- **示例**：
  ...
  响应（HTTP 上是流的首帧）
  ```json
  { "jsonrpc": "2.0", "id": "1", "result": { "runId": "run_01", "userItemId": "item_00" } }
  ```
  随后事件（节选）
  ```json
...
- **示例**（批准一个 toolCall interrupt）：
  ...
  { "jsonrpc":"2.0", "id":"7", "method":"runs.resume",
    "params": { "parentRunId":"run_01",
                "responses": [ { "itemId":"item_09", "response": { "type":"approval", "decision":"approve" } } ] } }
  ```
...
| 方法 | 入参 | 返回 | 说明 |
| --- | --- | --- | --- |
| `workspace.listFileChanges` | `WorkspaceQuery & { cursor?; limit? }` | `Page<FileChange>` | 工作区改动 |
| `workspace.getDiff` | `WorkspaceQuery & { path?: string }` | `DiffRow[]` | 结构化 diff（单文件/全量，非分页） |
...
> `getDiff` / `getFileHead` / `grep` 返回的是**单一聚合结果**（非集合列表），故保留专用 shape、不套 `Page<T>`；
> `grep` 的截断由 `GrepResult.total ≥ matches.length` 自描述。
...
| --- | --- | --- | --- | --- |
| a | `"rpc"` | 调用本身就错：未握手 / `session_not_found` / `invalid_params` / `cwd_unavailable` / `capability_not_negotiated` | 该 method 的**同步 JSON-RPC error response**（带 `error.code`，§8.2） | 调用未起 run |
| b | `"run"` | run 已起、执行中整体失败 | 流内 **`run.finished{ outcome:{ type:"error", result.error } }`** | 是 |
...
`ProblemData`（§4.6）是 **RFC 9457 *Problem Details* 数据模型的传输无关裁剪** —— 去掉 HTTP 专属的 `status`（与 JSON-RPC
...
"忽略未知"自动容忍，**不 bump 契约**（能力协商本就是协议最该可加的扩展点）。已知 key（缺省视为关闭）：
...
10. **Minimal Profile**：最小客户端只需 initialize + sessions.create + runs.start + item/run 事件 + items.list；其余分层可选、capability 门控（§3.2）。
...
- **日期串 pin 版本**：`protocolVersion` 用日期（§12），破坏性变更新日期 + 协商，非递增 major。
