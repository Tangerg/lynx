# Lyra Runtime Protocol · 旁路 API（定稿 `2026-06-07`）
...
>
> 文内裸 `§x` 指**本文**小节；引 `API.md` 一律写全 `API.md §x.y`。`protocolVersion`：**`2026-06-07`**（与 `API.md` 同）。
...
```ts
interface WatchSpec { watchId: string; cwd?: string; path?: string }   // watchId 客户端起名、回显于 files.changed；cwd 缺省 serve 目录；path 当前未用（见下）
...
type WorkspaceEvent =
  ...
  | { type: "mcp.serverChanged"; server: string; status?: McpStatus; toolCount?: number; error?: ProblemData }  // 见 §5
...
}                                      // 延续 run（resume / edit）无开场用户轮 → 省略
```

- **`toRunId` = inclusive-keep**：保留的**最后一个 root run**（其延续链一并保留），其后全部丢弃。省略 `toRunId` = 丢弃全部、
  回到空会话（覆盖"编辑第一条消息重跑"）。
...
> 向 `needsAuth` server 递交 token 的 `workspace.mcp.authenticate` 属提案面（613 B12，后端方法表尚未注册），见 `API.md` 附录 C.6。
...
> 审批记忆的**读 + 全局策略**面（`ApprovalMode`、`approval.listRemembered` / `forget`）属提案面（613 B9），见 `API.md` 附录 C.3。
