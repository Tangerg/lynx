...
- `runs.start` 是非幂等创建：客户端不得盲重试一个结果未知的创建请求。响应中断时，先用 `runs.list{sessionId}`
  对账，再对仍在运行的 Run 调 `runs.subscribe`；只有确认服务端没有创建 Run 后才重新 start。
...
| `todos`         | bool | `todo_write` 的 `state.snapshot{todos}` 投影                              |
| `compaction`    | bool | 自动上下文压缩及 `compaction` Item                                        |
