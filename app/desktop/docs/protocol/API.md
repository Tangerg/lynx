...
> [`app/runtime/contract/schema.json`](../../../runtime/contract/schema.json)（所有 shape 与跨字段约束）、
> [`openrpc.json`](../../../runtime/contract/openrpc.json)（方法的入/出 schema）、
> [`manifest.json`](../../../runtime/contract/manifest.json)（方法 / 能力门禁 / 错误注册表 / union / state 策略 /
> canonical 样本索引）、[`API_REFERENCE.md`](../../../runtime/contract/API_REFERENCE.md)（人读索引：每个方法的
...
> **字段表在 [`schema.json`](../../../runtime/contract/schema.json)**（含跨字段 presence 规则）与
...
> 新工具只需在客户端展示注册表登记一行；不约定也能 JSON 兜底渲染。**协议核心永不感知这张表。**

### 4.5 可复用结构（Diff / Search / 文件）

供 §7.5 `workspace.*` 直接返回，并被 §4.4.2 的工具 `result` 约定复用。它们是领域便利结构，不是核心原语。

> **三个文件类型共用 `FileStatus` 词汇，但意图不同、故各自独立**：`WorkspaceFileChange`（VCS 工作区扫描态，含
> `untracked`）/ `FileDiff`（逐文件结构化 diff，带 `rows`）/ `FileEdit`（agent 一次编辑的应用结果，无
> `untracked`）。共用过去式 `status`（描述"变更后的态"）。`added`/`removed`（±行）binary 时省略（不伪造 0），
> `previousPath` 仅 `renamed` 给。
> **`Diff` 是 sum-type**（`files` ⊕ `patch`，按 `format`），不是松对象同时带两者。
> **本地搜索命中（`SearchHit`）与网络检索结果（`WebSearchResult`）是两个互斥类型**，不合并成一个"同时带 path 和
> url"的非法可表达态。
...
`Interrupt` 三变体（`approval` / `question` / `toolResult`），每个都带 `itemId` + `runId` + 自包含 `payload`。
...
> file_path），所以记的是「`npm run *` 在本 project」而非笼统「整个 shell」。`decision:"deny" + remember` 合法 =
...
> `idempotency_in_progress`。**幂等重放不返回一个"新的" ack**：重试 `runs.start` 拿回的是同一个 run 的 ack。
