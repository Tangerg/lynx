# Lyra Runtime Protocol（定稿 `2026-07-19`）
...
- `params._meta` 承载协议版本 / clientInfo / clientCapabilities 这类**请求自描述元数据**；dispatch 在业务参数解码前剥离。
  传输元数据（trace / 门禁 token / 流游标）仍走带外通道。判定见 [`TRANSPORT.md`](./TRANSPORT.md) §2。
...
- 客户端只生成 JSON-RPC envelope `id`；业务资源 id 全由 server 生成。
...
- 事件名小写 `domain.action`：`segment.started` / `segment.finished` / `item.started` / `item.delta` / `item.completed` /
  `state.snapshot` / `custom`。运行时自有行为**必须**用一等事件/Item 类型；`custom` 只留第三方扩展。
...
interface Session {
  id: string; // ses_…
  revision: number; // 乐观并发版本；每次持久化修改递增
  title: string;
...
> 一个**全局审批姿态**（`ApprovalMode = "plan"`，见 §C.2），不是 run 的属性。
...
- **`compaction`** —— additive 第 7 变体，标"此处压缩了 N 条更早消息"。turn 边界的自动压缩产出该 Item（`item.started` + `item.completed`，带 `droppedMessages` = 压缩前后净减条数），fold 成时间线分隔条。
...
- collection 在翻页期间变化且 cursor 锚点消失时，server 返回 `invalid_params`，client 从第一页重启；按同一排序键分页的文件列表
  可安全 seek 到下一项。服务端不得猜测不匹配的排序键并静默跳项。
...
推论：客户端可排除高频 delta（§9 `excludedEvents`）而仍保持正确；不流式的 runtime 可不发任何 delta，
completed item 一样必发权威终值。

### 5.3 state.snapshot 必发边界

共享状态只以整份 `state.snapshot` 传播（无增量事件）。当 run 期间创建或改变了共享状态时，server **必须**在以下边界发 `state.snapshot`：

- 状态每次改变时（快照即当前完整视图）；
- `segment.finished` 之前（权威终值落点）；
- 重连后，让客户端拿到最新完整状态。
...
> **`remember`（审批 scope，AUX_API §6）**：持久化成一条**细粒度规则**（`ApprovalRule`，§C.2）。规则按 `(scope, tool, subject)`
...
- 返回 `DiscoverResponse`：
  ...
  | 字段              | 类型                           | 说明                                        |
  | ----------------- | ------------------------------ | ------------------------------------------- |
  | `protocol`        | `{ current; minSupported }`    | server 支持的协议日期范围                   |
  | `serverInfo`      | `{ name; version; cwd; home }` | serve 进程目录上下文（冷启动默认 cwd 兜底） |
  | `capabilities`    | `ServerCapabilities`（§9）     |                                             |

进程 shutdown 属于宿主生命周期；请求取消由 transport context 传播；HTTP 存活/就绪检查使用 sidecar。协议不暴露无效果的镜像方法。
...
- 入参 `UpdateSessionRequest`：
  ...
  | ------------------ | ------ | ---- | -------------------------------------------------- |
  | `sessionId`        | string | 是   |                                                    |
  | `expectedRevision` | number | 是   | 必须等于最近读取的 `Session.revision`              |
  | `title`            | string | 否   |                                                    |
  | `cwd`              | string | 否   | **改 cwd = relocate**；受 `features.relocate` 门控 |
  | `model`            | string | 否   |                                                    |
  | `favorite`         | bool   | 否   | 置顶/取消置顶；favorited 会话在列表里排在前面      |

- 这是局部 patch：省略字段保持不变。单条 SQL 原子校验 revision、修改字段并递增 revision；并发写入只有一个成功。
- 返回更新后的 `Session`。错误 `session_not_found` / `revision_conflict` / `capability_not_negotiated`（relocate 关闭时改 cwd）/ `cwd_unavailable`。
...
- 入参 `StartRunRequest`：
  ...
  > "计划"通过**全局审批姿态**实现：先 `approval.setMode{"plan"}`（§C.2，只读门禁——写/exec/network 工具一律拒），
...
- `runs.start` 携带稳定 `Idempotency-Key` 后可安全重试：服务端在执行前原子 claim key；同 key + 同请求会重放首个响应并
  订阅既有 run，不会创建第二个 run；首个执行尚未完成时返回可重试的 `idempotency_in_progress`，同 key + 不同请求返回
  `idempotency_conflict`。若业务已完成但响应缓存暂时写失败，也返回 `idempotency_in_progress`；同 key 重试只补写并重放已生成
  的响应，不会再次执行 mutation。未携带 key 的创建请求仍不得在结果未知时盲重试。
- **一会话一 run（`session_busy`）**：该 session 已有 run 在跑（**正在 pump 或 parked 在 HITL interrupt 上**）时拒绝——
  两个 run 同时写一条会话的消息日志会被 turn 末压缩（整段重写）吞掉彼此的消息。run admission 以 session claim
  原子串行化并发 start / resume / 破坏性 session 修改；继续既有 run 用 `runs.resume`、插话用 `runs.steer`。
- 错误（通道 a，§8.1）：`session_not_found` / `cwd_unavailable` / `invalid_params`（含 `provider`/`model` 只给其一）/
  `session_busy` / `idempotency_conflict` / `idempotency_in_progress`。
...
`workspace.listFiles` / `workspace.readFile` 的文件浏览语义见附录 C.1；模型侧代码导航通过 `features.lsp`
门控的 `lsp_*` 工具提供，不另设一套 UI 直调 RPC。
...
运行时在固定的 turn 生命周期点跑用户自写的 hook（外部命令，事件 JSON 走 stdin，按 exit code 决策；或声明式 `inject` 零进程注入上下文）。hook 来自两层 `hooks.json`：**全局 `~/.lyra/hooks.json`（永远信任）** 与**项目 `<root>/.lyra/hooks.json`**。**信任模型（安全要点）**：clone 来的仓库其项目 hook **默认不跑**（防供应链 RCE），须经 `hooks.setTrust` 显式信任后、下一个 turn 起生效。

`hooks.list` 列出该 cwd 发现的**全部** hook（含未信任的项目 hook，便于审阅 `command` 后再决定信任），`active` 标注它当前是否真的会跑（全局恒 `true`；项目仅在 `projectTrusted` 时 `true`）。
...
错误：`hooks.list` 同其它读方法可返 `cwd_unavailable`；`setTrust` 的 `projectRoot` 为空 → `invalid_params`。
...
客户端不发送协议通知；请求取消由 transport context 处理。
...
| ------------------ | ---------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `schedules.list`   | `{ cursor?; limit? }`                                             | `Page<Schedule>`            | 最新创建在前；游标分页                                                                                             |
...
| `schedules.update` | `{ id; expectedRevision; title?; prompt?; cwd?; provider?; model?; cron?; enabled? }` | `Schedule` | 局部 patch；revision 原子校验并递增；cron/enabled 变化时重算 `nextRunAt`                                           |
| `schedules.delete` | `{ id }`                                                         | 无                          | 幂等                                                                                                               |
| `schedules.runNow` | `{ id }`                                                         | `{ sessionId; runId }`      | 立即额外触发一次并返回可追踪 run 句柄；不移动下次到点时间                                                         |
...
错误：`prompt` / `cron` 为空或 cron 不可解析、`provider`/`model` 未成对 → `invalid_params`;未知 `id`(update / runNow)→ `invalid_params`；过期 `expectedRevision` → `revision_conflict`。
...
| `codebase.status`  | `{ cwd? }`                | `CodebaseStatus`          | 该 cwd 索引状态；索引中带当前 `operationId`                                                          |
| `codebase.reindex` | `{ cwd? }`                | `{ operationId }`         | **后台**全量重建并立即返回句柄；相同 cwd 的并发请求合并到同一 operation                              |
...
interface CodebaseStatus {
  ...
  operationId?: string; // state=indexing 时的当前后台操作
...
| `-32019` | `revision_conflict`         | `expectedRevision` 已过期；客户端重拉聚合后合并/重试                                                                                            |
| `-32020` | `idempotency_conflict`      | 同一个 `Idempotency-Key` 被用于不同 method/params                                                                                                |
| `-32021` | `idempotency_in_progress`   | 同 key + 同请求的首个执行尚未完成；稍后以同 key 重试                                                                                             |

`error.data` 为 `ProblemData`，**必须含 `type`（= 上表 name）**。

> 业务码 `-32001..-32021` 落在 JSON-RPC 2.0 的 `-32000..-32099`「implementation-defined server-error」保留段；
...
interface ServerCapabilities {
  ...
  features: Record<string, FeatureCapability>; // 开放 map：未声明或 enabled=false 即关闭
...
| key             | 含义                                                                               |
| --------------- | ---------------------------------------------------------------------------------- |
| `reasoning`     | 产出 `reasoning` item / delta                                                      |
| `mcp`           | MCP 工具 / `mcp.*`                                                                |
| `multimodal`    | 图片输入：`runs.start.input` 的 image ContentBlock（mime + base64，§4.3）          |
| `git`           | `workspace.listFileChanges` / `getDiff`（git 二进制在 PATH）                       |
| `checkpoints`   | `restoreType`（v2 影子 git 文件快照）                                              |
...
| **state**  | run 期共享可变视图态 | run 进行中的可变面板（todo board / 进度面板），有**终值快照**    | snapshot ✅           | 顶层 key（§2.6）   |
| **custom** | 一次性信号           | **不进历史、不改状态**的瞬时提示（"已连接 MCP"一类）             | 帧上自带（默认 ⬜）    | `name`（§2.6）     |

- 选 **Item**：它是工作回看的一部分吗？是 → Item（享受 `items.list` 历史 + run 树）。
- 选 **state**：它是 run 期一直在变、最后有个稳定终态的视图吗？是 → state（`state.snapshot` 落状态，§5.3）。
- 选 **custom**：它既不回看、也不构成状态，只是一次性提示吗？是 → custom（若需可恢复，必须自带 durable 落点，§5.2）。

> `state` 用整份 `state.snapshot` 传播共享可变态 —— 存在的理由就是"有终值的共享可变态"
...
- `protocolVersion` 是日期串（本定稿 `2026-07-19`）。
- **前向兼容是硬约定**：client 必须忽略响应中的未知字段、对未知 method/事件容忍。加 method / 加可选响应字段 /
  加事件 / 加 `features` map key → **同版本号**；新增请求字段会被旧 server 严格拒绝，必须新日期版本。
...
5. **元数据带外、业务进 params**：cwd/sessionId/runId 进 params；trace/版本/token/游标走带外（TRANSPORT §2）。
...
### C.1 · `workspace.listFiles` / `workspace.readFile`（基础读，不门控）—— 文件树浏览 + 查看
...
> `includeIgnored`/`cursor`/`limit` 全支持；分页游标来自确定性目录优先 + 路径排序。每个返回项在同一次查询中完成
> `lstat`，因此每一项的 `type` / `sizeBytes` / `modifiedAt` 来自同一次检查；超过安全扫描边界时明确返回 `invalid_params` 并要求收窄 path，不静默截断。
...
- `readFile`：`startLine`/`endLine` **1-based 闭区间**（面向编辑器）；`truncated` 自描述触顶 `maxBytes`。
...
`compaction` Item 是 §4.3 Item 联合的 additive 第 7 变体：runtime 在 turn 边界需要压缩时产出它（`item.started` +
`item.completed`，`droppedMessages` = 压缩前后净减条数），客户端 fold 成时间线分隔条。摘要文本已经折进重写后的
对话历史，因此 Item 只表达边界和删减计数。压缩策略属于 runtime 领域服务，不开放会与自动策略竞争的手动 RPC。
...
模型的 `todo_write` 清单（**非**已删的 `background.*` 任务注册表）。更新走既有 durable
