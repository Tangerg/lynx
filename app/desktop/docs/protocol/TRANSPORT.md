...
- **JSON-RPC envelope `id` 必须是 string**（硬约束）。虽然 JSON-RPC 2.0 本身也允许 number id，本协议**只接受字符串 id**——number id 会被拒为 `invalid_request`（`detail: "id must be a JSON string"`）。本文所有示例用字符串即遵此约定；其他客户端请勿发整数 id。（`null` id 仅用于无法解析 request 时的错误响应，按 JSON-RPC 2.0。）
...
server **不需要**重放：`item.delta` / `state.delta` / `run.progress` / 任何 `durable=false` 事件。
