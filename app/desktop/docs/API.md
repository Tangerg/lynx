...
`number`）：客户端用单调递增 integer，服务端不接 string id。
...
| 取消对象 | 机制 | 关键字段 |
| --- | --- | --- |
| 在飞的 JSON-RPC **Request**（如慢 `sessions.list`） | `notifications/canceled`（client→server） | `{ id: number, reason? }` —— `id` 是被取消 **Request 的 `id`**（§1.1 的 number id） |
| long-running **run** | `runs.cancel`（正经 Request，§5.2） | `{ runId, reason? }` —— 停 run，流以 `run/closed` stopReason `"canceled"` 收尾 |

> **`id` vs `requestId` 不同概念**：取消在飞 Request 用 `id`（number，就是那条
> Request 的 JSON-RPC id）；HITL 审批 / 提问用 `requestId`（string，业务 id，
> §4.3/§4.4）。同名易混，故取消侧直接用 `id`。
...
notification 流**按连接**投递：run R 的事件发给「启动 R 的那条连接」。

| Transport | 关联机制 |
| --- | --- |
| InProcess | 隐式 —— 客户端直接持有 `<-chan`，无歧义 |
| Wails IPC | 隐式 —— 单 WebView ↔ 宿主连接 |
| HTTP | 客户端生成 **connection id**；`runs.start` 等 POST 用 `Lyra-Connection-Id` header 带上；SSE 订阅用 `GET /v1/rpc/stream?conn=<id>` query 带上（`EventSource` 无法设自定义 header，故走 query —— 仍是 transport URL 层，不进 envelope）。Server 把该连接启动的 run 的 notification 路由到匹配 `conn` 的 SSE 流。 |
...
- **崩溃恢复（丢了 runId）**：客户端重启后用 `runs.list { sessionId }`（§5.2）
  发现会话里**仍在跑 / 等待输入**的 run（`RunSummary`：runId + status +
  lastEventId），再按 runId 重开流续传。这是 durable HITL（plan-mode 暂停
  跨重启恢复）的发现入口 —— Runtime 已能持久化并恢复 `waiting` 的 run，缺的
  就是这个发现 + 重连协议。`runs.list` **只返活跃 / 等待态**，不返已完成的 run
  （历史走 `messages.list`）。
...
`approve` / `deny`（+ 可选 `editedArgs`）。审批超 `expiresAt`（若给）未回，server
按默认策略（deny/abort）收尾并推 `lyra.approval-result`。
...
`Question` / `Answers` 形状见 §6.9。`expiresAt` / 默认应答语义同审批。**与审批
共享同一条 "Notification + Request" 通道形态**，不引入 server→client request。
...
`Page<T>` 游标分页（§6.4）；`Stream<R, E>` 立即返 `R` + 事件经 notification 流出；
`void` 无返回（typed 上 `null` / 空 object，不返哨兵）。"参数类型"列给该 method
的 params/result 类型名（§8.1 规则）。
...
| **Runs** | | |
| `runs.start` | `Stream<StartRunResponse, AgUiEvent>` | `StartRunRequest`（§6.3，含 `maxTurns?` / `maxBudgetUsd?`）；事件经 `notifications/run/event`，终态经 `run/closed.result` |
| `runs.list` | `RunSummary[]` | `ListRunsRequest { sessionId }` —— 仅活跃 / 等待态的 run（崩溃恢复 + durable HITL 发现，§3.3） |
...
| `sessions.export` | `ExportSessionResponse { url }` | `ExportSessionRequest { id, format: "md"\|"json" }` |
| **Messages** | | |
| `messages.list` | `Page<Message>` | `ListMessagesRequest { sessionId, …PageQuery }` |
| `messages.edit` | `EditMessageResponse { runId, checkpoint }` | `EditMessageRequest { sessionId, messageId, content }` —— 新 run 推 `lyra.checkpoint` |
| **Workspace** | | |
...
interface ApprovalRequest {
  requestId: string;             // 业务 id（string；区别于取消用的 number id，§2.3）
  ...
  reason?: string;
  risk?: string;
  expiresAt?: string;            // ISO-8601；超时按默认策略收尾
}
...
interface QuestionRequest { requestId: string; parentMessageId: string; questions: Question[] }
...
- 加 method / 加可选字段 / 加事件 → 同版本号（client 声明 `unknownMethodTolerance:
  true` 时）。客户端**必须忽略未知字段**。
- 改语义 / 删字段 → 新日期版本 + 协商。
...
client 传 `X-Lyra-Trace-Id` → 服务端缺失则生成（UUIDv7）→ 响应 `X-Lyra-Request-Id`
回显 → 结构化日志 `trace_id` 绑定。未来 facade 接入时透传 W3C `traceparent`。
