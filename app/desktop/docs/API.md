...
| 取消对象 | 机制 | 说明 |
| --- | --- | --- |
| 在飞的 JSON-RPC **Request**（如慢 `sessions.list`） | `notifications/canceled { requestId, reason? }`（client→server） | 取消单个还没返回的 Request |
| long-running **run** | `runs.cancel { runId, reason? }`（正经 Request，§5.2） | 停止一个 run，流以 `run/closed` status `"canceled"` 收尾 |
...
```ts
interface Question {
  id: string;                                    // 稳定 key（answers 按此索引）
  question: string;                              // 题干
  header: string;                                // 短标签（≤ 12 字符）
  options: { label: string; description: string; preview?: string }[]; // 2-4 项
  multiSelect: boolean;
}
// answers: question.id → 选中 label（multiSelect 为 label[]；自由文本直接用文本）。
// 按 id 而非题干索引 —— 题干重复 / 超长 / 改一字都不会断。
...
| `runs.cancel` | `void` | `{ runId, reason? }` —— 停止 run（与 `notifications/canceled` 不同，§2.3） |
...
> `notifications/canceled` 是**客户端→服务端**发的（§2.3 取消在飞
...
interface RunSummary {
  runId: string;
  // waiting = 卡在 HITL 审批/提问/plan-mode，可重连恢复。
  // 注意 RunSummary.status（run 终态: …|done）与 SessionStatus（会话静息态:
  // …|idle，§6.2）**有意不同** —— run 有"完成"终态，session 只有"静息"，
  // 别把两者"对齐"成一个 enum。
...
行为 SSOT 是后端的 Go interface（已 ship，非 "待建"）：
...
Go ↔ Go（in-process / Wails）两端直接 import `rpc/protocol`，**不需要
codegen**。TS / Rust / Python client 从 schema 生成（codegen 管线待接，当前
前端类型手写并以此 Go interface 为准对齐）。
...
- **Notification 同样适用**：`POST /v1/rpc/notifications/canceled`，响应
...
> **后端列指向真 Runtime（lynx 仓 `lyra/`，已 ship）**。本仓
> `internal/agui/` 是 Go dev mock（前端 CI / 本地 `wails dev` 用），不是
> 协议的权威实现。
...
| 流式 reducer / CUSTOM 事件 handler | `frontend/src/plugins/builtin/{core-reducer,agui-handlers}/` | `internal/engine/` + `internal/agui/`（事件 marshal） |
| HITL approval | `frontend/src/lib/agent/useApprovalSubmit.ts` | `internal/service/approval/`（Console/Gate） |
| dev mock（非权威） | — | 本仓 `internal/agui/`（lyra frontend repo） |
