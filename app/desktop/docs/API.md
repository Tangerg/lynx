...
- **一条下行事件流**：`notifications.run.event` 承载 run/item/state 全部事件。
...
- 元数据（连接 id / trace / 协议版本 / 幂等键 / 门禁 token / 流游标）走**带外通道**，不进 `params`；
...
- 客户端只生成三种非业务 id：JSON-RPC envelope `id`、transport 连接 id、幂等键。
...
interface ToolResultResponse{ kind: "toolResult"; output: string }                           // client-side 工具结果
```
...
> 约定：**Stream<R>** = 立即返 `R`（含 `runId`）+ 随后在 `notifications.run.event` 推 `RunEvent` 流。
...
- 返回（同步）`{ runId: string }`；随后流式 `RunEvent`（§5），以 `run.finished{outcome}` 收尾。
- 幂等：经 `X-Idempotency-Key` 头（见 TRANSPORT §10），重复键重新接上既有 run、不新建。
- 错误（同步通道 a）：`session_not_found` / `cwd_unavailable` / `invalid_params`。
...
把一个已存在 run（root）的事件流重新绑到当前连接（重连/崩溃恢复）。

- 入参 `{ runId: string }`；返回 `{ runId }` + 流式 `RunEvent`（订阅整棵 run 树，§5.4）。带 `Last-Event-Id` 续传。
- 错误：`run_not_found`。
...
interface ClientCapabilities {
  ...
  optOutNotificationMethods?: string[]; // 按连接抑制高频通知，如 ["item.delta"]
}
```
...
1. 尽量用同一连接 id 重开 transport 流；
2. 对每个仍显示的 root run 调 `runs.subscribe`；
3. 支持时带 last-seen event id（`Last-Event-Id`）；
4. 调 `items.list` 补 durable 缺口；
5. 按 `itemId` 与 `eventId` 去重。**ephemeral delta 不重放**（§5.2 保证正确）。
