...
> **Lyra** — Wails 桌面应用（Go 后端 + React/TS 前端）。Lyra Runtime Protocol v2（JSON-RPC，Session→Run→Item）流式协议驱动的插件化 agent client。
...
- **协议**: 自研 Lyra Runtime Protocol v2（JSON-RPC 2.0，`frontend/src/rpc/`，见 `docs/API.md` + `docs/TRANSPORT.md`）。已弃用 AG-UI。
- **桌面壳**: Wails v2
...
- **Radix first（硬规则，不是"考虑"）**：任何带交互行为 / 焦点管理 / 键盘导航 / aria 语义的组件，**一律先用 Radix Primitives，不要手写**。已在用：dialog / popover / select / dropdown / tooltip / context menu / tabs / slider / toggle-group(segmented) / checkbox / progress。需要新的这类组件时第一反应是查 Radix 有没有对应 primitive，有就用 `common/` 下薄包一层（复刻现有设计 token 的 className，见 `Checkbox` / `Slider` / `Segmented`），**绝不手写 focus trap / roving tabindex / aria-\* / 键盘事件**。
  - **唯一豁免**（换 Radix 零收益甚至有害时才保留手写，且必须在代码注释里写明理由）：(a) 纯展示、无交互（如 `Avatar` 只渲染 initials、没有图片 fallback 故事）；(b) Radix 版本会引入实测开销且无 a11y 收益（如 `ScrollArea` 故意用原生滚动）；(c) 定制行为是 Radix 模型套不进去的（如 `ReasoningBlock` 的"流式时自动展开"逻辑）。判据：**Radix 是否带来真实 a11y / 行为收益**——有就换，纯为统一而换不做。
  - **测试注意**：Radix 的 `Tabs.Trigger` 等在 `mousedown`(+focus) 激活而非 `click`，写测试用 `fireEvent.mouseDown`。
...
- **重构是节奏，不是可选项** —— 分两档：
  ...
  - **信号**（任何一项命中就该开始）：单文件 > 500 行、同一结构不变量（如复合键格式、归一逻辑）手写 > 3 处未收敛、新建插件需要改 kernel 多处、命名漂移导致两个名字指同一个东西、最近 commit 里有反复改同一段代码（说明抽象方向错了）。
...
- **边界校验用 Zod，内部数据流不加** —— 信任边界（sideload plugin spec / v2 RunEvent envelope / 用户输入的 URL 等）必须用 Zod schema 拒掉脏数据，RunEvent 信封校验在 `src/rpc/stream.ts`，其余就近文件。内部流（Zustand store / React props / 已被 codegen 类型守护的 StreamEvent payload / 已被 SDK 类型守护的 plugin SDK 调用）**不加 Zod** —— TS 已经够了，再加是噪音 + bundle 浪费。
- **现代特性只在"可读性/正确性提升 _且_ 不引入分配/退化"时采用** —— target 是 ESNext，Wails 2 WebView 带全套现代 API（`Array.at` / `findLast` / `findLastIndex` / `Object.groupBy` / `structuredClone` / `Intl.Segmenter` / 异步迭代器 等），但"追新"本身不是理由。判据三连：(a) **真有等价老写法可替换**（`arr[arr.length-1]` → `.at(-1)`、原地反向 find 循环 → `findLast(Index)`、`indexOf(x) !== -1` → `includes`），不为没有的场景硬套（无 `JSON.parse(JSON.stringify)` 就别提 `structuredClone`，regex 替换别改 `replaceAll`）；(b) **不引入额外分配 / 不退化热路径**——`toReversed()` / `toSorted()` 会复制整个数组，在 reducer / 每 token delta 调用的函数里用就是性能回归，保留 allocation-free 的原地反向循环；(c) **类型与行为完全保留**——`noUncheckedIndexedAccess` 已开，`.at(-1)` 与索引形式同为 `T | undefined`，互换安全。**反例（别动）**：同槽位读+写只换读会造成读/写 idiom 不一致；反向 `splice` mutation 循环必须倒序索引，换 `findLast` 会损坏；`.then()` 链 / `.reduce` 数值累加已是地道写法，换 async-await / `groupBy` 是硬套。地道 ≠ 最新。
...
- ❌ 把贡献面退回 `registry.ts` 的 per-slot `addX/removeX` map —— **已经反向做完了**：40 个命名 map 全塌进单一 `extensions` 底座，贡献统一走 `host.extensions.contribute(POINT, …)`（L2+L3，见 `docs/EXTENSION_POINTS.md`）。加新贡献面 = `kernelPoints.ts` 加一个 `defineExtensionPoint` + 一个 selector，**不再动 registry/registryState**。`addOwnedMulti` 已删；`registry.ts` 只剩 `addContribution`/`removeContribution` + declared-placeholder 的 `addOwned` + bookkeeping。别再提"抽 factory"或"加一对 addX/removeX"。
...
- ❌ **把 method 名斜杠化成 `/v1/rpc/runs/start`**。HTTP transport 上 method 名照搬 method 表里的字符串，点保留（如 `/v1/rpc/runs.start` / `/v1/rpc/workspace.terminal.subscribe`）。斜杠化会跟 REST shadow 混淆，引诱"再加一个 GET /v1/sessions" 的诱惑。详见 `docs/API.md` §5.1。
...
- ❌ **把业务 error 映射到 HTTP status code**（如 `session_not_found` 返 404）。HTTP status 仅反映 transport 层状态；业务 error 一律走 JSON-RPC `error.code`。详见 `docs/API.md` §7.3 + §10.6。
...
- **热路径不要 `console.debug` 完整对象**。run streams ~30 events/sec，DevTools console 留每个 event 的引用，长会话累积几万个对象无法 GC，肉眼可见的 lag。要看就去 Diagnostics view。
...
- **UI shell 不要在 async query 上做 join 后 filter**。tab strip / sidebar 列表 / 任何"反映 store 状态"的 UI 必须 **1:1 镜像 store 形状**。query 没 ready 时用 placeholder，**不要 filter 掉**——filter 会让 store 已确认存在的 id 在 DOM 上消失，用户报"点击 tab 没反应"。例：`PanelHeader` 之前 `tabIds.map(id => sessions.find(...)).filter(Boolean)`，cache 没命中时把已 open 的 tab filter 掉了；改成 `tabIds.map(id => found ?? {id, title: id, status: "idle"})` 后即使 query mid-refetch 也保持 1:1，下次 refetch 把真 title 补上 React reconciliation 保 node identity（commit `5dd7789`）。
- **先判定是不是 dev-only HMR 假象，再决定要不要修——很多根本不用修**。症状只在 `wails dev` / `vite dev` 里出现、**一次整页刷新（Cmd+R）就消失、生产构建里不可能发生**的，几乎都是 HMR 把模块级副作用重跑/叠加造成的假象，不是真 bug。判据：(a) 触发路径只在 dev（`import.meta.hot` 存在）才会重入吗？(b) 生产里该入口是不是只执行一次（如 `PluginProvider` 只 mount 一次 → `loadPlugins` 只跑一次）？(c) reload 后是否立刻干净？三个都 yes → **别动正确性敏感的热路径（plugin loader / registry / reducer）去"修"它**，刷新即可；真要消除 dev 摩擦就用纯 dev 范围的手段（`import.meta.hot.dispose` 清理、或 `import.meta.hot.accept(() => location.reload())` 自动整页刷新），不进生产代码。**反例（踩过）**：流式文本逐字重复其实是 HMR 重跑 `loadPlugins` 让 `onCore` handler 叠加（每个 delta 被 apply N 次）——我去给 plugin loader 加"幂等卸载"，结果因为 single 点（主题/命令/layout slot）按稳定 key 覆盖、multi 点（如 `onCore`）才是 `mintId` 追加，"setup 后再 dispose 旧的"把刚加的 layout slot/主题/命令按同 key 删掉了，HMR 后工具栏直接消失——比原症状更糟。已回退（`1c56be2` revert `06fdf94`）。教训：dev-only 假象别用生产代码绕，刷新就行。
...
│   │   ├── host.ts                    createHost(pluginName) + contribute 写路径
│   │   ├── defineExtensionPoint.ts    defineExtensionPoint<T>(def)
│   │   ├── kernelPoints.ts            ~35 个内置 ExtensionPoint（THEME / COMMAND / …）
│   │   ├── pointIds.ts                registry 内部 firing 循环用的 point id 常量（破环）
│   │   ├── registry.ts                usePluginStore + addContribution/removeContribution + declared* 的 addOwned + clearByPlugin
│   │   ├── selectors/                 按 domain 分组的 useXxx / lookupXxx + extensions.ts（底座读侧）
│   │   └── definePlugin.ts            loadPlugin
│   └── builtin/                       内置插件，按 domain 分组
├── protocol/run/                      reducer（StreamEvent 派发器）+ viewState（Item→message/block 投影）
...
├── rpc/                               Runtime Protocol boundary（JSON-RPC client + methods + shapes + transports）— 唯一 outbound 副作用层
├── main/container.ts                  composition root（DI 单例：wire rpc client + methods() + sidecar）
...
internal/agui/                         Go v2 Runtime Protocol mock（JSON-RPC + SSE），监听 :17171（server.go + runtime.go + plugins.go）
```
...
4. **路径在 `frontend/src/protocol/run/` 或 `frontend/src/rpc/`**：动了协议 fold 层或 wire 层 → 重点跑 `reducer.*.test.ts` + `rpc/*.test.ts`
