...
- **用户体验细节是一等公民，不是"以后再说"** —— 功能正确只是底线；真正拉开产品观感的，是那些**不影响功能、却天天硌用户**的细节。做任何 UI 都按"打磨后的终态"交付，而不是"能渲染出来"就收工。每次动 UI 至少扫这几类：(a) **尺寸稳定**——列表 / 卡片行高不随内容长度跳动（长文本单行 `truncate` + 省略号，配 `title` 或展开兜底，**绝不靠换行把高度撑起来**；例：ToolCard 标题、Diagnostics span 行）；(b) **行高 / 间距节奏**——同类元素统一 `leading` + `gap`，疏密一致，不靠零散 `margin` 凑；(c) **字号 / 字重 / 颜色的层级对比**——主次信息用 token 阶梯拉开（`fg` / `fg-muted` / `fg-faint`、semibold vs normal），对比足够又不过冲；(d) **对齐与数字**——同列边界对齐（固定 grid 列宽），数字用 `tabular-nums` 不抖动；(e) **截断 / 空态 / hover 的可达性**——被截断的内容要有办法看全（hover `title` / 展开），空态给提示而非一片空白。**判据**：改完一处 UI，自己当用户走一遍——"长内容会炸吗？切窄会挤吗？信息层级一眼分得清吗？尺寸会跳吗？"。`DESIGN.md` 给的是**规范**，这一条给的是**态度**：宁可多花一轮打磨，也不交"功能对、但硌手"的 UI。
...
- **注释纪律：轻易不要写注释** —— 优先让代码通过**命名、结构和抽象**自解释；想写注释时先问"能不能改个名字 / 抽个函数 / 调整结构让它不言自明"，能就别写。注释是代码表达力用尽后的**最后手段**，不是第一反应。只有以下情况**应该**写（写的是代码无法表达的信息）：
  1. **接口定义**：公开 API / 扩展点 / hook 的语义、参数、返回值、异常、副作用与调用约束（调用方看不到实现，契约必须写在签名旁）。
  2. **特殊约定**：业务规则、历史原因、兼容要求、外部系统约束（如协议条款 `API.md §x.y`、wire 形状方言、后端行为依赖）——这些不在代码里，只在上下文里。
  3. **特殊算法**：算法思路、边界条件、复杂度与非显然优化（看得懂每一行 ≠ 看得懂为什么这样算）。
  4. **反直觉实现**：为什么**不能**用更常见、更简单的写法（防止下一个人"好心"改回去——本文件 §3/§5 的反例条目就是这类注释的文件级版本）。
  5. **并发 / 事务 / 安全等关键约束**：必须遵守的顺序、隔离级别、竞态窗口、信任边界与风险点（违反不报编译错、只在生产炸）。
  - **判据一句话**：注释只写 _why_ 与 _约束_，不写 _what_ 与 _how_——what/how 该由代码自己说；复述代码的注释在代码变更时必然腐烂成误导。
  - **配套要求**：改了代码必须同步改注释，宁可删掉也不留过期的；注释对着**下一个读者**说话，不对 reviewer 说话（"此处修复 bug X"、"按 review 意见调整"一类一律不写）。
...
- ❌ 把贡献面退回 `registry.ts` 的 per-slot `addX/removeX` map —— **已反向做完**：40 个命名 map 塌进单一 `extensions` 底座，贡献统一走 `host.extensions.contribute(POINT, …)`。加新贡献面 = `kernelPoints.ts` 加一个 `defineExtensionPoint` + 一个 selector，**不动 registry/registryState**。别再提"抽 factory"或"加一对 addX/removeX"。
...
- ✅ **开放扩展点底座（L2+L3）**：40 个命名贡献 map → 单一 `extensions` 底座 + `defineExtensionPoint`/`contribute`；删 ~24 个纯 spec-register host facade；capability-on-point + 风险分级（`capabilities.ts`）+ sideload 默认 deny（`pluginOrigin.ts`）+ `host.commands.execute` + `definePluginPack`。
