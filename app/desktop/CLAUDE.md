...
- **重构是节奏，不是可选项** —— 分两档：
  ...
  - **大型重构（每 15-20 轮 feature）**：跨整个 `frontend/src/` 扫。跑 `npx knip` 找未引用的 export / 文件 / 依赖；找 > 500 行的文件考虑拆 SRP；找跨模块的 3+ 重复（不只是局部）；考虑是否要分 monorepo / 改架构层级。产出通常是 multi-batch 重构计划（A/B/C 三批），用户确认后逐批 commit，每批之间跑全套 `tsc + vitest + eslint + knip` 全绿。
