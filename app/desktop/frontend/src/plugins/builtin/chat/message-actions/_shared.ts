// Shared Tailwind chrome + helpers for the per-message action button plugins
// (copy / edit / regenerate / feedback).

/** Base Tailwind class string for the hover-revealed icon buttons in
 *  the message action bar. Rounding is applied per-role via `roleShape`. */
export const ACTION_BTN_BASE =
  "inline-flex h-7 w-7 items-center justify-center border-0 bg-transparent text-fg-faint transition-colors hover:bg-fg/[0.06] hover:text-fg";

/** Per-role corner rounding — pill on the user bubble, md on assistant. */
export function roleShape(role: string): string {
  return role === "user" ? "rounded-full" : "rounded-md";
}
