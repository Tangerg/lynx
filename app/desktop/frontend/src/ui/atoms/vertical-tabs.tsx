            ...
            <div key={group.id} className="flex flex-col gap-px">
              ...
              {group.items.map((item) => (
                <TabsPrimitive.Tab
                  key={item.id}
                  value={item.id}
                  className="flex h-[var(--control-height-md)] items-center gap-2.5 rounded-[var(--button-radius)] border-0 bg-transparent px-2.5 text-left font-sans text-ui-md leading-none text-fg transition-[background-color] duration-[var(--dur-fast)] ease-out hover:bg-hover focus-visible:outline-none data-[active]:bg-selected"
                >
