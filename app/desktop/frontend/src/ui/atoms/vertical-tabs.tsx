          ...
          <div key={group.id} className="flex flex-col gap-px">
            ...
            {group.items.map((item) => (
              <TabsPrimitive.Tab
                key={item.id}
                value={item.id}
                className="flex h-8 items-center gap-2.5 rounded-md border-0 bg-transparent px-2.5 text-left font-sans text-ui-lg leading-none text-fg transition-[background-color] duration-[120ms] ease-out hover:bg-fg/[0.04] focus-visible:outline-none data-[active]:bg-fg/[0.06]"
              >
