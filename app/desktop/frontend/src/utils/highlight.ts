...
function highlight(src: string, keywords: RegExp): string {
  let out = String(src).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const stash: string[] = [];
  ...
  out = out.replace(/('[^']*'|"[^"]*"|`[^`]*`)/g, (m) =>
    hold(`<span class="token-str">${m}</span>`),
  );
  out = out.replace(keywords, (m) => hold(`<span class="token-kw">${m}</span>`));
  out = out.replace(/\b([a-zA-Z_$][a-zA-Z0-9_$]*)(?=\()/g, (m) =>
    hold(`<span class="token-fn">${m}</span>`),
  );
