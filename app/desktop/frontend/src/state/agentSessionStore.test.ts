...
describe("selectTab after empty state", () => {
  ...
  it("adds the first session to tabIds from a zero-tab state", () => {
    useAgentSessionStore.setState({ tabIds: [], activeSessionId: "" });
  ...
  it("closeTab on the last tab → selectTab chain leaves correct state", () => {
    ...
    useAgentSessionStore.setState({ tabIds: ["s1"], activeSessionId: "s1" });
    ...
    // closed session.
    expect(useAgentSessionStore.getState().tabIds).toEqual([]);
    expect(useAgentSessionStore.getState().activeSessionId).toBe("");

    useAgentSessionStore.getState().selectTab("s2");
    expect(useAgentSessionStore.getState().tabIds).toEqual(["s2"]);

    useAgentSessionStore.getState().selectTab("s3");
    expect(useAgentSessionStore.getState().tabIds).toEqual(["s2", "s3"]);
  });
});
...
describe("agentSessionStore draft lifecycle", () => {
  ...
  it("takePendingMessage returns then clears the queued first message", () => {
    const s = useAgentSessionStore.getState();
    s.setPendingMessage("d1", { input: [{ type: "text", text: "hello" }], runOptions: {} });
...
describe("reconcileTabs reconciles persisted tabs against backend truth", () => {
  ...
  it("drops every tab + clears active when the backend has no sessions (db reset)", () => {
    ...
    useAgentSessionStore.getState().reconcileTabs([]);
    const s = useAgentSessionStore.getState();
  ...
  it("keeps live sessions and a not-yet-graduated draft, prunes the rest", () => {
    ...
    useAgentSessionStore.getState().reconcileTabs(["s1"]);
    const s = useAgentSessionStore.getState();
  ...
  it("re-targets the active session to a survivor when it was dropped", () => {
    useAgentSessionStore.setState({ draftSessionIds: new Set() });
    useAgentSessionStore.getState().reconcileTabs(["s2", "s3"]); // active s1 is gone
    const s = useAgentSessionStore.getState();
  ...
  it("is a no-op (same tabIds reference) when every persisted tab is still live", () => {
    useAgentSessionStore.setState({ draftSessionIds: new Set() });
    const before = useAgentSessionStore.getState().tabIds;
    useAgentSessionStore.getState().reconcileTabs(["s1", "s2", "s3"]);
    const s = useAgentSessionStore.getState();
