...
| Fonts fetched from a CDN | A desktop app ships its fonts; it doesn't phone Google on launch | ✅ Done — Geist + Geist Mono are self-hosted via `@fontsource-variable/*`, imported in `main.tsx` and bundled as `woff2` assets (subset per script). Dropped the four `fonts.googleapis.com` `<link>`s in `index.html` and the `@import` in `globals.css`. `--font-ui` / `--font-mono` lead with the bundled families (`Geist Variable` / `Geist Mono Variable`). Works offline, no render-blocking CDN round-trip, no launch leak. |
