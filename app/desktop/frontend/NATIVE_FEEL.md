...
| Launch flicker | `_doAfterNextPresentationUpdate` — don't show the window until the WebView has drawn | ◻︎ Wails shows the window on ready; verify there's no white/empty flash before first paint. Background colour is set dark to mask it; confirm it actually masks. |
