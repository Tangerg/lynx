...
// Sentinel errors for the [openai] package's input-shape validators.
...
var (
	...
	ErrNilConfig = errors.New("openai: config must not be nil")

	// ErrMissingApiKey is returned when a config supplies a nil
	// [model.ApiKey].
	ErrMissingApiKey = errors.New("openai: ApiKey is required")

	// ErrMissingDefaultOptions is returned when a *ModelConfig
	// supplies nil DefaultOptions.
	ErrMissingDefaultOptions = errors.New("openai: DefaultOptions is required")

	// ErrNilRequest is returned by [Api] methods when the SDK request
	// pointer is nil.
	ErrNilRequest = errors.New("openai: request must not be nil")
)
