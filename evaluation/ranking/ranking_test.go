...
import (
	...
	"github.com/Tangerg/scope/evaluation/ranking"
)
...
func TestEvaluatorCalculatesRankingMetricsAtCutoff(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(string(test.metric), func(t *testing.T) {
			...
			if report.Metric.Namespace != "ranking" || report.Metric.Name != evaluation.MetricName(test.metric) {
			...
			gotThreshold, found, err := report.Metric.Parameters.Decode[evaluation.Score]("threshold")
...
func TestPrecisionUsesConfiguredCutoff(t *testing.T) {
	...
	if report.Verdict != evaluation.VerdictUnspecified {
...
func TestEvaluatorValidatesConfigAndCancellation(t *testing.T) {
	if _, err := ranking.NewEvaluator(ranking.Config{Cutoff: 1}); !errors.Is(err, evaluation.ErrInvalidEvaluatorConfig) {
		t.Fatalf("missing metric error = %v", err)
	}
	if _, err := ranking.NewEvaluator(ranking.Config{Metric: ranking.MetricRecall}); !errors.Is(err, evaluation.ErrInvalidEvaluatorConfig) {
