# AGENTS.md — lynx monorepo 项目级上下文

> Go monorepo，多个 sub-module 各自有 `go.mod`。本文件只放**跨模块共用的法则 —— 只宏观、不写具体**（具体案例、符号、文件名活在代码 / git / 各模块自己的 `AGENTS.md` 里，会随演化变动，不进本则）。设计哲学的"为什么"见 [`DESIGN_PHILOSOPHY.md`](DESIGN_PHILOSOPHY.md)，落手重构的标尺见 [`REFACTORING.md`](REFACTORING.md)。
...
`lynx` 是一套面向 AI agent / RAG / LLM 集成的 Go 基础设施：`core` 定协议、`models` 适配 LLM provider、`vectorstores` 适配向量库、`tools` 提供工具集、`agent` 跑 planner 驱动的 runtime、`lyra` 是 in-house backend runtime；其余模块（`pkg` / `rag` / `chatmemory` / `mcp` / `a2a` / `skills` / `documentreaders` / `otel`）各司一域。每个 sub-module 的形态、关键类型、模块特有反向不变量见其自己的 `AGENTS.md`。所有模块共享下文这套法则。
...
- **可观测性 = OTel 三驾马车，sink 到 `log/slog`（vendor-neutral）**：观测 = Traces（span）+ Metrics（instrument）+ Logs，用全局 `otel.Tracer` / `otel.Meter`、零 DI，组合根在启动时一次性绑定 exporter + W3C propagator。规约：
  ...
  - 细节见 [`doc/OBSERVABILITY.md`](doc/OBSERVABILITY.md) + [`otel/AGENTS.md`](otel/AGENTS.md)。
...
- **加文档先问**：每个 sub-module 已有 `AGENTS.md`，其他默认不写。
