...
- **具体能力各自拥有实现**：`fs`、`shell` 等保持独立子包。两层 SPI：**Tool 层**对 LLM(JSON in/out + schema + 交互)，**Backend Port / Provider 层**做真正执行(本地 / 远程 / 沙箱后端可换)。消费方只依赖单操作最小端口，不得要求一个 backend 实现无关能力。演示工具属于 `examples`。
...
- **两层 SPI 是核心**:Tool 层只做 JSON ↔ Go + schema 校验 + LLM 交互;**所有业务逻辑**(行号、binary 检测、写锁、不可扩张的路径 authority …)都在 Backend Port 层 —— 这样远程 backend 能独立优化,不必往返整个文件。
...
- **bulk 查询下沉 Backend Port**:glob / grep 各有最小端口,远程 backend 一次 RPC 完成,而非多轮 list + read。
...
- ❌ **在 Tool 层做业务逻辑** —— 业务全在 Backend Port,Tool 只是 JSON ↔ Go + schema。
...
- **加新后端**(远程沙箱 / 容器):只实现实际提供的单操作端口,在调用处注入；不得为了注入一个 read 强迫实现 write/search。
