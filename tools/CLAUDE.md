...
> 给 LLM 调用的 schema 派生便利层和具体工具集(shell / 文件系统 / HTTP / 网页抓取 / 网页搜索 / skill 等)。共享执行协议、typed function 核心适配与实例 Registry 属于底层 `tool` 模块；本模块所有实现统一满足 `tool.Tool`，模型可见定义只使用 `core/chat` 协议值。
...
- **只有一套 Registry**:底层 `tool.Registry` 是唯一实例 registry，同时满足 `agent/toolloop.ToolResolver`；工具定义直接投影成 `core/chat.ToolDefinition`，不建立第二套 registry 或 bridge。
- **schema 从 Input struct 自动推导**：`tools/function.New` 只接受 struct/`*struct` 输入并派生 schema，然后委托 `tool.NewFunc` 完成 strict JSON 编解码；手写具体 Tool 时显式提供 schema。
- **typed helper 不承载 runtime policy**：`tool.NewFunc` 与 `tools/function.New` 都不处理并发、重试、HITL、直接返回或 tool-loop 终止；这些属于 agent/runtime decorator。
