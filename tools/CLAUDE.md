...
> 给 LLM 调用的 typed function adapter 和具体工具集(shell / 文件系统 / HTTP / 网页抓取 / 网页搜索 / skill 等)。共享执行协议与实例 Registry 属于底层 `tool` 模块；本模块所有实现统一满足 `tool.Tool`，模型可见定义只使用 `core/chat` 协议值。
...
- **实现 `tool.Tool` 的工具集合**,两层 SPI:**Tool 层**对 LLM(JSON in/out + schema + 交互),**Executor / Provider 层**做真正执行(本地 / 远程 / 沙箱后端可换)。
