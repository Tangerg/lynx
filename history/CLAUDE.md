# CLAUDE.md — history package family
...
- 每个外部 provider 位于 `history/<provider>` 独立叶子 module；本目录不存在聚合 module，数据库 SDK 和 OTel 不得跨 provider 扩散。
...
- **加新后端**：在 `history/<backend>` 建独立 module，实现 `core/history` 的真实能力，按 conversation_id 分区，禁止导入兄弟 provider。
