...
func TestContextualAugmenterConfig_validate(t *testing.T) {
	tests := []struct {
		name    string
		config  *ContextualQueryAugmenterConfig
		wantErr bool
		errMsg  string
	}{
		{
...
func TestNewContextualAugmenter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			augmenter, err := NewContextualQueryAugmenter(tt.config)
			if tt.wantErr {
				assert.Error(t, err)
				...
				assert.Implements(t, (*QueryAugmenter)(nil), augmenter)
			}
		})
	}
}
...
func TestContextualAugmenter_Augment(t *testing.T) {
	tests := []struct {
		...
		wantErr        bool
		validateResult func(t *testing.T, result *Query)
	}{
		{
			name:    "nil query",
			config:  &ContextualQueryAugmenterConfig{},
...
func TestContextualAugmenter_handleEmptyContext(t *testing.T) {
	tests := []struct {
		...
		validateResult    func(t *testing.T, original, result *Query)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			augmenter, err := NewContextualQueryAugmenter(config)
			require.NoError(t, err)
