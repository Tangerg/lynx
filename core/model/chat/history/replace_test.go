...
// empty set clears it. history.Replace routes through the backend's Replacer
...
func TestReplace_AtomicSetsExact(t *testing.T) {
	...
	if err := history.Replace(ctx, s, "c", chat.NewUserMessage("a")); err != nil {
	...
	if err := history.Replace(ctx, s, "c"); err != nil {
...
// count so the test can tell history.Count dispatched to the capability rather
...
// dispatch: history.Count uses a backend's Counter when present (so a backend
...
func TestCount_PrefersCounterElseFallsBackToRead(t *testing.T) {
	...
	plain := history.NewInMemoryStore()
	if err := plain.Write(ctx, "c", chat.NewUserMessage("a"), chat.NewUserMessage("b")); err != nil {
		t.Fatalf("Write: %v", err)
	}
	if n, err := history.Count(ctx, plain, "c"); err != nil || n != 2 {
	...
	cs := counterStore{Store: history.NewInMemoryStore(), n: 99}
	if n, err := history.Count(ctx, cs, "anything"); err != nil || n != 99 {
