...
func TestFileWriter_Write(t *testing.T) {
	t.Run("write single document without markers", func(t *testing.T) {
		...
		err = writer.Write(context.Background(), []*Document{doc})
		require.NoError(t, err)
	...
	t.Run("write with page metadata", func(t *testing.T) {
		...
		doc.Metadata["start_page_number"] = 1
		doc.Metadata["end_page_number"] = 5
	...
	t.Run("write in append mode", func(t *testing.T) {
		...
		err = writer1.Write(context.Background(), []*Document{doc1})
		require.NoError(t, err)
		...
		err = writer2.Write(context.Background(), []*Document{doc2})
		require.NoError(t, err)
	...
	t.Run("write empty document list", func(t *testing.T) {
		...
		err = writer.Write(context.Background(), []*Document{})
		require.NoError(t, err)
...
func TestFileWriter_buildDocumentContent(t *testing.T) {
	...
	t.Run("with markers and page metadata", func(t *testing.T) {
		...
		doc := mustCreateDocument(t, "Test content", nil)
		doc.Metadata["start_page_number"] = 10
		doc.Metadata["end_page_number"] = 15
	...
	t.Run("with markers and partial page metadata", func(t *testing.T) {
		...
		doc := mustCreateDocument(t, "Test content", nil)
		doc.Metadata["start_page_number"] = 5
		// Missing end page
...
func TestFileWriter_Integration(t *testing.T) {
	t.Run("complete write workflow with all features", func(t *testing.T) {
		...
		doc1.Metadata["start_page_number"] = 1
		doc1.Metadata["end_page_number"] = 10
		doc1.Metadata["author"] = "John Doe"

		doc2 := mustCreateDocument(t, "Chapter 2: Methods", nil)
		doc2.Metadata["start_page_number"] = 11
		doc2.Metadata["end_page_number"] = 25

		doc3 := mustCreateDocument(t, "Chapter 3: Results", nil)
		// No page metadata

		// Write documents
		err = writer.Write(context.Background(), []*Document{doc1, doc2, doc3})
		require.NoError(t, err)
...
func mustCreateDocument(t *testing.T, text string, media interface{}) *Document {
	t.Helper()
