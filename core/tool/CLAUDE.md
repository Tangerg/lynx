...
- **契约、通用适配与实例集合**：`Tool`、`Binding`、`Authorizer`、`Guard`、`WrappingTool`、`Capability`、`Func`、`NewFunc` 与 `Registry` 属于本包；schema 的派生、解析与验证只由 `core/jsonschema` 拥有；shell、文件系统、HTTP、搜索、skill 等具体执行能力属于 `tools` module family。
...
- `Guard` 只在已验证 `Invocation` 的执行边界询问 `Authorizer`；身份、租户、同意流程和策略存储由调用方通过 context 与具体实现持有，不进入 Core。
...
- ❌ 在 Core 固化角色、权限表、审批流程、提示注入分类或应用级安全策略。
