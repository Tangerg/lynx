...
func TestDispatchRejectsUnsupportedOperators(t *testing.T) {
	...
	if err := filter.DispatchBinary(nullTest, filter.BinaryHandlers{}); err == nil {
		t.Fatal("DispatchBinary accepted a null operator")
	}

	invalidUnary := &filter.UnaryExpr{Op: filter.OpAnd, Right: filter.EQ("field", 1)}
	if err := filter.DispatchUnary(invalidUnary, nil); err == nil {
...
func TestLiteralAsKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := filter.LiteralAsKey(tt.literal)
...
func TestLiteralToValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := filter.LiteralToValue(tt.literal)
			if (err != nil) != tt.wantErr {
...
func TestConversionRejectsMalformedOperands(t *testing.T) {
	if _, err := filter.ExtractValue(filter.NewIdent("field")); err == nil {
		t.Fatal("ExtractValue accepted an identifier")
	}
	if _, err := filter.CollectKeyPath(filter.NewLiteral("field")); err == nil {
	...
	if _, err := filter.CollectKeyPath(invalidIndex); err == nil {
...
func TestConvertListLiteralRejectsInvalidLists(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, _, err := filter.ConvertListLiteral(tt.list); err == nil {
...
func TestRequireListLiteralRejectsEmptyList(t *testing.T) {
	...
	if _, err := filter.RequireListLiteral(expr); err == nil {
...
func TestExactNumberConversions(t *testing.T) {
	t.Run("exact text", func(t *testing.T) {
		...
		actual, err := filter.NumberText(literal)
	...
	t.Run("int64 rejects fraction and overflow", func(t *testing.T) {
		if _, err := filter.NumberToInt64(filter.NewLiteral(1.5)); err == nil {
			t.Fatal("NumberToInt64 accepted a fraction")
		}
		if _, err := filter.NumberToInt64(filter.NewLiteral(uint64(math.MaxUint64))); err == nil {
			t.Fatal("NumberToInt64 accepted uint64 overflow")
		}
	})

	t.Run("float64 rejects rounded integer", func(t *testing.T) {
		if _, err := filter.NumberToFloat64(filter.NewLiteral(uint64(1<<53 + 1))); err == nil {
			t.Fatal("NumberToFloat64 accepted a rounded integer")
		}
		if actual, err := filter.NumberToFloat64(filter.NewLiteral(uint64(1 << 54))); err != nil || actual != 1<<54 {
			t.Fatalf("NumberToFloat64(exact power of two) = %v, %v", actual, err)
		}
	})

	t.Run("float32 rejects rounded integer", func(t *testing.T) {
		if _, err := filter.NumberToFloat32(filter.NewLiteral(1<<24 + 1)); err == nil {
			t.Fatal("NumberToFloat32 accepted a rounded integer")
		}
		if actual, err := filter.NumberToFloat32(filter.NewLiteral(1.5)); err != nil || actual != 1.5 {
