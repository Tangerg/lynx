...
func NumberText(lit *Literal) (string, error) {
...
func NumberIsInteger(lit *Literal) (bool, error) {
...
func NumberToInt64(lit *Literal) (int64, error) {
	number, err := numberRat(lit)
	if err != nil {
		return 0, err
	}
...
func NumberToInt(lit *Literal) (int, error) {
...
func NumberToFloat64(lit *Literal) (float64, error) {
	number, err := numberRat(lit)
	if err != nil {
		return 0, err
	}
...
func NumberToFloat32(lit *Literal) (float32, error) {
	number, err := numberRat(lit)
	if err != nil {
		return 0, err
	}
...
func numberRat(lit *Literal) (*big.Rat, error) {
...
// keys.
func LiteralAsKey(lit *Literal) (string, error) {
	switch {
	case lit.IsString():
		return lit.AsString()
	case lit.IsNumber():
...
func LiteralToValue(lit *Literal) (any, error) {
	if lit == nil {
...
func numberValue(lit *Literal) (any, error) {
	if lit == nil || !lit.IsNumber() {
		return nil, fmt.Errorf("filter: expected number literal, got %v", lit)
	}
	value := lit.Value
...
// ExtractValue asserts expr is an [Literal] then delegates to
// [LiteralToValue]. Used by the comparison branch of every visitor.
func ExtractValue(expr Expr) (any, error) {
	lit, ok := expr.(*Literal)
	if !ok {
...
//
//   - For a bare *Ident the path is just [ident].
//   - For *IndexExpr chains (profile["name"]["first"]) it returns
//     ["profile", "name", "first"].
//
// Callers can extend this by joining the slice with "." (for nested
// dotted paths) or by treating the first element as a flat key.
func CollectKeyPath(expr Expr) ([]string, error) {
