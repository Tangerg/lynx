...
|-------------|----------|------|
| `@Agent` / `@Action` 注解 | DSL `agent.New(...).Action(...)` | Go 没有运行时注解；显式 builder 比反射更快、更可调试 |
...
| P2 | **不反射热路径** | 反射只在 register 时跑一次（reflect package），运行时全是直调闭包 |
...
import (
    "context"
    agentreflect "github.com/Tangerg/lynx/agent/reflect"
    "github.com/Tangerg/lynx/agent/core"
)

type IntentAgent struct {
    _   struct{} `agent:"name=IntentBot,description=Routes customer intents,version=1.0.0"`
    LLM core.ChatClient
}

// Action：方法签名 (ctx, In) → (Out, error)
func (a *IntentAgent) ClassifyIntent(ctx context.Context, in UserInput) (Intent, error) {
    // ...
}

// Condition：方法签名 (ctx, *core.OperationContext) → core.Determination
func (a *IntentAgent) IntentClassified(ctx context.Context, oc *core.OperationContext) core.Determination {
    if _, ok := core.Get[Intent](oc.Blackboard, "it"); ok { return core.True }
    return core.False
}

// Goal：方法名以 "Achieves" 开头，返回 *core.Goal
func (a *IntentAgent) AchievesIntent() *core.Goal {
    return core.GoalProducing[Intent]("intent classified").WithValue(1.0)
}

func main() {
    a, err := agentreflect.Register(&IntentAgent{LLM: chatClient})
    if err != nil { log.Fatal(err) }
    // ... platform.Deploy(a) ...
}
```

反射只在 Register 时跑一次；运行时通过缓存的 `reflect.Method.Func` 闭包调用，比直接调用慢约 10×（但在 LLM 调用毫秒级背景下可忽略）。
...
### 13.1 为什么核心放 Action 接口而不是反射注册全用反射？

**答**：embabel 大量用 Kotlin 反射读 JVM 签名信息——这在 JVM 上几乎免费且元信息丰富。Go 反射比直接调用慢 10-50×，且对自定义泛型参数的支持很弱。

我们的折衷：
- **泛型构造器** `NewAction[In, Out]`（默认）：编译期保留所有类型，零反射
- **反射注册**（`reflect.Register`）：只在启动时跑一次反射，运行时调缓存的闭包

这样 95% 的用户用 DSL 完全无反射，从 Spring 迁移的用户用 reflect 得到熟悉的体验，两者性能差异在 LLM 调用主导的应用场景下不可见。
