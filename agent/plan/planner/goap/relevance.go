...
func relevantActions(actions []core.Action, goal *core.Goal) []core.Action {
	...
	for {
		...
		for _, action := range actions {
			...
			relevant[meta.Name] = struct{}{}
			changed = true

			for key, value := range meta.Preconditions {
				needed[needTuple{key: key, value: value}] = struct{}{}
			}
		}
		if !changed {
