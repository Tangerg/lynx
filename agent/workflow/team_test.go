...
func teamAgentA() *core.Agent {
	return agent.New(agent.Config{Name: "team:A", Actions: []agent.Action{agent.NewAction("A:tokenize", func(_ context.Context, _ *core.ProcessContext, input teamInput) (teamLength, error) {
		return teamLength{V: len(input.Text)}, nil
	}, core.ActionConfig{})}})
}

func teamAgentB() *core.Agent {
	return agent.New(agent.Config{Name: "team:B", Actions: []agent.Action{agent.NewAction("B:double", func(_ context.Context, _ *core.ProcessContext, length teamLength) (teamResult, error) {
...
func TestTeamValidatesConfiguration(t *testing.T) {
	duplicate := func(name string) *core.Agent {
		return agent.New(agent.Config{Name: name, Actions: []agent.Action{agent.NewAction("same-name", func(context.Context, *core.ProcessContext, teamInput) (teamResult, error) {
...
func TestTeamPreservesMemberSnapshotBindings(t *testing.T) {
	first := teamAgentA()
	firstWithState := agent.New(agent.Config{
		Name:             first.Name(),
		Actions:          first.Actions(),
		SnapshotBindings: []core.Binding{core.NewBinding[int]("team_first_state")},
	})
	second := teamAgentB()
	secondWithState := agent.New(agent.Config{
		Name:             second.Name(),
		Actions:          second.Actions(),
		Goals:            second.Goals(),
		SnapshotBindings: []core.Binding{core.NewBinding[string]("team_second_state")},
	})
	...
	state := definition.SnapshotBindings()
	if len(state) != 2 || state[0].Name != "team_first_state" || state[1].Name != "team_second_state" {
		t.Fatalf("snapshot bindings = %#v", state)
	}
}

func TestTeamRejectsDuplicateMemberSnapshotBindings(t *testing.T) {
	first := teamAgentA()
	firstWithState := agent.New(agent.Config{
		Name:             first.Name(),
		Actions:          first.Actions(),
		SnapshotBindings: []core.Binding{core.NewBinding[int]("shared_state")},
	})
	second := teamAgentB()
	secondWithState := agent.New(agent.Config{
		Name:             second.Name(),
	...
	definition, err := workflow.Team(workflow.TeamConfig{
		Name:   "team:duplicate-snapshot",
		Agents: []*core.Agent{firstWithState, secondWithState},
	})
	if definition != nil || err == nil || !strings.Contains(err.Error(), "duplicate snapshot binding") {
		t.Fatalf("Team = %#v, %v; want duplicate snapshot-state error", definition, err)
	}
}
