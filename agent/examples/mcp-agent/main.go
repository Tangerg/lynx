// mcp-agent shows the complete client-side MCP integration in one file:
//
...
//
// The example uses an in-memory transport pair so it runs offline; swap
// the transport for a CommandTransport / StreamableClientTransport in
// real deployments.
//
// Run from repo root:
//
