...
### 1.2 Baseline 39 observation 修订

ADR-A2-090 将 Framework Event 从“严格 JSON envelope + 由消费者自行猜 payload”收敛为自校验的 typed fact：

- `Event.Valid`、构造与反序列化共同拒绝未知 Framework event、错误 phase、错配的 Process/Step/Effect identity、未知字段、缺失必填 duration/Usage 和非法领域枚举；
- Event 新增 `ProcessFinished`、`SignalAccepted`、`StepFinished`、`StepCommitted`、`EffectStarted`、`EffectFinished` 与 `DeltaDropped` typed fact读取，外部 observer 不再复制 Kernel 私有 payload struct；
- `agent.process.finished` 直接携带 authoritative terminal `Usage`；Strategy Step 提交 Paused 后发布独立 `agent.process.paused` fact，durable pause/terminal lifecycle 只在对应 tree checkpoint callback 成功返回后发布；
- 同一 tree 的初始 started/restored Event 按 parent-before-child canonical order 发布，恢复 trace 不依赖 map 遍历。
...
- Event/Delta：自足携带 exact execution attribution 的已发生观察事实与 best-effort 临时增量；Event vocabulary/name/phase/identity/payload 自校验并提供 immutable typed fact读取，terminal fact携带 authoritative Usage；durable value携带 current incarnation，listener 无 veto/error 通道，Engine 以不可变饱和计数快照暴露被隔离的 listener panic。
...
- Framework Event/Delta observation wire：`252f8d49c23eb4182b1e85c66cebb35383e409677ab2e312baa3f1eefc43286d`
