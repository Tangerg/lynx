...
- 原因：这些 seam 都能脱离 ScopeApp 产品词汇独立说明，也分别由 admission correctness、Strategy adapter attribution、冻结 manifest 状态与 Engine-owned tree invariant 证明；因此是合理框架能力。反过来复制 Runtime DTO、把 transcript 建在 Delta 上、让 Host 改 Interaction wire 或保留旧 engine 作为旁路，都会重新制造抽象泄漏和双重所有权。
...
- 决策：三类 mutable owner 保持 concrete pointer API，不增加 interface facade、registry 或共享 service object。Engine 必须由 `NewEngine` 构造，Observer 必须由 `agentotel.NewObserver` 构造，二者与 Platform 都不得在首次使用后复制；公共 GoDoc 与 digest 直接冻结该约束。不可变值、Deployment/Definition 与只读 handle 继续可安全复制，不把 no-copy 规则泛化到整个 Framework。
...
## ADR-A2-089：以 root tree 单一提交线建立 durable effect kernel

- 状态：已接受并在 Scope kernel 实施；形成 Baseline 38。取代 ADR-A2-031/033 中以独立 Process loop、Process prepared snapshot 和临时 tree quiescence 拼接一致性的实现合同，保留其“Effect 前必须先有可恢复 intent”“root tree 是完整恢复单位”的目标；取代 ADR-A2-060 中 `StartedAt` 属于 admission 的部分；取代 ADR-A2-064 中 durable prepared tree change 缺少 acknowledged source-head 的部分。Flame adapter 仍是独立 harness 实施，不属于本 ADR 的 agent 基线完成条件。完整设计与 crash matrix 见 [`DURABLE_KERNEL_DESIGN.md`](DURABLE_KERNEL_DESIGN.md)。
- 证据：当前 Engine 的 Framework 状态由每个 `processLoop` 独立提交，child start、budget、child wait、parent settlement 和 TreeSnapshot 只能借助 Engine map、跨 loop command 与 quiescence barrier 临时协调。恢复单位已经是完整 root tree，mutation owner 却仍是 Process，形成结构性错位。现有 `PreparedStepAcknowledger` 只接收单 Process snapshot，不能原子表达 parent/child topology、Parked/Terminal 纯状态边界或 writer fencing；`Signal.ReceivedAt` 与 admission `StartedAt` 又把本地 wall clock 注入可重放输入或稳定 admission identity。
- 决策：每棵 root tree 由一个私有 `treeRuntime` 拥有唯一 Framework commit line。Process command、topology、mailbox、budget/capability、Effect phase、control intent、termination bookkeeping 和 acknowledged tree head 都只能由该 owner 改变。Strategy `Step` 与 Dispatcher I/O 是 owner 外的有界 job：同一 Process 至多一个 Step，sibling 可并行计算；completion 携带 tree incarnation 与 attempt identity，只有 owner 可以验证并提交，过期或取消结果整体丢弃。Step 只接收由 Runtime 从 `context.Background()` 构造的 cancellation-only context，不继承 value、deadline 或 cause。
- 决策：Dispatcher Effect 使用 `planned → pending → settled` 的明确状态机。pending boundary 先提交完整 prospective TreeSnapshot，成功后才 Dispatch；settled boundary 将 definite/Unknown Settlement 与 Host 产品事实原子提交。一个 Process 内 Effect batch 保持 declaration order，尚未派发的 planned Effect 不能被误记为 Unknown。Unknown resolution 是显式 `resolved` boundary，成功后仍回到 settled definite 状态；Kill 或 durability fault 只把 pending/settled-Unknown EffectID 冻结进 `Termination`。
- 决策：新增闭合、typed 的 `TreeDurability` Host port，嵌入现有 `ProcessStartOutcomeAcknowledger`，并只增加 tree activation、Effect boundary、Parked/Terminal checkpoint 三类方法。它取代 `PreparedStepAcknowledger`；`EngineConfig{}` 仍是零配置 ephemeral mode，单独配置 start-outcome acknowledger 仍是合法的 ephemeral lifecycle 用法，同时配置 standalone acknowledger 与 `TreeDurability` 明确失败。Framework 不增加 Store、transaction、journal、lease、Run、Session 或通用 commit hook。
- 决策：`TreeSnapshot` 是唯一恢复单位和完整 durable program counter。`Snapshot`/`Process.Capture` 分别重命名为 `ProcessSnapshot`/`Process.Snapshot`，仅保留诊断与 Strategy inspector；删除 `Engine.Restore`。durable snapshot 只以存在有效、crypto-random `TreeIncarnationID` 表示其模式，每次 restore 在发布 Process 前以 previous digest/incarnation CAS 激活新 incarnation；不增加可能与之矛盾的 durability boolean 或递增 generation。durable Engine 不开放非权威 `CaptureTree`，而返回 `ErrTreeCaptureUnavailable`。
- 决策：Signal 不再携带 Engine 生成的 `ReceivedAt`；影响行为的时间必须由 Host 放入稳定 payload，观察时间只属于 Event。`ProcessAdmission` 不再携带 `StartedAt`；accepted admission 后 Runtime 生成 tentative time，只有 started outcome 与 prospective tree 成功提交后才成为 Process 生命周期事实，aborted outcome没有该字段。
- 决策：child admission job 的预算先作为 Runtime 私有 provisional reservation 参与资源门禁，不提前写入 committed `ProcessSnapshot.ReservedBudget`；started outcome prospective tree 同时安装 child、topology、parent settlement 与 committed reservation，aborted/stale/fault 路径释放。durable restore 同理先建立不可见 whole-tree local reservation，再进行 activation CAS，成功后只做无失败 publication。
- 决策：tree-scoped durability fault 先收集每个 Process 已有 Unknown、失败 boundary 和 sibling in-flight Dispatcher/child-admission EffectID，再按 canonical Process 顺序完成全树终止决议，第二阶段才发布 Result 与父子 bookkeeping；不得让 map 遍历或 propagation 先后改变终态或丢失对账身份。
- 决策：Host 主动 waiting-subtree cancellation 继续使用具体的一次性 prepared capability，不泛化为任意 tree mutation。durable Prepare 必须从 last acknowledged head 冻结 source，暴露 `SourceTreeDigest`，resulting snapshot 沿用 current incarnation；Host 以 `(incarnation, source digest)` CAS 提交产品事实与 resulting head，再调用 contextless `Apply`，由 owner line 同步安装 Framework state和新的 acknowledged head。ambiguous commit 必须 reread head 后选择 Apply/Discard/fail-closed。
- 决策：capability 继续只表示 Framework authorization/gating。child authority 必须是 parent 子集，Effect 在 pending intent 前校验 required capability；真实 credential、filesystem、network 和 sandbox enforcement 属于 Flame/Host。Event/Delta 继续是 observation，但在 durable mode携带 current incarnation，旧 writer observation 不能更新 current Run。
- 原因：这套结构直接闭合 Scope/Flame 的恢复、并发、授权和事务边界，同时保持 Definition/Execution、Dispatcher、DeploymentResolver、ProcessAdmitter、TreeDurability 与 observation 为少数真实开放边缘。它不依赖 OTP/Temporal 类比，也不复制 supervisor、workflow history、activity registry、sandbox 或应用 Store。
- 后果：Scope 以有意 breaking 的 Baseline 36～38 完成 owner runtime、唯一恢复单元、稳定输入、TreeDurability、fencing/Unknown、prepared admin transform、删除与文档同步，不提供 alias、dual-read、dual-write 或 compatibility shim。Process Snapshot/TreeSnapshot 直接升级为 v9/v7；durable Event/Delta wire携带 incarnation；`agenttest.MemoryTreeDurability` 与公开 conformance 通过真实 Engine 路径验证三类 Effect boundary、checkpoint、幂等重投和 activation 竞争。Flame 只有完成自己的 schema/transaction/boot integration 后才能宣称 active recovery。

## ADR-A2-090：Framework Event 必须是自校验的 typed fact

- 状态：已接受并实施；形成 Baseline 39。
- 证据：Event 名称集合在架构中已经封闭，但 `Event.Valid` 过去只验证名称符合 qualified-name 语法且 payload 是合法 JSON；外部 JSON 可以把错误 phase、缺失 EffectID、未知 event name 或非法 payload 组合成“有效”Event。`otel/agent` 为取得 status/target/duration 又复制了一份 Kernel 私有 payload struct并忽略 `json.Unmarshal` error，导致 observation owner 有两份协议真相。Process 终态 Event 也没有携带同一 owner 已拥有的 terminal Usage，中心 telemetry 只能依赖调用方另行拼接 Result。
- 决策：Event 构造、`Valid` 与 JSON 反序列化共同执行封闭 name、phase、Process/Step/Effect identity 和 event-specific payload 合同。公开 immutable `ProcessFinishedFact`、`SignalAcceptedFact`、`StepFinishedFact`、`StepCommittedFact`、`EffectStartedFact`、`EffectFinishedFact` 与 `DeltaDroppedFact` 由 Event 自身按准确名称读取；raw `Payload` 继续服务 wire 转发与审计，但 observer 不再复制私有 JSON shape。finished fact 直接投影 authoritative terminal Usage；duration wire 的必填性显式表达，旧 observation shape 不双读。
- 决策：Strategy Step 提交 `StatusPaused` 后发布独立 committed process-paused fact，不要求观察者从 Step payload 反推 Process lifecycle。durable tree 的 Strategy/control pause 与 terminal publication 收敛到 tree owner 的同一个 checkpoint publication aggregate，只能在 Host checkpoint callback 成功返回后发布；crash prefix matrix 精确守卫 after-commit/before-publication 边界。tree 初始 started/restored facts 使用既有 canonical parent-before-child 顺序，避免恢复时 map 遍历让 child trace 失去 parent。
- 决策：`otel/agent` 把一次 start/restore 到 terminal 视为 Process activation：duration 统一使用 OTel 秒单位，增加 activation duration 与 terminal committed-step/prepared-effect/accepted-signal histograms，并为 Step/Effect metric 增加低基数 Deployment attribution、为 span 增加 Process/tree/Deployment 与 durable incarnation attribution。Process error outcome、失败的 Step attempt 与非成功 Effect settlement 从 typed fact 投影私有稳定 error，既 `RecordError` 形成标准 exception event，也设置 span error status；不伪造原始 stack、不泄露 Failure message 或 opaque payload。Observer 只保存 Span，不保存 callback context；`Close` 在线性化拒绝新观察后等待 in-flight callback，再结束残留 span。Kernel 仍不 import OTel，也不增加 logger、metric hook、dynamic attribute map 或产品 identity。
- 后果：root public/GoDoc 与 Framework observation wire 形成 Baseline 39；Process Snapshot v9、TreeSnapshot v7、Framework/Strategy recovery protocol 和其他 public package不变。OTel 的旧毫秒单位与 `agent.process.starts` 名称不保留兼容双写，新的 activation 语义可明确区分 started/restored。Event/Delta 仍不是恢复源，terminal Usage metric也不是预算或账本真相。

## ADR-A2-091：Definition 合同与性能纪律必须有可执行证据

- 状态：已接受并实施；形成 Baseline 40。
- 证据：`TreeDurability` 已有第三方 adapter 可复用的公开 conformance，但更常由第三方编写的 Definition/Execution 仍主要依赖文字合同。实例共享可变状态、Restore 丢字段或 Step 读取 hidden global input，最终可能表现为很难回溯的 durability conflict。与此同时，root-tree 单提交线的正确性已有 crash-prefix 证明，但 snapshot scaling、pure replay cost 和 sibling fairness 仍只有设计推理；没有测量就不应继续优化 owner 模型。
- 决策：`agenttest.RunDefinitionConformance` 接收一个 Definition、合法 Input、初始 Signals 与可选 restored cases，只通过公开边界并发读取 Descriptor/Start，比较 independently owned Snapshot/Restore 和 Step 后的 canonical bytes，并验证 Transition 合法性与 Signal consumption。Interaction、Planning、Workflow 各自以真实 Definition 接入。suite 只证明给定成功样本实际观察到的稳定性，不能静态证明实现从不访问 clock/random/global/I/O；失败、取消、畸形状态和领域不变量继续由 owner 自己测试。
- 决策：性能验证不增加 logger、timing callback、通用 hook 或 Host 类型。Scope 用 test-only benchmark 测量 1/15/63/255 Process 的 TreeSnapshot bytes/build/parse、1 KiB/64 KiB state 的 Restore→Step→Snapshot→Restore，以及慢 sibling 存在时 fast sibling 的 committed completion latency；绝对结果不进入合同，回归只在相同机器/toolchain 下用统计比较。首轮测量证明 private prospective tree construction 存在一次完整的 encode→strict parse→re-encode，内部路径因此改为取得输入 ownership 后直接复用 typed validation/canonicalization 并只 encode/hash 一次；外部 strict parse与 wire不变。callback/transaction/lock 等 Host 边界由 Flame trace 与 benchmark 拥有。
- 后果：只有 `agenttest` public/GoDoc digest 形成 Baseline 40；root、Interaction、Planning、Workflow、GOAP、Platform、全部 snapshot/Strategy protocol 与 observation wire不变。tree/Process/job 三种尺度在架构中明确分工，当前没有证据要求改变 root-tree owner、并行 Effect batch 或 snapshot wire。
