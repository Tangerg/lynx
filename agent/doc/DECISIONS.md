...
- 状态：已接受，实施中；取代 ADR-A2-031/033 中以独立 Process loop、Process prepared snapshot 和临时 tree quiescence 拼接一致性的实现合同，保留其“Effect 前必须先有可恢复 intent”“root tree 是完整恢复单位”的目标；取代 ADR-A2-060 中 `StartedAt` 属于 admission 的部分；取代 ADR-A2-064 中 durable prepared tree change 缺少 acknowledged source-head 的部分。完整目标设计与 crash matrix 见 [`DURABLE_KERNEL_DESIGN.md`](DURABLE_KERNEL_DESIGN.md)。
...
- 后果：下一公共基线将是有意 breaking revision，不提供 alias、dual-read、dual-write 或 compatibility shim。实施按 owner runtime、唯一恢复单元、稳定输入、TreeDurability、fencing/Unknown、prepared admin transform、Flame adapter、删除与文档八个可独立验证的切片推进；每个切片必须保持 build/vet/test/race 和 owner wire/architecture gate 自洽。
