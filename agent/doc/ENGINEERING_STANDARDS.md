...
`agent` 是可独立导入、可在任意 Go 程序中装配的库，不是某个产品后端的内部实现。任何公共抽象都必须能脱离具体 Host 完整说明；只能用某个产品名、数据库表或 UI 流程解释的类型没有进入 Framework 的资格。
...
- public tree durability conformance 只能通过正常 Engine 路径驱动 opaque boundary，覆盖 base head、same-content retry、Effect 三类 boundary、Parked/Terminal、random incarnation fencing 与 delayed old-writer transaction/activation 竞争；不得为测试暴露 boundary constructor、private wire 或 Store mutation hook。content conflict、commit ambiguity/rollback 由 adapter 存储层测试制造，child outcome atomicity与 checkpoint coalescing 由 Kernel owner tests证明；十行 crash prefix 由 `agenttest` 内部 typed before/after commit gate 逐项证明，gate 不得进入公共 API。
- child start 的 provisional budget 必须参与所有资源门禁但不得进入 committed snapshot；只有 started outcome prospective tree 能把它与 child/topology/parent settlement 一起转正，aborted、stale 与 durability fault 都必须释放。
