...
//
// SequenceAgents 编译 a₁ → a₂ → ... → aₙ 串行链为单个 agent。
...
func SequenceAgents(name string, agents ...*core.Agent) *core.Agent

// workflow/parallel.go
//
// ParallelAgents 编织 N 个同 In/Element 型的 sub-agent 并行 fan-out,
...
func ParallelAgents[In, Element, Result any](spec ParallelAgentsSpec[In, Element, Result]) *core.Agent

// workflow/loop.go
//
// LoopAgent 反复运行 body sub-agent 直到 Until predicate 返回 true。
...
func LoopAgent[In, Out any](spec LoopAgentSpec[In, Out]) *core.Agent
```
...
4. **goal 形态**：编译产物是单 action（`CanRerun=false`）+ 单 goal（`Pre: ["{name}-done"]`），与现有 `RepeatUntilAgent` / `ScatterGatherAgent` 同形。
...
| 并行调 N 个 generator 函数（无 agent 身份） | `workflow.ScatterGatherAgent`（action 级） |
| 反复调一个 task 闭包直到满意 | `workflow.RepeatUntilAgent`（action 级） |
| **`research → write → edit` 三个独立 agent 串行** | **`workflow.SequenceAgents`（新）** |
| **同时跑 3 个 evaluator agent，各自带 LLM/tools 树** | **`workflow.ParallelAgents`（新）** |
| **反复运行一个完整 critic agent 直到通过** | **`workflow.LoopAgent`（新）** |
...
1. **先做 P1 agent 级 workflow agents**（`SequenceAgents` / `ParallelAgents` / `LoopAgent`）—— 真实补全：用户当前要做"researcher → writer → editor"这类确定性 agent 流水线必须自己手写 child process plumbing
...
真正能跨范式带回 lynx 的只有两项：
1. **agent 级 workflow agents**（`SequenceAgents` / `ParallelAgents` / `LoopAgent`）—— 确定性 sub-agent 编排，弥补 `AsChatTool` 必须 LLM-driven 的缺口
