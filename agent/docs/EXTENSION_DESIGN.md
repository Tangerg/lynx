...
都是配置错误，Engine 构造或 Process 启动会 fail fast。

## 2. 作用域与合并

- Engine scope：`runtime.Config.Extensions`，对该 Engine 创建的 Process 生效；
- Process scope：`core.ProcessOptions.Extensions`，只对当前 Process 生效。
...
| Capability | 分发语义 | 所有权 |
|---|---|---|
| `planning.Planner` | 按 Agent 的 PlannerName 选择一个 | 规划算法 |
| `core.ActionMiddleware` | onion chain；先注册者最外层 | Action 横切行为 |
| `core.ToolMiddleware` | wrap chain；先注册者最内层 | Tool 横切行为 |
| `core.AgentValidator` | 全部执行并聚合错误 | deploy-time 验证 |
| `core.GoalApprover` | 全部同意才保留 Goal | Goal 选择策略 |
| `core.ToolGroupResolver` | 第一个 `ok=true` 的 resolver 获胜 | role 到工具组解析 |
| `core.ChatProvider` | Process 优先，首个非 nil Model 获胜 | 每进程模型选择 |
| `core.StopPolicy` | 任一返回 stop 即终止 | tick 边界策略 |
| `core.IDGenerator` | 最近注册者获胜 | Process ID |
| `core.Blackboard` | 最近注册的 prototype 获胜，`Clone` 新实例 | Process 状态 |
| `runtime.EventListener` | 全部订阅者接收事件 | 观测与投影 |

`ProcessStore`、`SessionStore`、默认 Chat、`ChatGuardrails` 和 snapshot policy 是
`runtime.Config` 的稳定构造依赖，不因为“可替换”就进入 Extension registry。动态领域依赖
