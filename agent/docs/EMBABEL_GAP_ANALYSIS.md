...
> **第七轮重写**（2026-05-08）。基线：embabel-agent (Kotlin/Spring) v0.4 / lynx/agent (Go) HEAD。
>
> 本轮相对第六轮的关键变动：刚结束一轮系统性重构 —— Wave B 死代码清理（Goal/AgentConfig/ActionConfig/EffectSpec/事件/Determination 上一批未启用字段全部砍光）、Wave C 防御式 nil 检查全面退场、Wave D 现代 Go 标准库迁移（`strconv.Quote` / `slices.SortStableFunc` / `slices.Sorted(maps.Keys)`）、Wave E 单实现接口去虚化（`Planner.Prune` / `worldStateDeterminer`）、Wave F 文档语气统一与错误信息标准化。**非测试代码从 ≈9512 LOC 收敛至 8853 LOC**，所有 examples + 测试包仍绿。
>
...
| Tool 模型 / advanced policies | ✅ `core/tool_group.go`（与 `chat.Tool` 共用类型） + `dsl/toolpolicy`（OnceOnly + Unlock） | ✅ Tool / ToolObject + `OneShotPerLoopTool` / `PlaybookTool` | **基线追平** |
...
| `agent/dsl/` + `dsl/workflow/` + `dsl/toolpolicy/` | `embabel-agent-api/src/main/kotlin/com/embabel/agent/api/{dsl,common/workflow}/` + `agent/agentic/` |
...
- 参数化的 subagent factory —— `runtime.AsChatTool[In, Out]` / `AsChatToolFromAgent[In, Out]` / `AsMCPTool[In, Out]` 三个 type-parameterised helper 顶替 embabel 的 `Subagent.byName()`/`ofClass()`/`ofInstance()`/`ofAnnotatedInstance()` 4 路 Builder + 注解。
- 显式的 PlannerType 枚举 —— `core.PlannerGOAP / PlannerHTN / PlannerReactive`，编译时穷举 `switch`。embabel 是 Spring bean 的字符串拼装。
- concept-per-package 的 layout —— `dsl/workflow/` 与 `dsl/toolpolicy/` 是独立 sub-package，import 路径与文件位置一致，不必在 200+ 文件平面里用 IDE 定位。
...
| **One-shot-per-loop** | ✅ `dsl/toolpolicy.WithOnceOnly` + `WithLoopScope` ctx | ✅ `OneShotPerLoopTool.kt` |
| **Playbook tool（条件解锁）** | ✅ `dsl/toolpolicy.WithUnlock(tool, cond)`（含 reason 文本回写 LLM） | ✅ `agentic/playbook/PlaybookTool.kt` |
...
| 子 agent → chat tool | ✅ `runtime.AsChatTool[In, Out]`（[runtime/subagent.go:43](../runtime/subagent.go)） + `AsChatToolFromAgent[In, Out]`（[runtime/subagent.go:67](../runtime/subagent.go)） | ✅ `Subagent.byName / ofClass / ofInstance / ofAnnotatedInstance` |
| 工厂路径数 | **2**（按名 + 直传 agent 实例） | **4**（多两条来自注解类） |
| Awaitable 子 agent 优雅退化 | ✅ child Waiting 时返回 `{status:"waiting", agent, processId, awaitableId, prompt}` JSON（[runtime/subagent.go:167](../runtime/subagent.go)） | ✅ `ProcessWaitingException` → tool result text |
| `runtime.AsMCPTool[In, Out]` | ✅ ([runtime/subagent.go:205](../runtime/subagent.go)) —— 独立 process（无父 ctx 要求），同样支持 waiting graceful-degrade | ✅ `PerGoalMcpExportToolCallbackPublisher` |
| `RunSubagent` 注解 | ❌（DSL 路线无注解） | ✅ |

**lynx 2 工厂 vs embabel 4 工厂**：因为 lynx 没有注解类，所以 `ofClass / ofAnnotatedInstance` 两条工厂自然不存在；2 工厂（按名 + 直传）已覆盖 lynx 风格的全部用例。
...
lynx：`dsl.Builder`（[dsl/builder.go](../dsl/builder.go)），编译期类型安全 + IDE rename 100% 友好 + 启动时静态可推。embabel：`@Agent / @Action / @AchievesGoal / @LlmTool` + Spring scan。Go 没有 Spring AOT，反射注册不合 Go 哲学。**永久分歧**，请把它当哲学差异而非缺口。
...
| 维度 | lynx | embabel | 谁更好 |
|---|---|---|---|
| **Top-level surface** | **5 个 constructor** —— `agent.New / NewAction / NewCondition / GoalProducing / NewPlatform`（[agent.go:27/31/40/46/49](../agent.go)） + 几个 helper（`runtime.AsChatTool / AsChatToolFromAgent / AsMCPTool / MCPToolGroupResolver`） | 100+ Spring beans + DSL | **lynx** |
...
| 代码量 | **8853 LOC** non-test（30+ 文件） | **~30k+ LOC**（200+ 文件） | **lynx** |
...
第七轮没有新增功能。**纯减负**：删了 30+ 个未启用的死字段 / 死接口 / 防御式重复检查 / 单实现虚接口，把核心模型与 embabel 的核心抽象再贴近一档（Goal 字段表对齐、Determination 不再有谓词糖、event 类型只保留 runtime 真发的那 14 个），同时利用 Go 1.21+ 标准库（`slices` / `cmp` / `strconv.Quote`）替换三处自手写工具代码。lynx agent 的功能面 vs embabel 仍是上一轮收尾时的状态——**P0–P3 主体全闭合，只剩独立子模块（A2A / RAG / Skills / Shell）和 niche LLM 优化（Matryoshka / StateMachine / Replanning）等用例驱动**。代码量 **8853 LOC** vs embabel 的 **~30k+ LOC**，同样的领域模型，3-4 倍的密度差距来自 Spring 工程基础设施 + 内置 RAG/Onnx/Shell 等独立模块。两个项目都正确——只是站在 Go 与 Kotlin/Spring 哲学的两端。
