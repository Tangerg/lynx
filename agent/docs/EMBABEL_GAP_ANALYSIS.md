# lynx/agent vs embabel-agent — 深度对比与缺口分析（第十一轮）

> **2026-05-11**。基线：embabel-agent (Kotlin/Spring) HEAD `1042d5870` ≈ **89.7k LOC** / 723 main 文件 / 16 Maven 子模块 vs lynx/agent (Go 1.26.3) HEAD `3dfbe25` / `feat/core-polish`，agent 模块 **10.9k LOC**（仓库总 43.7k LOC，9 个 workspace 模块）。
>
> 本轮**增量刷新**前一份（第十轮，2026-05-09），主要对齐：
> - embabel 近 ~30 个 commit（含 LoopMemo / OneShotPerLoopTool / per-call cost tracking / ephemeral process / retry classification 等）
> - lynx 完成的 SPI flattening（BlackboardFactory / PlannerFactory / PlannerType enum / ProcessControl 全部去掉）
> - 旧文档里残留的几个"`runtime.PlannerFactory`"、"`core.PlannerType`" 等过时引用
>
> 核心结论与上一轮一致：**lynx 在 framework 抽象层面已与 embabel 持平**。本轮新增的"应该闭合的 gap"主要在 observability 细化（per-LLM-call event）和 tool-loop 内部状态（LoopMemo 类比），不在抽象层。
>
> 姊妹篇 [`./EMBABEL_DEEP_COMPARISON.md`](./EMBABEL_DEEP_COMPARISON.md)（2026Q2）按"架构赌注"组织，跟本文的逐项打分形成互补 —— 本文更偏功能清单 / API 名称速查，新文档更偏战略定位 / 设计取舍分析。
>
...
| **Extension 模型** | ✅ 单注册入口 `PlatformConfig.Extensions` / `ProcessOptions.Extensions` + **10 capability**（含 `Blackboard` / `Planner` / `EarlyTerminationPolicy`，全部直接嵌入 `core.Extension`，**已去掉 factory 层**） | ✅ Spring DI 多 SPI 注入点（`BlackboardProvider` / `PlannerFactory` / 13 个 SPI） | **lynx 更整洁**：扁平 SPI 形态、无 factory 中间层 |
| **事件系统** | ✅ 15 个事件类型 + `event.Multicast` + `event.Listener` + `runtime.EventListener`（extension 包装）单一注册路径 | ✅ 约 30 个事件类型 + `AgenticEventListener` + Micrometer | **基线追平**；embabel 多了 per-tool-call / per-LLM-invocation 事件（见 §16 G1-G2） |
| **OTel observability** | ✅ `core.AgentTracer()` + 标准 attrs（`lynx.agent.tick` / `.action` / `.planner.astar` ...） | ✅ Micrometer + OTel exporter；新增 per-call cost tracking（commit `a31cef363`） | **基线追平**（embabel 多了 per-LLM-call cost / token 内置 metrics，见 §16 G3） |
| **持久化** | ⚙️ `core.Blackboard` extension（prototype + `Spawn()`）+ [`PERSISTENCE.md`](./PERSISTENCE.md) 实现指南 | ✅ Spring Data + `ContextRepository` SPI | **抽象等价**；开箱实现欠（按设计） |
| **多 LLM provider 内置 / RAG / Onnx / Shell / Skills / A2A / 注解 / classpath scan** | ❌（按定位） | ✅ 7+ 独立 Maven module | **故意分歧** |
| **代码量** | **10,975 LOC** non-test（agent 模块）/ 43.7k LOC（仓库总，9 module） | **89.7k LOC** non-test, 16 Maven 子模块 / 723 main 文件 | **lynx agent 1/8 ~ 1/9**（同核心模型，密度差源于 Spring 工程基础设施 + 独立子模块） |

**一句话**：lynx 在 framework 抽象层 1:1 追平 embabel 内核；多出来的能力（HTN / Reactive 严格更强 / Concurrent action mode / 单一 Extension 注册路径 / agent 级 workflow / dynamic publish / **扁平 SPI 无 factory 层**）是 lynx 自有创新，不是迫近的 gap。embabel 近期方向（LoopMemo / OneShotPerLoopTool / per-call cost）提示了 lynx 几个可以闭合的小 gap，见 §16。
...
| **Planner 扩展点** | ✅ `plan.Planner` 直接嵌入 `core.Extension`，按 `AgentConfig.PlannerName` dispatch（[plan/planner.go](../plan/planner.go)） | ✅ Spring `@Bean` 注入点 |
| **内置 planner** | "goap"（默认）/ "reactive" 框架自带；"htn" 必须用户提供 library | ✅ |
...
| **Core capability**（7 个） | `ActionInterceptor / ToolDecorator / AgentValidator / GoalApprover / EarlyTerminationPolicy / IDGenerator / ToolGroupResolver / Blackboard`（[core/](../core/)） | 对应 7 个 SPI |
| **跨包 capability**（3 个） | `runtime.EventListener`（[runtime/extension.go](../runtime/extension.go)） + `plan.Planner`（[plan/planner.go](../plan/planner.go)，按 `AgentConfig.PlannerName` 选）| 对应 3 个 SPI |
| **总计** | **10**（统一形态：直接嵌入 `core.Extension`，无 factory 中间层） | **9 直接对应 + 4 LLM ops 专属**（LlmMessageSender / Streamer / ToolInjectionStrategy / EmbeddingService） |
...
| **`core.Blackboard` extension**（**唯一** blackboard SPI；prototype + `Spawn()`） | ✅ ([core/blackboard.go](../core/blackboard.go)) | ✅ `BlackboardProvider` |
...
### 16.X embabel 近期新增 → lynx 可考虑闭合的小 gap

embabel 在 2026-04/05 的几个 commit 引入了 lynx 没有的细节能力，按 ROI 排序：

- **G1. `LLMInvocationEvent` + per-call cost 追踪** —— embabel commit `a31cef363`。低成本（约 80 LOC）；lynx 现有 `Process.RecordUsage` 只记聚合总和，没法做 per-call 归因。**建议做**：在 `event/` 加 `LLMInvocationEvent`，chat middleware 调用 listener。
- **G2. `ToolCallRequestEvent` / `ToolCallResponseEvent`** —— embabel `agent/api/event/AgentProcessEvent.kt` 已有。低成本（约 60 LOC）；今天 lynx 观测 per-tool-call 只能写 `ToolDecorator`，做成一等事件更干净。
- **G3. `LoopMemo` / `OneShotPerLoopTool` 类比** —— embabel commits `d5d428456` + `01ebec8f6`。中成本；真实用例：tool 每个 LLM tool loop 触发一次，或 tool 需要记住 "这轮调过没"。今天 lynx 用户得给每个 tool 写黑板胶水。**建议加到 `toolpolicy/`**。
- **G4. Sub-agent `ProcessOptions.Extensions` 父子继承** —— embabel commit `1042d5870` 刚为 listener 做了。低成本；lynx 应该对任何应该传播的 process-scope extension 镜像（或文档化为什么不传 —— 当前 `AsChatTool` **不**传 process-scope extension 给 child）。
- **G5. Action retry 按错误类型分类（retriable / non-retriable）** —— embabel commit `35b12148b`。中成本；lynx 的 `ActionQoS.MaxAttempts` 对所有错误一视同仁；分类后能做更聪明的 retry。
- **G6. Ephemeral process 标记** —— embabel commit `8247bc512`。仅在 lynx 将来引入 Process repository SPI 时需要镜像；现在 N/A。

每项都不紧急；优先级 G1 > G2 > G3 > G4 > G5 > G6。
...
- **第十一轮 SPI flattening + docs alignment** (2026-05-11, `70fe9d3` → `c75068c`)：**抹掉 factory 层**。`BlackboardFactory` → `Blackboard` 自己嵌入 `Extension`（prototype + `Spawn()`）；`PlannerFactory` + `PlannerType` enum → `plan.Planner` 嵌入 `Extension`，`AgentConfig.PlannerName` 按 name dispatch；`ProcessControl` 包装类删除，`EarlyTerminationPolicy` → Extension，OR-compose，Budget 隐式参与。所有 user-facing 文档同步 SPI 形态。Agent 模块 LOC **10,975**（含部分代码移入 + workflow / examples 扩充）。
...
lynx 在 framework 抽象层 1:1 追平 embabel 内核，并在 HTN / Reactive / agent 级 workflow / 单注册 Extension / dynamic publish / **扁平 SPI（无 factory 层）** 等方向有自有创新。剩余差距全部是**故意分歧**或**职责边界外**（Spring DI / 注解 / 多 provider / RAG / Shell 等），不是 framework 抽象的缺口。

embabel 近期在深挖 tool-loop 原语（`LoopMemo` / `OneShotPerLoopTool`）+ 细化 cost 归因（per-call event）—— lynx 可低成本闭合的 6 个小 gap 列在 §16.X，按 ROI 排序 G1-G6，最高 ROI 的是 G1（`LLMInvocationEvent`，约 80 LOC）。

代码量上：lynx agent 模块 **10,975 LOC** non-test vs embabel **89.7k LOC** Kotlin / 16 Maven 模块。同样的领域模型、≈ 1/8 的密度差距全部来自 Spring 工程基础设施 + 内置 RAG/Onnx/Shell 等独立模块——这是分工不同，不是优劣。
