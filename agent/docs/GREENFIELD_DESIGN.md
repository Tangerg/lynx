...
Tool loop 当前在 `agent/toolloop/`。**greenfield:留在 agent 侧,不搬回 core。** 理由:

- core chat 只定义 request/response/tool/middleware 原语；复杂 tool loop 涉及 return-direct、HITL park/resume、loop detection、round steering，已经是 agent runtime 策略。
- agent/toolloop 仍返回普通 `chat.CallMiddleware` / `chat.StreamMiddleware`，所以外层应用可组合它，但 core 不需要知道这些策略。
- lyra 的 `kernel/engine.go` 作为应用组合根直接装配 `toolloop.NewMiddleware(cfg)`，并把结果注入 agent guardrails。
...
| tool loop | tool loop 在 `agent/toolloop/`。core chat 只保留 request/response/tool/middleware 原语；lyra 作为应用组合根直接装配 `toolloop.NewMiddleware` 并注入 guardrails |
