...
>
> 对照 embabel：embabel 提供 `InMemoryAgentProcessRepository` / `InMemoryContextRepository` 这类参考实现 + Spring Boot starters；lynx 等价物是 BlackboardFactory extension capability + 用户自己的 backend 实现。**抽象层等价，开箱实现差异**。
...
| 层 | 默认实现 | 是否已有扩展点 |
|---|---|---|
| **Blackboard**（process 内的工件 / 条件 / 事件） | [`runtime.inMemoryBlackboard`](../runtime/in_memory_blackboard.go) | ✅ `core.BlackboardFactory` |
...
## 2. 实现 `core.BlackboardFactory`

### 2.1 接口契约

```go
type BlackboardFactory interface {
    core.Extension                   // 继承 Name() string

    NewBlackboard() core.Blackboard  // 每次 createProcess 调用一次
}
```

返回的 `core.Blackboard` 必须实现 [`core/blackboard.go`](../core/blackboard.go) 的全部读写方法：`Bind` / `Set` / `Get` / `GetValue` / `HasValue` / `Objects` / `Keys` / `SetCondition` / `GetCondition` / `Spawn` / `Clear` / `Protect` / `IsProtected`。
...
platform := agent.NewPlatform(runtime.PlatformConfig{
    Extensions: []core.Extension{
        myredis.NewBlackboardFactory(redisClient, ...),
    },
})
```

注册后，**所有** 通过 `Platform.RunAgent` / `Platform.StartAgent` / `Platform.CreateChildProcess` 创建的 process 都会调用 `factory.NewBlackboard()` 拿一份持久化 blackboard。`ProcessOptions.Blackboard` 仍可逐次覆盖。

### 2.3 子 process 的 Spawn 语义

`Platform.CreateChildProcess` 会从父 blackboard 调 `Spawn()` 生成子 blackboard。后端实现的 `Spawn()` 必须返回一个**新的、与父隔离的、共享父 snapshot 的** blackboard——典型做法是复制父的工件 map 后绑定新前缀；或者引用父 + 私有覆写层。
...
| 字段 | 说明 |
|---|---|
| `ProcessOptions.Blackboard` | 直接传 backend 实现；优先级最高（甚至高于 `BlackboardFactory`），适合 per-call 选择不同存储 |
...
如果用户的 BlackboardFactory 是持久化的，HITL 暂停期间的 awaitable 也要落到 blackboard——用户在 awaitable 的 `OnResponse` 里写"approval"到 blackboard，host 重启后只需重建 process（按 agent 配置 + bindings 重新 RunAgent），新 process 读到 blackboard 已有的 approval 就跳过 HITL。
...
| | embabel | lynx |
|---|---|---|
| Blackboard 抽象 | `Blackboard` 接口 + `BlackboardProvider` SPI | `core.Blackboard` 接口 + `core.BlackboardFactory` extension |
...
- ✅ `BlackboardFactory` capability — 已落地
