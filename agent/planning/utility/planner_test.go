...
func TestUtility_ExcludedActionsSkipped(t *testing.T) {
	...
	pl, err := utility.NewPlanner().PlanToGoal(t.Context(), start, domain, g,
...
func TestHybridUtility_SatisfiedFirstShortCircuit(t *testing.T) {
	...
	plH, err := utility.NewGoalFirst().PlanToGoal(t.Context(), start, domain, g, planning.Options{})
	if err != nil {
		t.Fatalf("goal-first PlanToGoal: %v", err)
	}
	...
	plU, err := utility.NewPlanner().PlanToGoal(t.Context(), start, domain, g, planning.Options{})
	if err != nil {
		t.Fatalf("utility PlanToGoal: %v", err)
	}
...
func TestHybridUtility_OneStepReachesGoal(t *testing.T) {
	...
	pl, err := utility.NewGoalFirst().PlanToGoal(t.Context(), start, domain, g, planning.Options{})
	if err != nil {
		t.Fatalf("PlanToGoal: %v", err)
	}
	if pl == nil || len(pl.Actions()) != 1 || pl.Actions()[0].Metadata().Name != "a" {
