...
func TestMissingProcessErrorsHaveStableIdentity(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "resume", run: func() error { return engine.Respond(t.Context(), "proc_missing", "susp_1", true) }},
	}
...
func TestAsyncEntryPointsReturnAdmissionErrorsSynchronously(t *testing.T) {
	...
	runHandle, err := engine.Start(t.Context(), nil, core.Bindings{}, core.ProcessOptions{})
	if runHandle != nil || err == nil {
		t.Fatalf("Start = %#v, %v; want nil run handle and error", runHandle, err)
	}
	runHandle, err = engine.ContinueAsync(t.Context(), "proc_missing")
	if runHandle != nil || !errors.Is(err, ErrProcessNotFound) {
		t.Fatalf("ContinueAsync = %#v, %v; want nil run handle and ErrProcessNotFound", runHandle, err)
	}
}
...
func TestContinuationAdmissionCannotCrossRegistryReservation(t *testing.T) {
	...
	if err := engine.Respond(t.Context(), process.ID(), "suspension", true); !errors.Is(err, ErrProcessNotFound) {
...
func TestResumeContextBoundsProcessTreeAdmission(t *testing.T) {
	...
	cancel()
	if err := engine.Respond(ctx, process.ID(), "suspension", true); !errors.Is(err, context.Canceled) {
		t.Fatalf("Resume = %v, want context cancellation", err)
	}
}

func TestStartClaimsRunOwnershipBeforeReturning(t *testing.T) {
	...
	runHandle, err := engine.Start(t.Context(), definition, core.Bindings{}, core.ProcessOptions{})
	if err != nil {
		t.Fatalf("Start: %v", err)
	}
	if started, err := runHandle.Process().state.beginRun(); started || !errors.Is(err, ErrProcessRunning) {
		t.Fatalf("second beginRun = (%v, %v), want false and ErrProcessRunning", started, err)
	}
	close(release)
	ctx, cancel := context.WithTimeout(t.Context(), time.Second)
	defer cancel()
	if _, err := runHandle.Await(ctx); err != nil {
...
func (*constructorExtension) ShouldStop(core.ProcessView) (bool, string) {
	return false, ""
}
