...
func TestDeployedDefinitionAccessorMutationDoesNotChangeRun(t *testing.T) {
	...
	process, err := engine.Run(
		t.Context(),
		source,
		map[string]any{core.DefaultBindingName: deploymentRunInput{Value: 21}},
		core.ProcessOptions{},
	)
	if err != nil {
		t.Fatal(err)
...
func TestRunCatalogsDefinitionForExactRestore(t *testing.T) {
	...
	process, err := engine.Run(
		t.Context(),
		source,
		map[string]any{core.DefaultBindingName: deploymentRunInput{Value: 20}},
		core.ProcessOptions{},
	)
	if err != nil {
		t.Fatal(err)
...
func TestAdvancedExecutionRejectsForeignDeployment(t *testing.T) {
	...
	parent, err := engine.createProcess(parentDef, nil, core.ProcessOptions{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestChildSpawnBindsCompiledDeploymentAndSessionIdentity(t *testing.T) {
	...
	parent, err := engine.createProcess(parentDef, nil, core.ProcessOptions{Session: &parentSession})
	if err != nil {
		t.Fatal(err)
	}
