...
// parked round on resume, write one on interrupt. Pass it to [Config.ParkStore];
// nil means the middleware falls back to [buildInterruptResponse] (the
// conversation-based tail path that the engine intercepts).
type ParkStore interface {
	// Consume atomically loads AND removes the parked round for a conversation,
	// returning (nil, nil) when nothing is parked. Read-and-remove is ONE
	// operation by design: a separate clear that failed after a successful read
	// would leave a stale round that the next turn — possibly a brand-new one on
	// the same conversation — re-injects, wrongly resuming onto a dead tail.
	// Atomic consumption makes resume idempotent.
	Consume(ctx context.Context, conversationID string) (*ParkState, error)

	// Write persists an interrupted round under the conversation id so a later
	// turn can Consume and resume it.
