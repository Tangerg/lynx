...
// ErrInvalidHistoryConfig reports a missing database-system identity.
var ErrInvalidHistoryConfig = errors.New("otel: invalid history config")

// HistoryConfig identifies the storage system observed by history
// instrumentation.
...
// HistoryMiddleware instruments chathistory capabilities without entering a
...
// NewHistory constructs history instrumentation for a composition root.
func NewHistory(config HistoryConfig) (*HistoryMiddleware, error) {
	system := strings.ToLower(strings.TrimSpace(config.System))
	if system == "" {
		return nil, fmt.Errorf("%w: system is required", ErrInvalidHistoryConfig)
	}
...
func (m *HistoryMiddleware) Store(next chathistory.Store) chathistory.Store {
...
func (m *HistoryMiddleware) Conversations(next chathistory.Lister) chathistory.Lister {
	if isNilCapability(next) {
		return nil
	}
...
func (m *HistoryMiddleware) Replace(next chathistory.Replacer) chathistory.Replacer {
	if isNilCapability(next) {
		return nil
	}
...
func (m *HistoryMiddleware) Count(next chathistory.Counter) chathistory.Counter {
	if isNilCapability(next) {
		return nil
	}
...
func (m *HistoryMiddleware) start(
	ctx context.Context,
	operation string,
