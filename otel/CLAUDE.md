...
- **按领域拆分的观测外挂**：`otel` 是无根包的集成层命名空间；`otel/agent`、`otel/chat`、`otel/tool`、`otel/history`、`otel/rag`、`otel/vectorstore` 分别包装自己领域的能力，`otel/slog` 只拥有开发态 exporter。领域模块不 import 本模块或官方 OTel。wrapper 是普通 decorator，不引入 `Observation`/`Tracer` 自定义接口。`a2a` / `mcp` 是协议 integration，在其协议调用边界直接使用官方 OTel，不在这里复制一层同义 wrapper。
...
- 要接生产后端不动这里,换组合根绑定的 exporter 即可。
