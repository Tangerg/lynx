...
- **按领域拆分的观测外挂**：`otel` 是无根包的集成层命名空间；`otel/agent`、`otel/chat`、`otel/tool`、`otel/history`、`otel/vectorstore` 分别包装自己领域的能力，`otel/slog` 只拥有开发态 exporter。领域模块不 import 本模块或官方 OTel。wrapper 是普通 decorator，不引入 `Observation`/`Tracer` 自定义接口。
