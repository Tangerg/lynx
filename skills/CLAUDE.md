...
- **覆盖以完整 bundle 为单位**:Merge 中高优先级 skill 获胜后,它的资源也只能来自同一 Source;不能从低优先级同名 skill 混入文件。
- **context 是 I/O 合同**:List / Load / OpenResource / ReadResource 在访问前后都观察取消;自定义 Source 也必须遵守。
- **坏 skill 不拖垮列表**:列表跳过缺少 `SKILL.md`、目录名非法或内容不合规范的项(与生态行为一致);权限、介质或远程 FS 等仓库访问故障必须返回,不能伪装成空列表。
- **懒读 per-call**:不缓存、不预扫,外部编辑即时可见;要缓存 / 预载是调用方的事。

## 模块特有反向不变量

- ❌ **import core / chat / 任何业务模块** —— 本模块是裸能力层,`tool.Tool` 适配封装在 `tools/skills`。
