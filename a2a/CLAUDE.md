...
- **根包放 A2A 周边 helper 与 chat tool adapter**:resolve AgentCard、打开 client、内容投影、server executor、把远端 agent 折成 `chat.Tool`。
...
- **远端 agent 只以 `[]chat.Tool` 暴露**:批量 resolve 是一个普通函数,生命周期由它返回的 close 函数表达 —— 不公开 SDK client、不做 Provider / cache / Registry。
