...
func TestNewRejectsNilStore(t *testing.T) {
	if _, err := chathistory.NewMiddleware(nil); !errors.Is(err, chathistory.ErrNilStore) {
		t.Fatalf("New(nil) error = %v", err)
	}
	var typedNil *recordingStore
	if _, err := chathistory.NewMiddleware(typedNil); !errors.Is(err, chathistory.ErrNilStore) {
...
func TestStreamWithoutConversationForwardsWithoutHistory(t *testing.T) {
	...
	if !errors.Is(gotErr, chathistory.ErrNilStream) {
