...
- **契约与通用适配，不是实现集合**：`Tool`、`WrappingTool`、`Capability`、`Func`、`NewFunc` 与 schema 派生属于本包；工具集合 Registry 属于复数 `tools` 包，shell、文件系统、HTTP、搜索、skill 等具体实现属于对应能力包。
- **生产依赖只到 Core**：除 `core/chat.ToolDefinition` 与标准库外，不依赖 Agent、MCP、A2A、具体工具或应用。
- **零全局状态**：运行时通过 `tools.Registry` 显式持有自己的工具集合。
