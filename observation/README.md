...
- **Users who want spans forwarded to their business log stream**:
  `go get github.com/Tangerg/lynx/otelbridge/slog` (or `/log`)
...
> the `otelbridge/<backend>` convention. They collide with the stdlib packages
...
models/           same
vectorstores/     same
otelbridge/       ← the only module that depends on go.opentelemetry.io/otel/sdk
    ├─ slog/
    └─ log/
```

Users who do not import any sub-package under `otelbridge/` never see the
OTel SDK dependency.
