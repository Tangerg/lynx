...
```text
core ------> chatclient/chathistory/embeddingclient/otel/documentreaders/*/
             vectorstores/tools/documentpipeline/models/rag/a2a/mcp/agent/app-runtime
...
embeddingclient -------------> vectorstores/app-runtime
...
| `vectorstores` | `core v1.0.0`、已发布的 `embeddingclient`/`pkg` |

`models` 发布前必须跑 provider constructor/protocol conformance；`vectorstores` 发布前必须跑 27 backend conformance。
...
`app/runtime` 最后更新到已发布的 `a2a`、`agent`、`chatclient`、`chathistory`、`core`、`embeddingclient`、`mcp`、`models`、`otel`、`pkg`、`skills`、`tokenizer` 和 `tools` 版本，关闭 workspace 后完成构建、测试与部署验证。它不应成为任何库 module 的依赖。
