...
| `core/model/embedding` 协议 | `core/embedding` | 使用单方法 `Model`；需要维度时由 `embeddingclient.Client.Dimensions` 发起显式探测 |
...
| `core/document/id` 与 formatter/transformer/batcher | `documentpipeline` module | Core `Document` 只保留数据与校验；消费方按需定义 Reader/Writer 小接口 |
...
`metadata.New` 已删除；`Map` 的零值就是可写空值，避免为了初始化引入只包装 `make` 的 API。
...
Core 不再定义没有实际消费方依赖的 `document.Reader/Writer`，也不再提供 `vectorstore.NewDocumentWriter`。需要编排 reader/writer 时，在消费模块按实际调用方法定义一方法接口；将 Indexer 接入 pipeline 时直接写一个局部函数 adapter，不把它提升为 Core 协议。
...
if err == nil {
    err = request.ValidateMatches(matches)
}
```

官方 VectorStore backend 已在成功出口执行同一校验；自定义 Searcher 也必须保证文档有效、分数有限且处于 `[0,1]`、不低于 `MinScore`、不超过 `TopK` 并按分数降序。
...
`Literal.AsNumber` 现在返回 `json.Number`，需要目标 SDK 的 `float64` 时由 adapter 显式调用 `Float64` 并承担精度策略；不要在 Core 公共出口先把大整数舍入。
...
Moderation 的 `Categories` 是开放的 `map[string]Verdict`，调用方按 provider 返回键读取，不能再访问 `categories.Hate` 等固定字段。`Flagged` 仍聚合全部键；自定义 Model 必须返回非空分类且每个 Score 有限并处于 `[0,1]`。
...
Embedding、Image 与 Moderation 的多结果响应使用 `response.First()` 取得首项；Image 的每个 `Result.Media` 都是标准 `media.Media`，provider 返回多张图片时不会再丢弃尾部结果。五个非 Chat Response 都必须通过 `Validate`。Speech 的音频容器字段为 `Options.OutputFormat`，生成结果字节为 `Result.Audio`；对应 JSON 字段是 `output_format` 与 `audio`，不再使用移植期的 `response_format` 与 `speech`。
...
`embeddingclient.Client` 只公开 `EmbedText`、`EmbedTexts`、`EmbedDocuments` 与显式探测用的 `Dimensions`，返回与 provider Response 不共享底层数组的向量。旧 Client 的 `Call` 和额外 `*embedding.Response` 返回值没有迁入新模块：需要 response metadata/usage 的调用方应直接构造 `embedding.Request` 并调用 `embedding.Model.Call`，避免两个调用入口产生语义漂移。
