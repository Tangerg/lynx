...
`core/model` 包本身仍存在，但只包含真正跨模态共享的 `Usage` 和 `RateLimit` 值。旧泛型 `Model`、Handler、Middleware、APIKey、catalog 和控制流类型均不存在。
...
`model.Usage` 只保留跨 provider 可比较的规范化计数。provider SDK 原始 usage 如需诊断，由 adapter 自己记录，或在明确命名并转为 JSON 后写入 response metadata；不能把 SDK 对象放回 Core Usage。
...
- [ ] `model.Usage` 不持有 provider 原始 SDK usage 对象。
