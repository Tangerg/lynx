...
| `core/model/embedding` | `core/embedding` | 使用单方法 `Model`；维度查询只依赖可选 `Dimensioner` 或显式 probe helper |
...
| `core/vectorstore/filter/{ast,lexer,parser,token,visitors}` | `core/vectorstore/filter` | 只使用稳定 Expr、构造函数和 `Parse`；解析实现已 internal 化 |
...
| `core/model.Usage` / `core/model.RateLimit` | `embedding.Usage` / 删除 | embedding 用量改用 `embedding.Usage{PromptTokens}`；chat 用量一直是 `chat.Usage`；RateLimit 零消费直接删除 |

`core/model` 包已整体删除。它声称的"跨模态共享 Usage/RateLimit"与事实不符：Usage 只被 embedding 消费且仅 `PromptTokens` 有填充者，RateLimit 零生产零消费。embedding 自有输入-only 的 `embedding.Usage{PromptTokens}`；chat 保留自己的 `chat.Usage`（input/output 词汇）；RateLimit 无替代（需要时由 provider adapter 按真实语义建模）。
...
type FilterDeleter interface { DeleteWhere(context.Context, filter.Expr) error }
```

消费方接口只组合实际调用的能力。搜索请求是普通值：

```go
expr, err := filter.Parse(`kind == "guide" AND year >= 2025`)
