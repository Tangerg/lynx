...
- Core 公共面从重构前 24 个 package、1,205 个导出标识符收敛为 11 个公共 package、320 行自动冻结的 exported API 快照。
...
| `core/document` | 可序列化 Document 值与校验；reader/writer 接口由真实消费方定义 |
| `core/embedding` | text-to-vector 协议与单方法 Model；维度探测属于 `embeddingclient` 工作流 |
...
| `core/vectorstore/filter` | 单一 metadata AST、Predicate/Selector、递归下降 Parse、校验、Visitor 契约、顺序组合入口与 Formatter |
...
- `ToolDefinition.Clone` 返回独立 schema 快照，Tool/Registry/Agent/MCP/A2A 不再各自维护浅拷贝约定。
...
- Embedding/Image/Moderation/Speech/Transcription 的 Options 扩展与响应 metadata 附加值统一使用 `metadata.Map`；Options 通过 `SetExtension` 写入 namespaced `Extensions`，metadata 继续通过 `Set` 写入 `Extra`，调用方必须传播编码错误。
- 删除所有 modality Request-level `Params` 参数袋；typed Options 之外的 provider JSON 仅能进入 `Options.Extensions`。
...
- provider 原生 options key 统一为 `<provider>/options`，删除 Spring 移植期 `lynx:ai:model:*` key；Core typed Options 只保留多 provider 共享且语义一致的字段。
...
- `SearchRequest.ValidateMatches` 验证成功输出的文档、分数范围、MinScore、TopK 与降序；25 个真实 backend 在返回前统一执行。
...
- Filter 使用单一 token-free AST；同包私有 scanner/token/递归下降 parser 直接构造 `Predicate`，不再维护 internal AST/转换层。`Validate` 由私有 analyzer visitor 承担，严格拒绝残缺、typed-nil、循环节点、非法 operator 与非精确整数索引；`Parse` 校验后通过私有无状态 optimizer 收敛双重否定、关联重复项、深层吸收与公共因子。优化保持输入不可变且不改写非逻辑叶子。公开 `Formatter` visitor 将树稳定输出为文本 DSL；provider compiler 与 inmemory interpreter 共享公开 `Visitor`，selector 始终保留完整 metadata 路径；`Visit(predicate, visitors...)` 统一校验并按传入顺序分派原树，首错即停。
- VectorStore adapter 的数字编译不再默认经 `float64`：Milvus 保留精确文本，Bedrock/Cassandra 保留 int64/uint64；Qdrant 对只接受整数的 match/IN 拒绝小数，Chroma/Pinecone/Redis/Weaviate 在目标 float32/float64 会改变整数值时明确报错。

### Other modalities

- `core/model/<modality>` 扁平为 `core/<modality>`。
- 删除各 modality 的 ClientCaller/Handler/Middleware/Chain/ModelMetadata framework。
- `core/embedding.Client` 移至独立 `embeddingclient` module；Core 只保留协议与单方法 `Model`。新 Client 只返回调用方实际使用的独立向量，并以 `Dimensions` 承担显式、无缓存探测，不复制原始 Response，也不提供与 `Model.Call` 重复的入口。
- Embedding 删除没有端到端闭环的多模态、编码格式、`Dimensioner` 与 Core probe helper。
...
- Image Response 保存 provider 返回的全部结果，每项直接复用 `media.Media`；Moderation Categories 保留开放的 provider 原始键，不冻结为单一 taxonomy。
- 五个非 Chat Response 均提供递归 `Validate`；构造器与官方 provider 成功出口遵守同一不变量。
...
- Request 不再有任意 `Params`；provider 扩展在 adapter 中通过 JSON-safe、namespaced 的 `Options.Extensions` 编解码。
- 请求边界拒绝带首尾空白的显式 model id、空的列表元素和非有限数值；VectorStore `MinScore` 拒绝 `NaN`。provider 无法表达的显式通用选项在 I/O 前返回稳定错误，不再静默丢弃。
...
- Filter `AtomicExpr`、`ComputedExpr`、`ExprBuilder` 与 precedence API；根输入改为 `Predicate`
- `metadata.New`、`document.Reader/Writer`、`vectorstore.NewDocumentWriter` 与 `AcceptAllScores`
- Embedding `EncodingFormat`、`ModalityType`、`Dimensioner`、`DimensionFunc` 与 Core 维度探测函数
- Image `Image`/`NewImage`、`ResponseFormat`、单数 `Response.Result`，以及 provider-only `Style`/`Quality` options
- Transcription 的 `Prompt`、`Temperature`、`ResponseFormat` 与 `TimestampGranularity` 通用 options
- Moderation 固定字段 `Categories` struct；当前使用保留 provider 原始键的开放 map
...
v1 wire 基线冻结 47 个带 JSON tag 的导出 struct，并用 17 个代表性 root 生成 478 行聚合 golden，覆盖 Metadata、Media、Chat、Document、Embedding、Image、Moderation、Speech、Transcription 与 VectorStore Search/Match。
...
- 320 行 exported API baseline 自动检测声明和方法签名增删。
- 47 项 wire DTO inventory 防止新 JSON struct 绕过 fixture review。
- AST 协议安全门禁拒绝任何可序列化公共 DTO 重新引入 `any`/`interface{}` 字段或 Request `Params` 参数袋。
- 11/11 公共 package docs 与 runnable examples 自动守卫。
- 当前 12 个 Core 生产 package 的逐包 coverage budget 全部不低于 P0 baseline；删除的旧 Filter internal 子包不再残留无效门禁项，新增协议面采用更高目标。
