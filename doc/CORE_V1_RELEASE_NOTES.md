...
- Core 公共面从重构前 24 个 package、1,205 个导出标识符收敛为 11 个公共 package、330 条自动冻结的 exported declaration/method signature。
...
| `core/vectorstore/filter` | 单一 metadata AST、Predicate/Selector、递归下降 Parse、校验与 Visitor 契约 |
...
- Filter 使用单一 token-free AST；同包私有 scanner/token/递归下降 parser 直接构造 `Predicate`，不再维护 internal AST/转换层或自动 optimizer。provider compiler 与 inmemory interpreter 共享公开 `Visitor`，selector 始终保留完整 metadata 路径。
...
- 330 条 exported API baseline 自动检测声明和方法签名增删。
