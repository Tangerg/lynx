...
> 状态：架构冻结；最终审查与全量发布门禁已通过，尚未创建 tag
...
- Core 公共面从重构前 24 个 package、1,205 个导出标识符收敛为 11 个公共 package、349 条自动冻结的 exported declaration/method signature。
...
- `core/model` 删除：其 Usage 实际只被 embedding 消费且仅 `PromptTokens` 有填充者，RateLimit 零生产零消费。embedding 自有输入-only 的 `Usage{PromptTokens}`；chat 保留自己的 `chat.Usage`（input/output 词汇）。没有跨模态共享的 usage 类型——各模态的用量形状本就不同构。
...
- 349 条 exported API baseline 自动检测声明和方法签名增删。
...
- 7 个 serialization/parser fuzz target 各独立运行 5 分钟，累计 609,846,214 次执行且无失败语料。
