...
- Filter 使用单一 token-free AST；同包私有 scanner/token/递归下降 parser 直接构造 `Predicate`，不再维护 internal AST/转换层。`Validate` 由私有 analyzer visitor 承担，`Parse` 校验后通过私有 optimizer 收敛双重否定、幂等与吸收结构；公开 `Formatter` visitor 将树稳定输出为文本 DSL。provider compiler 与 inmemory interpreter 共享公开 `Visitor`，selector 始终保留完整 metadata 路径；`Visit(predicate, visitors...)` 统一校验并按传入顺序分派原树，首错即停。
