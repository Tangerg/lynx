...
> 状态：Release Candidate；P7-07 最终架构审查通过后冻结
...
- Core 公共面从重构前 24 个 package、1,205 个导出标识符收敛为 12 个公共 package、367 条自动冻结的 exported declaration/method signature。
...
v1 wire 基线冻结 50 个带 JSON tag 的导出 struct，并用 19 个代表性 root 生成 535 行聚合 golden，覆盖 Metadata、Media、Chat、Document、Embedding、Image、Moderation、Speech、Transcription、Usage/RateLimit 与 VectorStore Search/Match。
...
- 367 条 exported API baseline 自动检测声明和方法签名增删。
