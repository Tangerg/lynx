...
- [x] **P14-01 收窄 ProcessStore 与 Snapshot 协议**（完成：2026-07-21）
  - ProcessSnapshot 升级 schema v4，删除 Revision/CAS/Mutation；ProcessStore 收敛为
    Save/Load/Delete，列表拆为可选 ProcessLister。
...
| **总计** | **完成** | **117/117（100%）** | **P0–P14 全部关闭；tag/release 仍是独立授权动作** |

### 15.2 当前焦点

- 当前阶段：P14 单机 Framework 边界收正，4/4，已关闭。
- 下一任务：无；tag/release 仍是独立授权动作，本批不创建。
- 当前决策门：已解除；按 BB-01 至 BB-08 直接迁移，不保留兼容层。
- 最近完成：移除 Snapshot CAS/Action retry/公开 Session sequencer，ProcessStore 最小化，
  App SQLite v15 adapter 与独立 module gate 收口。
...
- 状态：已接受并实现。
- 决策：Snapshot v4 删除 revision；ProcessStore 只表达保存完整树快照、读取和按根删除
  持久化树，列表拆为可选 `ProcessLister`。CAS、事务、幂等和分布式协调属于 adapter，
  不进入 Framework 合同。
