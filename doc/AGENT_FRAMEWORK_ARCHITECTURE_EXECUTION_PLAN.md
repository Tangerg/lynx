...
### P14：单机 Framework 边界收正

目标：让 Agent Framework 只整合单机领域能力，不替存储或 Host 声明原子性、幂等、分布式
owner 与可验证的副作用安全。

- [x] **P14-01 收窄 ProcessStore 与 Snapshot 协议**（完成：2026-07-21）
  - ProcessSnapshot 升级 schema v4，删除 Revision/CAS/Mutation 与 Framework 协调语义；
    列表拆为可选 ProcessLister。
- [x] **P14-02 删除 Framework 级 Action retry 与公开 Session sequencer**（完成：2026-07-21）
  - Action 每次调度只执行一次；同 Session FIFO 顺序器内收为 Engine 本机实现细节。
- [x] **P14-03 迁移 App SQLite adapter 与独立模块依赖**（完成：2026-07-21）
  - SQLite schema v15 投影 parent_id 并以递归 CTE 清理进程树；App 固定到 Agent
    `338f7a625` 对应 pseudo-version，无 workspace-only 编译路径。
- [x] **P14-04 同步契约、文档并执行完整门禁**（完成：2026-07-21）
  - API/wire/deployment golden 已审查刷新；Agent/App 全量 test/vet/tidy、Agent full race、
    App 高风险 race、App `GOWORK=off` 全量门禁与 diff check 全绿。

退出标准：Framework 公开面不再承诺底层原子性、幂等、分布式协调或 Action replay；本机
Process/Session 顺序保持确定；App adapter 独立实现并验证自己的 SQLite 策略。

### P15：纯能力抽象收口

目标：按单机 Framework 定位逐项删除仍由抽象层偷带的存储协调、重试标签、缓存策略和
具体 adapter，实现层通过最小接口明确承担自己的语义。

- [x] **P15-01 完整表达 Process 持久化变更**（完成：2026-07-21）
  - `ProcessStore.Apply` 一次接收 `ProcessSnapshotChange`；Framework 只表达完整树替换与
    旧树清理意图，App SQLite adapter 自己选择事务实现。
- [x] **P15-02 明确 Extension scope 与并发所有权**（完成：2026-07-21）
  - 构造边界拒绝无能力或作用域错误的 extension；实现自行保证并发安全、状态与生命周期，
    Runtime 不增加串行、重试或分布式协调。
- [x] **P15-03 删除 committed/retry 伪事务协议**（完成：2026-07-21）
  - 删除 `ErrCommitted`、`CommittedError` 与 `Commit`；交互错误保留原始 cause，Action
    每次调度一次的语义直接由 executor 保证。
- [x] **P15-04 删除 ToolGroup 具体策略实现**（完成：2026-07-21）
...
退出标准：Agent 生产代码不 import 存储 backend/`chathistory`，不提供 extension adapter 的
...
### P24：Waiting child checkpoint settlement

- [x] **P24-01 ToolLoop 支持 host-settled pause**（完成：2026-07-30）
  - `Checkpoint.CompletePausedCall` 以不可变变换结算任意 paused call，不执行工具、不推进
    publication；`AwaitingInput` 区分外部输入边界与 ready continuation。
...
- [x] **P39-01 删除测试专用生产入口**（完成：2026-08-05）
  - 删除仅测试调用的 `deploymentCompiler.compile`；测试在同 package 的 `_test.go` 中自行组合
    snapshot、definition validation 与 compileSnapshot，生产 deployment 只保留 Engine 真实路径。
  - 删除仅测试读取的 `processState.runActive`；并发所有权测试改用真实 `beginRun` 冲突行为证明
    `Start` 返回前已完成 run ownership admission，不为断言暴露第二条状态读取路径。
- [x] **P39-02 测试术语与失败语义收正**（完成：2026-08-05）
  - process-tree sequencer 测试、key 与错误文案统一使用 tree，不再残留 Session 术语；AgentTool
    测试名、注释与 JSON 反例同步真实 API，不再沿用 AsChatTool/subagent/legacy 迁移语言。
  - AgentTool 测试不再丢弃构造与 JSON 编码错误；child event 测试删除历史修复叙述，只陈述当前
    subtree listener 合同；plan value 测试注释删除迁移历史，只锁定当前评分语义。
- [x] **P39-03 反证复核与完整门禁**（完成：2026-08-05）
  - 复核 `StreamCall` 一跳边界后确认其隔离公开无名返回签名与内部 panic 恢复所需命名返回值，
    因而保留，不以“减少函数数目”为由制造无收益 API 变化。
  - Agent public API 与 wire baseline hash 均不变；空目录/空文件、TODO/FIXME/HACK/XXX、宿主术语、
    生产静态 `fmt.Errorf`、test-only production entry 与 targeted ignored-error 扫描收口。
  - Agent standalone 全量 build/vet/test/staticcheck/golangci-lint/tidy、core/planning/runtime/arch race、
    examples 与 diff check 全绿。
...
- 状态：已接受并实现。
- 决策：删除 QoS、`RetryPolicy` 与 `RetrySafety`；每次调度只调用 Action 一次。provider、tool
  或业务写入的 retry 由真正理解副作用的实现自行处理。
