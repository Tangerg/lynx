...
- snapshot v1 的 status 使用稳定字符串；`LLMInvocation`、`EmbeddingInvocation`、`Session` 全部增加显式 snake_case JSON tag；不再依赖枚举整数和 encoding/json 默认大写字段名。
...
type ProcessControl interface {
    ...
    TerminateToolCall()
    Suspend(context.Context, Suspension) ActionStatus
}

type UsageRecorder interface {
    RecordUsage(context.Context, float64, int)
    RecordLLMInvocation(context.Context, LLMInvocation)
    RecordEmbeddingInvocation(context.Context, EmbeddingInvocation)
}
```
...
| **总计** | **完成** | **90/90（100%）** | **P0–P9 全部关闭；发布动作仍需单独授权** |

### 15.2 当前焦点

- 当前阶段：P9 Go API 语义化收口已完成，15/15。
- 下一任务：无；后续改动按 API/wire/SemVer 治理另行立项。
- 当前决策门：已解除；按 BB-01 至 BB-08 直接迁移，不保留兼容层。
- 最近完成：P9-15 完整 Agent/App gate；公开 API 为 16 package / 626 declaration / root 46，wire 为 14 struct / 456 行；未执行 git 或发布动作。
