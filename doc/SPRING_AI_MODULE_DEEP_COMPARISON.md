...
| **vectorstores** | **lynx 架构领先** | 组合+泛型 dispatch、参数化防注入、ISP、零运行时依赖手写 Pratt parser | `IS NULL`/`NIN`/日期归一化、按 id 删除、SQL 数值类型细分 |
...
| **chatmemory** | **lynx 领先** | **序列化保真度**（完整 round-trip Parts/tool）、排序键正确性、ctx 取消、OTel span | 关系型 SQL dialect 广度、`findConversationIds()`、dialect 自动探测 |
...
| IN | ✅ | ✅ |
| **NIN（NOT IN）** | ❌（靠 `not(x in ...)` 组合） | ✅ 一等公民 + `expandNin` fallback |
| **IS NULL / IS NOT NULL** | ❌ | ✅ 一等公民 |
| **LIKE** | ✅ 一等公民 | ❌ |
| **索引/嵌套路径** | ✅ `metadata['a']['b']`、`arr[0]` | ⚠️ 仅单层 `a.b` |
| 日期 | ❌ | ✅ ISO-8601 自动转 Date |
| 数值类型 | NUMBER 统一 float64 | Long/Integer/Decimal 细分 |

**Spring 更"数据库化"**（NIN/IS NULL/日期/Long），**lynx 更"通用过滤化"**（LIKE + 任意深度索引）。
...
- **spring-ai 反超**：`IS NULL`/`IS NOT NULL` 一等公民（lynx 无法表达"字段缺失"）；`NIN` + 德摩根下推 + `expandIn/Nin` 代数 fallback；日期归一化；按 id 删除；数值类型细分；AND/OR 多别名。
- **取舍而非高下**：backend 广度持平；DSL 表达力是不同子集（lynx 偏通用过滤，Spring 偏 SQL WHERE）。**最高优先级 gap**：补 `IS NULL`/`IS NOT NULL`（inmemory `lookupField` 已返 nil、语义现成）。
...
关键差异：(1) Spring `findConversationIds()` 能枚举所有会话——lynx 完全按 conversationID 分区、从不跨会话扫描（**真实 gap**：Spring 能列举/后台清理）。(2) Spring `saveAll` 是**全量替换**（先 delete 再 batch insert），lynx `Write` 是 **append**。
...
- **spring-ai 反超**：关系型 SQL 广度（单 jdbc + dialect 覆盖 8 库）；`findConversationIds()` 可枚举会话；dialect 自动探测 + builder 开箱省事。
...
5. **协议/DSL 完整度**：MCP resources/prompts/complete 全覆盖；filter DSL 的 IS NULL/NIN/日期归一化。
...
| **P1** | filter `IS NULL`/`IS NOT NULL` + vectorstore 按 id 删除 | vectorstores | 常见过滤/删除形态表达不了 | 小-中 |
| **P2** | chatmemory `findConversationIds()` + 关系型 dialect 广度 | chatmemory | 运维列举/批量清理 | 中 |
