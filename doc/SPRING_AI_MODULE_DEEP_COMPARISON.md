...
| **vectorstores** | **lynx 架构领先** | 组合+泛型 dispatch、参数化防注入、ISP、零运行时依赖手写 Pratt parser、`IS NULL`（✅ 已补） | `NIN`/日期归一化、按 id 删除、SQL 数值类型细分 |
...
最该补的 5 项：(1) ~~OTel metrics（token usage counter）~~ ✅ 已闭合（2026-05-29）；(2) ~~embedding token-aware batching~~ —— 非缺口，已由 document 层 `TokenCountBatcher` 覆盖；(3) ~~Document 自动 id + chunk 溯源 metadata~~ ✅ 已闭合；(4) ~~vectorstore filter `IS NULL`/`IS NOT NULL`~~ ✅ 已补（语言层 + 10 backend）—— vectorstore **按 id 删除**仍可补；(5) ~~chatmemory `findConversationIds()`~~ ✅ 已补（`memory.Lister`，6 backend + core）—— 关系型 dialect 广度仍可补。其余多为 spring-ai 的成熟度细节或 lynx 有意识的 YAGNI 取舍。
...
- **spring-ai 反超**：~~`IS NULL`/`IS NOT NULL`~~ ✅ **已补齐（2026-05-29）** —— 新增 `is`+`null` 两个 keyword token，`field IS NULL` 复用 `BinaryExpr{IS, field, NULL}`、`field IS NOT NULL` 复用 `UnaryExpr{NOT, …}`（每个 backend 的 NOT handler 免费给出 NOT NULL）；语言层 + inmemory 参考语义 + 10 个 backend（pgvector/cockroachdb/supabase/mariadb/oracle/tidb/clickhouse/couchbase/mongodb/neo4j）落地，其余 backend（redis/cassandra/pinecone/chroma/milvus/qdrant/weaviate/es/opensearch/… 部分 vector-native 或 IS NULL 不可表达）按既有惯例优雅拒绝（"unsupported operator"，与"redis 不支持 IN"同机制）。`NIN` + 德摩根下推 + `expandIn/Nin` 代数 fallback；日期归一化；按 id 删除；数值类型细分；AND/OR 多别名仍是 Spring 领先。
...
| **P1** | vectorstore 按 id 删除 | core/vectorstore | 动 exported `DeleteRequest`，需先报方案 | 小-中 |
