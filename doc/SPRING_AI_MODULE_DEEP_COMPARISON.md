...
最该补的 5 项：(1) ~~OTel metrics（token usage counter）~~ ✅ 已闭合（2026-05-29）；(2) ~~embedding token-aware batching~~ —— 非缺口，已由 document 层 `TokenCountBatcher` 覆盖；(3) ~~Document 自动 id + chunk 溯源 metadata~~ ✅ 已闭合；(4) ~~vectorstore filter `IS NULL`/`IS NOT NULL`~~ ✅ 已补（语言层 + 10 backend）—— ~~vectorstore 按 id 删除~~ ✅ 已补（`IDDeleter`，20 backend）；(5) ~~chatmemory `findConversationIds()`~~ ✅ 已补（`memory.Lister`，6 backend + core）。

> **2026-05-29 收口**：P0/P1 的可补 gap 已全部落地。剩余两项经评估**故意不做**（见 §12.4）：chatmemory 关系型 SQL dialect 广度（postgres 已覆盖主流 + `lyra/internal/storage/sqlite` 已有 SQLite；为多 dialect 引入 N 个近重复模块或破坏"一 backend 一 go.mod"拓扑，不划算）；filter 日期归一化（与各 backend 日期存储强耦合、使用频率低，对元数据过滤 DSL 属 YAGNI）。
...
| K | chatmemory 关系型多 SQL dialect（MySQL/SQLite/Oracle/...）| postgres 覆盖主流 + `lyra/.../storage/sqlite` 已有 SQLite；多 dialect 要么加 N 个近重复 `go.mod`、要么破坏"一 backend 一 driver"拓扑把所有 SQL driver 打包进来，不划算 |
| L | filter 日期字符串归一化（ISO-8601 → Date）| 与各 backend 日期存储/类型强耦合、使用频率低；元数据过滤 DSL 用字符串比较已够，属 YAGNI |
