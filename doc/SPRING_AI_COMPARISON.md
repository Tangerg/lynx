...
> Spring AI 代码参考：`spring-projects/spring-ai` main 分支。
> - **初版基线**（2026-04-17）：1.0 GA 后
> - **2026-04-20 复核基线**：`2.0.0-SNAPSHOT`（见 §12 新增生态）
> Lynx 代码参考：`/Users/tangerg/Desktop/lynx`（2026-04-20 HEAD = `63e4bb2`）。

> **2026-04-20 修订摘要**：
> - §3「ToolCallingManager vs ToolMiddleware」：Lynx 侧 commit `8e58479` 已把 ToolMiddleware 从硬编码改为 opt-in，本节倒退点已部分消解（仍缺 `ToolExecutionMode` 枚举与 `ToolInvoker` 注入点）。
> - §9 清单 #8「Document.Builder + IdGenerator 集成」：上游 Spring AI 2.0 已集成（`Document.java:300-306, 379`），结论改为「Lynx 自己仍缺，参照上游补即可」。
> - 新增 §12：Spring AI 2.0 带来的两个生态级变化（**MCP 全家桶** + **ChatMemory 六种持久化后端**），原文档未覆盖。
...
### 3.2 Lynx：opt-in 中间件（修订）

> **2026-04-20 复核**：commit `8e58479` 已把自动注入去掉，`core/model/chat/client.go:294-298, 354-359` 现在的注释明确声明「Tool execution is NOT injected automatically; register ToolMiddleware explicitly via WithMiddlewares」。

现状：
```go
// core/model/chat/tool_middleware.go:15
func NewToolMiddleware() (CallMiddleware, StreamMiddleware) { ... }
// 用法
client.WithMiddlewares(NewToolMiddleware())
```

- ✅ 用户可替换 ToolMiddleware 实现（自定义并行 / retry / 超时 / 拒绝执行）
- ✅ 用户可控制它在中间件链中的位置
- ✅ 可完全不挂（等效 Spring 的 `isInternalToolExecutionEnabled = false`）
- ⚠️ **仍缺**：`ToolExecutionMode` 枚举 + `ToolInvoker` 注入点——当前只有「挂 / 不挂」二元，Spring 则提供「internal / external / custom」三态 + 可替换 invoker

**剩余倒退面**：比 Spring 的 `ToolCallingManager` 少一层「可配置工具解析与执行策略」的 API。用户要做自定义编排，目前必须重写整个 CallMiddleware/StreamMiddleware。

### 3.3 剩余必要补齐

「硬编码注入」已于 `8e58479` 解决。剩下的差距是「执行模式可切换」+「invoker 可替换」：

```go
// 1. 执行模式枚举（对应 ToolCallingChatOptions.isInternalToolExecutionEnabled 的三态泛化）
type ToolExecutionMode int
const (
    ToolExecutionInternal ToolExecutionMode = iota  // 默认：进程内递归
    ToolExecutionExternal                            // 返回 raw tool calls，由上层处理
    ToolExecutionCustom                              // 调用用户提供的 ToolInvoker
)

// 2. 可注入的 invoker（对应 Spring 的 ToolCallingManager）
...
这 10 行改动打开 agentic 场景（外部执行、MCP 服务器转发、自定义并行策略等）。
...
### 8.1 🟡 ToolCallingManager 能力仍不完整（§3，2026-04-20 部分修复）
**现状**：commit `8e58479` 把硬编码改为 opt-in，但仍无 `ToolExecutionMode` 三态开关 / `ToolInvoker` 注入点。
**代价**：只能「挂 / 不挂」，不能 external execution 也不能换执行策略。
**修复**：~10 行补齐 Mode + Invoker。
...
| 8 | `Document.Builder.idGenerator(IdGenerator)` 集成 | `Document.java:300-306, 379` 🆕 上游 2.0 已实装 | S | 去除「ID 必须调用方生成」的负担（Lynx 仍缺） |
...
## 11. Spring AI 2.0 新生态（2026-04-20 新增）

本次复核在 `/Users/tangerg/Desktop/spring-ai` 上（版本：`2.0.0-SNAPSHOT`，见 `spring-ai-model/pom.xml:24`）发现了三处上游已落地、原文档未覆盖的生态变化。Lynx 对标设计需要回填。

### 11.1 🔴 MCP 全家桶（最重要的新生态）

Spring AI 2.0 把 MCP 从「协议适配」升级成「第一方 client + server 框架」：

```
spring-ai/
├── mcp/
│   ├── mcp-annotations/                     # 注解驱动的 tool/resource/prompt 定义
│   └── transport/
│       ├── mcp-spring-webmvc/               # HTTP/SSE 传输
...
**关键设计点**：
- **MCP Annotations 模块**把 `@Tool / @McpResource / @McpPrompt` 这类注解下沉为独立制品，可被外部项目复用
- **Transport 分层**：HTTP (webmvc) 与响应式 (webflux) 并行，都挂 Spring Boot starter
- **Client & Server 对称设计**：两侧都有对应的 autoconfig + starter，开箱即用

**Lynx 对标策略**：

| 层面 | Spring AI 2.0 | Lynx 规划 |
|-----|--------------|----------|
| 协议定义 | `mcp-annotations`（AspectJ/注解） | **没有**；doc/agent/README.md 规划 `agents/mcp/` 但未开工 |
| 传输层 | `mcp-spring-webmvc` + `mcp-spring-webflux` | **没有** |
| Client 注入 | `@EnableMcpClient` + autoconfig | **没有** |
| Server 暴露 | `@McpServer` + autoconfig | **没有** |

**建议**：
1. `agents/mcp/` 走独立顶层 module（和 `vectorstores/` 一个级别），内分 `client/` + `server/` + `transport/http/`。
2. 不仿照 Spring 的 `@Tool` 注解——Go 走 `mcp.Register(name, fn)` DSL。
3. Transport 用 `net/http` + `golang.org/x/exp/events` 做 SSE，不引入额外响应式框架。
4. ToolGroup 概念（见 `doc/agent/` 中的 embabel 0.4 新抽象）天然匹配 MCP server 的能力聚合——可以打通。

### 11.2 🟠 ChatMemory 六种持久化后端

Spring AI 2.0 新增六个 repository module，把 ChatMemory 从单一 in-memory 实现扩展成多后端矩阵：
...
**Lynx 现状**：`core/model/chat/memory/` 只有 `MessageWindowStore`（in-memory），无任何持久化实现。

**Lynx 对标策略**：
- 若要补，走 `memories/` 顶层 module，子包 `redis/`、`postgres/`、`mongo/` 各自挂各自 SDK 依赖
- **不要**把后端实现塞进 `core/`，按现有「重依赖走外挂」原则
- `memory.Store = Reader + Writer + Clearer` 这套 ISP 接口（Lynx 的 🏆 胜点）对接后端天然友好，不需要额外抽象

### 11.3 🟢 `BaseAdvisor` 抬升为具名接口

上游细节：`spring-ai-client-chat/src/main/java/org/springframework/ai/chat/client/advisor/api/BaseAdvisor.java:42`

```java
public interface BaseAdvisor extends CallAdvisor, StreamAdvisor {
    // before(request) / after(request, response) 模板方法
    // 默认 adviseCall / adviseStream 用 before/after 拼
}
```

- 被 `RetrievalAugmentationAdvisor` 采用为父接口
- 和 Lynx 规划中的 `AroundMiddleware`（见 `MIDDLEWARE_DESIGN.md`）心智模型完全一致，命名可以直接类比

**Lynx 影响**：不改变原提案，但给 `MIDDLEWARE_DESIGN.md §7「与 Spring AI 对照」`增加一条强参照——既然 Spring 自己也在把「高层模板」具名化，Lynx 实现 `AroundMiddleware` 就更有路径依据。

### 11.4 其他上游变化（概略）

| 概念 | 2.0 中的变化 | Lynx 影响 |
|-----|-----------|---------|
| `ToolExecutionResult` | 从隐式 Map 升级为富接口（`conversationHistory()` / `returnDirect()` / `buildGenerations()`） | `SPRING_AI_COMPARISON §3` 描述需刷新 |
| `Document.Builder.idGenerator(...)` | 已集成，默认 `RandomIdGenerator` | §9 清单 #8 从「缺失」改为「上游已有、Lynx 仍缺」✅ 已修订 |
| `QueryRouter` / `DocumentSelector` / `DocumentRanker` | 仍未出现 | §4 仍然成立 |
| `isNull` / `isNotNull` 算子 | 仍不在 `Filters.g4` | §5 / §9 清单 #7 仍然成立 |
| `ObservationRegistry` | 仍是 Micrometer，无 OTel 迁移 | 反证 `OBSERVABILITY_DESIGN.md` 放弃仿 Spring、直接用 OTel 的决策是对的 |
