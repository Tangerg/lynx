...
| 后处理 | backward + forward planning optimization | 同（`backwardOptimize` + `forwardOptimize` 在 reconstruct 之后顺序跑） |
| 可达性预检 | `isGoalReachable()` 反向扫描候选 actions | 同（`goalReachable` 一步反向扫；不满足直接 nil 短路，避免 A\* 烧 maxIter） |

planner trace span 记录 `plan_length_raw`（A\* 出来的）和 `plan_length`（后处理之后的）两个值，方便观察后处理裁掉了多少冗余 action。
...
| Subtree 预算 | `cost() = ownCost + childProcesses.sum { it.cost }` 递归汇总 | 同 — `AgentProcess.Usage()` 递归走 children；`MaxActionsPolicy` / `BudgetPolicy` 自动是子树作用域 |

子树预算汇总两边语义一致：parent agent 调用 child agent 时，child 的 cost / tokens / actions 自动汇入 parent 的预算上限。lynx 的对外 API 是 `pc.RecordUsage(cost, tokens)` — 集成层（监听 `LLMResponseEvent` 的 listener、wrap chat client 的 adapter）负责往里报数，框架本身不发明数字。
...
~~1. **Backward + forward planning optimization**（A\* 后处理）~~ ✅ 已完成
~~2. **子进程预算汇总**（递归累计 cost / tokens）~~ ✅ 已完成
~~3. **goal reachability 预检**（在跑 A\* 前快速判 goal 是否可达，省掉 worst-case 搜索）~~ ✅ 已完成

（清单已清空 — 这一节保留作为历史记录，方便回看曾经的差距。）
...
7. **事件原生 JSON 友好** — 21 个 event 类型全部实现 `MarshalJSON`，自带 `event` 字段做 discriminator；接口字段（Action / WorldState / Awaitable / error）自动 collapse 成 wire-friendly 摘要。embabel 用 Jackson `@JsonTypeInfo` 才有同等效果。
