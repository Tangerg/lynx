...
ResultMetadata, NewOptions, NewRequest, NewResponse, NewResult
```
...
```text
Model, ModelFunc, Options, Request, Response, ResponseMetadata, Result,
ResultMetadata, Usage, NewOptions, NewRequest, NewResponse, NewResult
```

`Model` 只含 `Call`。旧 `ClientRequest`、
`ClientCaller`、handler/middleware/chain、`ModelMetadata`、`GetDimensions` 和
全局维度 cache 已物理删除；P9 又删除无端到端闭环的 `Dimensioner`、探测 helper、
多模态与编码格式表面，维度探测由 `embeddingclient.Client.Dimensions` 拥有。
`ResultMetadata` 只保留 JSON-safe Extra。所有真实 provider/vectorstore/runtime
消费点已切换，未保留 alias、bridge 或 deprecated wrapper。
...
`Options.OutputFormat` 是普通图片 MIME 字符串；`Options.Merged` 使用标准库解析并
规范化，拒绝非 `image/*`、带参数或语法错误的值。P9 删除重复的 Image URL/base64
DTO 与只服务少数 provider 的 Style/Quality/ResponseFormat，结果直接复用
`media.Media` 并以复数 `Response.Results` 保留全部图片。
...
Result, ResultMetadata, Verdict, NewOptions, NewRequest,
NewResponse, NewResult
```

P9 将 `Categories` 从封闭的固定字段 struct 收敛为开放 `map[string]Verdict`，provider
原始分类键不会因 Core taxonomy 丢失；`Flagged` 聚合全部键，构造器严格校验有限
`[0,1]` 分数。
...
```text
DefaultTopK, MaxSimilarityScore, MinSimilarityScore, ErrEmptyDocuments,
ErrMissingFilter, FilterDeleter, IDDeleter, Indexer, Match, Searcher,
SearchRequest
```

`Match` 只包含 `Document *document.Document` 与 `Score float64`。`Document.Score` 已删除，RAG 在自身边界使用 `Candidate` 并显式完成映射。旧 `Store`、`Creator`、`Retriever`、`Deleter`、request wrapper、metadata/native client 探测面均已删除；P9 继续删除无消费者的 `NewDocumentWriter` 与重复常量。能力由四个单方法接口独立表达，`SearchRequest` 以 `Validate` 校验输入、以 `ValidateMatches` 校验成功输出。
...
| `embedding.Model` | ~~Call、Dimensions、DefaultOptions、Metadata~~（P5-01/P9 已删除复合与伪能力） | 当前只含 `Call`；维度探测由 `embeddingclient` 消费工作流拥有 |
