...
- 每次调度的 Action 只执行一次；retry 由真正拥有副作用语义的 provider/tool/业务实现处理。
...
- 相同 Session 的并发 turn 由 Engine 内部 FIFO 协调器稳定排序，取消 waiter 会从队列移除；
...
- Framework 定位为可嵌入的单机能力整合层，不提供分布式执行 owner 或存储并发协议。
