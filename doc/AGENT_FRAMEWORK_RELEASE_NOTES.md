...
|---|---:|
| public package | 17 |
| exported declaration | 632 |
| root façade | 47 / 50 |
| exported JSON struct | 14 |
| wire fixture | 456 行 |

- API baseline SHA-256：`886b20bf3714fc898daf794ad726c01d279aa68bf0074736c2208e7bd1588966`
...
当前基线包含 `ChildOptionsFunc` / `ProcessOptions.ChildOptions` 与
`interaction.Limits.MaxModelCalls` 两项 additive API；wire contract 未变化。正式版本
仍必须在内部依赖改为精确 tag 后，以 `GOWORK=off` 复验 module DAG、完整门禁、干净
consumer 和数据迁移。本轮开发 commit 已获授权；tag、数据库迁移或 release 仍需要
维护者另行授权。
