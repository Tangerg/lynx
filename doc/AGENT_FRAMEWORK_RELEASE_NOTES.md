...
|---|---:|
| public package | 17 |
| exported declaration | 633 |
| root façade | 47 / 50 |
| exported JSON struct | 14 |
| wire fixture | 456 行 |

- API baseline SHA-256：`96399e00c1df1d624c9007b299a56e732460a21ffd3bfb80f4fcbfe88656fc3d`
...
当前基线包含 `ChildOptionsFunc` / `ProcessOptions.ChildOptions`、
`interaction.Limits.MaxModelCalls` 与 `ProcessSnapshotFailed.MarshalJSON` 三项
additive API。durable wire fixture 未变化；`ProcessSnapshotFailed` 的观测事件 JSON
已从 Go 默认 struct 编码纠正为统一 event envelope。正式版本仍必须在内部依赖改为
精确 tag 后，以 `GOWORK=off` 复验 module DAG、完整门禁、干净 consumer 和数据迁移。
本轮开发 commit 已获授权；tag、数据库迁移或 release 仍需要维护者另行授权。
