...
func New(store Store, ids IDs, signals *Signals, events Publisher) (*Service, error) {
	if store == nil || ids == nil || signals == nil || events == nil {
		return nil, errors.New("goalflow: store, ids, signals and event publisher are required")
	}
...
func (service *Service) Start(ctx context.Context, request protocol.StartGoalRequest) (*protocol.Goal, error) {
	...
	} else if !errors.Is(err, goaldomain.ErrNotFound) {
		return nil, err
	}
...
func (service *Service) Update(ctx context.Context, request protocol.UpdateGoalRequest) (*protocol.Goal, error) {
	...
	if err := value.ReplaceObjective(request.Objective, incarnation, service.now()); err != nil {
		return nil, fmt.Errorf("%w: goal cannot be edited", protocol.ErrInvalidParams)
	}
	value, err = service.store.SaveGoal(ctx, value, expectedIncarnation, expectedRevision)
	if err != nil {
		return nil, service.project(err)
	}
	service.changed(request.SessionID)
	return Present(value), nil
}
...
func (service *Service) Stop(ctx context.Context, request protocol.GoalRequest) (*protocol.Goal, error) {
	...
	expectedIncarnation, expectedRevision := value.IncarnationID(), value.Revision()
	if err := value.Pause(goaldomain.ReasonStoppedByUser, "", service.now()); err != nil {
...
func (service *Service) Resume(ctx context.Context, request protocol.GoalRequest) (*protocol.Goal, error) {
	...
	expectedIncarnation, expectedRevision := value.IncarnationID(), value.Revision()
	if err := value.Resume(service.now()); err != nil {
		return nil, fmt.Errorf("%w: goal is not resumable or its budget is exhausted", protocol.ErrInvalidParams)
	}
	value, err = service.store.SaveGoal(ctx, value, expectedIncarnation, expectedRevision)
	if err != nil {
		return nil, service.project(err)
...
func (service *Service) Clear(ctx context.Context, request protocol.GoalRequest) error {
	...
	if err := service.store.RemoveGoal(ctx, value.SessionID(), value.IncarnationID(), value.Revision()); err != nil {
...
func (service *Service) Current(ctx context.Context, sessionID string) (goaldomain.Goal, bool, error) {
	...
	if errors.Is(err, goaldomain.ErrNotFound) {
		return goaldomain.Goal{}, false, nil
	}
...
func (service *Service) continueRun(ctx context.Context, runID string) (goaldomain.Goal, error) {
	...
	if err != nil {
		return goaldomain.Goal{}, err
	}
	expectedRevision := value.Revision()
	if err := value.ContinueRun(runID, service.now()); err != nil {
		return goaldomain.Goal{}, err
	}
...
func (service *Service) Report(ctx context.Context, sessionID, runID, outcome, reason string) (string, error) {
	value, err := service.store.LoadGoal(ctx, sessionID)
	if errors.Is(err, goaldomain.ErrNotFound) {
		return "No active Goal exists for this session.", nil
	}
	if err != nil {
		return "", err
	}
	if value.ActiveRunID() != runID {
		return "This Run does not own the current Goal; no outcome was reported.", nil
	}
	...
	if err := value.Report(expectedIncarnation, completed, reason, service.now()); err != nil {
		if outcome == "blocked" && strings.TrimSpace(reason) == "" {
	...
	if _, err := service.store.SaveGoal(ctx, value, expectedIncarnation, expectedRevision); err != nil {
		if errors.Is(err, goaldomain.ErrVersionConflict) {
			return "The Goal changed concurrently; inspect it before reporting an outcome.", nil
		}
		return "", err
	}
	service.changed(sessionID)
	if completed {
		return "Goal outcome reported as completed. The loop will stop after this Run settles.", nil
	}
...
func (service *Service) claimRun(ctx context.Context, sessionID, incarnationID, runID string) (goaldomain.Goal, error) {
	...
	if value.IncarnationID() != incarnationID {
		return goaldomain.Goal{}, goaldomain.ErrVersionConflict
	}
	expectedRevision := value.Revision()
	if err := value.ClaimRun(runID, service.now()); err != nil {
		return goaldomain.Goal{}, err
	}
...
func (service *Service) awaitInput(ctx context.Context, value goaldomain.Goal) (goaldomain.Goal, error) {
	expectedRevision := value.Revision()
	if err := value.AwaitInput(value.ActiveRunID(), service.now()); err != nil {
		return goaldomain.Goal{}, err
	}
	stored, err := service.store.SaveGoal(ctx, value, value.IncarnationID(), expectedRevision)
	if err == nil {
...
func (service *Service) pause(ctx context.Context, value goaldomain.Goal, code goaldomain.ReasonCode, detail string) (goaldomain.Goal, error) {
	expectedRevision := value.Revision()
	if err := value.Pause(code, detail, service.now()); err != nil {
		return goaldomain.Goal{}, err
	}
	stored, err := service.store.SaveGoal(ctx, value, value.IncarnationID(), expectedRevision)
	if err == nil {
...
func (service *Service) settleRun(ctx context.Context, value goaldomain.Goal, runID, outcome string, usage protocol.RunMetrics) (bool, error) {
	cost := 0.0
	if usage.Usage != nil && usage.Usage.CostUSD != nil {
	...
	if remove {
		if err := service.store.RemoveGoal(ctx, value.SessionID(), value.IncarnationID(), expectedRevision); err != nil {
			return false, err
		}
		service.changed(value.SessionID())
		return true, nil
	}
	if _, err := service.store.SaveGoal(ctx, value, value.IncarnationID(), expectedRevision); err != nil {
		return false, err
	}
	service.changed(value.SessionID())
	return false, nil
...
func (service *Service) recoverWithoutRun(ctx context.Context, value goaldomain.Goal) (goaldomain.Goal, error) {
	expectedRevision := value.Revision()
	if err := value.RecoverWithoutRun(service.now()); err != nil {
		return goaldomain.Goal{}, err
	}
	stored, err := service.store.SaveGoal(ctx, value, value.IncarnationID(), expectedRevision)
	if err == nil {
...
func (service *Service) load(ctx context.Context, sessionID string) (goaldomain.Goal, error) {
	value, err := service.store.LoadGoal(ctx, sessionID)
	if errors.Is(err, goaldomain.ErrNotFound) {
		return goaldomain.Goal{}, fmt.Errorf("%w: no goal for session", protocol.ErrInvalidParams)
	}
	return value, err
}
...
func Present(value goaldomain.Goal) *protocol.Goal {
	...
	if reason.Code != goaldomain.ReasonNone {
		result.Reason = &protocol.GoalReason{Code: protocol.GoalReasonCode(reason.Code), Detail: reason.Detail}
	}
	return result
}
