...
func (driver *Driver) Recover(ctx context.Context) error {
	...
	if err != nil { return fmt.Errorf("goalflow: list goals for recovery: %w", err) }
	for _, value := range values {
		if err := driver.recoverOne(ctx, value); err != nil {
			if errors.Is(err, goaldomain.ErrVersionConflict) { continue }
...
func (driver *Driver) Start() {
	driver.mu.Lock()
	if driver.started || driver.closing { driver.mu.Unlock(); return }
	driver.started = true
	driver.tasks.Add(1)
	driver.mu.Unlock()
	go driver.loop()
...
func (driver *Driver) loop() {
	...
	for {
		...
		case <-driver.signals.Wake():
			for _, sessionID := range driver.signals.Drain() { driver.reconcile(sessionID) }
...
func (driver *Driver) reconcile(sessionID string) {
	...
	if err != nil { driver.retry(sessionID); return }
	watchOwnedRun := false
	if found && value.ActiveRunID() != "" && (value.Status() == goaldomain.Completing ||
		...
		if runErr != nil { driver.retry(sessionID); return }
	...
	if driver.suppressed[sessionID] {
		...
		if existing != nil { cancel = existing.cancel; delete(driver.drives, sessionID) }
		driver.mu.Unlock()
	...
	if keep { driver.mu.Unlock(); return }
	var cancel context.CancelFunc
	var oldRunID string
	settlePaused := false
	if existing != nil {
		cancel, oldRunID = existing.cancel, existing.runID
		if oldRunID == "" && found && existing.incarnationID == value.IncarnationID() { oldRunID = value.ActiveRunID() }
	...
	if oldRunID != "" {
		...
		if settlePaused { driver.settlePausedRun(sessionID, value.IncarnationID(), oldRunID) }
	}
	if shouldStart { driver.startDrive(value) }
}
...
func (driver *Driver) startDrive(value goaldomain.Goal) {
	...
	go func() {
		...
		defer func() {
			driver.mu.Lock()
			if driver.drives[value.SessionID()] == state { delete(driver.drives, value.SessionID()) }
			driver.mu.Unlock()
...
func (driver *Driver) driveGoal(ctx context.Context, state *drive, sessionID, incarnationID string) {
	for ctx.Err() == nil {
		...
		if err != nil || !found || value.IncarnationID() != incarnationID { return }
		if value.ActiveRunID() == "" {
			if value.Status() != goaldomain.Active { return }
			if err := driver.runs.WaitSessionStartable(ctx, sessionID); err != nil { return }
			opened, err := driver.runs.StartAutonomous(ctx, runflow.AutonomousStart{
				...
				Claim: func(claimCtx context.Context, runID string) error {
					if !driver.prepareClaim(sessionID, state, runID) { return context.Canceled }
					_, err := driver.goals.claimRun(claimCtx, sessionID, incarnationID, runID)
					return err
				},
			})
			if err != nil {
				if errors.Is(err, protocol.ErrSessionHasActiveRun) { continue }
				if ctx.Err() != nil || errors.Is(err, goaldomain.ErrVersionConflict) || errors.Is(err, goaldomain.ErrInvalidTransition) || errors.Is(err, goaldomain.ErrNotFound) { return }
			...
			if err := driver.waitRunSettlement(ctx, value.ActiveRunID()); err != nil { return }
		}
		if ctx.Err() != nil { return }
		if !driver.settleCurrent(ctx, sessionID, incarnationID) { return }
	}
}
...
func (driver *Driver) prepareClaim(sessionID string, state *drive, runID string) bool {
	...
	if driver.closing || driver.suppressed[sessionID] || driver.drives[sessionID] != state { return false }
	state.runID = runID
	return true
}
...
func (driver *Driver) waitRunSettlement(ctx context.Context, runID string) error {
	...
	for {
		...
		if err != nil { return err }
		if current.Status != protocol.RunStatusRunning { return nil }
...
func (driver *Driver) settleCurrent(ctx context.Context, sessionID, incarnationID string) bool {
	value, found, err := driver.goals.Current(ctx, sessionID)
	if err != nil || !found || value.IncarnationID() != incarnationID || value.ActiveRunID() == "" { return false }
	current, err := driver.runs.Get(ctx, value.ActiveRunID())
	if err != nil { return false }
	switch current.Status {
	case protocol.RunStatusWaiting:
		if value.Status() == goaldomain.Active { _, _ = driver.goals.awaitInput(ctx, value) }
		return false
	case protocol.RunStatusFinished:
		if current.Outcome == nil { return false }
		removed, err := driver.goals.settleRun(ctx, value, current.ID, string(current.Outcome.Type), current.Metrics)
		if err != nil || removed { return false }
...
func (driver *Driver) ObserveResumed(ctx context.Context, runID string) error {
	...
	if errors.Is(err, goaldomain.ErrNotFound) { return nil }
	if err != nil { return err }
	driver.mu.Lock()
	suppressed := driver.closing || driver.suppressed[value.SessionID()]
	driver.mu.Unlock()
	if suppressed { return errors.New("goalflow: session lifecycle is changing") }
...
func (driver *Driver) CancelDetached(runID string) {
	if runID != "" { driver.cancelRun(runID, "Goal cleared") }
}
...
func (driver *Driver) SuppressSession(ctx context.Context, sessionID string) (bool, error) {
	...
	stateRunID := ""
	if state != nil { state.cancel(); stateRunID = state.runID; delete(driver.drives, sessionID) }
	driver.mu.Unlock()
	value, found, err := driver.goals.Current(ctx, sessionID)
	if err != nil { driver.ReleaseSession(sessionID, false); return false, err }
	runID := stateRunID
	if found && value.ActiveRunID() != "" { runID = value.ActiveRunID() }
	if runID != "" { driver.cancelRun(runID, "Session lifecycle changed") }
	return found, nil
}
...
func (driver *Driver) ReleaseSession(sessionID string, changed bool) {
	...
	driver.mu.Unlock()
	if closing { return }
	if changed { driver.goals.changed(sessionID); return }
	driver.signals.Publish(sessionID)
}
...
func (driver *Driver) pauseStartFailure(ctx context.Context, sessionID, incarnationID string, cause error) {
	value, found, err := driver.goals.Current(ctx, sessionID)
	if err != nil || !found || value.IncarnationID() != incarnationID || value.Status() != goaldomain.Active || value.ActiveRunID() != "" { return }
...
func (driver *Driver) cancelRun(runID, reason string) {
	...
	if errors.Is(err, protocol.ErrRunFinished) || errors.Is(err, protocol.ErrRunNotFound) { return }
}
...
func (driver *Driver) settlePausedRun(sessionID, incarnationID, runID string) {
	...
	defer cancel()
	value, found, err := driver.goals.Current(ctx, sessionID)
	if err != nil || !found || value.IncarnationID() != incarnationID || value.Status() != goaldomain.Paused || value.ActiveRunID() != runID { return }
	current, err := driver.runs.Get(ctx, runID)
	if err != nil || current.Status != protocol.RunStatusFinished || current.Outcome == nil { return }
	_, _ = driver.goals.settleRun(ctx, value, runID, string(current.Outcome.Type), current.Metrics)
}

func (driver *Driver) Close() {
	driver.mu.Lock()
	if driver.closing { driver.mu.Unlock(); return }
	driver.closing = true
	driver.cancel()
	runIDs := make([]string, 0, len(driver.drives))
	for _, value := range driver.drives {
		value.cancel()
		if value.runID != "" { runIDs = append(runIDs, value.runID) }
	}
	driver.mu.Unlock()
	...
	for _, goal := range goals {
		if goal.Status() == goaldomain.Active { _, _ = driver.goals.pause(ctx, goal, goaldomain.ReasonRuntimeRestarted, "") }
		if goal.ActiveRunID() != "" { runIDs = append(runIDs, goal.ActiveRunID()) }
	}
	seen := make(map[string]bool, len(runIDs))
	for _, runID := range runIDs { if !seen[runID] { seen[runID] = true; driver.cancelRun(runID, "Runtime shutting down") } }
	for _, goal := range goals {
		if goal.ActiveRunID() == "" { continue }
