...
func newLSPTool(service *codeintel.Service, paths lspPathResolver) (toolcontract.Tool, error) {
	if service == nil || paths == nil {
		return nil, errors.New("agenttools: code intelligence service and path resolver are required")
	}
	return toolcontract.NewFunc(toolcontract.FuncConfig{Name: "lsp", Description: "Query a workspace language server. Position operations use path plus 1-based line and character; document_symbols and diagnostics use path; workspace_symbols uses query."}, func(ctx context.Context, request lspRequest) (string, error) {
		...
		switch {
		case position && (strings.TrimSpace(request.Path) == "" || request.Line < 1 || request.Character < 1):
			return "", errors.New("lsp: position operations require path, line, and character")
		case (request.Operation == "document_symbols" || request.Operation == "diagnostics") && strings.TrimSpace(request.Path) == "":
			return "", errors.New("lsp: operation requires path")
		case request.Operation == "workspace_symbols" && strings.TrimSpace(request.Query) == "":
			return "", errors.New("lsp: workspace_symbols requires query")
		}
		if request.Path != "" {
			resolved, err := paths.AbsolutePath(request.Path)
			if err != nil {
				return "", err
			}
		...
		if errors.Is(err, codeintel.ErrUnsupported) || errors.Is(err, codeintel.ErrUnavailable) {
			return "No language server is available for that file type or workspace.", nil
		}
		return result, err
	})
}
