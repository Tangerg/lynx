...
func newShellTools(service *shellflow.Service, sessionID, workspace string) ([]toolcontract.Tool, error) {
	...
	shell, err := toolcontract.NewFunc(toolcontract.FuncConfig{
		...
		if request.AutoBackgroundAfterSeconds > 0 {
			after = time.Duration(request.AutoBackgroundAfterSeconds) * time.Second
		}
	...
	read, err := toolcontract.NewFunc(toolcontract.FuncConfig{
		...
		if request.Wait {
			if err := service.Await(ctx, sessionID, request.ShellID, time.Duration(request.TimeoutMillis)*time.Millisecond); err != nil {
				if errors.Is(err, shellflow.ErrNotFound) {
					return "No such background shell.", nil
				}
				return "", err
			}
		}
		...
		prefix := ""
		if snapshot.Dropped {
			prefix = "[earlier output dropped]\n"
		}
		state := "still running"
		if snapshot.Finished {
			state = fmt.Sprintf("finished (exit %d)", snapshot.ExitCode)
		}
	...
	stop, err := toolcontract.NewFunc(toolcontract.FuncConfig{
		...
		if running {
			return "Background shell stop requested.", nil
		}
