...
func (catalog *Catalog) goalTools(ctx context.Context, scope agentexec.ToolScope) ([]scopedTool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("agenttools: create Goal tool: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("agenttools: get Goal tool: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("agenttools: inspect Goal ownership: %w", err)
	}
	if !owned {
		return values, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("agenttools: report Goal tool: %w", err)
	}
...
func encodeGoalToolResult(value *protocol.Goal) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("agenttools: encode Goal result: %w", err)
	}
