...
func (catalog *Catalog) planTools(scope agentexec.ToolScope) ([]scopedTool, error) {
	enter, err := toolcontract.NewFunc(toolcontract.FuncConfig{
		...
		if !changed {
			return "This session is already in Plan mode. Continue investigating or update the Plan with set_plan.", nil
		}
		return "Plan mode entered. Investigate with read-only tools and maintain the complete proposal with set_plan.", nil
	})
	if err != nil {
		return nil, fmt.Errorf("agenttools: enter Plan mode tool: %w", err)
	}
	...
	set, err := toolcontract.NewFunc(toolcontract.FuncConfig{
		...
		for index, step := range request.Steps {
			steps[index] = protocol.PlanStep{Description: step.Description, Status: protocol.PlanStatus(step.Status)}
		}
	...
	if err != nil {
		return nil, fmt.Errorf("agenttools: set Plan tool: %w", err)
	}
	...
	return toolcontract.NewFunc(toolcontract.FuncConfig{
		...
		if continuation, resumed := agentexec.ToolInputContinuationFromContext(ctx); resumed {
			...
			if value.Revision != state.Revision {
				return "", errors.New("exit_plan_mode: Plan changed after the approval request; review it again")
			}
		...
		if len(value.Steps) == 0 {
			return "", errors.New("exit_plan_mode: current Plan is empty; call set_plan first")
		}
...
func (tool *planModeGate) Call(ctx context.Context, arguments string) (string, error) {
	...
	if active {
		return "", errors.New("tool blocked by this session's read-only Plan mode")
	}
