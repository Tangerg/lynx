...
func New(lifetime context.Context, specs []ServerSpec) (*Service, error) {
	...
	for index, spec := range specs {
		if strings.TrimSpace(spec.Name) != spec.Name || strings.TrimSpace(spec.Command) != spec.Command || strings.TrimSpace(spec.LanguageID) != spec.LanguageID || spec.Name == "" || spec.Command == "" || spec.LanguageID == "" {
			return nil, errors.New("codeintel: every server needs a trimmed name, command, and language id")
		}
		if len(spec.Extensions) == 0 {
			return nil, fmt.Errorf("codeintel: server %q needs at least one extension", spec.Name)
		}
		if _, duplicate := seen[spec.Name]; duplicate {
			return nil, fmt.Errorf("codeintel: duplicate server %q", spec.Name)
		}
		...
		for extensionIndex, extension := range spec.Extensions {
			...
			if len(extension) < 2 || !strings.HasPrefix(extension, ".") || strings.ContainsAny(extension, `/\\`) {
				return nil, fmt.Errorf("codeintel: invalid extension %q", extension)
			}
			if _, duplicate := byExtension[extension]; duplicate {
				return nil, fmt.Errorf("codeintel: extension %q has multiple owners", extension)
			}
		...
		for markerIndex, marker := range spec.RootMarkers {
			...
			if marker == "." || filepath.IsAbs(marker) || marker == ".." || strings.HasPrefix(marker, ".."+string(filepath.Separator)) {
				return nil, fmt.Errorf("codeintel: invalid root marker %q", marker)
			}
...
func (service *Service) server(ctx context.Context, root string, spec ServerSpec) (*client, error) {
	...
	if value := service.clients[key]; value != nil {
		service.mu.Unlock()
		return value, nil
	}
	if pending := service.starting[key]; pending != nil {
		service.mu.Unlock()
		select {
		case <-pending.done:
	...
	launchContext, cancel := context.WithCancel(service.ctx)
	pending := &launch{done: make(chan struct{}), cancel: cancel}
	service.starting[key] = pending
	service.mu.Unlock()
...
func (service *Service) launchClient(launchContext context.Context, key, root string, spec ServerSpec, pending *launch) {
	value, err := startClient(launchContext, root, spec)
	pending.cancel()
	service.mu.Lock()
	closed := service.closed
	if !closed {
		delete(service.starting, key)
		if err == nil {
			service.clients[key] = value
			pending.client = value
			service.watchers.Add(1)
			go service.watchClient(key, value)
		} else {
			pending.err = err
		}
		close(pending.done)
...
func startClient(lifetime context.Context, root string, spec ServerSpec) (*client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("codeintel: open %s stdout: %w", spec.Name, err)
	}
	stdin, err := command.StdinPipe()
	if err != nil {
		_ = stdout.Close()
		return nil, fmt.Errorf("codeintel: open %s stdin: %w", spec.Name, err)
	}
	if err := command.Start(); err != nil {
		_ = stdout.Close()
		_ = stdin.Close()
		if errors.Is(err, exec.ErrNotFound) {
			return nil, fmt.Errorf("%w: %s", ErrUnavailable, spec.Name)
		}
		return nil, fmt.Errorf("codeintel: start %s: %w", spec.Name, err)
	}
	...
	if err := value.connection.Call(initializeContext, "initialize", params, &initialized); err != nil {
		_ = value.close()
		return nil, fmt.Errorf("codeintel: initialize %s: %w", spec.Name, err)
	}
	if err := value.connection.Notify(initializeContext, "initialized", struct{}{}); err != nil {
		_ = value.close()
		return nil, fmt.Errorf("codeintel: acknowledge %s initialization: %w", spec.Name, err)
	}
	return value, nil
}
...
func (value *client) Handle(ctx context.Context, connection *jsonrpc2.Conn, request *jsonrpc2.Request) {
	...
	case "textDocument/publishDiagnostics":
		...
		if signal := value.updates[params.URI]; signal != nil {
			close(signal)
		}
	...
	case "workspace/configuration":
		if !request.Notif {
			...
			_ = connection.Reply(ctx, request.ID, make([]any, len(params.Items)))
		}
	default:
		if !request.Notif {
			_ = connection.Reply(ctx, request.ID, nil)
		}
	}
}
...
func (value *client) sync(ctx context.Context, path string) (string, <-chan struct{}, bool, error) {
	...
	if err != nil {
		return "", nil, false, fmt.Errorf("codeintel: read %s: %w", path, err)
	}
	uri := pathURI(path)
	...
	signal := value.updates[uri]
	if signal == nil {
		signal = make(chan struct{})
		value.updates[uri] = signal
	}
	if opened && previous.Digest == digest {
		return uri, signal, false, nil
	}
	...
	if notifyErr != nil {
		return "", nil, false, fmt.Errorf("codeintel: synchronize %s: %w", path, notifyErr)
	}
...
func readDocument(path string) ([]byte, error) {
	...
	if len(contents) > maxDocumentBytes {
		return nil, errors.New("language-server document exceeds the 8 MiB limit")
	}
	return contents, nil
}
...
func (value *client) close() error {
	value.closeOnce.Do(func() {
		...
		defer cancel()
		_ = value.connection.Call(ctx, "shutdown", nil, nil)
		_ = value.connection.Notify(ctx, "exit", nil)
		value.cancel()
		_ = value.connection.Close()
		select {
		case <-value.exited:
			value.mu.Lock()
			value.closeErr = value.waitErr
			value.mu.Unlock()
		case <-ctx.Done():
			_ = killCommand(value.command)
			<-value.exited
		}
	})
...
func (service *Service) Close() error {
	service.once.Do(func() {
		...
		service.clients = make(map[string]*client)
		starting := make([]*launch, 0, len(service.starting))
		for _, pending := range service.starting {
			starting = append(starting, pending)
		}
		service.mu.Unlock()
		for _, pending := range starting {
			pending.cancel()
		}
		...
		service.cancel()
		service.closeErr = errors.Join(errs...)
	})
