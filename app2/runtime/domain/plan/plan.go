...
func NewBoundary(steps []Step) (Boundary, error) {
	...
	for index := range normalized { normalized[index].Description = strings.TrimSpace(normalized[index].Description) }
	if err := validateSteps(normalized); err != nil { return Boundary{}, err }
...
func New(sessionID string) (State, error) {
	...
	if err := value.Validate(); err != nil { return State{}, err }
	return value, nil
}
...
func (value State) Replace(steps []Step, now time.Time) (State, error) {
	if value.revision == math.MaxUint64 { return State{}, fmt.Errorf("%w: revision overflow", ErrInvalid) }
	normalized := cloneSteps(steps)
	for index := range normalized { normalized[index].Description = strings.TrimSpace(normalized[index].Description) }
	next := State{sessionID: value.sessionID, steps: normalized, revision: value.revision + 1, updatedAt: now.UTC()}
	if !value.updatedAt.IsZero() && next.updatedAt.Before(value.updatedAt) { return State{}, fmt.Errorf("%w: replacement time moved backwards", ErrInvalid) }
	if err := next.Validate(); err != nil { return State{}, err }
	return next, nil
}

func (value State) Validate() error {
	if value.sessionID == "" { return fmt.Errorf("%w: session identity is required", ErrInvalid) }
	if value.revision == 0 {
		if len(value.steps) != 0 || !value.updatedAt.IsZero() { return fmt.Errorf("%w: unwritten Plan must be empty", ErrInvalid) }
		return nil
	}
	if value.updatedAt.IsZero() { return fmt.Errorf("%w: committed Plan needs an update time", ErrInvalid) }
...
func validateSteps(steps []Step) error {
	...
	for index, step := range steps {
		if strings.TrimSpace(step.Description) == "" || !step.Status.Valid() { return fmt.Errorf("%w: invalid step %d", ErrInvalid, index) }
		if step.Status == InProgress { inProgress++ }
	}
	if inProgress > 1 { return fmt.Errorf("%w: at most one step may be in_progress", ErrInvalid) }
	return nil
}
