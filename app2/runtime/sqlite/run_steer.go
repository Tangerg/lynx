...
func (database *Database) CommitSteer(ctx context.Context, write runflow.SteerWrite) error {
	...
	if changed != 1 { return rundomain.ErrInvalidTransition }
	if err := insertItem(ctx, transaction, write.Item); err != nil { return err }
	if err := insertConversationMessage(ctx, transaction, write.Message); err != nil { return err }
	if err := insertRunEvents(ctx, transaction, []rundomain.EventRecord{write.Event}); err != nil { return err }
	if err := transaction.Commit(); err != nil { return fmt.Errorf("sqlite: commit steer: %w", err) }
	return nil
}
