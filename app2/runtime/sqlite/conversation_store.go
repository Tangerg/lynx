...
func insertConversationMessage(ctx context.Context, transaction *sql.Tx, record conversation.Record) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: append conversation message %d: %w", record.Ordinal, err)
	}
	return nil
}
...
func (database *Database) ListConversationMessages(ctx context.Context, sessionID string) ([]conversation.Record, error) {
	...
	if through >= 0 {
		compacted.Ordinal = through
		values = append(values, compacted)
	}
	for rows.Next() {
		var value conversation.Record
		if err := rows.Scan(&value.SessionID, &value.RunID, &value.Ordinal, &value.Body); err != nil {
			return nil, err
		}
