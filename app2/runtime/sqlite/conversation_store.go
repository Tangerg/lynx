...
func (database *Database) ListConversationMessages(ctx context.Context, sessionID string) ([]conversation.Record, error) {
	rows, err := database.database.QueryContext(ctx, `SELECT session_id,run_id,ordinal,body FROM conversation_messages WHERE session_id=? ORDER BY ordinal`, sessionID)
