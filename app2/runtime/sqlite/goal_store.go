...
func (database *Database) CreateGoal(ctx context.Context, value goaldomain.Goal) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: create goal: %w", err)
	}
	...
	if changed != 1 {
		return goaldomain.ErrVersionConflict
	}
	return nil
}
...
func (database *Database) SaveGoal(ctx context.Context, value goaldomain.Goal, expectedIncarnation string, expectedRevision uint64) (goaldomain.Goal, error) {
	...
	if err != nil {
		return goaldomain.Goal{}, fmt.Errorf("sqlite: save goal: %w", err)
	}
	changed, err := result.RowsAffected()
	if err != nil {
		return goaldomain.Goal{}, err
...
func (database *Database) RemoveGoal(ctx context.Context, sessionID, incarnation string, revision uint64) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite: remove goal: %w", err)
	}
	changed, err := result.RowsAffected()
	if err != nil {
		return err
...
func scanGoal(row goalScanner) (goaldomain.Goal, error) {
	...
	if err := json.Unmarshal([]byte(encoded), &body); err != nil {
		return goaldomain.Goal{}, fmt.Errorf("sqlite: decode goal: %w", err)
	}
...
func encodeGoalBody(value goaldomain.Goal) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("sqlite: encode goal: %w", err)
	}
