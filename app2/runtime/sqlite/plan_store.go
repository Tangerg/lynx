...
type planBody struct { Steps []planStepBody `json:"steps"` }
...
func (database *Database) LoadPlan(ctx context.Context, sessionID string) (plandomain.State, error) {
	...
	if !errors.Is(err, plandomain.ErrNotFound) { return value, err }
	var exists int
	...
	} else if lookupErr != nil { return plandomain.State{}, lookupErr }
...
func savePlanCAS(ctx context.Context, execer planExecer, value plandomain.State, expectedRevision uint64) error {
	...
	if err != nil { return fmt.Errorf("sqlite: save Plan: %w", err) }
	changed, err := result.RowsAffected()
	if err != nil { return err }
	if changed != 1 { return plandomain.ErrVersionConflict }
	return nil
}
...
func (database *Database) EnterPlanMode(ctx context.Context, sessionID string, now time.Time) (bool, error) {
	...
	if err != nil { return false, fmt.Errorf("sqlite: enter Plan mode: %w", err) }
	changed, err := result.RowsAffected()
...
func (database *Database) ExitPlanMode(ctx context.Context, sessionID string) (bool, error) {
	...
	if err != nil { return false, fmt.Errorf("sqlite: exit Plan mode: %w", err) }
	changed, err := result.RowsAffected()
	return changed == 1, err
}
...
func scanPlan(row planScanner) (plandomain.State, error) {
	...
	if err := row.Scan(&sessionID, &revision, &body, &updated); errors.Is(err, sql.ErrNoRows) {
		return plandomain.State{}, plandomain.ErrNotFound
	} else if err != nil { return plandomain.State{}, fmt.Errorf("sqlite: scan Plan: %w", err) }
	var stored planBody
	if err := json.Unmarshal([]byte(body), &stored); err != nil { return plandomain.State{}, fmt.Errorf("sqlite: decode Plan: %w", err) }
	steps := planSteps(stored)
	updatedAt, err := decodeTime(updated)
	if err != nil { return plandomain.State{}, err }
...
func decodePlanBoundary(body string) (plandomain.Boundary, error) {
	var stored planBody
	if err := json.Unmarshal([]byte(body), &stored); err != nil { return plandomain.Boundary{}, fmt.Errorf("sqlite: decode Plan boundary: %w", err) }
	value, err := plandomain.NewBoundary(planSteps(stored))
	if err != nil { return plandomain.Boundary{}, fmt.Errorf("sqlite: invalid Plan boundary: %w", err) }
	return value, nil
}

func planSteps(stored planBody) []plandomain.Step {
	steps := make([]plandomain.Step, len(stored.Steps))
	for index, step := range stored.Steps { steps[index] = plandomain.Step{Description: step.Description, Status: plandomain.Status(step.Status)} }
	return steps
}

func emptyPlanBody() (string, error) {
	body, err := encodePlanSteps(nil)
	if err != nil { return "", fmt.Errorf("sqlite: encode empty Plan boundary: %w", err) }
	return body, nil
}
...
func encodePlanSteps(steps []plandomain.Step) (string, error) {
	...
	for index, step := range steps { stored[index] = planStepBody{Description: step.Description, Status: string(step.Status)} }
	body, err := json.Marshal(planBody{Steps: stored})
	if err != nil { return "", fmt.Errorf("sqlite: encode Plan: %w", err) }
