...
export function HookSettings(props: HookSettingsProps) {
  ...
  const hooks = useQuery({
    queryKey: key,
    queryFn: ({ signal }) =>
      listHooks(props.connection, props.workspace ?? { path: "" }, signal),
  ...
  const trust = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({
        queryKey: runtimeQueryKeys.hooks(props.connection),
      });
    },
  });
  const projectHooks =
    hooks.data?.hooks.filter((hook) => hook.scope === "project") ?? [];
  ...
  if (props.workspace === undefined) {
    return (
      ...
      >
        <header>
          <div>
            <h2 id="hook-settings-title">{t("settings.hook.effective")}</h2>
            <p>{t("settings.hook.effectiveDetail")}</p>
          </div>
        </header>
      ...
      <section className="settings-section" aria-labelledby="hook-trust-title">
        ...
        {hooks.isPending ? (
          ...
          <HookState
            ...
            onAction={() => void hooks.refetch()}
          >
            {messageOf(hooks.error, t)}
          </HookState>
        ) : (
          <div
            className="hook-trust-card"
            data-trusted={hooks.data.projectTrusted || undefined}
          >
            <div>
              <span className="hook-trust-status">
                {hooks.data.projectTrusted
                  ? t("settings.hook.trusted")
                  : t("settings.hook.notTrusted")}
              </span>
              <strong>
                {t(
                  projectHooks.length === 1
                    ? "settings.hook.projectCountOne"
                    : "settings.hook.projectCountMany",
                  { count: formatNumber(projectHooks.length) },
                )}
              </strong>
              <code title={hooks.data.projectRoot}>
                {hooks.data.projectRoot || t("settings.hook.noProjectRoot")}
              </code>
            </div>
            {hooks.data.projectRoot ? (
              hooks.data.projectTrusted ? (
                <button
                  ...
                  onClick={() => trust.mutate(false)}
                >
                  {trust.isPending
                    ? t("settings.hook.revoking")
                    : t("settings.hook.revoke")}
                </button>
                ...
                <div className="hook-trust-confirm">
                  ...
                  <button
                    type="button"
                    disabled={trust.isPending}
                    onClick={() => setConfirmingTrust(false)}
                  >
                  ...
                  <button
                    ...
                    type="button"
                    disabled={trust.isPending}
                    onClick={() => trust.mutate(true)}
                  >
                    {trust.isPending
                      ? t("settings.hook.trusting")
                      : t("settings.hook.trustProject")}
                  </button>
                </div>
              ) : (
                <button
                  ...
                  type="button"
                  onClick={() => setConfirmingTrust(true)}
                >
                  {t("settings.hook.reviewAndTrust")}
                </button>
              )
            ) : null}
            {trust.isError ? (
              <p className="settings-inline-error" role="alert">
                {messageOf(trust.error, t)}
              </p>
            ) : null}
          </div>
        )}
        ...
        <header>
          ...
          {hooks.data ? (
            <span className="hook-count">
              {t(
                hooks.data.hooks.length === 1
                  ? "settings.hook.countOne"
                  : "settings.hook.countMany",
                { count: formatNumber(hooks.data.hooks.length) },
              )}
            </span>
          ) : null}
        </header>
...
function HookCard(props: { hook: HookInfo }) {
  ...
  const actionKind = props.hook.command
    ? t("settings.hook.command")
    : t("settings.hook.inject");
  return (
    <article className="hook-card" data-active={props.hook.active || undefined}>
      <header>
        ...
        <span className="hook-active-state">
          {props.hook.active
            ? t("settings.hook.active")
            : t("settings.hook.inert")}
        </span>
      </header>
      <div className="hook-action">
        <span>
          {actionKind}
          {props.hook.timeoutMillis
            ? t("settings.hook.timeout", {
                count: formatNumber(props.hook.timeoutMillis),
              })
            : ""}
        </span>
        <pre>{action}</pre>
...
function messageOf(error: unknown, t: Translate) {
  if (error instanceof MissingProjectRootError)
    return t("settings.hook.missingProjectRoot");
