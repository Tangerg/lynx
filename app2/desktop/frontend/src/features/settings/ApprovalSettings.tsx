	...
	return (
		<>
			<section className="settings-section" aria-labelledby="approval-mode-title">
				<header>
					<div>
						<h2 id="approval-mode-title">{t("settings.approval.effectStance")}</h2>
				...
				{mode.isPending ? (
					...
					<ApprovalState action={t("settings.approval.tryAgain")} onAction={() => void mode.refetch()}>
					...
					<div className="approval-mode-grid">
						{modes.map((candidate) => {
							...
							return (
								...
								>
									<span><strong>{t(candidate.name)}</strong><small>{t(candidate.badge)}</small></span>
				...
				{changeMode.isError ? <p className="settings-inline-error" role="alert">{messageOf(changeMode.error, t)}</p> : null}
			</section>

			<section className="settings-section" aria-labelledby="approval-rules-title">
				<header>
					<div>
						<h2 id="approval-rules-title">{t("settings.approval.remembered")}</h2>
						<p>{t("settings.approval.rememberedDetail")}</p>
					</div>
					{rules.data ? <span className="approval-rule-count">{t("settings.approval.visibleCount", { count: formatNumber(rules.data.rules.length) })}</span> : null}
				</header>
				{props.sessionId === undefined ? (
					...
					<ApprovalState action={t("settings.approval.tryAgain")} onAction={() => void rules.refetch()}>
				...
				{forget.isError ? <p className="settings-inline-error" role="alert">{messageOf(forget.error, t)}</p> : null}
			</section>
		</>
	);
}
...
function ApprovalRuleCard(props: {
	...
	const subject = props.rule.subject === "" ? t("settings.approval.everyInvocation") : props.rule.subject;
	return (
		<article className="approval-rule-card" data-decision={props.rule.decision}>
			<header>
				<div>
					<span className="approval-rule-verdict">{decisionName(props.rule.decision, t)}</span>
					<span className="approval-rule-scope">{scopeName(props.rule.scope, t)}</span>
				</div>
				{props.confirming ? (
					<div className="approval-forget-confirm">
						<span>{t("settings.approval.forgetQuestion")}</span>
						<button type="button" disabled={props.pending} onClick={props.onCancel}>{t("settings.approval.cancel")}</button>
						<button className="danger" type="button" disabled={props.pending} onClick={props.onForget}>{props.pending ? t("settings.approval.forgetting") : t("settings.approval.forget")}</button>
					</div>
				) : (
					<button className="text-action danger" type="button" onClick={props.onConfirm}>{t("settings.approval.forget")}</button>
				)}
			</header>
			...
			{props.rule.dir ? <p title={props.rule.dir}>{t("settings.approval.projectPath", { path: props.rule.dir })}</p> : null}
		</article>
	);
}
...
function scopeName(scope: string, t: Translate) {
	switch (scope) {
	case "session": return t("settings.approval.scope.session");
	case "project": return t("settings.approval.scope.project");
	case "global": return t("settings.approval.scope.global");
...
function decisionName(decision: string, t: Translate) {
	switch (decision) {
	case "approve": return t("settings.approval.decision.approve");
	case "deny": return t("settings.approval.decision.deny");
	default: return decision;
	}
}

function messageOf(error: unknown, t: Translate) {
	return error instanceof Error ? error.message : t("settings.approval.requestFailed");
}
