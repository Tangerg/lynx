...
import { useLocalization, type MessageKey, type Translate } from "../localization/Localization";
				...
				{summary.isPending ? (
					...
					<>
						<div className="usage-metrics">
							<UsageMetric label={t("settings.usage.tokens")} value={formatTokens(totalTokens(summary.data.total), formatNumber)} />
							<UsageMetric label={t("settings.usage.cost")} value={formatCost(summary.data.total.costUsd, t, formatNumber)} />
							<UsageMetric label={t("settings.usage.runs")} value={formatNumber(summary.data.runs ?? 0)} />
							<UsageMetric label={t("settings.usage.sessions")} value={formatNumber(summary.data.sessions ?? 0)} />
						</div>
						<p className="usage-cost-note">
							{t("settings.usage.costNote")}
						</p>
						<UsageBreakdown title={t("settings.usage.providers")} values={summary.data.byProvider ?? []} t={t} formatNumber={formatNumber} />
						<UsageBreakdown title={t("settings.usage.models")} values={summary.data.byModel ?? []} t={t} formatNumber={formatNumber} />
						<UsageBreakdown title={t("settings.usage.days")} values={summary.data.byDay ?? []} t={t} formatNumber={formatNumber} />
					</>
				) : null}
			</section>
			...
			<section className="settings-section">
				...
				{props.sessionId === undefined ? (
					...
					<div className="usage-metrics usage-session-metrics">
						<UsageMetric label={t("settings.usage.tokens")} value={formatTokens(totalTokens(session.data), formatNumber)} />
						<UsageMetric label={t("settings.usage.cost")} value={formatCost(session.data.costUsd, t, formatNumber)} />
						<UsageMetric
							label={t("settings.usage.models")}
							value={formatNumber(Object.keys(session.data.byModel ?? {}).length)}
						/>
					</div>
				) : null}
			</section>
			...
			<div>
				{props.values.slice(0, 10).map((value) => (
					<article key={value.key}>
						...
						<span>{formatCost(value.costUsd, props.t, props.formatNumber)}</span>
						<small>{props.t((value.runs ?? 0) === 1 ? "settings.usage.runCountOne" : "settings.usage.runCountMany", { count: props.formatNumber(value.runs ?? 0) })}</small>
					</article>
				))}
			</div>
		</section>
...
function formatCost(value: number | undefined, t: Translate, formatNumber: NumberFormatter): string {
...
function messageOf(error: unknown, t: Translate): string {
	return error instanceof Error ? error.message : t("settings.usage.loadFailed");
}
