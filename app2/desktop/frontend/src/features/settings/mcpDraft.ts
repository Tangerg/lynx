...
export function durableMCPServerSignature(server: MCPServer) {
  return JSON.stringify({
    ...
    connection: {
      ...server.connection,
      headersMasked: Object.entries(server.connection.headersMasked ?? {}).sort(
        ([left], [right]) => left.localeCompare(right),
      ),
      envMasked: Object.entries(server.connection.envMasked ?? {}).sort(
        ([left], [right]) => left.localeCompare(right),
      ),
    },
...
export function candidateFromDraft(draft: MCPDraft): MCPServerCandidate {
  ...
  return {
    ...
    ...(draft.disabledTools.length === 0
      ? {}
      : { disabledTools: draft.disabledTools }),
    ...(draft.autoApproveTools.length === 0
      ? {}
...
export function requestFromDraft(
  ...
  if (draft.description.trim() !== (server.description ?? ""))
    request.description = draft.description.trim();
  const timeout = timeoutFromDraft(draft);
  if (timeout !== (server.timeoutSeconds ?? 0))
    request.timeoutSeconds = timeout;
  if (!sameValues(draft.disabledTools, server.disabledTools ?? []))
    request.disabledTools = draft.disabledTools;
  if (!sameValues(draft.autoApproveTools, server.autoApproveTools ?? []))
    request.autoApproveTools = draft.autoApproveTools;
  if (connectionChanged(server, draft))
    request.connection = connectionFromDraft(draft, false);
  return request;
}

export function mcpDraftChanged(server: MCPServer, draft: MCPDraft) {
  const currentTimeout = server.timeoutSeconds
    ? String(server.timeoutSeconds)
    : "";
  return (
    draft.enabled !== (server.status.type !== "disabled") ||
...
export function withToolPolicy(
  ...
  const disabledTools = draft.disabledTools.filter(
    (candidate) => candidate !== name,
  );
  const autoApproveTools = draft.autoApproveTools.filter(
    (candidate) => candidate !== name,
  );
  ...
  return {
    ...
    autoApproveTools: autoApproveTools.sort(),
  };
}
...
function connectionFromDraft(
  ...
  if (draft.transport === "streamableHttp") {
    const authorization = draft.clearAuthorization
      ...
      : draft.authorization.trim() === ""
        ? undefined
        : { type: "set", value: draft.authorization };
    const headers = draft.clearHeaders
      ...
      : draft.headersJSON.trim() === ""
        ...
        : {
            type: "set",
            value: parseSecretObject(draft.headersJSON, "headers"),
          };
    return {
      type: "streamableHttp",
      url: required(draft.url, "endpointRequired"),
      ...((candidate && authorization?.type === "clear") ||
      authorization === undefined
        ? {}
        : { authorization }),
      ...((candidate && headers?.type === "clear") || headers === undefined
        ? {}
        : { headers }),
    };
  }
  const environment = draft.clearEnvironment
    ...
    : draft.environmentJSON.trim() === ""
      ...
      : {
          type: "set",
          value: parseSecretObject(draft.environmentJSON, "environment"),
        };
  return {
    type: "stdio",
    ...
    ...((candidate && environment?.type === "clear") ||
...
function parseSecretObject(
  ...
  if (
    entries.length === 0 ||
    entries.some(([name, entry]) => name === "" || typeof entry !== "string")
  ) {
