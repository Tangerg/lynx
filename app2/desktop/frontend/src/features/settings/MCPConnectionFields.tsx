			...
			<div className="mcp-field-grid">
				{props.includeName ? (
					<label>
						...
						<input dir="ltr" value={props.draft.name} maxLength={128} autoComplete="off" placeholder="filesystem-tools" onChange={(event) => update("name", event.currentTarget.value)} />
					</label>
				) : null}
				<label>
				...
				<label>
					...
					<input dir="ltr" type="number" min="0" max="3600" inputMode="numeric" value={props.draft.timeoutSeconds} placeholder="15" onChange={(event) => update("timeoutSeconds", event.currentTarget.value)} />
				</label>
	...
	return (
		<>
			<label>
				...
				<input dir="ltr" type="url" value={props.draft.url} maxLength={8192} placeholder="https://tools.example.com/mcp" onChange={(event) => props.update("url", event.currentTarget.value)} />
			</label>
			<div className="mcp-secret-grid">
				<div className="mcp-secret-field">
					<label>
						...
						<input dir="ltr" type="password" autoComplete="off" value={props.draft.authorization} disabled={props.draft.clearAuthorization} placeholder={props.draft.clearAuthorization ? t("settings.mcp.storedValueRemoved") : t("settings.mcp.optionalReplacement")} onChange={(event) => props.update("authorization", event.currentTarget.value)} />
					</label>
				...
				<div className="mcp-secret-field">
					<label>
						...
						<textarea dir="ltr" value={props.draft.headersJSON} disabled={props.draft.clearHeaders} spellCheck={false} placeholder={'{\n  "X-API-Key": "…"\n}'} onChange={(event) => props.update("headersJSON", event.currentTarget.value)} />
					</label>
		...
		<>
			<div className="mcp-field-grid mcp-field-grid-wide">
				<label>
					...
					<input dir="ltr" value={props.draft.command} maxLength={4096} placeholder="npx" onChange={(event) => props.update("command", event.currentTarget.value)} />
				</label>
				<label>
					<span>{t("settings.mcp.workingDirectory")} <small>{t("settings.mcp.absolutePath")}</small></span>
					<input dir="ltr" value={props.draft.dir} placeholder="/absolute/path" onChange={(event) => props.update("dir", event.currentTarget.value)} />
				</label>
			</div>
			<div className="mcp-secret-grid">
				<label>
					<span>{t("settings.mcp.arguments")} <small>{t("settings.mcp.onePerLine")}</small></span>
					<textarea dir="ltr" value={props.draft.argsText} spellCheck={false} placeholder={'-y\n@scope/server'} onChange={(event) => props.update("argsText", event.currentTarget.value)} />
				</label>
				<div className="mcp-secret-field">
					<label>
						...
						<textarea dir="ltr" value={props.draft.environmentJSON} disabled={props.draft.clearEnvironment} spellCheck={false} placeholder={'{\n  "TOKEN": "…"\n}'} onChange={(event) => props.update("environmentJSON", event.currentTarget.value)} />
					</label>
