...
export function AgentNarrative(props: AgentNarrativeProps) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const target = [
        ...(reader.viewportRef.current?.querySelectorAll<HTMLElement>(
          "[data-item-id]",
        ) ?? []),
    ...
    <div className="narrative-reader">
      ...
      >
        <div className="narrative-timeline" ref={reader.contentRef}>
            ...
            <div className="history-navigator">
              <label>
                ...
                <span className="sr-only">
                  {t("narrative.searchLoadedHistory")}
                </span>
                <input
                  ref={searchInput}
                  type="search"
              ...
              {searchMatches.length > 0 ? (
                <span className="history-match-actions">
                  <button
                    type="button"
                    aria-label={t("narrative.previousMatch")}
                    onClick={() => moveMatch(-1)}
                  >
                    ↑
                  </button>
                  <button
                    type="button"
                    aria-label={t("narrative.nextMatch")}
                    onClick={() => moveMatch(1)}
                  >
                    ↓
                  </button>
              ...
              {props.historyError ? (
                <p role="alert">{props.historyError}</p>
              ) : null}
            </div>
          ) : null}
          {props.streamError ? (
          ...
          {material.rootItems.length === 0 &&
          material.orphanRuns.length === 0 ? (
      ...
      return (
        ...
        >
          ...
          {item.question?.fields.map((field, index) => (
            <p key={`${item.id}:${index}`}>
              <HighlightedText text={field.prompt} query={searchQuery} />
            </p>
          ))}
        </article>
      );
