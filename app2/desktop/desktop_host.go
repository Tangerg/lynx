...
func (host *DesktopHost) Bootstrap() (DesktopBootstrap, error) {
	if host.remote.Active() {
		...
		connection,err:=host.remote.Bootstrap(ctx);if err!=nil{return DesktopBootstrap{},fmt.Errorf("desktop host: bootstrap remote Runtime: %w",err)}
...
func(host *DesktopHost)ConnectRemoteRuntime(endpoint,token string)(remote.State,error){ctx,cancel:=context.WithTimeout(context.Background(),30*time.Second);defer cancel();return host.remote.Configure(ctx,endpoint,token)}

func(host *DesktopHost)UseLocalRuntime()(remote.State,error){if err:=host.remote.UseLocal();err!=nil{return remote.State{},err};return host.remote.State(),nil}

func(host *DesktopHost)UseRemoteRuntime()(remote.State,error){ctx,cancel:=context.WithTimeout(context.Background(),15*time.Second);defer cancel();return host.remote.UseRemote(ctx)}

func(host *DesktopHost)ForgetRemoteRuntime()(remote.State,error){if err:=host.remote.Forget();err!=nil{return remote.State{},err};return host.remote.State(),nil}
