...
| U22 | Theme/accent/light-dark/i18n | shell/settings/UI tokens | R10 | in_progress | typed boundary 已覆盖全部主 Agent/Session/Shell/Goal/Plan/file、Resources 与 Settings production surfaces，仅激活 complete English；TSX AST 显式文案盘点已清零，MCP/Interrupt 纯转换校验、Desktop bridge/discovery infrastructure 错误已改为 presentation adapter 映射的 typed code，Agent stream/action/history fallback 已收口。8 个 parity locale、Arabic RTL、locale owner/selector 与 logical CSS 继续生产迁移；最终门禁待 R11 |
