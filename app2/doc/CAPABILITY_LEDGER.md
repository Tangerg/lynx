...
| U22 | Theme/accent/light-dark/i18n | shell/settings/UI tokens | R10 | in_progress | typed boundary/hidden error channel 与 TSX AST copy audit 已闭合；9 个 exact 1012-key static dictionaries（含 Arabic）、ShellPreferences v2 locale owner/native selector、active-only lang/dir/Intl 已实现，0 missing/extra/placeholder mismatch 且无旧 app runtime dependency/raw fallback。Arabic logical CSS 与最终 native-language/visual/package 门禁继续；完成前仍为 `in_progress` |
