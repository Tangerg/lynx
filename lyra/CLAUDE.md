...
判断"这段代码该不该这么写 / 这个 PR 该不该 merge"的硬尺子。每条都标了本次 session 真实命中的例子，方便后续对号入座。
...
- **低耦合** = 跨包依赖**通过最小接口**而不是具体类型。chat 服务依赖 `chat.Engine`（5 方法）不直接抱 `*engine.Engine`、autonomy 包内定义 `platform` 接口（2 方法）不抱 `*runtime.Platform` —— 这就是低耦合。
- **二者矛盾时**：宁可包多一点（更高内聚）也别让一个包横跨多个 domain（低内聚）；宁可接口多一点（更低耦合）也别让一个具体类型变成跨包枢纽。
- ❌ 反例（已修）：lyra batch B 之前 `chat.New(*engine.Engine)` —— 一处改 engine API 整个 chat 测试都得重写。
...
- 信号：一个 struct field > 8 + method > 10 / 一个函数 > 80 行 / 一个文件 > 500 行 —— 通常都是 SRP 信号，考虑拆。
- ❌ 反例：原 `ProcessContext` 把 chat client、guardrails、tool resolver、cancel hook、blackboard、options 混在一起 11 个字段 —— audit 想拆但分析后发现是 inherent complexity（11 个真实依赖），最后用**字段分区**注释表达 SRP 而非物理拆分（lyra agent batch D）。

### OCP — Open/Closed（开放扩展、关闭修改）

- **加新能力靠加新类型，不靠改老 dispatch loop**。`runtime.collectExtensions[T any]([]Extension)` 是范例：要加新 ToolDecorator / ActionMiddleware / GoalApprover —— 实现接口就自动被 dispatch 发现，runtime 不动。
- Go 里通常用 **interface + type assertion / generic 类型分发** 实现 OCP，不用继承。
- ❌ 反例（已修）：approval `EarlyTerminationPolicy` 一开始想拆掉 `Extension` embed —— 后来发现 embed 是承重的（`collectExtensions` 靠它），删了反而要新加注册路径，**OCP 反而被破坏** → audit 误判，保留。
...
- ❌ 反例（避免）：一个 `Service.Update(ctx, x)` 实现里悄悄忽略 ctx —— 调用方 cancel 不生效。LSP 不是只看签名匹配，还看行为契约。

### ISP — Interface Segregation（接口隔离）

- **接口里只放调用方真用的方法**。`approval.Console`（4 方法：ListPending / Decide / SetMode / GetMode）给 client side；`approval.Gate`（2 方法：Register / GetMode）给 producer side；`Service = Console + Gate` union 给 runtime 装配。chat 服务只 import `Gate`。
- Rob Pike: "The bigger the interface, the weaker the abstraction." 大接口逼实现方塞 stub、逼 caller 多了解不该知道的。
- ❌ 反例（已修）：原 `approval.Service` 把 5 个方法揉一个接口，chat 只用其中 2 个 —— lyra batch D 已拆 Console / Gate。
...
- ❌ 反例（已修）：chat / tool / autonomy 都曾直接 import `*engine.Engine` / `*runtime.Platform` 整体 —— 本次 session 三处全改了。
...
- ✅ 例（已做）：config.MCPTransport 和 engine.MCPTransport enum 双定义 → 合一份；`fmt.Errorf("constant")` 58 处 → `errors.New(...)`。
- ❌ 反例（避免）：把所有 provider adapter 的 `requestHelper` 强行抽公共基类 —— SDK shape 差异 > 相似度，会成累赘。

### KISS — Keep It Simple, Stupid

- **简单 > 巧妙**。读代码的人 90% 时间在维护，写代码的人 10% 时间在创造。
- 信号：嵌套泛型 > 2 层 / 反射 / interface{} / type-switch 长尾 / 函数闭包嵌套 —— 通常都是"我可以这么写"但不该这么写。
- ✅ 例（已做）：autonomy 引入 `platform` interface 时只 2 个方法（仅消费的），不预留"未来可能要的"方法；`agentdoc.Render` 用线性扫一遍而不是上 LRU。
- ❌ 反例（避免）：approval 的 `atomicMode{value int32}` 自家 wrapper —— `atomic.Int32` 一句话搞定，wrapper 是 KISS 反例（已删）。
...
- ✅ 例（已做）：删 `trace.Service` 整个 stub 包；删 `Platform()` / `ChatAgent()` 零调用方 getter；模型 audit 写到 `ServiceProvider` 的 sync.Map 现代化 —— 一看零 caller，整个 modernize 都不值得做。
...
- **Make zero values useful**：struct 零值能用就别要构造函数。`bytes.Buffer{}` / `sync.Mutex{}` 都是范例。我们的 `pendingRequest` / `turnState` 都尽量这样。
- **Composition over inheritance**：Go 没有继承，只有 embed + interface。embed 是 has-a 不是 is-a，慎用（容易让方法集变得难追）。
- **Smaller interfaces are better**（Rob Pike）：1 方法的 interface 经常是最有用的。`io.Reader` / `io.Writer` / 我们的 `tool.ToolSource`（1 方法）都是这条。
