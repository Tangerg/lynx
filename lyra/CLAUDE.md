...
- **目前开发阶段**：不写 legacy 兼容代码，schema 变了直接换；不写 migration；注释里不提"Legacy …"
- **加文档？先问** —— `lyra/CLAUDE.md` / `lyra/doc/ARCHITECTURE.md` / `lyra/doc/ROADMAP.md` 已经存在；其他默认不写
- **重构是节奏**：每加 3-5 个 feature 扫一轮（删死代码 + 整合 3+ 重复 + 改 1-2 个名字，touch < 5 文件 / 净 < 100 LOC）。每 15-20 个 feature 扫整个 module（dead code / SOLID / DRY 全面），分批 commit，每批走 `go build && go vet && go test ./...` 全绿
