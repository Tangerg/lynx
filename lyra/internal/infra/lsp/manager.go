...
//
// The Manager is the entry point. It starts one server per (workspace root,
// language) on first use and keeps it warm for the engine's lifetime. Adding a
// language is a one-line ServerSpec (see server.go); nothing here is
// Go-specific.
package lsp
...
var ErrClosed = errors.New("lsp: manager closed")
...
// Manager owns the live language-server connections for one engine. It is safe
...
// NewManager builds a manager over specs (see DefaultServers for the built-in
// table). It starts no processes — servers launch lazily on first use.
func NewManager(specs []ServerSpec) *Manager {
	return &Manager{table: newServerTable(specs), clients: map[string]*client{}}
}
...
func (m *Manager) WorkspaceSymbols(ctx context.Context, root, query string) ([]Symbol, error) {
	specs := m.table.forRoot(root)
	if len(specs) == 0 {
...
func (m *Manager) Supported(file string) bool {
	_, ok := m.table.forFile(file)
	return ok
}
