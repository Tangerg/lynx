...
> **真正落后的仍是「自主性与触达」**——调度/自动化、远程 IM 桥接（**flue 的头牌**：17 个 channel 适配器）、多 agent 团队（**cline 18 team 工具 / kimi 128 swarm**）、hooks（**claude_code 27 / pi 27**）、OS sandbox、evals。
...
**单 turn 执行**：agent loop（复用 lynx `agent/runtime` 的 `for{}`，不手写）· 工具循环 + **并行工具 + per-path 写锁** · **HITL R 模型**（park-on-interrupt + resume，可持久化/审计/**跨重启 rehydrate**）· **plan-as-stance**（agent/chat/plan 三模式塌缩成一个 loop + `exit_plan_mode`）· steering 注入 · **MaxBudget/MaxCostUSD + idle/stall backstop**（codex 式空闲超时，非总时长硬切）。
...
> **自 2026-06-14 已落地、别再当缺口的**：多模态图片输入（旧头号 todo，已接）、plan-as-stance、idle/stall backstop、checkpoint 精修、schema-from-struct、原子 memory.Replace、type-preserving snapshot、一批并发竞态加固（kill-race / cancel-park / concurrent-HITL）。
...
**3.2 Agent loop / 工具循环 / 并行** ✅
lyra：复用 SDK `for{}` welded loop + 并行工具 + per-call 取消 + **per-path 写锁**。
field：人人都有 loop。**新范式**：codex **Code Mode**（模型写 JS 在 V8 里 `await tools.*()`，一步编排多工具）、eve 同类 "code mode" workflow 工具。flue 把 loop **委派给外部 pi-agent-core**（自己只管 loop 外的一切）。
判断：齐平；Code Mode 是值得关注的新方向（lyra 暂不需要）。
...
**3.17 多 agent 团队 / 并行编排** 🟡（lyra 仅单委派）
lyra：`SpawnChildProtectedOnly` 单委派（3 种 spawn 模式），**无并行团队**。
...
判断：**这是本轮竞品提升最猛的维度**——lyra 明显落后（仅单委派）。成本高（团队生命周期 + 消息板 + 成本 roll-up）。
...
| **多 agent 团队** | 🟡(单委派) | 🟢 | 🟢(task) | ✅ | 🟢 | 🟢 | 🟢 | **cline(18)/kimi(128)** |
...
| 缺口 | 谁领先 | 成本 | 备注 |
|---|---|---|---|
| **多 agent 团队 / 并行编排** | cline(18 工具) · kimi(128 swarm) · AionUi(ACP Team) | 高 | 本轮竞品升级最猛；lyra 仅单委派 |
