...
- **原则**：这是把上次的 A/B/C 选项**新增了一个更优的 D 选项**。⚠️ 涉及 `agent/runtime` 的 tick 循环和 `core.Process` 语义，是核心改动。

> **结论（2026-05-29 调查后）—— 不做这条 runtime 改动，现状已经更优。**
>
> 深读当前 `agent/runtime` + `agent/hitl` + lyra 工具审批流后确认：lyra 的工具审批**已经在工具边界做到"不重跑 LLM"**，只是机制不是 SDK 的"turn 退出 + resume"，而是 **in-process 同步阻塞**——
>
> - `lyra/internal/engine/observer.go` 的 `observedTool.Call` 在调真正工具**之前**调 `OnToolCallApprove`；后者 `select` 在 `approval.Decide` 的 decision channel 上**park 住该工具调用的 goroutine**（带 `<-ctx.Done()` 兜底），决策一到就放行执行那个工具。planner / LLM **完全不重跑**。
> - 这正是 SDK 用 `tool_deferred` 想达到的效果。SDK 之所以要"退出 + 跨 session resume",是因为它**跨进程**(client ≠ agent 子进程);lyra **in-process**,一个 channel select 就够,不需要把状态序列化出去再拉回来。
>
> 反过来,**真按 D 去套 `agent/hitl` 的 `AwaitInput` 反而是 regression**:`AwaitInput` 冻结在 **action 边界**,resume 走 `ContinueProcess` → **下一 tick 重新 plan → 重新跑整个 chat action = LLM 重跑**——恰好是这条想避免的代价。而且它需要 **mid-action snapshot/restore**(当前 `process_snapshot.go` 只存进程元数据,不存"执行到哪个工具"),要做就是一大坨侵入式改动。
>
> 同步阻塞唯一的劣势是"等审批期间进程不可持久化、重启会丢在飞的审批"。但审批本就是**有界的交互等待**(前端协议把 `lyra.approval` 配 `expiresAt` + 超时默认 deny/abort),不是跨重启的持久暂停;为这点边际收益上 mid-action snapshot 不划算。**保留同步 gate。**
>
> **唯一相邻的真实改进**:`agent/core/process_context.go:236` 的 `buildChatRequest` 把工具循环写死成 `chat.NewToolMiddleware()`(默认 config),所以 lyra 的 chat agent **拿不到** `FeedbackOnUnknownTool` / `FeedbackOnEmptyResponse`(模型幻觉出不存在的工具名会直接 abort,而不是把"可用工具列表"喂回让它改选)。`agent/workflow/supervisor.go` 已经用了 `FeedbackOnUnknownTool:true`。要让 lyra 也吃到,得把 `ProcessContext` 的工具中间件 config 做成可配(经 `ActionConfig` 或 `ProcessOptions`)——**这是 agent-core 改动,属用户活跃域,先咨询再动**(见下方"## 后续"段)。
...
2. **吸收 SDK 验证过的控制面 ergonomics**——但**逐项按前后端协议契约 + 当前代码实际验证**,不盲抄。
3. **补可观测性事件**——复用 "chat turn = AgentProcess + event listener" 通路,顺水推舟。

### 实施记录（2026-05-29 一轮重构）

把上面 A–H 真正落地时,有一半在对照**当前代码 + 前端 `[CLOSED]` 协议对齐决议**后改了方向。诚实记录:

| 项 | 结局 | 一句话 |
|---|---|---|
| 组织重构 | ✅ 已做 | dispatch 查表化、chat inmemory 拆 4 文件、OnlineConfig 去重 |
| G 可观测事件 | ✅ 已做 | `CompactBoundary` / `MemoryUpdated` 事件 + AG-UI CUSTOM 翻译 |
| C 富审批结果 | ❌ **砍** | 违反前端 `[CLOSED]` 决议(wire 只 `approve/deny`、"记住"是前端 UI);反而**删了死的 `DecisionAllowAlways`**、enum 对齐 `approve/deny` |
| D permission 梯度 | ✅ 已做 | 加 `ModeReadOnly`(3-way gate pass/prompt/deny),纯后端策略 |
| H budget | ✅ 部分 | token 上限 `MaxBudget`(内部,工具循环边界处停);per-model 分解 + CostUSD **砍**(单 model、无定价表,YAGNI) |
| A steering RPC | ⏭️ **缓** | 前端只*预留* `lyra.interrupt/resume`,无 spec,单方面加 wire 方法违反"动协议先对齐" |
| E 后台通知 | ⏭️ **缓** | lyra 没有后台任务特性(`task` 工具都不在),没东西可通知 |
| B deferred tool | ✅ **判定不做** | **现状的 in-process 同步 gate 已经"工具边界不重跑 LLM",`AwaitInput` 路反而 regress**(见 §B 结论) |
| F session tag / resume-at | 未做 | 随 M3 session 工作做 |

**教训**:这份文档最初把 §B 标成"最高价值单点",但深读当前代码后发现它是个**伪命题**——in-process 运行时不需要 SDK 那套跨进程 defer/resume。"对照真实代码再下结论"比"照着另一个框架的 API 推演"重要。
...
## 后续（待用户决定的 agent-core 改动）

这轮重构刻意只动 lyra,没碰用户活跃开发的 `agent` 模块。以下一条**真实改进**需要 agent-core 改动,留给用户:

- **让 `ProcessContext` 的工具循环 config 可配**。现状 `agent/core/process_context.go:236` 把 `buildChatRequest` 里的工具中间件写死成 `chat.NewToolMiddleware()`(默认 config),所以**任何**经 `ChatWithActionTools` 跑工具循环的 action(含 lyra 的 chat agent)都吃不到 `FeedbackOnUnknownTool` / `FeedbackOnEmptyResponse`。`agent/workflow/supervisor.go:97` 已经手动 `chat.NewToolMiddleware(ToolLoopConfig{...})` 绕过这条限制。
- **价值**:开了之后,模型幻觉出不存在的工具名时,lyra agent 会把"可用工具列表"喂回让它改选,而不是整轮 abort —— 健壮性明显提升,且是 dogfood 用户刚加的 core 特性。
- **做法**:给 `ActionConfig` 或 `ProcessOptions` 加一个 `ToolLoopConfig`(或等价开关),`buildChatRequest` 读它;lyra 的 chat action 在 `ActionConfig` 里开 `FeedbackOnUnknownTool: true`。
- **为什么不在这轮做**:跨 `agent` 模块 + 用户活跃域 + 改 `ProcessContext`/`ActionConfig` 是 exported 签名 —— 按 CLAUDE.md 先咨询再动。
