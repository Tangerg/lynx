...
### C2. cost 导管已接好（✅ `1b8f3a2`），定价表 = operator 提供
导管全通了：`engine.Config.Pricing`（经 `runtime.Config` 透传）一旦给，`invocationFrom` 填 `LLMInvocation.Cost` → `chatOutput` 汇总 → `ChatOutput.CostUSD` + per-model → `TurnEnd.CostUSD`。不给则 cost=0（诚实，框架不臆造价格）。**插上定价表即端到端出钱数,无需改其他代码** —— 表本身（per-provider 费率）由 operator 维护。`maxBudgetUsd`（cost 上限）可在表就位后基于 `CostUSD` 加；现有 token 上限够用。
