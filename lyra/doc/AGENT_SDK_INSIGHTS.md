...
| 工具循环 recovery | ✅ 已做(跨 agent) | `ActionConfig.ToolLoop` 可配 + lyra 开 `FeedbackOnUnknownTool`——幻觉工具名喂回让模型改选,不整轮 abort |
...
- ✅ **让 `ProcessContext` 的工具循环 config 可配** —— 已做(commit `feat(agent,lyra): per-action tool-loop config`)。`ActionConfig`/`ActionMetadata` 加了 `ToolLoop chat.ToolLoopConfig`,`buildProcessContext` 把它从 action metadata 透传到 `ProcessContext.actionToolLoop`,`buildChatRequest` 用它建中间件(零值=旧默认,additive)。lyra 的 chat action 开了 `FeedbackOnUnknownTool` + `FeedbackOnEmptyResponse`——模型幻觉工具名会被喂回真实工具列表让它改选,而不是整轮 abort。dogfood 了用户刚加的 recovery policies(`51db432`)。
- ⏭️ 其余跨 `agent` 模块的想法(如 `ProcessContext` 暴露 steering/中断的更细粒度控制)继续按"动协议/exported 签名前先咨询"走。
