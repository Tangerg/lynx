...
func ServerCapabilities() coreapi.ServerCapabilities {
	return coreapi.ServerCapabilities{
		...
		Features: coreapi.ServerFeatureCapabilities{
			Reasoning:     true,
			MCP:           true,
			Multimodal:    false,
			Checkpoints:   false,
			...
			Skills:        false,
			SessionExport: false,
			Attachments: coreapi.AttachmentLimits{
				Enabled: false,
			},
		},
		Providers: []string{}, // future: derive from chat.Client metadata
		Limits:    coreapi.Limits{MaxConcurrentRuns: 8},
	}
}
...
func notImpl(method string) error {
	return fmt.Errorf("%w: %s", coreapi.ErrNotImplemented, method)
}
