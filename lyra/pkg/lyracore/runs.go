...
func (i *Server) StartRun(ctx context.Context, in coreapi.StartRunRequest) (*coreapi.StartRunResponse, <-chan coreapi.AgUiEvent, error) {
	...
	handle, err := i.rt.Chat().StartTurn(ctx, chat.StartTurnRequest{
		SessionID: sessionID,
		Message:   userMsg,
		PlanMode:  in.Mode == coreapi.RunModePlan,
	})
	if err != nil {
		return nil, nil, err
	}
...
func lastUserMessage(msgs []coreapi.Message) string {
	for idx := len(msgs) - 1; idx >= 0; idx-- {
		if msgs[idx].Role == coreapi.MessageRoleUser {
