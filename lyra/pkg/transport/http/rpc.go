...
// broadcast registry. Events arrive through rpcadapter.EncodeRunEvent.
func (s *Server) attachStream(ctx context.Context, runID string, events <-chan coreapi.AgUiEvent) {
	if runID == "" || events == nil {
		return
	}
	...
	go func() {
		...
		for {
			...
			case ev, ok := <-events:
				if !ok {
					closed, _ := rpcadapter.EncodeRunClosed(runID, "completed")
					if closed != nil {
