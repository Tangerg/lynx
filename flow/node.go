...
// need to wait for the operation to complete. It returns a channel that will
// receive the result when processing completes, allowing the caller to:
//   - Continue execution without waiting for results
//   - Optionally check for results later
//   - Implement fan-out processing patterns
//
// The channel returned by Run will receive either the processed result
// or an error, and will be closed when processing completes.
//
// Example:
//
...
//	resultChan, _ := backgroundProcessor.Run(ctx, data)
//
...
//	    // Handle result
//	}()
type AsyncNode[I any] struct {
	core[I, any]
}

// Run executes processing asynchronously and returns a channel for results.
//
...
//   - Will receive exactly one value (either result or error)
//   - Will be closed after the value is sent
//   - Has a buffer size of 1 to prevent goroutine leaks
//
// The caller is responsible for consuming from the channel to prevent
// potential resource leaks.
func (a *AsyncNode[I]) Run(ctx context.Context, input I) (any, error) {
