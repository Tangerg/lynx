...
func TestNewInMemoryMemory(t *testing.T) {
	...
	assert.NotNil(t, memory.store)
	assert.Equal(t, 0, len(memory.store))
}
...
func TestWrite_SingleMessage(t *testing.T) {
	...
	assert.Equal(t, chat.MessageTypeUser, messages[0].Type())
}
...
func TestWrite_MultipleMessages(t *testing.T) {
	...
	msg1 := chat.NewUserMessage("Question 1")
	msg2 := chat.NewAssistantMessage("Answer 1")
	msg3 := chat.NewUserMessage("Question 2")
	...
	assert.Equal(t, chat.MessageTypeUser, messages[0].Type())
	assert.Equal(t, chat.MessageTypeAssistant, messages[1].Type())
	assert.Equal(t, chat.MessageTypeUser, messages[2].Type())
}
...
func TestWrite_AppendToExistingConversation(t *testing.T) {
	...
	msg2 := chat.NewUserMessage("Second")
...
func TestWrite_DifferentConversations(t *testing.T) {
	...
	msg1 := chat.NewUserMessage("Conv A Message")
	msg2 := chat.NewUserMessage("Conv B Message")
...
func TestWrite_DifferentMessageTypes(t *testing.T) {
	...
	systemMsg := chat.NewSystemMessage("System instruction")
	userMsg := chat.NewUserMessage("User query")
	assistantMsg := chat.NewAssistantMessage("Assistant response")
...
func TestWrite_AssistantMessageWithToolCalls(t *testing.T) {
	...
	msg := chat.NewAssistantMessage([]*chat.ToolCall{toolCall})
	err := memory.Write(ctx, "conv1", msg)
	require.NoError(t, err)

	messages, _ := memory.Read(ctx, "conv1")
	assert.Len(t, messages, 1)
	assert.Equal(t, chat.MessageTypeAssistant, messages[0].Type())
}
...
func TestWrite_ToolMessage(t *testing.T) {
	...
	msg, err := chat.NewToolMessage([]*chat.ToolReturn{toolReturn})
	require.NoError(t, err)
	...
	messages, _ := memory.Read(ctx, "conv1")
	assert.Len(t, messages, 1)
	assert.Equal(t, chat.MessageTypeTool, messages[0].Type())
}
...
func TestWrite_CompleteConversationFlow(t *testing.T) {
	...
	// System message
	systemMsg := chat.NewSystemMessage("You are a helpful assistant")
	memory.Write(ctx, "conv1", systemMsg)

	// User message
	userMsg1 := chat.NewUserMessage("What's the weather?")
	...
	assistantMsg1 := chat.NewAssistantMessage([]*chat.ToolCall{toolCall})
	...
	assistantMsg2 := chat.NewAssistantMessage("The weather in New York is sunny with 72°F")
...
func TestRead_ReturnsImmutableCopy(t *testing.T) {
	...
	msg1 := chat.NewUserMessage("Original")
	...
	messages = append(messages, chat.NewUserMessage("Added"))
...
func TestRead_AfterMultipleWrites(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		msg := chat.NewUserMessage("Message")
		memory.Write(ctx, "conv1", msg)
	}
...
func TestClear_ExistingConversation(t *testing.T) {
	...
	msg := chat.NewUserMessage("To be cleared")
	memory.Write(ctx, "conv1", msg)
...
func TestClear_IsolationBetweenConversations(t *testing.T) {
	...
	memory.Write(ctx, "convA", chat.NewUserMessage("A"))
	memory.Write(ctx, "convB", chat.NewUserMessage("B"))
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(index int) {
			...
			for j := 0; j < messagesPerGoroutine; j++ {
				msg := chat.NewUserMessage("concurrent message")
...
func TestConcurrentReadsAndWrites(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		memory.Write(ctx, convID, chat.NewUserMessage("init"))
	}
	...
	for i := 0; i < 50; i++ {
		...
		go func() {
			defer wg.Done()
			err := memory.Write(ctx, convID, chat.NewUserMessage("writer"))
			assert.NoError(t, err)
		}()
...
func TestConcurrentClears(t *testing.T) {
	...
	for i := 0; i < conversations; i++ {
		...
		memory.Write(ctx, convID, chat.NewUserMessage("test"))
	}
...
func TestConcurrentOperationsOnDifferentConversations(t *testing.T) {
	...
	for i := 0; i < operations; i++ {
		...
		go func(id string) {
			defer wg.Done()
			memory.Write(ctx, id, chat.NewUserMessage("test"))
		}(convID)
...
func TestConcurrentWritesDifferentMessageTypes(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		...
		go func() {
			...
			msg, _ := chat.NewToolMessage([]*chat.ToolReturn{toolReturn})
			memory.Write(ctx, "conv1", msg)
		}()
	}
