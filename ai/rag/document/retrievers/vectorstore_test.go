...
func TestNewVectorStoreRetriever(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			retriever, err := NewVectorStoreRetriever(tt.config)
			if tt.wantErr {
				assert.Error(t, err)
				...
				assert.Implements(t, (*rag.DocumentRetriever)(nil), retriever)
...
func mustCreateQueryWithFilter(t *testing.T, text string, filterExpr ast.Expr) *rag.Query {
	query := mustCreateQuery(t, text)
	query.Set(FilterExprKey, filterExpr)
	return query
}

func mustCreateQueryWithFilterString(t *testing.T, text string, filterStr string) *rag.Query {
	query := mustCreateQuery(t, text)
