...
func TestChatModel_Stream(t *testing.T) {
	...
	response := newModel.Stream(context.Background(), request)
	for res, err := range response {
		if err != nil {
			t.Fatal(err)
		}
		t.Log(res.Result().Output().Text())
	}
}
...
func TestChatModel_Stream_Tool(t *testing.T) {
	...
	for res, err := range response {
		...
		for _, toolCall := range res.Result().Output().ToolCalls() {
			t.Log(toolCall.ID)
			t.Log(toolCall.Name)
			...
			messages.NewUserMessage(messages.UserMessageParam{
				...
				Media: []*content.Media{
					content.
						...
						WithData("https://upload.wikimedia.org/wikipedia/commons/thumb/d/dd/Gfp-wisconsin-madison-the-nature-boardwalk.jpg/2560px-Gfp-wisconsin-madison-the-nature-boardwalk.jpg").
						MustBuild(),
				},
			}),
		},
