...
func newMessage(textContent string, metadata map[string]any) message {
	var clonedMetadata map[string]any
	if metadata != nil {
		clonedMetadata = maps.Clone(metadata)
	} else {
		clonedMetadata = make(map[string]any)
	}
...
type MessageParams struct {
	Type        MessageType      // The type of the message (MessageTypeSystem, MessageTypeUser, MessageTypeAssistant, or MessageTypeTool)
	Text        string           // The text content of the message
...
type AssistantMessage struct {
	message
	mediaAttachments []*content.Media // Media attachments included in the assistant's response
...
func (a *AssistantMessage) HasMedia() bool {
	return len(a.mediaAttachments) > 0
}
...
func NewAssistantMessage[T string | []*content.Media | []*ToolCall | map[string]any | MessageParams](param T) *AssistantMessage {
	...
	if messageParams.Media == nil {
		messageParams.Media = make([]*content.Media, 0)
	}
	...
	return &AssistantMessage{
		message:          newMessage(messageParams.Text, messageParams.Metadata),
		mediaAttachments: slices.Clone(messageParams.Media),
		toolCalls:        slices.Clone(messageParams.ToolCalls),
	}
}
...
type UserMessage struct {
	message
	mediaAttachments []*content.Media // Media attachments included with the user's message
}
...
type ToolMessage struct {
	message
	toolReturnResults []*ToolReturn // Results from executed tool functions
}
...
func NewToolMessage[T []*ToolReturn | MessageParams](param T) (*ToolMessage, error) {
	...
	return &ToolMessage{
		message:           newMessage("", messageParams.Metadata),
		toolReturnResults: slices.Clone(messageParams.ToolReturns),
	}, nil
}
...
func MergeUserMessages(messages []Message) *UserMessage {
	...
	textBuilder := strings.Builder{}
	mergedMetadata := make(map[string]any)
	combinedMedia := make([]*content.Media, 0)
	...
	for _, msg := range userMessages {
		...
		combinedMedia = append(combinedMedia, userMessage.mediaAttachments...)
	}
...
func MergeToolMessages(messages []Message) (*ToolMessage, error) {
	...
	for _, msg := range toolMessages {
		...
		combinedToolReturns = append(combinedToolReturns, toolMessage.toolReturnResults...)
	}
