...
// It allows configuring the model, middlewares, options, input text, and additional parameters
...
// WithInput sets the text content to be moderated
// Returns the ClientRequest for method chaining
func (r *ClientRequest) WithInput(input string) *ClientRequest {
	if input != "" {
		r.input = input
	}
	return r
...
func (c *ClientCaller) Moderation(ctx context.Context) (*Moderation, *Response, error) {
	...
	return response.Result().Moderation, response, nil
}
...
func (c *ClientCaller) Moderations(ctx context.Context) ([]*Moderation, *Response, error) {
	...
	moderations := make([]*Moderation, 0, len(response.Results))
	for _, result := range response.Results {
		moderations = append(moderations, result.Moderation)
	}
	return moderations, response, nil
}
...
// Copies the input, options, and params from the provided request
...
// ModerateText creates a ClientRequest for moderating a single text input
