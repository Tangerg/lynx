...
func TestNewTokenCountBatcher(t *testing.T) {
	...
	t.Run("with valid config", func(t *testing.T) {
		...
		assert.Equal(t, MetadataModeAll, batcher.metadataMode)
	})
	...
	t.Run("multiple instances are independent", func(t *testing.T) {
		...
		config2 := &TokenCountBatcherConfig{
			...
			ReservePercentage:   0.2,
			Formatter:           &mockFormatter{},
			MetadataMode:        MetadataModeEmbed,
		}
		...
		assert.Equal(t, MetadataModeAll, batcher1.metadataMode)
		assert.Equal(t, MetadataModeEmbed, batcher2.metadataMode)
	})
}
...
func TestTokenCountBatcher_Batch(t *testing.T) {
	...
	t.Run("empty document list", func(t *testing.T) {
		...
		batches, err := batcher.Batch(ctx, []*Document{})
	...
	t.Run("single small document", func(t *testing.T) {
		...
		doc, _ := NewDocument("This is a short document.", nil)

		batches, err := batcher.Batch(ctx, []*Document{doc})
	...
	t.Run("multiple small documents in single batch", func(t *testing.T) {
		...
		doc1, _ := NewDocument("First document.", nil)
		doc2, _ := NewDocument("Second document.", nil)
		doc3, _ := NewDocument("Third document.", nil)

		batches, err := batcher.Batch(ctx, []*Document{doc1, doc2, doc3})
	...
	t.Run("documents requiring multiple batches", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			doc, _ := NewDocument(strings.Repeat("word ", 20), nil)
	...
	t.Run("single document exceeding max tokens", func(t *testing.T) {
		...
		doc, _ := NewDocument(strings.Repeat("word ", 100), nil)
	...
	t.Run("preserves document order", func(t *testing.T) {
		...
		doc1, _ := NewDocument("First", nil)
		doc2, _ := NewDocument("Second", nil)
		doc3, _ := NewDocument("Third", nil)
	...
	t.Run("with document metadata", func(t *testing.T) {
		...
		doc, _ := NewDocument("Document with metadata", nil)
	...
	t.Run("different metadata modes", func(t *testing.T) {
		...
		for _, mode := range modes {
			t.Run(mode.String(), func(t *testing.T) {
				...
				doc, _ := NewDocument("Test content", nil)
	...
	t.Run("exact boundary case", func(t *testing.T) {
		...
		doc1, _ := NewDocument("This is document one.", nil)
		doc2, _ := NewDocument("This is document two.", nil)

		batches, err := batcher.Batch(ctx, []*Document{doc1, doc2})
	...
	t.Run("many small documents", func(t *testing.T) {
		...
		for i := range docs {
			docs[i], _ = NewDocument("Small doc.", nil)
		}
...
func TestTokenCountBatcher_RealWorldScenarios(t *testing.T) {
	...
	t.Run("chat message batching", func(t *testing.T) {
		...
		for _, msg := range messages {
			doc, _ := NewDocument(msg.content, nil)
    ...
    return u.Name
		...
		docs := make([]*Document, 0, len(snippets))
		for i, snippet := range snippets {
			doc, _ := NewDocument(snippet, nil)
...
func TestTokenCountBatcher_EdgeCases(t *testing.T) {
	...
	t.Run("very small max tokens", func(t *testing.T) {
		...
		doc, _ := NewDocument("Short text.", nil)
...
// Helper function
func mustNewDocument(text string) *Document {
