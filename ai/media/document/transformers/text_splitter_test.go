...
func TestTextSplitter_Transform(t *testing.T) {
	...
	t.Run("empty string results in empty output", func(t *testing.T) {
		...
		doc, err := document.NewDocument(" ", nil) // Space to avoid NewDocument error
		require.NoError(t, err)
	...
	t.Run("multiple documents", func(t *testing.T) {
		...
		doc1, err := document.NewDocument("line1\nline2", nil)
		require.NoError(t, err)

		doc2, err := document.NewDocument("line3\nline4", nil)
		require.NoError(t, err)
...
Content line 2`
		...
		doc, err := document.NewDocument(markdown, nil)
		require.NoError(t, err)
...
func main() {
	fmt.Println("Hello")
		...
		doc, err := document.NewDocument(code, nil)
		require.NoError(t, err)
...
2,Bob,bob@example.com`
		...
		doc, err := document.NewDocument(csv, nil)
		require.NoError(t, err)
...
2025-01-01 10:00:02 ERROR Failed`
		...
		doc, err := document.NewDocument(logs, nil)
		require.NoError(t, err)
...
Third paragraph.`
		...
		doc, err := document.NewDocument(article, nil)
		require.NoError(t, err)
...
func BenchmarkTextSplitter_MultipleDocuments(b *testing.B) {
	...
	for i := range docs {
		docs[i], _ = document.NewDocument("line1\nline2\nline3", nil)
	}
